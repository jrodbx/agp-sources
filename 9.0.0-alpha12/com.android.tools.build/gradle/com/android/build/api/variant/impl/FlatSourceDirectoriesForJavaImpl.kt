 ...
 *
 * The KAPT and KSP generators need to be stored in dedicated storage so we can
 * provide all user's source generated folders without creating a circular dependency.
 */
class FlatSourceDirectoriesForJavaImpl(
    name: String,
    ...
    /**
     * Internal API to retrieve all registered source generators filtering out the KSP and
     * KAPT generators. This is used by the KSP task and hence filtering prevents a circular
     * dependency (KAPT also needs the KSP java generated sources so it can creates bindings)
     */
    ...
    override val all: Provider<out Collection<Directory>> by lazy {
        directories.zip(internalGenerators) {
            genericDirectories: List<Directory>, generatorDirectories: List<Directory> ->
    ...
    override fun addSource(directoryEntry: DirectoryEntry){
        ...
        when(directoryEntry.kind) {
            ...
            DirectoryEntry.Kind.KAPT -> {
                ...
                kaptGenerator.set(
                    (directoryEntry as ConfigurableFileTreeBasedDirectoryEntryImpl)
                        .asListOfDirectories(variantServices.projectInfo.projectDirectory))
            }
