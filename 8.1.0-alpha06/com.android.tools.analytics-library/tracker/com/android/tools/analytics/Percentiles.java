 ...
 * based on the P-square algorithm found at: http://pierrechainais.ec-lille.fr/Centrale/Option_DAD/IMPACT_files/Dynamic%20quantiles%20calcultation%20-%20P2%20Algorythm.pdf
...
public class Percentiles {
     ...
     * @param rawDataSize Number of raw samples to store, only relevant if no estimator in toMerge
     *                    is interpolated.
     */
    public static Percentiles merge(double[] targets, List<Percentiles> toMerge, int rawDataSize)
        ...
        if (totalCount > p.mRawDataSize) {
            ...
            throw new MergeException(
                    "At least one interpolated estimator, but not of large enough size");
        }
        else {
        ...
        for (double targetMarker : markers) {
            ...
            while (!queue.isEmpty()) {
                ...
                if (countAtValue == targetCount) {
                    ...
                    p.mBuckets[currentBucket++] = new Bucket(targetMarker,
                                                             markerValue,
                                                             countAtValue,
                                                             totalCount);
                    if (m.increment()) {
                ...
                double ratio = ((double)(targetCount - lastCount)) / ((double)(countAtValue
                                                                               - lastCount));
                double estimatedMarkerValue = lastMarkerValue
                                              + (markerValue - lastMarkerValue) * ratio;

                p.mBuckets[currentBucket++] = new Bucket(targetMarker,
                                                         estimatedMarkerValue,
                                                         targetCount,
                                                         totalCount);
    ...
    public static Percentiles fromProto(PercentileEstimator e, double[] targets, int rawDataSize)
        ...
        if (e.getRawSampleCount() > 0) {
            ...
            for (int i = 0; i < e.getBucketCount(); ++i) {
                ...
                r.mBuckets[i] = new Bucket(bucket.getTargetPercentile(),
                                           bucket.getValue(),
                                           bucket.getCount(),
                                           r.mCount);
            }
    ...
    private void createBuckets() {
        ...
        for (double t : mTargets) {
            ...
            mBuckets[currentBucketIndex] = new Bucket(target,
                                                      mInitialData[index],
                                                      index,
                                                      mRawDataSize);
            currentBucketIndex++;
        ...
        currentBucketIndex++;
        mBuckets[currentBucketIndex] = new Bucket(1.0,
                                                  mInitialData[mRawDataSize - 1],
                                                  mRawDataSize,
                                                  mRawDataSize);
    ...
    private static void update(Bucket b, Bucket prev, Bucket next, double d) {
        ...
        double numerator = ((b.count - prev.count + d) * (next.value - b.value) / (next.count
                                                                                   - b.count)) +
                           ((next.count - b.count - d) * (b.value - prev.value) / (b.count
                                                                                   - prev.count));
