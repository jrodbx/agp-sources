...
open class FontLoader {
  ...
  companion object {
    ...
    @JvmStatic
    fun getInstance(sdkPath: File?): FontLoader {
      if (instance == null) {
        instance = FontLoader()
      }
  ...
  /** After downloading a new directory, the instances maintained by this class can be updated by calling this method. */
  ...
  private fun parseXml(source: InputSource, handler: DefaultHandler) {
    ...
    } catch (ex: SAXException) {} catch (ex: ParserConfigurationException) {} catch (ex: IOException) {}
  }

  /**
   * Perform a merge sort of the existing fonts and the list form a newly completed font directory download. This method will remove fonts
   * that are from the specified provider, and add newly found fonts. If there are multiple providers this method will be called after each
   * completed download. The result is a list of fonts ordered by font name. Fonts from different providers will be mixed together.
   *
