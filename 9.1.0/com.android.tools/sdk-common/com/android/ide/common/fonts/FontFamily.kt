...
class FontFamily : Comparable<FontFamily> {
  ...
  /**
   * Special use for creating synonyms in font-family files with references to other fonts. The fontFamily property of each [FontDetail]
   * will not be the [FontFamily] created here.
   */
  constructor(
    provider: FontProvider,
    fontSource: FontSource,
  ...
  constructor(provider: FontProvider, name: String) : this(provider, FontSource.LOOKUP, name, "", "", Collections.emptyList())

  override fun compareTo(other: FontFamily): Int {
    return Comparator.comparing(FontFamily::name).thenComparing(FontFamily::provider).compare(this, other)
  }
