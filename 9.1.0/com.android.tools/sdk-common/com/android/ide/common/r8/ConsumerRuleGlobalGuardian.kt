 ...
 *
 * The custom filtering/detection logic in this class is planned to be replaced by logic built into R8, which is more robust in rule parsing
 * / exception generation. See b/437139566.
 */
object ConsumerRuleGlobalGuardian {
    ...
    fun asExceptionMessage(isDynamicFeature: Boolean, path: String): String {
      ...
      return baseMessage +
        if (isBannedOption(option)) {
          if (isDynamicFeature) {
            "It should not be specified in this module. It can be specified in the base" + " module instead."
          } else {
          ...
          if (isDynamicFeature) {
            ...
            "It should not be used in a consumer configuration file without specifying a" + " package."
          }
        }
    }
  }
   ...
   * - `([a-zA-Z0-9]+)`: Capturing Group 1 (the "option"). Matches one or more alphanumeric characters
   * - `(?:\s+([^#]*?))?`: An optional non-capturing group for the arguments.
   * - `\s+`: Matches one or more whitespace characters (required separator).
   * - `([^#]*?)`: Capturing Group 2 (the "args"). Non-greedily matches any character that is not a '#' (comment marker).
   ...
   * - `(?:#.*)?`: An optional non-capturing group for comments. Matches '#' followed by any characters to the end of the line.
   */
  ...
  class DiscoveredIssue(val requiresArgumentInConsumerRules: Boolean, val globalOption: String, val errorMessage: String)

  /**
   * Checks consumer proguard file for invalid global options, used for validating locally defined consumer rules.
   *
   * These checks should be guarded by [BooleanOption.R8_GLOBAL_OPTIONS_IN_CONSUMER_RULES_DISALLOWED].
   *
   * Note that consumer files may not exist, since [BooleanOption.FAIL_ON_MISSING_PROGUARD_FILES] may be false.
   */
   ...
   */
  fun readConsumerKeepRulesRemovingBannedGlobals(inputStream: InputStream, shouldRemoveBannedGlobals: Boolean): String {
