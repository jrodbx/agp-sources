...
object GenericBuiltArtifactsSplitOutputMatcher {
   ...
   *
   * This uses the same logic as the store, using two passes: First, find all the compatible outputs. Then take the one with the highest
   * versionCode.
   *
   ...
   */
  fun computeBestOutput(outputs: GenericBuiltArtifacts, variantAbiFilters: Collection<String>, deviceAbis: List<String?>): List<File> =
    computeBestArtifact(outputs.elements, variantAbiFilters, deviceAbis)?.let { ImmutableList.of(File(it.outputFile)) }
  ...
  fun computeBestArtifact(
    ...
    val matches: MutableList<GenericBuiltArtifact> = Lists.newArrayList()
    ...
    val match =
      Collections.max(matches) { splitOutput: GenericBuiltArtifact, splitOutput2: GenericBuiltArtifact ->
    ...
    return if (isMainApkCompatibleWithDevice(match, variantAbiFilters, deviceAbis)) match else null
  }

  /**
   * Computes a - b.
   *
   * null values are considered to be the same as 1 since this is how the platform handles missing version code from the manifest
   */
   ...
   *
   * Higher score means a better match. Scores returned by different call are only comparable if the specified deviceAbi is the same.
   */
  private fun getAbiPreferenceOrder(builtArtifact: GenericBuiltArtifact, deviceAbi: List<String?>): Int {
    val abiFilter = getFilter(builtArtifact, "ABI")
  ...
  private fun isMainApkCompatibleWithDevice(
    ...
    if (
      (getFilter(mainBuiltArtifact, "ABI") == null) && !variantAbiFilters.isEmpty()
  ...
  private fun getFilter(variantOutput: GenericBuiltArtifact, filterType: String): String? =
