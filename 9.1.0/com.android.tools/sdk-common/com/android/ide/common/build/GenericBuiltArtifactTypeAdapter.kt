...
abstract class CommonBuiltArtifactTypeAdapter<T : CommonBuiltArtifact> : TypeAdapter<T>() {
  ...
  /**
   * Subclasses should use this hook to write their specific attributes to the json output stream.
   *
  ...
  override fun write(out: JsonWriter, value: T?) {
    ...
    value.versionCode?.let { out.name("versionCode").value(it) }
    value.versionName?.let { out.name("versionName").value(it) }
   ...
   *
   * Subclasses specific attributes should be handled by the [handleAttribute] lambda which will be called for any attributes this method
   * does not know about.
   *
   * Instantiation of the [T] object will be handled by the [instantiate] lambda that will be passed with all the parsed attributes as
   * intrinsic types.
   *
  ...
  fun read(
    ...
    instantiate: (outputFile: String, versionCode: Int, versionName: String) -> T,
  ): T {
    ...
    return instantiate(outputFile!!, versionCode, versionName.orEmpty())
  }
...
internal object GenericBuiltArtifactTypeAdapter : CommonBuiltArtifactTypeAdapter<GenericBuiltArtifact>() {
  ...
  override fun read(reader: JsonReader): GenericBuiltArtifact {
    ...
    return super.read(
      ...
      { outputFile: String, versionCode: Int, versionName: String ->
