...
abstract class CommonBuiltArtifactsTypeAdapter<T : CommonBuiltArtifacts, ArtifactTypeT, ElementT>(val projectPath: Path) :
  TypeAdapter<T>() {
  ...
  final override fun write(out: JsonWriter, value: T?) {
    ...
    out.endArray()
    getElementType(value)?.let { elementType -> out.name("elementType").value(elementType) }
        ...
        "baselineProfiles" -> {
          ...
          while (reader.hasNext()) {
            ...
            while (reader.hasNext()) {
              when (val attributeName = reader.nextName()) {
                ...
                "baselineProfiles" -> {
                  reader.beginArray()
                  while (reader.hasNext()) {
                    val baselineProfileFile = projectPath.resolve(reader.nextString()).normalize().toFile()
            ...
            baselineProfiles.add(
              BaselineProfileDetails(minApi ?: error("minApi is required"), maxApi ?: error("maxApi is required"), baselineProfileFiles)
            )
          }
          reader.endArray()
        }
...
class GenericBuiltArtifactsTypeAdapter(projectPath: Path) :
  CommonBuiltArtifactsTypeAdapter<GenericBuiltArtifacts, GenericArtifactType, GenericBuiltArtifact>(projectPath) {

  override val artifactTypeTypeAdapter
    get() = GenericArtifactTypeTypeAdapter
