...
/**
 * Returns a string list of resource symbols in the format 'resourceType:resourceName:resourceId' from the declared and referenced resources
 * from the ResourceDirectory()
 */
fun getResourcesFromDirectory(directory: File): List<String> {
  ...
  directory.walk().filter { it.isFile }.toSet().forEach { getDeclaredAndReferencedResourcesFrom(it, resourceUsageModel) }
...
fun getDeclaredAndReferencedResourcesFrom(
  resourceFile: File,
  resourceUsageModel: ResourceUsageModel = ResourceUsageModel(),
): List<ResourceUsageModel.Resource> {
  val resFolderType = ResourceFolderType.getTypeByName(resourceFile.parentFile.name)
  if (resourceFile.name.endsWith(SdkConstants.DOT_XML) && resFolderType != ResourceFolderType.RAW) {
