...
/**
 * Takes all the values folders from resources and extracts the folder locale names from them. Additionally, if pseudolocales are enabled,
 * add the folder locale names for the pseudolocales. Returns these folder locale names as a set.
 */
fun generateFolderLocaleSet(resources: Collection<File>, pseudoLocalesEnabled: Boolean = false): Set<String> {
  ...
  val allResources =
    resources
      .fold(mutableListOf<File>()) { acc, it ->
        if (it.isDirectory()) {
    ...
    allResources
      .filter { resourceFolder ->
        resourceFolder.name.startsWith("values-") &&
          FolderConfiguration.getConfigFromQualifiers(resourceFolder.name.substringAfter("values-").split("-"))?.localeQualifier != null
      }
...
/**
 * Takes the supported locales text file, and extracts the locales into SupportedLocales format. This reads the locale with the
 * [defaultLocaleSpecifier] before it, signifying it is the default locale, and adds the rest of the locales in folder-name format.
 *
 * The [SupportedLocales] will then provide access to the default locales, and the folder locales by property.
 */
...
fun writeSupportedLocales(output: File, locales: Collection<String>, defaultLocale: String?) {
  if (defaultLocale.isNullOrEmpty()) {
    ...
    output.writeText(listOf("$defaultLocaleSpecifier$defaultLocale", *(locales.toTypedArray())).joinToString("\n"))
  }
}
...
fun validateLocale(locale: String): String? {
  val localeQualifier = LocaleQualifier.parseBcp47(LocaleQualifier.BCP_47_PREFIX + locale.replace("-", "+"))
...
fun writeLocaleConfig(output: File, locales: Set<String>, defaultLocale: String? = null) {
  ...
  if (!defaultLocale.isNullOrBlank()) {
    outLines.add(
      "<locale-config xmlns:android=\"http://schemas.android.com/apk/res/android\" " + "android:defaultLocale=\"$defaultLocale\">"
    )
  } else {
    outLines.add("<locale-config xmlns:android=\"http://schemas.android.com/apk/res/android\">")
  }
  locales.forEach { localeString -> outLines.add("    <locale android:name=\"$localeString\"/>") }
...
fun readResourcesPropertiesFile(inputFiles: Collection<File>): String? {
  ...
  inputFiles.forEach { input ->
    ...
    FileInputStream(input).use { properties.load(it) }
    if (properties.containsKey("unqualifiedResLocale")) {
      ...
      if (defaultLocale != null && defaultLocale != unqualifiedResLocale) {
        throw RuntimeException(
          "Multiple resources.properties files found with " +
