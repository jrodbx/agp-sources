 ...
 * <ul>
 * <li>Resources from a single package. In this mode packageName inside resource objects is always null. Each resource in this mode is
 *   identified by type and name.
 * <li>Resources from multiple packages. In this mode resource is identified by triple: packageName, type and name. This mode is used for
 *   Android applications with dynamic feature modules where each module defines their resources inside its own package and the same
 *   resource name and type may be used by different resources from different modules.
 * </ul>
 */
class ResourceStore(val supportMultipackages: Boolean = false) {

  /** All known resources by id. */
  private val resourceById: MutableMap<ResourceId, Resource> = Maps.newLinkedHashMapWithExpectedSize(TYPICAL_RESOURCE_COUNT)

  /** All resources. */
  val resources: List<Resource>
    get() = Collections.unmodifiableList(_resources)

  private val _resources: MutableList<Resource> = Lists.newArrayListWithCapacity<Resource>(TYPICAL_RESOURCE_COUNT)

  /** Resources partitioned by type and name. */
  private val typeToName: MutableMap<ResourceType, LinkedListMultimap<String, Resource>> = Maps.newEnumMap(ResourceType::class.java)

  /** Map from resource ID value (R field value) to corresponding resource. */
  private val valueToResource: MutableMap<Int, Resource> = Maps.newHashMapWithExpectedSize(TYPICAL_RESOURCE_COUNT)

  /** Set of resource names that are explicitly marked as kept */
  private val keepResources: MutableSet<ResourceId> = Sets.newHashSet()

  /**
   * Recorded list of keep attributes: these can contain wildcards, so they can't be applied immediately; we have to apply them after
  ...
  /**
   * Recorded list of discard attributes: these can contain wildcards, so they can't be applied immediately; we have to apply them after
   * scanning through all resources.
   */
  ...
  /**
   * Whether we should attempt to guess resources that should be kept based on looking at the string pool and assuming some of the strings
   * can be used to dynamically construct the resource names. Can be turned off via `tools:shrinkMode="strict"`.
   */
  var safeMode = true
  ...
  /** Returns resource by triple: packageName, type and name. In a single package mode packageName must be null. */
  fun getResource(packageName: String?, type: ResourceType, name: String): Resource? {
    Preconditions.checkArgument(supportMultipackages || packageName == null, "In a single package mode packageName must be null.")
    return resourceById[ResourceId(type, name, packageName)]
  }

  /**
   * Returns resources by type and name. In single package mode may return empty collection or collection with only one element. In multiple
   * package mode returns resources that have specified type and name from all packages.
   */
  ...
  /**
   * Returns resources which are referenced by specified resource url. In single package mode may return empty collection or collection with
   * only one element. In multiple package mode returns resources that are referenced by specified url from all packages.
   */
  ...
  /**
   * Returns all resources that are referenced by provided {@param webUrl} (for example: file:///android_res/drawable/bar.png). Looks for:
   * <ul>
  ...
  fun getResourcesFromWebUrl(webUrl: String): List<Resource> {
    ...
    return FolderTypeRelationship.getRelatedResourceTypes(folderType).flatMap { getResources(it, name) }
  }

  /**
   * Adds resource to store. In a single package mode, packageName is cleared inside provided {@param resource}. If resource was already
   * added to store but without its resource ID value (value = -1) this method updates resource ID value.
   *
  ...
  fun addResource(resource: Resource): Resource {
    ...
    val updated =
      if (stored == null) {
        ...
        _resources += resource
        typeToName.computeIfAbsent(resource.type) { LinkedListMultimap.create() }.put(resource.name, resource)
        if (resource.value != -1) {
  ...
  /** Returns all resources as collection of maps where each map contains resources of the same type partitioned by name. */
  fun getResourceMaps(): Collection<ListMultimap<String, Resource>> {
    return typeToName.values
  }

  /** Records resources to keep explicitly. Input is a value of 'tools:keep' attribute in a &lt;resources&gt; tag. */
  fun recordKeepToolAttribute(value: String) {
    ...
    Splitter.on(',').omitEmptyStrings().trimResults().split(value).forEach { _keepAttributes += it }
  }

  /** Records resources to discard explicitly. Input is a value of 'tools:discard' attribute in a &lt;resources&gt; tag. */
  fun recordDiscardToolAttribute(value: String) {
    ...
    Splitter.on(',').omitEmptyStrings().trimResults().split(value).forEach { _discardAttributes += it }
  }

  /**
   * Processes all keep and discard rules which were added previously by [recordKeepToolAttribute] and [recordDiscardToolAttribute] and
   * marks all referenced resources as reachable/not reachable respectively.
   *
   * <p>If the same resource is referenced by some keep and discard rule then discard takes precedence.
   *
   * <p>Returns the resources that was marked as kept because of the rules (excluding any that was kept, but then discarded)
   */
  fun processToolsAttributes(): List<Resource> =
    _keepAttributes
      ...
      .toList()
      .minus(_discardAttributes.asSequence().flatMap { getResourcesForKeepOrDiscardPattern(it) }.onEach { it.isReachable = false })
  ...
  fun dumpConfig(): String =
    _resources
      ...
      .map { r ->
        ...
        val actions =
          listOfNotNull("remove".takeUnless { r.isReachable }, "no_obfuscate".takeIf { keepResources.contains(id) }).joinToString(",")
        "${r.type}/${r.name}#$actions"
      }
      .joinToString("\n", "", "\n")

  fun dumpKeepResources(): String = keepResources.map { it.name }.sorted().joinToString(",")
  ...
  fun dumpResourceModel(): String =
    _resources
      ...
      .flatMap { r ->
        ...
        sequenceOf("${r.url} : reachable=${r.isReachable}") + references.map { "    ${it.url}" }
      }
      .joinToString("\n", "", "\n")
  ...
  private fun getResourcesForKeepOrDiscardPattern(pattern: String): Sequence<Resource> {
    ...
    return try {
      ...
      resources.entries().asSequence().filter { regexp.matches(it.key) }.map { it.value }
  ...
  fun merge(other: ResourceStore) {
    ...
    other._discardAttributes.forEach { if (!_discardAttributes.contains(it)) _discardAttributes.add(it) }
    other._keepAttributes.forEach { if (!_keepAttributes.contains(it)) _keepAttributes.add(it) }
    ...
    other._resources.forEach { r ->
      val existing = getResource(r.packageName, r.type, r.name) ?: addResource(Resource(r.packageName, r.type, r.name, r.value))
  ...
  companion object {
    ...
    /**
     * Serialize the given store into a compact string; this will allow the [ResourceStore] to be restored via a call to [deserialize]. This
     * method tries to generate compact output, generate semi-readable output and be low overhead.
     */
    ...
    fun serializeInto(sb: StringBuilder, store: ResourceStore, includeValues: Boolean = true) {
      // Format
    ...
    fun deserialize(s: String): ResourceStore {
      ...
      while (offset < length) {
        ...
        val fromClassName =
          ResourceType.fromClassName(typeString)
      ...
      // Populate resource store
      referenceMap.forEach { (resource, _) -> resourceStore.addResource(resource) }
...
private data class ResourceId(val type: ResourceType, val name: String, val packageName: String?)
