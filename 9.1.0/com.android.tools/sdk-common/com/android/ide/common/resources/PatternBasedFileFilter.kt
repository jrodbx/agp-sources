...
class PatternBasedFileFilter {
  ...
  /**
   * Creates default resource file filter. The default ignored patterns are the same as used by aapt but can be customized via the
   * `ANDROID_AAPT_IGNORE` environment variable.
   */
   ...
   * * Entry can have the flag `<dir>` to match only directories or `<file>` to match only files. Default is to match both.
   * * Entry can be a simplified glob `<prefix>*` or `*<suffix>` where prefix/suffix must have at least 1 character (so that we don't match
   *   a '*' catch-all pattern.)
   ...
   *
   * Similar to constructor(aaptStypeignoredPattern: String) above, but this constructor accepts a list of strings instead of a single
  ...
  fun isIgnored(filePath: String, isDirectory: Boolean): Boolean {
    ...
    for (token in ignoredPatterns) {
      ...
      if (token[tokenIndex] == '*') {
        ...
        ignore =
          token.length - tokenIndex == filePath.length - nameIndex &&
            token.regionMatches(tokenIndex, filePath, nameIndex, filePath.length - nameIndex, ignoreCase = true)
      }
