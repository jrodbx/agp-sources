...
@JvmField val GMAVEN_BASE_URL = System.getenv(GMAVEN_TEST_BASE_URL_ENV_VAR) ?: DEFAULT_GMAVEN_URL

/** Provides information about the artifacts and versions available on maven.google.com */
abstract class GoogleMavenRepository
@JvmOverloads
constructor(
  ...
  fun findVersion(dependency: Dependency, predicate: Predicate<Version>?, allowPreview: Boolean = false): Version? {
    val filter = getDependencyFilter(dependency, predicate)
    val group = dependency.group ?: return null
    if (group == "androidx.work") {
      handleWorkManager(dependency, allowPreview, filter)?.let {
        return it
      }
    }
  ...
  private fun getDependencyFilter(dependency: Dependency, predicate: Predicate<Version>?): (Version) -> Boolean {
  ...
  // snapshot versions.
  private fun getSnapshotVersionFilter(filter: ((Version) -> Boolean)?): (Version) -> Boolean = { candidate ->
    !candidate.isSnapshot && (filter == null || filter(candidate))
  }

  fun findVersion(groupId: String, artifactId: String, filter: Predicate<Version>?, allowPreview: Boolean = false): Version? =
    findVersion(groupId, artifactId, { filter?.test(it) != false }, allowPreview)

  fun findVersion(groupId: String, artifactId: String, filter: ((Version) -> Boolean)? = null, allowPreview: Boolean = false): Version? {
  ...
  fun getVersions(dependency: Dependency, predicate: Predicate<Version>?): Sequence<Version> {
  ...
  /** Like [findCompileDependencies], but you can specify a specific POM scope to match, such as "runtime" */
  ...
  protected open fun getPackageMap(): Map<String, PackageInfo> {
    if (packageMap == null) {
      ...
      findData("master-index.xml")?.use { readMasterIndex(it, map) { tag -> PackageInfo(tag) } }
      packageMap = map
    }
  ...
  protected fun <T : PackageInfo> readMasterIndex(stream: InputStream, map: MutableMap<String, T>, factory: (String) -> T) =
    try {
      stream.use {
  ...
  protected data class ArtifactInfo(val id: String, val versions: String) {
    ...
    fun getVersions(): Sequence<Version> = versions.splitToSequence(",").map { Version.parse(it) }

    fun getAgpVersions(): Sequence<AgpVersion> = versions.splitToSequence(",").map { AgpVersion.tryParse(it) }.filterNotNull()

    fun findVersion(filter: ((Version) -> Boolean)?, allowPreview: Boolean = false): Version? =
      getVersions().filter { filter == null || filter(it) }.filter { allowPreview || !it.isPreview }.maxOrNull()

    fun findDependencies(version: Version, packageInfo: PackageInfo, requiredScope: String = "compile"): List<Dependency> {
      return dependencyInfo[version] ?: loadDependencies(version, packageInfo, requiredScope)
    }

    private fun loadDependencies(version: Version, packageInfo: PackageInfo, requiredScope: String): List<Dependency> {
  ...
  protected open inner class PackageInfo(private val pkg: String) {
    ...
    private fun readDependenciesFromPomFile(stream: InputStream, file: String, requiredScope: String): List<Dependency> {
