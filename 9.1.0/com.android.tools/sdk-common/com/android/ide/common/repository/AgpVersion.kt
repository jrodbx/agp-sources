...
/**
 * This class is intended to represent versions of the Android Gradle Plugin (and things which by their nature have a version number
 * identical to AGP, such as individual artifacts), and no other versions. This class captures knowledge of how historical versions of AGP
 * were spelt, and its assumptions about version ordering and constraints on the form of the version are not generally applicable to
 * arbitrary Gradle artifacts. Do not use this to represent version numbers of software that is not the Android Gradle Plugin.
 *
 * This class is deliberately non-comparable with [Version], for two reasons: firstly, the ordering semantics we depend on for `-dev`
 * versions are incompatible with the specified ordering of Gradle version specifiers; secondly, we use Android Gradle Plugin versions as
 * version identifiers in various places in the IDE (particularly in sync-related areas) and it would be a semantic error to compare such a
 * version with a version of a generic (Gradle) artifact.
 */
class AgpVersion private constructor(val major: Int, val minor: Int, val micro: Int, val previewKind: PreviewKind, val preview: Int?) :
  ...
  @JvmOverloads constructor(major: Int, minor: Int, micro: Int = 0) : this(major, minor, micro, PreviewKind.NONE, null)
  ...
  fun isAtLeast(major: Int, minor: Int, micro: Int, previewType: String?, previewVersion: Int, isSnapshot: Boolean): Boolean {
  ...
  companion object {
    ...
    private fun isTwoDigitPreviewFormat(major: Int, minor: Int, micro: Int, previewKind: PreviewKind): Boolean {
      return major > 3 || major == 3 && minor > 1 || major == 3 && minor == 1 && micro == 0 && previewKind != PreviewKind.BETA
    }

    private fun parsePreviewString(major: Int, minor: Int, micro: Int, previewKind: PreviewKind, previewString: String): Int {
      if (isTwoDigitPreviewFormat(major, minor, micro, previewKind) && previewString.length != 2) {
        throw NumberFormatException(
          "AgpVersion $major.$minor.$micro$previewKind requires" + " a two digit preview, but received \"$previewString\"."
        )
      }
      if (!isTwoDigitPreviewFormat(major, minor, micro, previewKind) && previewString.startsWith("0")) {
        throw NumberFormatException(
          "AgpVersion $major.$minor.$micro$previewKind requires" + " no zero-padding, but received \"$previewString\"."
        )
      }
    ...
    /**
     * Attempt to parse {@param value} as a String corresponding to a valid AGP version. The (regular) language recognized is:
     *
     * NUM "." NUM "." NUM (PREVIEW-KIND digit digit? | "-dev")? where NUM = "0" | [1-9] digit* and PREVIEW-KIND = "-" ("alpha" | "beta" |
     * "rc")
     *
     * After the regular language is recognized, we additionally impose a constraint on the numeric preview value, corresponding to the
     * versions actually released: in the 3.0.0 series and earlier, and in the 3.1.0-beta series, numeric preview versions were used with no
     * padding; in 3.1.0-alpha and 3.1.0-rc and all later versions, numeric preview versions were zero-padded with a field width of 2.
     *
      ...
      return try {
        ...
        val preview: Int? =
          when (previewKind) {
            PreviewKind.ALPHA,
            PreviewKind.BETA,
            PreviewKind.RC -> parsePreviewString(major, minor, micro, previewKind, matchList[4])
    ...
    /**
     * Parse [value] as a String corresponding to a valid AGP version. See [tryParse] for details on the version string format recognized.
     *
     * @param value a String
    ...
    fun parse(value: String): AgpVersion {
      return tryParse(value) ?: throw IllegalArgumentException("'$value' is not a valid AGP version string.")
    }
    ...
    /**
     * Attempt to parse [value] as a String corresponding to a valid stable AGP version. The (regular) language recognized is:
     *
    ...
    /**
     * Parse [value] as a String corresponding to a valid stable AGP version. See [tryParseStable] for details on the version string format
     * recognized.
     *
     * @param value a String
     * @return an AgpVersion object corresponding to [value].
    ...
    fun parseStable(value: String): AgpVersion {
      return tryParseStable(value) ?: throw IllegalArgumentException("'$value' is not a valid stable AGP version string.")
    }
  }
}
