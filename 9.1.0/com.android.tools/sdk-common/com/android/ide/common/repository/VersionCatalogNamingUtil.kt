...
private infix fun ((String) -> String).compose(second: (String) -> String) = { arg: String -> second(this(arg)) }

/**
 * Pick name with next steps. Each step generates name and check whether it exists in reserved set where comparison is done in
 * case-insensitive way. If same name already exists, it tries the next step. Steps defined as follows:
 * - if artifact is androidx.compose and any reserved alias has androidx-compose prefix - add "androidx-compose-" prefix;
 * - if artifact is androidx and any reserved alias has androidx prefix - add "androidx-" prefix;
 * - in other case just use artifactID "org.jetbrains.kotlin:kotlin-reflect" => "kotlin-reflect";
 * - if name already exists - add group suffix if it's not similar to name "com.google.libraries:guava" => "libraries-guava";
 * - if already exists - add group prefix "org.jetbrains.kotlin:kotlin-reflect" => "jetbrains-kotlin-reflect";
 * - if already exists - add group "org.jetbrains.kotlin:kotlin-reflect" => "jetbrains-kotlin-kotlin-reflect";
 * - if already exists - add number at the end until no such name in reserved "jetbrains-kotlin-reflect" => "jetbrains-kotlin-reflect2".
 */
fun pickLibraryVariableName(dependency: Dependency, includeVersionInKey: Boolean, caseSensitiveReserved: Set<String>): String {
...
private val obfuscateReservedWords: (String) -> String =
  setOf(
    ...
    .let { set ->
      { alias: String ->
        if (!alias.contains("-")) alias else alias.split("-").joinToString("-") { if (set.contains(it)) "${it}1" else it }
      }
    }
...
private fun getMaybeTransformVersionToCamelCase(reserved: Set<String>): (String) -> String {
  ...
  return { version -> if (camelCaseOutput) CaseFormat.LOWER_HYPHEN.to(CaseFormat.LOWER_CAMEL, version) else version }
}

/**
 * Pick name with next steps. Each step generates name and check whether it exists in reserved set where comparison is done in
 * case-insensitive way. If same name already exists, it tries the next step. Steps defined as follows:
 * - pluginId will cut root domain and transform "." to hyphens "org.android.application" => android-application
 * - in other case it will transform "." to hyphens for whole pluginId "org.android.application" => org-android-application
 * - if already exists - add "X" + /number/ at the end until no such name in reserved "org.android.application" => org-android-applicationX2
 */
fun pickPluginVariableName(pluginId: String, caseSensitiveReserved: Set<String>): String {
  val reserved = TreeSet(String.CASE_INSENSITIVE_ORDER)
  reserved.addAll(caseSensitiveReserved)
...
internal fun String.toSafeHyphenKey(): String = this.toSafeKey().replace("_", "-")
...
/**
 * Variable name generator takes artifact id as a base. It analyses reserved aliases and chose same notation. This can be a lower camel or
 * hyphen case. notation otherwise. Lower camel is preferable - if no reserved, algorithm will pick lower camel.
 *
 * Picking variable name happens in steps. Each step generates some name around artifact id with hyphen and transform to camel case if it is
 * the style. Then check whether it's reserved with case-insensitive set. If yes, jumping to the next step. Steps defined as follows:
 ...
 */
fun pickVersionVariableName(dependency: Dependency, caseSensitiveReserved: Set<String>, includeVersionInKey: Boolean = false): String {
...
/**
 * Variable name generator takes plugin id as a base. It analyses reserved aliases and chose same notation. This can be a lower camel or
 * hyphen case. Picking variable happens in steps. Each step generates some name around updated plugin id and check whether it's reserved
 * with case-insensitive set. If yes, jumping to the next step. Steps defined as follows:
 ...
 * - use full pluginId.
 * - use pluginId + /number/. Those steps are similar to picking library version
 */
fun pickPluginVersionVariableName(pluginId: String, caseSensitiveReserved: Set<String>): String {
  val reserved = TreeSet(String.CASE_INSENSITIVE_ORDER)
  reserved.addAll(caseSensitiveReserved)
  val plugin = cutDomainPrefix(pluginId)
...
fun keysMatch(keyText: String?, reference: String): Boolean {
  ...
  for (i in keyText.indices) {
    if (isAfterDelimiter(i, keyText)) {
      ...
      if (keyText[i].normalizeIgnoreCase() != reference[i].normalize()) return false
...
private fun isAfterDelimiter(index: Int, s: String): Boolean = index > 0 && s[index - 1].normalize() == '.'
