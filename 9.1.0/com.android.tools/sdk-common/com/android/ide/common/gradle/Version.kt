...
/**
 * This class represents a single version with the semantics of version comparison as specified by Gradle. It explicitly does not represent:
...
class Version : Comparable<Version>, Serializable {
  ...
  /**
   * this is only non-private in order to provide error-checking in legacy class constructors; it should not be generally used. If true,
   * this represents the infimum of the set of versions with the specified parts as a prefix ("infimum" because there is no finite
   * representation of such a version).
   */
  val isPrefixInfimum: Boolean

  // Used for serialization by the IDE.
  private constructor() : this(parts = listOf(DEV("dev")), separators = listOf(Separator.EMPTY), isPrefixInfimum = true)
  ...
  private fun extendedParts(atLeast: Int): List<Part> =
    DEV("dev").let { dev -> parts.let { if (isPrefixInfimum) it + List(max(0, atLeast - it.size)) { dev } else it } }
  ...
  val previewInfimum
    get() =
      parts
        .indexOfFirst { it !is Numeric }
        .takeIf { it > -1 }
        ?.let {
          when (it) {
            0 -> prefixInfimum("dev")
            else -> Version(parts.subList(0, it), separators.subList(0, it - 1) + Separator.EMPTY, true)
          }
        }
    ...
    get() =
      parts
        ...
        ?.let {
          when (it) {
            0 -> parse("0")
            else -> Version(parts.subList(0, it), separators.subList(0, it - 1) + Separator.EMPTY, false)
          }
        }
  ...
  val minor
    get() = (parts.takeIf { it.size > 1 && it[0] is Numeric }?.get(1) as? Numeric)?.number.toIntOrNull()

  val micro
    get() = (parts.takeIf { it.size > 2 && it[0] is Numeric && it[1] is Numeric }?.get(2) as? Numeric)?.number.toIntOrNull()

  /** Return a Version suitable as an exclusive upper bound from considering the parts of this version up to [prefixSize] as a prefix. */
  fun nextPrefix(prefixSize: Int): Version {
    if (parts.size < prefixSize) {
      return Version(
        parts + List(prefixSize - parts.size) { Numeric("0") },
        separators.let { it.dropLast(1) + List(prefixSize - parts.size) { Separator.DOT } + it.last() },
        true,
      )
    } else {
      return Version(
  ...
  /** Return a Version suitable as an exclusive upper bound from considering this version as a prefix. */
  fun nextPrefix(): Version = nextPrefix(parts.size)

  /**
   * Return a [Version] corresponding to the prefix of this version: if it is not an infimum, the return value is equal to this version; if
   * it is an infimum, the return value is the specified prefix.
   */
  ...
  override fun equals(other: Any?) =
    when (other) {
      is Version -> (1 + max(this.parts.size, other.parts.size)).let { size -> this.extendedParts(size) == other.extendedParts(size) }
      else -> false
    }
  ...
  companion object {
    ...
    /**
     * Parse a string corresponding to an exact version (as defined by Gradle). The result is Comparable, implementing the ordering
     * described in the Gradle user guide under "Version ordering", compatible with determining whether a particular version is included in
     * a range (but not, directly, implementing the concept of "base version" used in conflict resolution).
     */
    @JvmStatic fun parse(string: String): Version = doParse(string, false)

    /**
     * Parse a string corresponding to a version, and return a [Version] which represents the least possible version with the given string
     * as a prefix (including an implicit separator). Conceptually, this is the same as the version denoted by [string] followed by an
     * infinite sequence of `dev` parts.
     */
    @JvmStatic fun prefixInfimum(string: String): Version = doParse(string, true)
  }
}

fun BigInteger?.toIntOrNull() =
  when {
    this == null -> null
...
class NonNumeric(string: String) : Part(string) {
  ...
  override fun compareTo(other: Part) =
    when (other) {
      is DEV -> 1
      is Special,
      is Numeric -> -1
...
class Numeric(string: String) : Part(string) {
  ...
  @Transient private var _number: BigInteger? = null
