 ...
 * - the range corresponding to a maven-style version range, meaning versions within that range (modified by prefix matching if the upper
 *   bound is exclusive).
 *
 * Note: This is not a [Range] by inheritance, as [Range] is final, while we need to customize printing. It nevertheless supports the public
 * interface of [Range] with [Version] domain.
 *
 * This is intended to support string specifiers of artifact versions in Gradle build files. It does not attempt to model directly the
 * strictly/required/preferred hierarchy of specifications.
 */
class VersionRange private constructor(private val range: Range<Version>, private val identifier: String?) {
  ...
  /** Return a string that will produce the same [VersionRange] when parsed, or `null` if no such identifier exists. */
  ...
  val isSingleton
    get() = range.isSingletonRange()

  /** If the range contains exactly one (non-prefixInfimum) version, return it. */
  val singletonVersion
    get() = if (range.isSingletonRange()) lowerEndpoint() else null

  @Suppress("WHEN_ENUM_CAN_BE_NULL_IN_JAVA")
  internal fun withoutUpperBound(): VersionRange =
    when {
      !hasUpperBound() -> this
  ...
  companion object {
    /**
     * Matches a String consisting entirely an opening range indicator, followed by zero or more non-commas, a comma, zero or more
     * non-commas, and a closing range indicator.
     */
    internal val MAVEN_STYLE_REGEX = "^[\\[(\\]][^,]*,[^,]*[\\[)\\]]$".toRegex()

    /**
     * Parse a string as a [VersionRange]. All strings are valid version ranges: they are one of: the universal range `+`; a Maven-style
     * range; a prefix range; or a range consisting of a single [Version]. Maven-style ranges representing an invalid range (e.g. because
     * the lower bound is higher than the upper bound) are treated as an empty range.
     */
    @JvmStatic
    ...
    private fun parseMavenRange(string: String): Range<Version> {
      ...
      val higherVersion = higherString.let { if (higherClosed) Version.parse(it) else Version.prefixInfimum(it) }
      return when {
  ...
  hasLowerBound() &&
    ...
    let {
      ...
      lowerBoundType() == BoundType.CLOSED &&
        lower.isPrefixInfimum &&
        upperBoundType() == BoundType.OPEN &&
        upper.isPrefixInfimum &&
...
private fun Range<Version>.isSingletonRange() =
  hasLowerBound() &&
    ...
    let {
      val lower = lowerEndpoint()
      val upper = upperEndpoint()
      lowerBoundType() == BoundType.CLOSED && !lower.isPrefixInfimum && upperBoundType() == BoundType.CLOSED && lower == upper
    }

private fun Range<Version>.validLowerBoundForMavenRange() = !hasLowerBound() || !lowerEndpoint().isPrefixInfimum
