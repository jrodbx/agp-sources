 ...
 *
 * A resource type identifies the group or resource. Resources in Android can have various types: drawables, strings, etc. The full list of
 * supported resource types can be found in `com.android.resources.ResourceType`.
 *
 * The name of the symbol has to be a valid java identifier and is usually the file name of the resource (without the extension) or the name
 * of the resource if the resource is part of an XML file.
 *
 * Names of resources declared in XML files can contain dots and colons, these are replaced by underscores when accessed from java. To
 * sanitize the resource name, call [canonicalizeValueResourceName].
 *
 * For example, the resource `drawable/foo.png` has name `foo`. The string `bar` in file `values/strings.xml` with name `bar` has resource
 * name `bar`.
 *
 * The java type is the java data type that contains the resource value. This is generally `int`, but other values (such as `int[]`) are
 * allowed. Type should not contain any whitespaces, be `null` or empty.
 *
 * The value is a java expression that conforms to the resource type and contains the value of the resource. This may be just an integer
 * like `3`, if the resource has type `int`. But may be a more complex expression. For example, if the resource has type `int[]`, the value
 * may be something such as `{1, 2, 3}`.
 *
...
sealed class Symbol {
  ...
  companion object {
     ...
     *
     * The `cannonicalName` of the symbol will be the sanitized resource name (See [canonicalizeValueResourceName].)
     *
    ...
    fun createSymbol(
      ...
      return createSymbol(resourceType, name, idProvider.next(resourceType), isMaybeDefinition, validation)
    }
    ...
    fun createSymbol(
      ...
      return if (resourceType == ResourceType.ATTR) {
        attributeSymbol(name = name, intValue = value, isMaybeDefinition = isMaybeDefinition)
      } else {
        normalSymbol(resourceType = resourceType, name = name, intValue = value)
      }
    }
     ...
     * ```
     *
     * corresponds to a StylableSymbol with value `[0x7f040001,0x7f040002]` and children `["attr1", "attr2"]`.
     *
     * @param name the sanitized name of the symbol
     * @param validation check the `name` of the symbol is a valid resource name.
     */
    @JvmStatic
    fun createStyleableSymbol(
      name: String,
    ...
    /**
     * Checks whether the given resource name meets all the criteria: cannot be null or empty, cannot contain whitespaces or dots. Also
     * checks if the given resource type is a valid.
     *
    ...
    private fun validateSymbol(name: String, resourceType: ResourceType) {
      try {
        ...
        throw IllegalArgumentException("Validation of a resource with name '$name' and type " + "'${resourceType.getName()}' failed.'", e)
      }
    }
    ...
    fun normalSymbol(
      ...
      Preconditions.checkArgument(resourceType != ResourceType.ATTR, "Internal Error: Attributes must be represented by AttributeSymbol.")
  ...
  abstract class NormalSymbol internal constructor() : Symbol() {
    final override fun toString(): String = "$resourceVisibility $resourceType $canonicalName = 0x${intValue.toString(16)}"
   ...
   *
   * The use of the data class generated equals method means that it is vital that this class is never used to represent a symbol that could
   * be represented by [BasicNormalSymbol]
   */
  ...
  internal data class BasicNormalSymbol(override val resourceType: ResourceType, override val name: String) : NormalSymbol() {
    override val intValue: Int
      get() = 0
   ...
   *
   * The use of the data class generated equals method means that it is vital that this class is never used to represent a symbol that could
   * be represented by [BasicAttributeSymbol]
   */
  ...
  internal data class BasicAttributeSymbol(override val name: String) : AttributeSymbol() {
    override val intValue: Int
      get() = 0
  ...
  abstract class StyleableSymbol : Symbol() {
    ...
    final override fun getValue(): String =
      StringBuilder(values.size * 12 + 2)
        .apply {
          append("{ ")
   ...
   *
   * The use of the data class generated equals method means that it is vital that this class is never used to represent a symbol that could
   * be represented by [BasicStyleableSymbol]
   */
  internal data class StyleableSymbolImpl
  constructor(
    override val name: String,
    override val values: ImmutableList<Int>,
    /**
     * list of the symbol's children in order corresponding to their IDs in the value list. For example:
     * ```
     * int[] styleable S1 {0x7f040001,0x7f040002}
     * int styleable S1_attr1 0
     * int styleable S1_attr2 1
     * ```
     *
     * corresponds to a Symbol with value `"{0x7f040001,0x7f040002}"` and children `{"attr1", "attr2"}`.
     */
  ...
  internal data class BasicStyleableSymbol constructor(override val name: String, override val children: ImmutableList<String>) :
    StyleableSymbol() {
    override val values: ImmutableList<Int>
