...
/**
 * Parser method that can read a [SymbolTable] from a resource XML file. Resource XML files contain zero or multiple resources of the
 * following types:
 * <table>
 ...
 * <tr><td>XML </td><td>`xml` </td><td>`xml` </td><td>`int` </td></tr> </table>
 *
 * (*1) Resources can be also declared in an extended form where the XML Tag is `item` and the attribute `type` specifies whether the
 * resource is an `attr`, a `string` et cetera. Therefore a construction like this:
 * <pre>
 * <resources>
 ...
 * </pre>
 *
 * It is also worth noting that some resources can be declared with a prefix like `aapt:` or `android:`. Following aapt's original
 * behaviour, we strip the type names from those prefixes. This behaviour is deprecated and might be the support for it might end in the
 * near future.
 *
 * (*2)The mapping of `declare-styleable` to symbols is complex. For each styleable, a symbol of resource type `styleable` is created of
 * java type `int[]`. For each attribute (`attr`) in the `declare-styleable` a child of that parent is created and added under that symbol
 * as well as a symbol of resource type `attr` with java type `int`. In case of the symbol with `styleable` type, its name is the symbol
 * name of the `declare-styleable` element joined with the name of the `attr` element by an underscore character. The value of the int array
 * in the `declare-styleable` contains the IDs of all `styleable` symbols. So, for example, the following XML:
 * <pre>
 * <resources>
 ...
 * }
 * </pre>
 * Constructs a [SymbolTable] from the given parsed XML document. The values for the resource are drawn from the given provider. For testing
 * purposes, this method guarantees that IDs are assigned in the order the resources are provided in the XML document. However, this
 * guarantee is only to make testing simpler and non-test code should not rely on this assumption as it may change, along with the required
 * refactoring of the test code.
 *
...
fun parseValuesResource(
  ...
  val root = xmlDocument.documentElement ?: throw ResourceValuesXmlParseException("XML document does not have a root element.")
  ...
  while (current != null) {
    if (current.nodeType == Node.ELEMENT_NODE) {
      parseChild((current as Element?)!!, builder, idProvider, enumSymbols, platformAttrSymbols, validation)
    }
...
private fun parseChild(
  ...
  val resourceType =
    ResourceType.fromXmlTag(child) ?: throw ResourceValuesXmlParseException("Unknown resource value XML element '${toXml(child)}'")
  ...
  when (resourceType) {
    ...
    ResourceType.XML -> builder.add(Symbol.createSymbol(resourceType, name, idProvider, validation))
    ResourceType.STYLEABLE ->
      // We also need to find all the attributes declared under declare styleable.
      parseDeclareStyleable(child, idProvider, name, builder, enumSymbols, platformAttrSymbols, validation)
    ResourceType.ATTR ->
    ...
    else -> throw ResourceValuesXmlParseException("Unknown resource value XML element '${toXml(child)}'")
  }
}

/**
 * Parses a declare styleable element and finds all it's `attr` children to create new Symbols for each them: a `styleable` Symbol with the
 * name which is a concatenation of the declare styleable's name, an underscore and the child's name; and a `attr` Symbol with the name
 * equal to the child's name.
 *
...
private fun parseDeclareStyleable(
  ...
  while (attrNode != null) {
    ...
    if (tagName != ResourceType.ATTR.getName() || attrElement.namespaceURI != null) {
      ...
      throw ResourceValuesXmlParseException("Illegal type under declare-styleable: was <$tagName>, only accepted is " + "<attr>")
    }
        ...
        if (platformAttrSymbols == null) {
          ...
          val attrSymbol =
            platformAttrSymbols.symbols.get(ResourceType.ATTR, realAttrName)
              ?: throw ResourceValuesXmlParseException("Unknown android attribute '$attrName' under '$styleableName")
  ...
  builder.add(Symbol.createStyleableSymbol(styleableName, attrValues.build(), attrNames.build(), validation))
}

/**
 * Parses an attribute element and finds all it's `enum` children to create new Symbols for each of them: an `id` Symbol with the name equal
 * to the child's name.
 *
...
private fun parseAttr(
  ...
  while (enumNode != null) {
    ...
    val newEnum = Symbol.createSymbol(ResourceType.ID, getMandatoryAttr(enumElement, "name"), idProvider, validation = validation)

    enumSymbols.add(newEnum)
    enumNode = enumNode.nextSibling
  }

  val newAttr = Symbol.createSymbol(ResourceType.ATTR, name, idProvider, isMaybeDefinition, validation)

  if (!builder.contains(newAttr)) {
    // If we haven't encountered this attribute yet, add the new one.
    builder.add(newAttr)
    return newAttr.intValue
  } else if (!isMaybeDefinition && (builder[newAttr] as Symbol.AttributeSymbol).isMaybeDefinition) {
...
private fun getMandatoryAttr(element: Element, attrName: String): String {
  val attr =
    element.getAttributeNodeNS(null, attrName)
      ?: throw ResourceValuesXmlParseException("Element '${element.tagName}' should have attribute '$attrName'")
  return attr.value
}
