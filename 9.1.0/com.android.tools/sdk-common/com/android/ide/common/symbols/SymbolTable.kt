...
/**
 * List of [Symbols] identifying resources in an Android application. [SymbolTables] do not only exist for applications: they can be used
 * for other building blocks for applications, such as libraries or atoms.
 *
 * A [SymbolTable] keeps a list of instances of [Symbol], each one with a unique pair of class and name. [SymbolTable]s also have a
 * [tablePackage] property. This should be unique and is used to generate the `R.java` file.
 */
@Immutable
abstract class SymbolTable protected constructor() {
  ...
  private data class SymbolTableImpl(
    ...
    override val symbols: ImmutableTable<ResourceType, String, Symbol>,
  ) : SymbolTable() {

    override fun toString() = symbols.values().joinToString("\n  ", "SymbolTable ($tablePackage)\n ")
  }

  /**
   * Produces and returns a subset of this [SymbolTable] including only [Symbol]s that are also included in [table]. In other words, a
   * [Symbol] `s` will exist in the result if and only if `s` exists in `this` and there is a symbol `s1` in [table] such that
    ...
    for (resourceType in table.symbols.rowKeySet()) {
      ...
      for (s1 in table.symbols.row(resourceType).values) {
        if (s1.canonicalName in allowedNames) {
          this.symbols.get(resourceType, s1.canonicalName)?.let { s -> builder.put(resourceType, s1.canonicalName, s) }
        }
      }
    }
  ...
  /** . Builds and returns a new [SymbolTable] with the same symbols as `this`, but renamed with [tablePackage]. */
  ...
  fun getSymbolByVisibility(visibility: ResourceVisibility): List<Symbol> {
    val symbols = symbols.values().filter { it.resourceVisibility == visibility }.sortedBy(Symbol::canonicalName)
  ...
  fun containsSymbol(type: ResourceType, canonicalName: String): Boolean {
    ...
    return type == ResourceType.STYLEABLE && maybeGetStyleableParentSymbolForChild(canonicalName) != null
  }
   ...
   *     </declare-styleable>
   * </pre>
   * Calling `containsStyleableSymbol("s1_foo")` would return the parent [Symbol], but calling `containsStyleableSymbol("foo")` or
   * `containsSymbol(STYLEABLE, "foo")` would both return `null`.
   */
  tailrec fun maybeGetStyleableParentSymbolForChild(canonicalName: String, start: Int = 0): Symbol.StyleableSymbol? {
    ...
    if (parent != null) {
      ...
      if (childName.startsWith(ANDROID_ATTR_PREFIX)) {
        val childNameWithColon = SdkConstants.ANDROID_NS_NAME_PREFIX + childName.drop(ANDROID_ATTR_PREFIX.length)
  ...
  /** Creates a table with all the symbols taken from [this] table, but the values are from [mainSymbolTable] */
  fun withValuesFrom(mainSymbolTable: SymbolTable): SymbolTable = mainSymbolTable.filter(this).rename(this.tablePackage)

  /** Returns the [ResourceType]s present in the table. */
  val resourceTypes: Set<ResourceType>
    get() = symbols.rowKeySet()
  ...
  class Builder {
    ...
    private val symbols: Table<ResourceType, String, Symbol> = Tables.newCustomTable(Maps.newEnumMap(ResourceType::class.java), ::HashMap)

    /** Adds [symbol] to the table to be built. The table must not contain a [Symbol] with the same resource type and name. */
    fun add(symbol: Symbol) = apply {
      require(!symbols.contains(symbol.resourceType, symbol.canonicalName)) {
        "Duplicate symbol in table with resource type '${symbol.resourceType}' and " + "symbol name '${symbol.canonicalName}'"
      }
      symbols.put(symbol.resourceType, symbol.canonicalName, symbol)
    }

    /** Adds all [Symbol]s in [symbols] to the table. This is semantically equivalent to calling [add] for each element in [symbols]. */
    ...
    fun addAllIfNotExist(symbols: Collection<Symbol>) = apply { symbols.forEach { if (!contains(it)) add(it) } }

    /** Adds [symbol] if it doesn't exist in the table yet. If [symbol] is already present, chooses the correct resource accessibility. */
    private fun addFromPartialHelper(symbol: Symbol) {
      ...
      if (
        existing.resourceVisibility != symbol.resourceVisibility &&
      ...
      val children = ImmutableList.copyOf((symbol.children + existing.children).distinct().sorted())

      val visibility = ResourceVisibility.max(symbol.resourceVisibility, existing.resourceVisibility)

      this.symbols.remove(existing.resourceType, existing.canonicalName)
      this.symbols.put(
        symbol.resourceType,
        symbol.canonicalName,
        Symbol.styleableSymbol(symbol.canonicalName, ImmutableList.of(), children, visibility),
      )
    }

    /**
     * Adds all [Symbol]s from [table] if they don't exist in the table yet. If a [Symbol] is already present, chooses the correct resource
     * accessibility.
     */
    internal fun addFromPartial(table: SymbolTable) = apply { table.symbols.values().forEach { addFromPartialHelper(it) } }

    /** Sets the table package to be [tablePackage]. See [SymbolTable] description. */
    fun tablePackage(tablePackage: String) = apply { this.tablePackage = validate(tablePackage) }

    /** Returns whether a [Symbol] with the same resource type and name as [symbol] has been added. */
    operator fun contains(symbol: Symbol): Boolean = contains(symbol.resourceType, symbol.canonicalName)

    /** Returns whether the table contains a [Symbol] with the [resourceType] and [canonicalName]. */
    fun contains(resourceType: ResourceType, canonicalName: String): Boolean = symbols.contains(resourceType, canonicalName)

    /** Returns the [Symbol] from the table matching the provided [symbol], or `null`. */
    operator fun get(symbol: Symbol): Symbol? = symbols.get(symbol.resourceType, symbol.canonicalName)

    fun remove(resourceType: ResourceType, canonicalName: String): Symbol? = symbols.remove(resourceType, canonicalName)
  ...
  class FastBuilder(private val symbolInterner: Interner<Symbol>) {
    ...
    private val symbols: ImmutableTable.Builder<ResourceType, String, Symbol> = ImmutableTable.builder()
    ...
    /** Adds all [Symbol]s in [symbols] to the table. This is semantically equivalent to calling [add] for each element of [symbols]. */
  ...
  companion object {
     ...
     *
     * The merge is order-sensitive: when multiple [Symbols] with the same class / name exist in multiple [SymbolTables], the first one will
     * be used. The package of the resultant [SymbolTable] will be either that of the first element of [tables] or the default if [tables]
     * is empty.
     */
    @JvmStatic
    ...
    /**
     * Merges all partial R [File]s in [tables], with package name [packageName]. See 'package-info.java' for a detailed description of the
     * merging algorithm.
     */
    @JvmStatic
      ...
      tables
        ...
        .forEach {
          try {
            ...
            throw PartialRMergingException("An error occurred during merging of the partial R files", e)
          }
        }
...
private fun validate(tablePackage: String): String {
  ...
  for (segment in Splitter.on('.').split(tablePackage)) {
    require(SourceVersion.isIdentifier(segment)) { "$msg as '$segment' is not a valid Java identifier." }
