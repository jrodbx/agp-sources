...
/**
 * Returns a path tree map containing the given pairs. All operations on a path tree map run in constant time with respect to the number of
 * elements, but there is a much higher constant overhead than for path hash maps. This conversion method runs in linear time.
 */
...
/**
 * Returns an empty path tree map. All operations on a path tree map run in constant time with respect to the number of elements, but there
 * is a much higher constant overhead than for path hash maps. This conversion method runs in linear time.
 */
fun <T> pathTreeMapOf(): MutablePathMap<T> = PathTreeMapImpl()

/**
 * Converts the given [Map] into a tree path map. All operations on a tree map run in constant time with respect to the number of elements,
 * but there is a much higher constant overhead than for path hash maps. This conversion method runs in linear time.
 */
...
/**
 * Represents a node in the [PathTreeMapImpl]. Parent nodes link to their children, but children do not link back to their parents.
 * Different subclasses will use different mechanisms for identifying and storing their children.
 */
internal abstract class PathMapEntry<T>(
  /**
   * Indicates the segment index that this entry will match against. A [PathString] will only match this entry if its [PathString.nameCount]
   * matches this value exactly. Negative values are permitted, and indicate an entry that can never be considered an exact match for any
   * [PathString]. This is used when creating entries to represent protocols or roots which are just containers for the subtrees that match
   * actual paths.
   */
  val segmentIndex: Int
) {
  /**
   * True iff this entry was explicitly inserted into the map. If false, this entry was just created as a container for other entries but it
   * should not be exposed as an entry in the map via the public interface.
   */
  ...
  abstract fun getOrCreateChild(filesystem: URI, root: String?, segments: List<String>): PathMapEntry<T>
...
internal class ProtocolPathMapEntry<T> : PathMapEntry<T>(-2) {
  ...
  override fun getOrCreateChild(filesystem: URI, root: String?, segments: List<String>): PathMapEntry<T> =
    filesystems.getOrPut(filesystem, { RootPathMapEntry() })

  override fun getChild(filesystem: URI, root: String?, segments: List<String>): PathMapEntry<T>? = filesystems[filesystem]

  override fun iterator(): Iterator<PathMapEntry<T>> = filesystems.values.iterator()
}
...
internal class RootPathMapEntry<T> : PathMapEntry<T>(-1) {
  ...
  override fun getChild(filesystem: URI, root: String?, segments: List<String>): PathMapEntry<T>? = roots[root]

  override fun iterator(): Iterator<PathMapEntry<T>> = roots.values.iterator()
}

/** Implementation of [PathMapEntry] that indexes its children using a particular segment of the path. */
internal class SegmentPathMapEntry<T>(nameCount: Int) : PathMapEntry<T>(nameCount) {
  ...
  override fun getChild(filesystem: URI, root: String?, segments: List<String>): PathMapEntry<T>? = children[segments[segmentIndex]]
...
/**
 * An implementation of [MutablePathMap] based on a tree. All operations run in constant time, but there is a higher coefficient than with
...
internal class PathTreeMapImpl<T> : MutablePathMap<T>, Sequence<T> {
  ...
  override fun containsKeyStartingWith(key: PathString): Boolean = findEntry(key) != null

  override fun containsPrefixOf(key: PathString): Boolean = findMostSpecificEntry(key) != null

  override fun findMostSpecific(key: PathString): T? = findMostSpecificEntry(key)?.value
  ...
  private fun findEntry(filesystem: URI, root: String?, segments: List<String>): PathMapEntry<T>? {
    var current: PathMapEntry<T> = tree
    while (current.segmentIndex < segments.size) {
  ...
  private fun getOrCreateEntry(filesystem: URI, root: String?, segments: List<String>): PathMapEntry<T> {
    var current: PathMapEntry<T> = tree
    while (current.segmentIndex < segments.size) {
