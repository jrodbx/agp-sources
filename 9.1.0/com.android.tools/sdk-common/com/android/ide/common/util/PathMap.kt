...
/**
 * A collection that maps [PathString] instances onto arbitrary data. Provides efficient algorithms for locating values based on a key
 * prefix and locating the value matching the longest prefix of a given path.
 */
interface PathMap<out T> {
  ...
  /**
   * Returns the value associated with the longest prefix of the given key. Returns null if either the value is null or the map contains no
   * values for the given key. Runs in O(1) with respect to the number of entries in the map.
   */
