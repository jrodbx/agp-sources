...
/**
 * Computes the system image repository hash from the information supplied on the managed managed device dsl.
 *
 ...
 * @return the hash for the system image repository with the given parameters. A system image is not guaranteed to exist with the given
 *   values, but this gives the hash that the sdkHandler can check.
 */
...
fun computeVersionString(version: Int, minorVersion: Int, extensionVersion: Int?): String {
  val minorSuffix =
    if (minorVersion != 0 || version >= MIN_MINOR_VERSION_API) {
...
fun computeVendorString(imageSource: String, pageAlignmentSuffix: String) = computeImageSource(imageSource) + pageAlignmentSuffix
...
fun canSourcePerformNdkTranslation(imageSource: String, version: Int) =
  when (computeImageSource(imageSource)) {
    ...
    // support translation.
    "google_apis",
    "google_apis_playstore" -> version >= 30
    else -> false
  }

fun isTvOrAutoSource(imageSource: String) = imageSource.contains("-tv") || imageSource.contains("-auto")

fun is16kPageSource(vendorString: String) = vendorString.contains(PAGE_16K_SOURCE_SUFFIX)
...
fun isTvOrAutoDevice(deviceName: String) = deviceName.contains("TV") || deviceName.contains("Auto")

private fun apiComponentFromHash(systemImageHash: String): String? = systemImageHash.split(";").getOrNull(API_OFFSET)
...
fun parseApiFromHash(systemImageHash: String): Int? {
  ...
  return apiComponent.substringAfter(API_PREFIX).substringBefore(MINOR_API_PREFIX).substringBefore(API_EXTENSION_PREFIX).toIntOrNull()
}
...
fun parseMinorApiFromHash(systemImageHash: String): Int? {
  ...
  return apiComponent.substringAfter(MINOR_API_PREFIX, missingDelimiterValue = "").substringBefore(API_EXTENSION_PREFIX).toIntOrNull() ?: 0
}
...
fun parseExtensionFromHash(systemImageHash: String): Int? {
  ...
  return apiComponent.substringAfter(API_EXTENSION_PREFIX, missingDelimiterValue = "").toIntOrNull()
}

/**
 * Returns the vendor string for the given system image
 *
 * The vendor string is different from the system image source as it may contain additional information such as page size.
 */
fun parseVendorFromHash(systemImageHash: String): String? = systemImageHash.split(";").getOrNull(VENDOR_OFFSET)

/**
 * Returns the system image source for the given system image.
 *
 * The source will be one of "google_apis", "google_apis_playstore", "default", "google_atd", "aosp_atd". For the actual vendor string
 * associated with the hash see [parseVendorFromHash]
 */
...
fun parseAbiFromHash(systemImageHash: String): String? = systemImageHash.split(";").getOrNull(ABI_OFFSET)
