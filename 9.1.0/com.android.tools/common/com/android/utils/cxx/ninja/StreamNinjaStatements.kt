...
/**
 * This function builds on and is one level higher than [streamNinjaBuildTokens]. Its purpose is to create a stream of Ninja statements
 * (called [NinjaStatement]) like compile 'build', 'rule', 'subninja', and 'include' statements.
 *
 * The 'subninja' and 'include' statements refer to another file. These statements are not expanded by this function. Instead, an outer
 * function that knows how to locate those files should be used.
 */
@VisibleForTesting
  ...
  streamNinjaBuildTokens { type, value ->
    ...
    do {
      ...
      when (state) {
          ...
          state =
            when (type) {
              ...
              TextType -> {
                ...
                action(
                  when (val receiver = stack.pop<NinjaStatement>()) {
        ...
        EXPECT_PROPERTY_EQUALS -> {
          state = if (value != "=") error(value) else EXPECT_PROPERTY_VALUE
        }
        ...
        SYNTAX_ERROR -> return@streamNinjaBuildTokens
        DONE -> {}
      }
    } while (!done)
  }
...
/**
 * This function streams [NinjaStatement] like Reader.streamNinjaStatements(...) above but it also knows how to locate external files
 * references from 'include' statements and expands them inline in the [NinjaStatement] stream.
 */
fun streamNinjaStatements(file: File, progress: ProgressCallback?, action: (NinjaStatement) -> Unit) {
...
/**
 * Stack-like helper functions over a [MutableList<Any>]. The main purpose is [pop] which can be used when the caller knows the expected
 * type to be popped from the stack. The [push] function is here to making the calling code look consistent with respect to stack semantics.
 */
...
sealed class NinjaStatement {
   ...
   * For example,
   *
   * build Externals/soundtouch/CMakeFiles/SoundTouch.dir/AAFilter.cpp.o: CXX_COMPILER__SoundTouch ../ext/soundtouch/AAFilter.cpp ||
   * cmake_object_order_depends_target_SoundTouch DEFINES = -DANDROID -DDATA_DIR=\"/usr/local/share/dolphin-emu/\" -DHAVE_EGL=1
   * -DUSE_ANALYTICS=1 -DUSE_MEMORYWATCHER=1 -DUSE_PIPES=1 -D_ARCH_64=1 -D_DEFAULT_SOURCE -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE
   * -D_M_ARM=1 -D_M_ARM_64=1 DEP_FILE = Externals/soundtouch/CMakeFiles/SoundTouch.dir/AAFilter.cpp.o.d FLAGS = -g -DANDROID
   * -fdata-sections -ffunction-sections -funwind-tables -fstack-protector-strong -no-canonical-prefixes -D_FORTIFY_SOURCE=2 -Wformat
   * -Werror=format-security -O2 -g -DNDEBUG -fPIC -w -march=armv8-a+crc -fdiagnostics-color -Wall -Wtype-limits -Wsign-compare
   * -Wignored-qualifiers -Wuninitialized -Wshadow -Winit-self -Wmissing-declarations -Wmissing-variable-declarations -fno-strict-aliasing
   * -fno-exceptions -fvisibility-inlines-hidden -fvisibility=hidden -fomit-frame-pointer INCLUDES = -I../Core -I../../../../../
   * -I../ext/enet/include -I../ext/minizip OBJECT_DIR = Externals/soundtouch/CMakeFiles/SoundTouch.dir OBJECT_FILE_DIR =
   * Externals/soundtouch/CMakeFiles/SoundTouch.dir
   *
   * Above,
  ...
  /**
   * A Ninja rule definition (https://github.com/ninja-build/ninja/blob/master/doc/manual.asciidoc#rule-variables)
   *
   * rule CXX_COMPILER__SoundTouch depfile = $DEP_FILE deps = gcc command = /path/to/clang++ --target=aarch64-none-linux-android21
   * --gcc-toolchain=/path/to/ndk/toolchains/llvm/prebuilt/darwin-x86_64
   * --sysroot=/path/to/ndk/toolchains/llvm/prebuilt/darwin-x86_64/sysroot $DEFINES $INCLUDES $FLAGS -MD -MT $out -MF $DEP_FILE -o $out -c
   * $in description = Building CXX object $out
   *
   * The only required property is 'command'. This defines the command to execute after variables are expanded.
   */
  data class RuleDef(val name: String, val properties: Map<String, String>) : NinjaStatement()

  /**
   * Pools allow you to allocate one or more rules or edges a finite number of concurrent jobs which is more tightly restricted than the
   * default parallelism.
   *
   * # No more than 4 links at a time.
   * pool link_pool depth = 4
   *
   * rule link ... pool = link_pool
   *
   * see: https://ninja-build.org/manual.html
   */
  data class PoolDef(val name: String, val properties: Map<String, String>) : NinjaStatement()
