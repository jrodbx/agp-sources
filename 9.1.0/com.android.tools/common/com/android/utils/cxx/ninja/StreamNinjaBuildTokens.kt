...
/**
 * This function builds on and is one level higher than [streamUnescapedNinja]. Its purpose is to create a stream of Ninja build tokens
 * (called [NinjaBuildToken]) that are literals or special characters recognized by ninja like '|', '||', and ':'.
 */
@VisibleForTesting
      ...
      when (type) {
        ...
        LiteralType -> {
          ...
          for (i in 0 until size) {
            state =
              when (val ch = type.charAt(value, i)) {
                ' ' ->
                  when (state) {
                    IN_PIPE -> {
                      action(PipeType, "|")
