 ...
 *
 * There is only one escape character, $, and it has the following behaviors: $ followed by a newline escape the newline (continue the
 * current line across a line break). $ followed by text a variable reference. ${varname} alternate syntax for $varname. $ followed by space
 * a space. (This is only necessary in lists of paths, where a space would otherwise separate filenames.) $: a colon. (This is only
 * necessary in build lines, where a colon would otherwise terminate the list of outputs.) $$ a literal $.
 */
  ...
  while (true) {
    ...
    if (next == -1) {
      if (sb.isNotEmpty()) {
        when (state) {
          START,
          START_AFTER_NON_WHITESPACE -> action(LiteralType, sb)
          IN_DOLLAR_VARIABLE -> action(VariableType, sb)
          IN_DOLLAR_CURLY_VARIABLE -> action(VariableWithCurliesType, sb)
          AFTER_COMMENT_HASH -> action(CommentType, sb)
          else -> error("$state")
        }
      }
      return
...
enum class NinjaUnescapeTokenType {
  ...
  fun charAt(value: CharSequence, index: Int) =
    when (this) {
      VariableType ->
        when (index) {
          0 -> '$'
