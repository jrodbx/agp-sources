...
/**
 * Writes source files and flags to compile_commands.json.bin. The parsing of command-lines and writing to compile_commands.json.bin is
 * multi-threaded.
 */
fun writeCompileCommandsJsonBin(
  ...
  CompileCommandsEncoder(compileCommandsJsonBin).use { encoder ->
    fun writeCommand(compiler: File, sourceFile: File, objectFile: File, flags: List<String>) {
      ...
      try {
        ...
        synchronized(ninjaBuildFile) { error = error ?: e }
        throw e
      }
    }

    fun popBuffer() = synchronized(buffers) { if (buffers.isEmpty()) intArrayOf(2048) to DoubleStringBuilder() else buffers.removeAt(0) }

    fun pushBuffer(buffer: Pair<IntArray, DoubleStringBuilder>) = synchronized(buffers) { buffers.add(buffer) }
    ...
    fun expandCommandThenWrite(unexpanded: NinjaBuildUnexpandedCommand) {
      ...
      try {
        ...
        for (subcommand in os.splitCommandLine(expanded)) {
          ...
          if (commandLineBuffer.size <= minimumSizeOfTokenizeCommandLineBuffer(subcommand)) {
            commandLineBuffer = allocateTokenizeCommandLineBuffer(subcommand)
          }
          val tokens = TokenizedCommandLine(subcommand, false, os.platform, commandLineBuffer)
          ...
          repeat(2) {
            tokens.removeNth(0)?.let { token ->
              if (isToolchainTool(token)) {
                ...
                writeExecutor.execute { writeCommand(compiler = compiler, sourceFile = source, objectFile = objectFile, flags = flags) }
                matched = true
              }
            }
          }
    ...
    try {
      streamNinjaBuildCommands(ninjaBuildFile, progress) {
        ...
        if (explicitOutputs.isNotEmpty() && explicitOutputs[0].hasExtensionIgnoreCase("o", "pch")) {
...
/**
 * Strip flags that would prevent coalescing with flags of other clang commands. This is mainly:
 * - The source file
 * - The output file
...
private fun TokenizedCommandLine.stripFlags(sourceFile: String) {
  ...
  removeTokenGroup(sourceFile, 0, filePathSlashAgnostic = true)
...
private fun isToolchainTool(exe: String) = exe.filenameEndsWithIgnoreCase("clang", "clang++", "ar") && exe.hasExtensionIgnoreCase("exe", "")

/** Reduce a list of source files to one source file by removing a .pch if there is one. */
@VisibleForTesting
fun deconflictSourceFiles(sources: List<String>) = sources.singleOrNull { !it.hasExtensionIgnoreCase("pch") } ?: sources.first()
