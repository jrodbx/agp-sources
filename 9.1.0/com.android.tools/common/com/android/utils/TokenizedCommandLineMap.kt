...
/**
 * Safely set up a session for tokenizing command-lines in bulk and mapping them to some result value.
 *
 * The [indexes] buffer is private, is shared between multiple tokenization calls, and is grown on demand for larger command-lines.
 */
class TokenizedCommandLineMap<T>(
  ...
  var hashFunction = { tokens: TokenizedCommandLine -> tokens.computeNormalizedCommandLineHashCode() }
  ...
  fun computeIfAbsent(commandLine: String, sourceFile: String, compute: (TokenizedCommandLine) -> T): T {
    if (indexes.size <= minimumSizeOfTokenizeCommandLineBuffer(commandLine)) {
      indexes = allocateTokenizeCommandLineBuffer(commandLine)
    }
    val tokens = TokenizedCommandLine(commandLine = commandLine, raw = raw, platform = platform, indexes = indexes)
