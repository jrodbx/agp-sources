...
/**
 * Parse a Windows or POSIX command-line without allocating per-character memory. It does this by amortizing with a shared a buffer from
 * parses of other command lines.
 *
 * The first element of the buffer is a generation counter. If the buffer is shared between multiple parses each parse will increment the
 * generation. If a buffer is used with an older [TokenizedCommandLine] then that's a bug in the calling code so an exception will be
 * thrown.
 *
 * After the first element, the buffer consists of indexes of characters within the original [commandLine] delimited by
 * [ZERO_ALLOC_TOKENIZER_END_OF_TOKEN] and terminated by [ZERO_ALLOC_TOKENIZER_END_OF_COMMAND]
 *
 * @param commandLine the command-line to tokenize
 * @param raw then special characters are sent to the receiver. On Windows, that's double-quote("), backslash(\), and caret(^). On POSIX,
 *   that's single-quote('), double-quote("), and backslash(\).
 * @param platform optional platform type. Default is the current platform.
 * @param indexes optional IntArray to hold indexes of characters, token delimiters (ZERO_ALLOC_TOKENIZER_END_OF_TOKEN), and end of string
...
class TokenizedCommandLine(
  ...
  /**
   * Remove tokens matching [token] and also remove up to [extra] additional tokens following that. If [matchPrefix] is set to true, [token]
   * is treated as a prefix and any token starts with this prefix would be matched and removed.
   *
   * This function updates [indexes] without resizing it or allocating memory. It works by maintaining a 'read' pointer and a 'write' that
   * are pointers within [indexes].
   *
   * In the beginning read and write are the same. When a matching token is found then the read pointer is moved forward past the end of the
   * matching token along with any [extra] tokens.
   *
   * So for example, if removeTokenGroup("-c", 1) is called then the pointers will look like this after the '-c' and one extra token have
   * been skipped.
   *
   *                            read
   *                            v
   ...
   *             write
   *
   * When the parameter [returnFirstExtra] is true, this function returns the first string value of the first parameter after [token]; this
   * requires an extra string allocation. When [returnFirstExtra] is false, null is returned; no extra string allocation is needed in this
   * case.
   */
  ...
  private fun tokenStartingAt(start: Int): String {
    ...
    for (read in start until indexes.size) {
      when (val offset = indexes[read]) {
        ...
        else -> token.append(commandLine[offset])
      }
    }
  ...
  /**
   * Remove and return the n-th token and return its value as a string. Will return null if the requested token [n] is out of range.
   *
   * This function operates in a manner similar to [removeTokenGroup] in that it uses a read pointer and write pointer to walk across the
   * [commandLine] and it relies on the read pointer being the same as write pointer or larger.
   */
   ...
   * - A string surrounded by double quotation marks ("string") is interpreted as a single argument, regardless of white space contained
   *   within. A quoted string can be embedded in an argument.
   * - A double quotation mark preceded by a backslash (\") is interpreted as a literal double quotation mark character (").
   * - Backslashes are interpreted literally, unless they immediately precede a double quotation mark.
   * - If an even number of backslashes is followed by a double quotation mark, one backslash is placed in the argv array for every pair of
   *   backslashes, and the double quotation mark is interpreted as a string delimiter.
   * - If an odd number of backslashes is followed by a double quotation mark, one backslash is placed in the argv array for every pair of
   *   backslashes, and the double quotation mark is "escaped" by the remaining backslash.
   *
   * @param raw if true, then special characters double-quote("), backslash(\), and caret(^) are returned, otherwise they are not returned.
   *   Regardless of this flag, the Windows command line escaping logic is applied and unquoted whitespace is removed.
    ...
    while (i < length) {
      ...
      when {
        ...
        c == '\\' -> {
          ...
          repeat(slashCount) { j -> indexes[offset++] = i + j }
  ...
  /**
   * Tokenize a string with POSIX rules. This function should operate in the same manner as the bash command-line.
   *
   * This is the zero-alloc (per char) part of the tokenizer.
   *
   ...
   * - Enclosing characters in double-quotes ( "" ) shall preserve the literal value of all characters within the double-quotes, with the
   ...
   * echo 3=[/$3]
   *
   * @param raw if true, then special characters single-quote('), double-quote("), and backslash(\) are returned, otherwise they are not
   *   returned. Regardless of this flag, the POSIX command line escaping logic is applied.
   */
  ...
  /**
   * Return true if the token at [offset] matches [token]. [offset] is a pointer into [indexes] so it is one-relative to account for the
   * generation counter.
   */
  @VisibleForTesting
  fun tokenMatches(token: String, offset: Int, matchPrefix: Boolean, filePathSlashAgnostic: Boolean = false): Boolean {
    checkGeneration()
    var i = 0
    ...
    while (index != ZERO_ALLOC_TOKENIZER_END_OF_COMMAND) {
      ...
      if (c1 != c2 && (!filePathSlashAgnostic || !bothSlash(c1, c2))) {
        return false
      }
      ++i
  ...
  /**
   * Skip to the start of the next token [offset] is the one-relative starting pointer. The return value is the offset to the start of the
   * next token.
   */
  @VisibleForTesting
  ...
  private fun isEndOfToken(i: Int) = i < indexes.size && indexes[i] == ZERO_ALLOC_TOKENIZER_END_OF_TOKEN

  private fun isEndOfCommand(i: Int) = i >= indexes.size || indexes[i] == ZERO_ALLOC_TOKENIZER_END_OF_COMMAND

  private fun isStartOfToken(i: Int) = i == 1 || isEndOfToken(i - 1)

  private fun charAt(i: Int) = if (isEndOfToken(i)) null else commandLine[indexes[i]]
  ...
  /**
   * Not hashCode() to avoid giving the impression [TokenizedCommandLine] can be directly stored in a hashtable. It is *not* the same same
   * as toString().hashCode(), but it is guaranteed to be a good hash function.
   */
  ...
  private fun checkGeneration() {
    if (generation != abs(indexes[0])) {
      throw Exception("Buffer indexes was shared with another " + "TokenizedCommandLine after this one")
    }
  }
}
 ...
 *
 * The +3 is derived as: one element to hold the generation counter
 ...
 *
 * This relies on the fact that Windows and POSIX parsing can only reduce the number of characters and not increase it. Reduction comes, for
 * example, from combining contiguous blocks of whitespace with a single [ZERO_ALLOC_TOKENIZER_END_OF_TOKEN].
 */
fun allocateTokenizeCommandLineBuffer(commandLine: String) = IntArray(minimumSizeOfTokenizeCommandLineBuffer(commandLine))

fun minimumSizeOfTokenizeCommandLineBuffer(commandLine: String) = commandLine.length + 3
