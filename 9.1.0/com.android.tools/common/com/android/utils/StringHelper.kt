...
fun StringBuilder.appendCapitalized(word: String): StringBuilder {
  ...
  if (c >= Character.MIN_HIGH_SURROGATE.code && c <= Character.MAX_HIGH_SURROGATE.code) {
    ...
    upperChar =
      if (result != -0x1) { // Character.ERROR (internal!)
        result.toChar()
      } else {
 ...
 *
 * This is unlikely to be what you need. Prefer to use [String.appendCapitalized], [StringBuilder.appendCapitalized] or
 * [String.capitalizeAndAppend].
 *
...
/**
 * Returns a string containing the given prefix, as is, and a capitalized version of the given <var>word</var>.
 *
...
/**
 * Returns a string containing the given prefix, as is, and a capitalized version of the given <var>word1</var> and <var>word2</var>.
 *
 * @param prefix the prefix to add before the word
 ...
 * @return the capitalized word.
 */
fun String.appendCapitalized(word1: String, word2: String): String {
...
/**
 * Returns a string containing the given prefix, as is, and a capitalized version of the given <var>words</var>.
 *
...
/**
 * Returns a capitalized version of the given <var>word</var>, including the given <var>suffix</var>
 *
 * @param word the word to be capitalized
...
fun <T> combineAsCamelCase(objectList: Collection<T>, mapFunction: (T) -> String): String {
  val sb = StringBuilder(objectList.size * 20)
  combineAsCamelCase(sb, objectList, mapFunction)
  return sb.toString()
}

fun <T> combineAsCamelCase(sb: StringBuilder, objectList: Collection<T>, mapFunction: (T) -> String) {
 ...
 *
 * If the objects are strings, they are directly added to the list. If the objects are collections of strings, the strings are added. For
 * other objects, the result of their toString() is added.
 *
...
fun String.tokenizeCommandLineToEscaped() = TokenizedCommandLine(this, false).toTokenList()

fun String.tokenizeCommandLineToRaw() = TokenizedCommandLine(this, true).toTokenList()
...
fun CharSequence.asSeparatedListContains(element: CharSequence, separators: CharSequence = ","): Boolean {
  var offset = 0
  while (offset < length) {
    if (startsWith(element, offset) && (length == offset + element.length || separators.contains(this[offset + element.length]))) {
      return true
    }
    @Suppress("ControlFlowWithEmptyBody") while (offset < length && !separators.contains(this[offset++])) {}
  }
  return false
}
