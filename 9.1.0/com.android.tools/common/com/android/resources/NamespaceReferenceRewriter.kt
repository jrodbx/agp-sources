...
/**
 * A utility to rewrite resource references in Xml file with the fully qualified names once including package names
 *
 ...
 */
class NamespaceReferenceRewriter(private val localPackage: String, private val packageProvider: (type: String, name: String) -> String) {

  /**
   * Method that rewrites references in a manifest file
   *
   * @param node a root node of a manifest xml document
   * @param localLib shows if the reference is for local library (not remote). In this case we should not allow private resources access
   */
  fun rewriteManifestNode(node: Node, localLib: Boolean) {
    ...
    node.attributes?.forEach { rewriteManifestNode(it, localLib) }

    // Now fix the children.
    node.childNodes?.forEach { rewriteManifestNode(it, localLib) }
  }
   ...
   */
  fun rewritePossibleReference(content: String, writeLocalPackage: Boolean = false, localLib: Boolean = false): RewrittenReference {
