...
data class Message(
  val kind: Kind,
  val text: String,
  /** A list of source positions. Will always contain at least one item. Must have type 'List' to satisfy KotlinBinaryCompatibilityTest */
  ...
  @Deprecated("Use sourceFilePositions", ReplaceWith("sourceFilePositions[0].position.startLine + 1"))
  val lineNumber: Int
    get() = sourceFilePositions[0].position.startLine + 1

  /** @return a legacy 1-based column number. */
  @Deprecated("Use sourceFilePositions", ReplaceWith("sourceFilePositions[0].position.startColumn + 1"))
  val column: Int
    get() = sourceFilePositions[0].position.startColumn + 1

  /**
   * Create a new message, which has a [Kind], a String which will be shown to the user and at least one [SourceFilePosition].
   *
  ...
  constructor(
    ...
    text = text,
    rawMessage = text,
    sourceFilePositions = ImmutableList.builder<SourceFilePosition>().add(sourceFilePosition).add(*sourceFilePositions).build(),
  )

  /**
   * Create a new message, which has a [Kind], a String which will be shown to the user and at least one [SourceFilePosition].
   *
   * It also has a rawMessage, to store the original string for cases when the message is constructed by parsing the output from another
   * tool.
   *
