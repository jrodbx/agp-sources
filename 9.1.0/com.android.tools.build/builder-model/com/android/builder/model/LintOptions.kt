...
interface LintOptions {
  ...
  /**
   * Returns the exact set of issues to check, or null to run the issues that are enabled by default plus any issues enabled via [enable]
   * and without issues disabled via [disable]. If non-null, callers are allowed to modify this collection.
   */
  ...
  /** Whether we should write an text report. Default false. The location can be controlled by [textOutput]. */
  ...
  /** Whether we should write an HTML report. Default true. The location can be controlled by [htmlOutput]. */
  val htmlReport: Boolean

  /** The optional path to where an HTML report should be written */
  val htmlOutput: File?

  /** Whether we should write an XML report. Default true. The location can be controlled by [xmlOutput]. */
  ...
  /**
   * An optional map of severity overrides. The map maps from issue id's to the corresponding severity to use, which must be "fatal",
   * "error", "warning", or "ignore".
   *
   * @return a map of severity overrides, or null. The severities are one of the constants [SEVERITY_FATAL], [SEVERITY_ERROR],
   *   [SEVERITY_WARNING], [SEVERITY_INFORMATIONAL], [SEVERITY_IGNORE].
   */
