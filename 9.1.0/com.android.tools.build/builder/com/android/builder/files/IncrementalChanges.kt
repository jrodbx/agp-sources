...
data class SerializableChange(val file: File, val fileStatus: FileStatus, val normalizedPath: String) : Serializable {
  companion object {
    private const val serialVersionUID = 1L
  }
}

data class SerializableInputChanges(val roots: List<File>, val changes: Collection<SerializableChange>) : Serializable {
  companion object {
    private const val serialVersionUID = 1L
  }
}

class SerializableFileChanges(val fileChanges: List<SerializableChange>) : Serializable {

  private val fileStatusMap by lazy { fileChanges.groupBy { it.fileStatus } }

  val removedFiles by lazy { fileStatusMap[FileStatus.REMOVED] ?: emptyList() }

  val modifiedFiles by lazy { fileStatusMap[FileStatus.CHANGED] ?: emptyList() }

  val addedFiles by lazy { fileStatusMap[FileStatus.NEW] ?: emptyList() }
 ...
 *
 * Changes that mix jars and directories must use `@Classpath` or `@CompileClasspath` sensitivity, which ensures that the normalized path
 * for jars at the root is the name of the jar.
 *
 * Do *not* use this with relative normalization, as an addition and removal of two files with the same normalized path but different
 * absolute paths may be reported as a change, which breaks the jar cache.
 */
fun classpathToRelativeFileSet(
  ...
  return Collections.unmodifiableMap(
    HashMap<RelativeFile, FileStatus>().apply {
...
fun MutableMap<RelativeFile, FileStatus>.addFileChange(change: SerializableChange) {
  val key = RelativeFile.fileInDirectory(change.normalizedPath, change.file)
  val previous = get(key)

  if (previous != null) {
    /**
     * Note that with classpath normalization, the same file may be reported as both added and removed if its order within the classpath
     * changes.
     *
