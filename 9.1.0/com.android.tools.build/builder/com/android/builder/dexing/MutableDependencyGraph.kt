...
/**
 * Directed graph to model the dependencies among objects of interest. An edge from A to B means that object A depends on object B (A is a
...
class MutableDependencyGraph<T> : DependencyGraphUpdater<T>, Serializable {
  ...
  /**
   * Returns the directly dependent nodes of a given node. The returned set does not include the given node.
   *
  ...
  /**
   * Returns all the directly and transitively dependent nodes of a given set of nodes. The returned set does not include the nodes in the
   * given set.
   *
