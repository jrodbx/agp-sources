...
/**
 * A bucket of class files. Multiple buckets are split from a [ClassBucketGroup] using an internal algorithm, and each one is identified by
 * a bucket number.
 */
class ClassBucket(val bucketGroup: ClassBucketGroup, val bucketNumber: Int) : Serializable {
      ...
      classFiles =
        Stream.concat(
          ...
          classFileInput.entries { rootPath, relativePath ->
            getBucketNumber(relativePath, bucketGroup.numOfBuckets, bucketGroup is JarBucketGroup) == bucketNumber &&
  ...
  private fun getBucketNumber(relativePath: String, numberOfBuckets: Int, isJarFile: Boolean): Int {
    check(!File(relativePath).isAbsolute) { "Expected relative path but found absolute path: $relativePath" }

    val pathOfPackageOrClass =
      if (isJarFile) {
