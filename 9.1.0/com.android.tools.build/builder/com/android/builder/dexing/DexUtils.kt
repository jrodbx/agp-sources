...
/** Returns a sorted list of files in the given directory whose relative paths satisfy the given filter. */
fun getSortedFilesInDir(dir: Path, filter: (relativePath: String) -> Boolean = { true }): List<Path> {
...
/** Returns a sorted list of the relative paths of the entries in the given jar where the relative paths satisfy the given filter. */
fun getSortedRelativePathsInJar(jar: File, filter: (relativePath: String) -> Boolean = { true }): List<String> {
...
/**
 * Returns a sorted map of the entries in the given jar whose relative paths satisfy the given filter. Each entry in the map maps the
 * relative path of a jar entry to its byte contents.
 *
 * The given jar must not have duplicate entries.
 */
fun getSortedRelativePathsInJarWithContents(jar: File, filter: (relativePath: String) -> Boolean = { true }): SortedMap<String, ByteArray> {
  return ZipFile(jar).use { zipFile ->
    zipFile
      .entries()
      .toList()
      .filter { entry -> filter(entry.name) }
      .map { entry -> entry.name to zipFile.getInputStream(entry).buffered().use { stream -> stream.readBytes() } }
      .toMap()
      .toSortedMap()
  }
}
