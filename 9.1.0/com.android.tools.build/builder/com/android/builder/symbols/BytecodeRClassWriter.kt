...
fun exportToCompiledJava(
  ...
  JarFlinger(outJar).use { jarCreator ->
    ...
    exportToCompiledJava(tables, finalIds, rPackage) { entryPath, content -> jarCreator.addEntry(entryPath, content.inputStream()) }

    if (packageNameToId != null) {
      writeRPackages(packageNameToId) { entryPath, content -> jarCreator.addEntry(entryPath, content.inputStream()) }
    }
  }
}
...
fun exportToCompiledJava(
  ...
  mergedTables.forEach { table -> exportToCompiledJava(table, finalIds, rPackage, consumer) }
}
...
private fun exportToCompiledJava(
  ...
  consumer.invoke(packageR + SdkConstants.DOT_CLASS, generateOuterRClass(resourceTypes, packageR))
}
...
private fun generateOuterRClass(resourceTypes: EnumSet<ResourceType>, packageR: String): ByteArray {
  ...
  cw.visit(Opcodes.V1_8, ACC_PUBLIC + ACC_FINAL + ACC_SUPER, packageR, null, "java/lang/Object", null)

  for (rt in resourceTypes) {
    cw.visitInnerClass(packageR + "$" + rt.getName(), packageR, rt.getName(), ACC_PUBLIC + ACC_FINAL + ACC_STATIC)
  }
...
private fun generateResourceTypeClass(table: SymbolTable, resType: ResourceType, finalIds: Boolean, rPackage: String?): ByteArray? {
  ...
  cw.visit(Opcodes.V1_8, ACC_PUBLIC + ACC_FINAL + ACC_SUPER, internalName, null, "java/lang/Object", null)

  cw.visitInnerClass(internalName, internalName(table, null), resType.getName(), ACC_PUBLIC + ACC_FINAL + ACC_STATIC)
  ...
  if (resType == ResourceType.ATTR) {
    ...
    cw.visitInnerClass("android/R\$attr", "android/R", resType.getName(), ACC_PUBLIC + ACC_FINAL + ACC_STATIC)
  }
      ...
      if (resType == ResourceType.STYLEABLE) {
        ...
        for ((i, value) in values.withIndex()) {
          if (isUnstableAndroidAttr(value, s.children[i])) {
            ...
            clinit.dup()
            clinit.push(i)
            clinit.visitFieldInsn(GETSTATIC, "android/R\$attr", canonicalizeValueResourceName(name), "I")
...
private fun isUnstableAndroidAttr(value: Int, name: String): Boolean {
  ...
  return value == 0 && (name.startsWith("android.") || name.startsWith("android_") || name.startsWith("android:"))
}
...
private fun writeRPackages(packageNameToId: Map<String, Int>, consumer: (String, ByteArray) -> Unit) {
