...
data class AaptPackageConfig(
  ...
  init {
    if (!mergeOnly && androidJarPath == null) {
      throw IllegalStateException("Android jar path must be provided except in merge-only " + "(auto namespace transform) case.")
    }
  }
  ...
  class Builder {
    ...
    fun setManifestFile(manifestFile: File): Builder {
      if (!manifestFile.isFile) {
        throw IllegalArgumentException("Manifest file '" + manifestFile.absolutePath + "' is not a readable file. ")
      }
    ...
    fun addResourceDir(resourceDir: File?): Builder {
      if (resourceDir != null && !resourceDir.isDirectory) {
        throw IllegalArgumentException("Path '" + resourceDir.absolutePath + "' is not a readable directory.")
      }
