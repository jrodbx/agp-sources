 ...
 *
 * This must be used in the following sequence: Call [compile] or [link] as many times as needed. These methods block until the operation
 * requested is complete. The first call to either of [compile] or [link] will start the underlying daemon process. Call [shutDown()], which
 * blocks until the daemon process has exited.
 *
 * Processes cannot be re-started.
 *
 * The state tracking in this class is separated from the actual process handling to allow test fakes that do not actually use AAPT.
 */
@NotThreadSafe
abstract class Aapt2Daemon(protected val displayName: String, protected val logger: ILogger) : Aapt2 {
  enum class State {
    NEW,
    RUNNING,
    SHUTDOWN,
    ...
    when (state) {
      State.NEW -> {
        ...
        try {
          ...
          if (SdkConstants.currentPlatform() != SdkConstants.PLATFORM_WINDOWS && !System.getProperty("os.arch").contains("64")) {
   ...
   *
   * If the daemon process could not be started, this method must throw an exception, having attempted to free any used resources (e.g.
   * threads, processes).
   */
   ...
   */
  @Throws(TimeoutException::class) protected abstract fun stopProcess()

  /** Wrap and propagate the original exception, but also try to shut down the daemon. */
  private fun handleError(action: String, exception: Exception, unexpected: Boolean = true): Nothing {
    throw Aapt2InternalException(
        "$displayName: $action" +
          (if (state == State.RUNNING) ", attempting to stop daemon.\n" else "\n") +
          (if (unexpected) "This should not happen under normal circumstances, " + "please file an issue if it does." else ""),
        exception,
      )
      .apply {
        try {
