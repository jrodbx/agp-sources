...
fun makeCompileCommand(request: CompileResourceRequest): ImmutableList<String> {
  ...
  if (!request.isPngCrunching) {
    ...
    if (lowerName.endsWith(SdkConstants.DOT_PNG) && !lowerName.endsWith(SdkConstants.DOT_9PNG)) {
...
fun makeLinkCommand(config: AaptPackageConfig): ImmutableList<String> {
  ...
  if (!config.resourceDirs.isEmpty()) {
    try {
      if (config.isListResourceFiles()) {
        ...
        val resourceListFile = File(config.intermediateDir!!, "resources-list-for-" + resourceOutputApk.name + ".txt")
        ...
        for (dir in config.resourceDirs) {
          FileOutputStream(resourceListFile).use { fos ->
            PrintWriter(fos).use { pw ->
              dir.listFiles()!!.filter { it.isFile }.sortedBy { it.path }.forEach { pw.print(it.absolutePath + " ") }
            }
          }
        }
        ...
        for (dir in config.resourceDirs) {
          dir.listFiles()!!.filter { it.isFile }.sortedBy { it.path }.forEach { builder.add("-R", it.absolutePath) }
        }
      }
    } catch (e: IOException) {
      throw AaptException("Failed to walk paths " + Joiner.on(File.pathSeparatorChar).join(config.resourceDirs), e)
    }
  }
  ...
  if (config.mainDexListProguardOutputFile != null) {
    builder.add("--proguard-main-dex", config.mainDexListProguardOutputFile.absolutePath)
  }

  if (config.splits != null) {
    for (split in config.splits) {
      val splitter = File.pathSeparator
      builder.add("--split", resourceOutputApk.toString() + "_" + split + splitter + split)
    }
  }
   ...
   */
  val densityResourceConfigs = Lists.newArrayList(AaptUtils.getDensityResConfigs(resourceConfigs))
  val otherResourceConfigs = Lists.newArrayList(AaptUtils.getNonDensityResConfigs(resourceConfigs))
  var preferredDensity = config.preferredDensity

  val densityResSplits = if (config.splits != null) AaptUtils.getDensityResConfigs(config.splits) else ImmutableList.of()

  if ((preferredDensity != null || densityResSplits.iterator().hasNext()) && densityResourceConfigs.isNotEmpty()) {
    throw AaptException(
      String.format(
  ...
  if (otherResourceConfigs.isNotEmpty() || localeFilters.isNotEmpty()) {
    ...
    val resourceConfigs = (otherResourceConfigs + localeFilters)
    if (resourceConfigs.none { it.isBlank() }) {
      builder.add("-c", joiner.join(resourceConfigs))
    }
  }
  ...
  if (config.isStaticLibrary()) {
    ...
    if (!config.staticLibraryDependencies.isEmpty()) {
      throw AaptException("Static libraries to link against should be passed as imports")
    }
  } else {
