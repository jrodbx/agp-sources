 ...
 *
 * @param deterministicEntryOrder whether or not the order of the entries in the created APK will be deterministic, regardless of any
 *   multi-threading in the implementation and/or regardless of the order of calls to [writeZip], [writeFile], and [deleteFile].
 *   Importantly, deterministicEntryOrder does not guarantee that the resulting APK will be deterministic for a given set of source files;
 *   that is true for clean builds, but not incremental builds. Instead, if deterministicEntryOrder is true, the resulting APK will be
 *   deterministic given the same initial APK and the same calls to [writeZip], [writeFile], and [deleteFile].
...
class ApkFlinger(
    ...
    when (creationData.nativeLibrariesPackagingMode) {
      ...
      NativeLibrariesPackagingMode.UNCOMPRESSED_AND_ALIGNED -> {
        ...
        noCompressPredicate = Predicate { name ->
          baseNoCompressPredicate.apply(name) || name?.endsWith(SdkConstants.DOT_NATIVE_LIBS) ?: false
        }
        pageAlignPredicate = Predicate { it?.endsWith(SdkConstants.DOT_NATIVE_LIBS) ?: false }
      }
        ...
        SignedApk(
          ...
          SignedApkOptions.Builder()
            ...
            .also { builder ->
              ...
              if (enableV4Signing) {
                builder.setV4Output(File("${creationData.apkPath.absolutePath}.idsig"))
              }
            }
            .build(),
        )
      }
    archive = SynchronizedArchive(if (deterministicEntryOrder) StableArchive(innerArchive) else innerArchive)
  }

  /**
   * Copies the content of a Jar/Zip archive into the receiver archive.
   *
   * <p>An optional predicate allows to selectively choose which files to copy over and an optional function allows renaming the files as
   * they are copied.
   *
   * <p>If any entries in zip already exist in this archive, they must be deleted with [deleteFile] before calling this method.
   *
   ...
   * @param isIgnored an optional filter or {@code null} to mark which out files should not be added, even through they are on the zip; if
   *   {@code transform} is specified, then this predicate applies after transformation
   * @throws IOException I/O error
   */
  @Throws(IOException::class)
  override fun writeZip(zip: File, transform: Function<String, String>?, isIgnored: Predicate<String>?) {
    ...
    for (entry in entries) {
      ...
      val newAlignment = getNewAlignment(entry, newName, newCompressionLevel, pageAlignPredicate, pageSize)
   ...
   *
   * <p>If this file entry already exists in this archive, it must be deleted with [deleteFile] before calling this method.
   *
   * <p>After calling this method, any calls at all to [deleteFile] will result in an error.
   *
   ...
   *
   * <p>If this method is called after any writeZip() or writeFile() calls, an error will be thrown.
   *
