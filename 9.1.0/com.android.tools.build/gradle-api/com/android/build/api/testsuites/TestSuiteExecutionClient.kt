...
class TestSuiteExecutionClient(
  ...
  fun getInputParameter(key: String): String =
    inputParameters.firstOrNull { it.name == key }?.value ?: throw RuntimeException("Missing value for key $key")

  companion object {

    /**
     * Name of the environment property the junit engine expect to be set. This environment property will point to a file location which
     * content will be an instance of [TestEngineInputProperties] serialized as a Json object.
     */
    @get:Incubating const val DEFAULT_ENV_VARIABLE = "com.android.junit.engine.input.parameters"

    /** Load the context using the default environment variable set by the test task */
    @Incubating fun default() = withEnvVariable(DEFAULT_ENV_VARIABLE)

    /** Load the context using a specific environment variable. */
    @Incubating
    fun withEnvVariable(variableName: String) =
      TestSuiteExecutionClient(
        Properties()
          .also { it.load(FileReader(System.getenv(variableName))) }
          .map { TestEngineInputProperty(it.key as String, it.value as String) }
      )
  }
}
