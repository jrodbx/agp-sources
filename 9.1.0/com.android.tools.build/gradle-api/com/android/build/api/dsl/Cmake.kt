 ...
 * directory.
 *
 * To include CMake projects in your Gradle build, you need to use Android Studio 2.2 and higher with Android plugin for Gradle 2.2.0 and
 * higher. To learn more about Android Studio's support for external native builds, read
...
interface Cmake {
   ...
   *
   * This directory also includes other build system files that should persist when performing clean builds, such as
   * [Ninja build files](https://ninja-build.org/). If you do not specify a value for this property, the Android plugin uses the
   ...
   * ```
   *
   * If you specify a path that's a subdirectory of your project's temporary `build/` directory, you get a build error. That's because files
   * in this directory do not persist through clean builds. So, you should either keep using the default `<project_dir>/<module>/.cxx/`
   ...
   *
   * When you specify a version of CMake, as shown below, the plugin searches for the appropriate CMake binary within your PATH
   * environmental variable. So, make sure you add the path to the target CMake binary to your PATH environmental variable.
   *
   * ```
   * android {
   *     ...
   ...
   * ```
   *
   * If you do not configure this property, the plugin uses the version of CMake available from the
   * [SDK manager](https://developer.android.com/studio/intro/update.html#sdk-manager). (Android Studio prompts you to download this version
   * of CMake if you haven't already done so).
   *
