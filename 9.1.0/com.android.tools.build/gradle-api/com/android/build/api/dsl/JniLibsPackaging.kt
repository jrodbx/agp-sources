...
interface JniLibsPackaging {
  /**
   * Whether to use the legacy convention of compressing all .so files in the APK. If null, .so files will be uncompressed and page-aligned
   * when minSdk >= 23.
   */
  ...
  /**
   * The set of patterns where the first occurrence is packaged in the APK. For each native library APK entry path matching one of these
   * patterns, only the first native library found with that path gets packaged.
   *
