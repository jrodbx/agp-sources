...
interface KotlinMultiplatformAndroidHostTest {
   ...
   * current subproject and all its dependencies. This property is available by default, or if the Gradle property
   * `android.enableUnitTestBinaryResources` is set to `true`.
   *
   * `android_merged_resources`: the path to the directory containing merged resources, which includes all the resources from the current
   * subproject and all its dependencies. This property is available only if the Gradle property `android.enableUnitTestBinaryResources` is
   * set to `false`.
   *
   * `android_merged_assets`: the path to the directory containing merged assets. For app subprojects, the merged assets directory contains
   * assets from the current subproject and its dependencies. For library subprojects, the merged assets directory contains only assets from
   ...
   *
   * `android_custom_package`: the package name of the final R class. If you modify the application ID in your build scripts, this package
   * name may not match the `package` attribute in the final app manifest.
   *
   * Note that starting with version 3.5.0, if the Gradle property `android.testConfig.useRelativePath` is set to `true`, the paths above
   * will be relative paths (relative to the current project directory, not the root project directory); otherwise, they will be absolute
   * paths. Prior to version 3.5.0, the paths are all absolute paths.
   */
