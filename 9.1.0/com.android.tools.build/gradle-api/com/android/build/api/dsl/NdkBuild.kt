...
/**
 * DSL object for per-module ndk-build configurations, such as the path to your `Android.mk` build script and external native build output
 * directory.
 *
 * To include ndk-build projects in your Gradle build, you need to use Android Studio 2.2 and higher with Android plugin for Gradle 2.2.0
 * and higher. To learn more about Android Studio's support for external native builds, read
...
interface NdkBuild {
   ...
   * in this directory do not persist through clean builds. So, you should either keep using the default `<project_dir>/.cxx/` directory or
   * specify a path outside the temporary build directory.
   *
   * since 3.0.0
   */
