...
interface NdkBuildFlags {
   ...
   * [Application.mk](https://developer.android.com/ndk/guides/application_mk.html) scripts. The following sample specifies the
   * `Application.mk` for the ndk-build project:
   * ```
   * android {
   ...
   * The properties you can configure are the same as those available in your
   * [Android.mk](https://developer.android.com/ndk/guides/android_mk.html) and
   * [Application.mk](https://developer.android.com/ndk/guides/application_mk.html) scripts.
   *
  ...
  /**
   * Specifies the Application Binary Interfaces (ABI) that Gradle should build outputs for. The ABIs that Gradle packages into your APK are
   * determined by [android.defaultConfig.ndk.abiFilter][Ndk.abiFilters]
   *
   * In most cases, you need to specify ABIs using only [android.defaultConfig.ndk.abiFilter][Ndk.abiFilters], because it tells Gradle which
   * ABIs to both build and package into your APK. However, if you want to control what Gradle should build, independently of what you want
   * it to package into your APK, configure this property with the ABIs you want Gradle to build.
   *
   ...
   *
   * For example, if your ndk-build project defines multiple libraries and executables, you can tell Gradle to build only a subset of those
   * outputs as follows:
   * ```
   * android {
   *     // Similar to other properties in the defaultConfig block, you can override
   ...
   * ```
   *
   * If you don't configure this property, Gradle builds all executables and shared object libraries that you define in your ndk-build
   * project. However, by default, Gradle packages only the shared libraries in your APK.
   *
   * since 2.2.0
   */
