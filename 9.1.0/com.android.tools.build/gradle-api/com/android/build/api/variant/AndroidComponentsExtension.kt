 ...
 *
 * Each component has a type, like application or library and will have a dedicated extension with methods that are related to the
 ...
 */
interface AndroidComponentsExtension<DslExtensionT : Any, VariantBuilderT : VariantBuilder, VariantT : Variant> :
  DslLifecycle<DslExtensionT>, AndroidComponents {

  /** [Action] based version of [finalizeDsl] above. */
  @Deprecated(level = DeprecationLevel.HIDDEN, replaceWith = ReplaceWith("finalizeDsl(callback)"), message = "Will be removed in 10.0")
  fun finalizeDSl(callback: Action<DslExtensionT>)

  /**
   * Creates a [VariantSelector] instance that can be configured to reduce the set of [ComponentBuilder] instances participating in the
   ...
   * [onVariants] callback does not have to deal with changing configuration and can focus on updating task inputs. See [ComponentBuilder]
   ...
   * additional new steps between existing tasks must be done via the [Component.artifacts] API. See
   ...
   *
   * A lambda must be provided to create and configure the variant scoped object that will be stored alongside the Android Gradle Plugin's
   * [com.android.build.api.variant.Variant] instance.
   *
   * Variant Scoped objects should use [org.gradle.api.provider.Property<T>] for its mutable state to allow for late binding. (see
   * [com.android.build.api.variant.Variant] for examples).
   *
   * The [DslExtension.Builder] allow you to choose if you want to extend Project, BuiltType or ProductFlavor. You can extend just one or up
   * to all of them.
   *
   ...
   * ```
   *
   * A Product flavor extension of type ProductFlavorDslExtension will allow users to specify the following declarations in their build
   * files:
   * ```
   * android {
   ...
   * @param configurator a lambda to create a variant scoped object. The lambda is provided with the [VariantExtensionConfig] that can be
   *   used to retrieve the [VariantT] instance as well as DSL extensions registered with [DslExtension]
   * @return an instance of a sub type of [VariantExtension] that will be stored with the [VariantT] instance and can be retrieved by
   ...
   *
   * The name of the source type will be used to create expected directories names in the various source sets. For instance, src/main/<name>
   * or src/debug/<name>.
   *
   * There is no notion of priorities between the build-type, flavor specific directories, all the expected directories will be interpreted
   * as a flat namespace.
   *
   * Therefore, any [org.gradle.api.Task] that needs access to the entire list of source folders can just use the [Sources.extras]'s
   * [Flat.all] method for that source type.
   *
   * However, If you need to have overriding priorities between the expected directories and therefore require a merging activity, you can
   * still use this API but you will need to create a merging task that will have all sources in input and produce a single output folder
   * for the merged sources.
   *
   ...
   *
   * The [suffix] parameter determines the naming convention for the generated configurations. For example, if [suffix] is "custom", the
   * generated configurations would be "custom", "debugCustom", "releaseCustom", "testCustom", etc.
   *
   ...
   */
  @Incubating fun addSourceSetConfigurations(suffix: String)

  /**
   * This API is provided only for the KSP Gradle plugin to use and should not be used otherwise. Use [addSourceSetConfigurations] instead.
   *
   * Adds a KSP configuration for each source set. The generated configurations are named "ksp", "kspDebug", "kspRelease", "kspTest", etc.
   *
   * @param useGlobalConfiguration whether dependencies added to the plain "ksp" configuration should be added to all resolvable
   *   configurations. Defaults to true.
   * @suppress This API is only intended to be used by the KSP Gradle plugin. Other users should instead use [addSourceSetConfigurations]
   */
