 ...
 *
 * Tasks in Android Gradle Plugin can produce more than one file in the output folder so any [Artifact] with a [Artifact.kind] of type
 * [com.android.build.api.artifact.ArtifactKind.DIRECTORY] can actually contain several produced [File]. For instance, when dealing with
 * multi-apk, there will be several manifest files or APKs produced by the Android Gradle Plugin.
 *
 * Each produced file can be identified with unique metadata like the list of filters and/or version code or version name. This instance
 * will allow producer and consumer [org.gradle.api.Task]s to easily match produced files with this metadata without relying on name
 * mangling or other custom solutions.
 *
 ...
 * </code></pre>
 *
 * This [BuiltArtifacts] will abstract access to these produced files and provided some metadata associated with each file to be able to
 * identify filters, version code or version name.
 */
interface BuiltArtifacts {
   ...
   *
   * @param out the [Directory] that can be used to save the metadata using a standard file name.
   */
  fun save(out: Directory)

  /**
   * Specialized version of Gradle's [WorkParameters] so we can retrieve the output file generated when transforming an instance of
   * [BuiltArtifacts] into a new one.
   */
