...
sealed class ScopedArtifact : Artifact.Single<Directory>(DIRECTORY, Category.INTERMEDIATES) {
  ...
  /**
   * .class files, result of sources compilation and/or external dependencies depending on the scope; includes users' transformation, but
   * does not include Jacoco instrumentation
   *
   ...
   *
   * If you need to generate bytecodes post compilation and also need to have access to the compiled classes, you cannot directly consume
   * [CLASSES] and append to it as it would create a circular dependency.
   *
   ...
   *
   * Do not use this unless you are producing bytecodes post compilation, see [CLASSES]. This artifact is only providing a readonly view of
   * the compiled classes, to add or replace classes, use [CLASSES]
   *
   * To add new bytecode or transform existing bytecode, use [CLASSES]. Bytecode added to [CLASSES], or transformed [CLASSES] will not
   * impact the content of [POST_COMPILATION_CLASSES]
   *
   ...
   */
  @Incubating object POST_COMPILATION_CLASSES : ScopedArtifact()
