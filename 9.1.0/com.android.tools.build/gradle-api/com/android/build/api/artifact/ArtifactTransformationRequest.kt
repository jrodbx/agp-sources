...
/**
 * When a [Directory] contains more than one artifact (for example, consider [SingleArtifact.APK] with multiple APKs for different screen
 * densities), this object will abstract away having to deal with [BuiltArtifacts] and manually load and write the metadata files.
 *
 * Instead, users can focus on writing transformation code that transform one file at at time into a newer version. It will also provide the
 * ability to use Gradle's [WorkQueue] to multi-thread the transformations.
 *
...
interface ArtifactTransformationRequest<TaskT : Task> {
   ...
   * @param parameterConfigurator The lambda to configure instances of [parameterType] for each [BuiltArtifact].
   */
