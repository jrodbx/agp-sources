 ...
 *
 * When replacing a stable method with a new API which is [org.gradle.api.Incubating], we cannot directly deprecate the old method as it
 * would leave the user to use either between a deprecated or an incubating API.
 *
 ...
 * - Once the new API becomes stable, the old API [ReplacedByIncubating] will be replaced with a [Deprecated] annotation.
 * - After the deprecation period has passed, the old API will be removed.
 *
 * Users can use this annotation presence to start moving their code to the new API if they do not object using an incubating potentially
 * unstable API.
 *
...
annotation class ReplacedByIncubating(val message: String, val bugId: Long)
