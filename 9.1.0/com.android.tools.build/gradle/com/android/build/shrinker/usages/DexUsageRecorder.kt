...
/**
 * Records resource usages, detects usages of WebViews and {@code Resources#getIdentifier}, gathers string constants from compiled .dex
 * files.
 *
...
class DexUsageRecorder(val root: Path) : ResourceUsageRecorder {
  ...
  override fun recordUsages(model: ResourceShrinkerModel) {
    ...
    Files.walk(root)
      ...
      .forEach { path -> runResourceShrinkerAnalysis(Files.readAllBytes(path), path, DexFileAnalysisCallback(path, model)) }
  }
}

private class DexFileAnalysisCallback(private val path: Path, private val model: ResourceShrinkerModel) : AnalysisCallback {
  companion object {
  ...
  override fun referencedStaticField(internalName: String, fieldName: String) {
    ...
    val realMethod = model.obfuscatedClasses.resolveOriginalMethod(ClassAndMethod(internalName.toSourceClassName(), fieldName))
    ...
    if (isValidResourceType(realMethod.className)) {
      ...
      ResourceType.fromClassName(typePart)?.let { type ->
        model.resourceStore.getResources(type, realMethod.methodName).forEach { ResourceUsageModel.markReachable(it) }
      }
    }
  }
  ...
  override fun referencedString(value: String) {
    ...
    if (value.all { Character.isJavaIdentifierPart(it) || isSpecialCharacter(it) } && value.any { Character.isJavaIdentifierPart(it) }) {
      model.addStringConstant(value)
      model.isFoundWebContent = model.isFoundWebContent || value.contains(ANDROID_RES)
    }
  }

  override fun referencedMethod(internalName: String, methodName: String, methodDescriptor: String) {
  ...
  private fun shouldIgnoreField(): Boolean {
    val visitingFromStaticInitRClass = (isRClass && visitingMethod.isVisiting && (visitingMethod.methodName == "<clinit>"))
    return visitingFromStaticInitRClass || isRClass && !visitingMethod.isVisiting
  }

  private fun isValidResourceType(className: String): Boolean = className.substringAfterLast('.').startsWith("R$")
}
