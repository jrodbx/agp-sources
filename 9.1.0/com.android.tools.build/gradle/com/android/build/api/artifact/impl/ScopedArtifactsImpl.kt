...
class ScopedArtifactsImpl(
  ...
  /**
   * Internal function to set the initial provider of a [ScopedArtifact]. Because AGP tasks creation happens after the variant API ran, we
   * cannot use the append method as they would be appended after a third party decided to replace the artifacts.
   *
   * Instead, AGP task creation must use these [setInitialContent] methods to declare itself as the initial provider of the artifact after
   * third party's variant block has ran.
   */
  internal fun setInitialContent(type: ScopedArtifact, artifacts: ArtifactsImpl, incomingType: Artifact.Single<*>) {
    getScopedArtifactsContainer(type).initialScopedContent.from(artifacts.get(incomingType))
  }

  internal fun setInitialContent(type: ScopedArtifact, fileCollection: FileCollection) {
    getScopedArtifactsContainer(type).initialScopedContent.from(fileCollection)
  }

  override fun <T : Task> use(taskProvider: TaskProvider<T>): ScopedArtifactsOperationImpl<T> =
    ScopedArtifactsOperationImpl(this, taskProvider, projectLayout, fileCollectionCreator)

  class ScopedArtifactsContainer(fileCollectionCreator: () -> ConfigurableFileCollection) {
    ...
    var currentScopedContent = fileCollectionCreator().also { it.from(this.initialScopedContent) }
    val finalScopedContent = fileCollectionCreator().also { it.from(currentScopedContent) }

    val artifactsAltered = AtomicBoolean(false)

    private val listOfProviders = mutableListOf(initialScopedContent)

    /** Reset the current provider of the artifact the new file collection and make sure the final version points to the new content. */
    @Synchronized
  ...
  internal fun getScopedArtifactsContainer(type: ScopedArtifact) =
    scopedArtifacts.getOrPut(type) { ScopedArtifactsContainer(fileCollectionCreator) }

  internal fun getScopedArtifactsContainer(type: InternalScopedArtifact) =
    internalScopedArtifacts.getOrPut(type) { ScopedArtifactsContainer(fileCollectionCreator) }

  /**
   * Publish the final version of [type] artifact under a different internal [into] type. This is useful when some code path produce
   * different versions of [into] artifacts from the ones registered under [type] and consumers just want to use the [into] type
   * irrespectively.
   */
  internal fun republish(type: ScopedArtifact, into: InternalScopedArtifact) {
    getScopedArtifactsContainer(into).initialScopedContent.from(getFinalArtifacts(type))
  }

  internal fun getFinalArtifacts(type: ScopedArtifact): FileCollection = getScopedArtifactsContainer(type).finalScopedContent

  internal fun getFinalArtifacts(type: InternalScopedArtifact): FileCollection = getScopedArtifactsContainer(type).finalScopedContent
  ...
  class ScopedArtifactsOperationImpl<T : Task>(
      ...
      taskProvider.configure {
        when (val provider = with(it)) {
          ...
          is DirectoryProperty -> setContentPath(to, provider, taskProvider.name)
            ...
            throw RuntimeException("Only RegularFileProperty or DirectoryProperty" + " instances are supported, got ${provider.javaClass}")
        }
      }
    ...
    override fun toGet(
      ...
      val scopedContent = scopedArtifacts.getScopedArtifactsContainer(type).finalScopedContent
    ...
    override fun <ArtifactT> toTransform(
      ...
      taskProvider.configure { task ->
        inputJars(task).set(currentScopedContent.getRegularFiles(projectLayout.projectDirectory))
        inputDirectories(task).set(currentScopedContent.getDirectories(projectLayout.projectDirectory))
    ...
    internal fun toTransform(
      ...
      // set the [Task] input and output fields.
      taskProvider.configure { task ->
        inputJars(task).setFrom(currentScopedContent.getRegularFiles(projectLayout.projectDirectory))
        inputDirectories(task).setFrom(currentScopedContent.getDirectories(projectLayout.projectDirectory))
      ...
      newContent.from(
        ...
        taskProvider.flatMap { task -> intoJarDirectory(task).map { it.asFileTree.files } },
    ...
    internal fun toFork(
      ...
      scopedArtifacts.getScopedArtifactsContainer(intoType).initialScopedContent.from(newContent)
    }
    ...
    override fun <ArtifactT> toReplace(type: ArtifactT, into: (T) -> RegularFileProperty)
      ...
      taskProvider.configure { task -> setContentPath(type, into(task), taskProvider.name) }
      resetContentProvider(type, into)
    }

    private fun setContentPath(type: ScopedArtifact, into: RegularFileProperty, vararg paths: String) {
      into.set(
    ...
    private fun setContentPath(type: ScopedArtifact, into: DirectoryProperty, vararg paths: String) {
      into.set(
        type.getIntermediateOutputPath(
          buildDirectory = projectLayout.buildDirectory,
          variantIdentifier = scopedArtifacts.variantIdentifier,
    ...
    private fun resetContentProvider(type: ScopedArtifact, into: (T) -> RegularFileProperty) {
      ...
      fileCollectionCreator.invoke().also { newScopedContent ->
        newScopedContent.setFrom(taskProvider.flatMap { task -> into(task) })
