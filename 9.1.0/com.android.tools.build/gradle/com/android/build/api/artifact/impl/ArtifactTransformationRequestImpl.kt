...
class ArtifactTransformationRequestImpl<TaskT : Task>(
  ...
  override fun <ParamT : WorkParameters> submit(
    ...
    if (sourceBuiltArtifacts == null) {
      builtArtifactsReference.set(
        BuiltArtifactsImpl(artifactType = outputArtifactType, applicationId = "unknown", variantName = "unknown", elements = listOf())
      )
    ...
    sourceBuiltArtifacts.elements.forEach { builtArtifact ->
      workQueue.submit(actionType) { parameters ->
        if (task is BaseTask && parameters is DecoratedWorkParameters) {
          ...
          parameters.analyticsService.get().workerAdded(task.path, workerKey)
        }

        mapOfBuiltArtifactsToParameters[builtArtifact] = parameterConfigurator(builtArtifact, outputLocation(task).get(), parameters)
      }
    }
    ...
    builtArtifactsReference.set(
      sourceBuiltArtifacts.copy(
        ...
        elements =
          sourceBuiltArtifacts.elements.map {
            val output = mapOfBuiltArtifactsToParameters[it] ?: throw RuntimeException("Unknown BuiltArtifact $it, file a bug")
  ...
  override fun submit(task: TaskT, transformer: (input: BuiltArtifact) -> File) {

    val sourceBuiltArtifacts =
      BuiltArtifactsLoaderImpl().load(inputLocation(task).get()) ?: throw RuntimeException("No provided artifacts.")

    builtArtifactsReference.set(
      sourceBuiltArtifacts.copy(
        artifactType = outputArtifactType,
        elements = sourceBuiltArtifacts.elements.map { it.newOutput(transformer(it).toPath()) },
      )
    )
  }
}
