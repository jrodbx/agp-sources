...
class ManifestFilesImpl(private val variantServices: VariantServices) : ManifestFiles {
  ...
  override fun addStaticManifestFile(relativeFilePath: String) {
    ...
    addSource(FileBasedFileEntry(name = "variant", file = file.asFile, isUserAdded = true, shouldBeAddedToIdeModel = true))
  }

  override fun <TASK : Task> addGeneratedManifestFile(taskProvider: TaskProvider<TASK>, wiredWith: (TASK) -> RegularFileProperty) {
    val mappedValue: Provider<RegularFile> = taskProvider.flatMap { wiredWith(it) }
    taskProvider.configure { task ->
      wiredWith
        .invoke(task)
        .set(variantServices.projectInfo.buildDirectory.file("${SdkConstants.FD_GENERATED}/$name/${taskProvider.name}/AndroidManifest.xml"))
    }
    addSource(TaskProviderBasedFileEntry("$name-${taskProvider.name}", mappedValue, isGenerated = true, isUserAdded = true))
  }

  /** For internal usage. File may not exist. */
  internal fun addSourceFile(file: File) {
    addSource(FileBasedFileEntry(name = "variant", file = file, isUserAdded = false, shouldBeAddedToIdeModel = true))
  }

  // this will contain all files
  private val files = variantServices.newListPropertyForInternalUse(type = RegularFile::class.java)
  ...
  private val staticFiles = variantServices.newListPropertyForInternalUse(type = RegularFile::class.java)
  ...
  fun addSource(fileEntry: FileEntry) {
    fileEntry.asFile(variantServices.provider { variantServices.projectInfo.projectDirectory }).let {
      files.add(it)
