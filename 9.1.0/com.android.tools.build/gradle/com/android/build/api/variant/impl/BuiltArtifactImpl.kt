...
private constructor(
  ...
  fun newOutput(newOutputFile: Path): BuiltArtifactImpl = copy(outputFile = newOutputFile.toFile().invariantSeparatorsPath)

  fun getFilter(filterType: FilterConfiguration.FilterType): FilterConfiguration? = filters.firstOrNull { it.filterType == filterType }

  fun toGenericBuiltArtifact() =
    GenericBuiltArtifact(
      outputType = outputType.toString(),
      filters =
        filters.map { filterConfiguration ->
          GenericFilterConfiguration(filterConfiguration.filterType.toString(), filterConfiguration.identifier)
        },
  ...
  companion object {
    ...
    ) = BuiltArtifactImpl(FileUtils.toSystemIndependentPath(outputFile), versionCode, versionName, variantOutputConfiguration, attributes)
  }
}
      ...
      { outputFile: String, versionCode: Int, versionName: String ->
        BuiltArtifactImpl.make(
          ...
          variantOutputConfiguration =
            VariantOutputConfigurationImpl(isUniversal = OutputType.UNIVERSAL.name == outputType, filters = filters.build()),
          attributes = attributes,
        )
      },
    )
