...
class BuiltArtifactsLoaderImpl : BuiltArtifactsLoader {
  ...
  fun load(folder: FileSystemLocation): BuiltArtifactsImpl? {
    return loadFromFile(getMetadataFile(folder))
  }

  override fun load(fileCollection: FileCollection): BuiltArtifactsImpl? {
    val metadataFile = fileCollection.asFileTree.files.find { it.name == BuiltArtifactsImpl.METADATA_FILE_NAME }
  ...
  companion object {
    @JvmStatic fun loadFromDirectory(folder: File): BuiltArtifactsImpl? = loadFromFile(File(folder, BuiltArtifactsImpl.METADATA_FILE_NAME))
    ...
    fun loadFromFile(inputFile: File?): BuiltArtifactsImpl? {
      ...
      val buildOutputs =
        try {
          JsonReader(reader).use { BuiltArtifactsTypeAdapter(relativePathToUse).read(it) }
        } catch (e: Exception) {
          throw IOException(
            "Error parsing build artifacts from ${if (redirectedFile!=null) "$redirectedFile redirected from $inputFile" else inputFile}",
            e,
          )
        }
      // resolve the file path to the current project location.
      return buildOutputs.copy(
        elements = buildOutputs.elements.map { it.newOutput(relativePathToUse.resolve(Paths.get(it.outputFile)).normalize()) }
      )
    }
