...
/**
 * Implementation of [DirectoryEntry] for an existing directory. The directory is provided as a [Provider] of [Directory] for convenience,
 * the directory must exist with sources present during configuration time.
 *
...
class FileBasedDirectoryEntryImpl(
  ...
  override fun asFileTree(fileTreeCreator: () -> ConfigurableFileTree): Provider<List<ConfigurableFileTree>> =
    getFileTree(fileTreeCreator).run { elements.map { listOf(this) } }

  override fun asFileTreeWithoutTaskDependency(fileTreeCreator: () -> ConfigurableFileTree): List<ConfigurableFileTree> =
    listOf(getFileTree(fileTreeCreator))

  private fun getFileTree(fileTreeCreator: () -> ConfigurableFileTree): ConfigurableFileTree =
