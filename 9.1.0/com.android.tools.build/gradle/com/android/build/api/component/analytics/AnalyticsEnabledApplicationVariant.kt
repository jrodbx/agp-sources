...
open class AnalyticsEnabledApplicationVariant
@Inject
constructor(override val delegate: ApplicationVariant, stats: GradleBuildVariant.Builder, objectFactory: ObjectFactory) :
  AnalyticsEnabledVariant(delegate, stats, objectFactory), ApplicationVariant {
  ...
  override val outputs: List<VariantOutput>
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.GET_OUTPUTS_VALUE
      return delegate.outputs
    }

  override val dependenciesInfo: DependenciesInfo
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.DEPENDENCIES_INFO_VALUE
      return delegate.dependenciesInfo
    }

  private val userVisibleSigningConfig: AnalyticsEnabledSigningConfig by
    lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
      objectFactory.newInstance(AnalyticsEnabledSigningConfig::class.java, delegate.signingConfig, stats)
    }
  ...
  private val userVisiblePackaging: TestedApkPackaging by
    lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
      objectFactory.newInstance(AnalyticsEnabledTestedApkPackaging::class.java, delegate.packaging, stats)
    }
  ...
  private val userVisibleBundleConfig: BundleConfig by
    lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
      objectFactory.newInstance(AnalyticsEnabledBundleConfig::class.java, delegate.bundleConfig, stats)
    }

  override val bundleConfig: BundleConfig
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.GET_BUNDLE_CONFIG_VALUE
  ...
  override val shrinkResources: Boolean
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.SHRINK_RESOURCES_VALUE
