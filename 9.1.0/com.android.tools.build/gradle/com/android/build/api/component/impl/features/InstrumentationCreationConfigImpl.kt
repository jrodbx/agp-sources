...
class InstrumentationCreationConfigImpl(private val component: ComponentCreationConfig, private val internalServices: VariantServices) :
  InstrumentationCreationConfig {

  override val instrumentation: InstrumentationImpl by lazy {
    InstrumentationImpl(component.services, internalServices, isLibraryVariant = component is LibraryCreationConfig)
  }
  ...
  override val projectClassesAreInstrumented: Boolean
    get() =
      registeredProjectClassesVisitors.isNotEmpty() || (component as? ApkCreationConfig)?.advancedProfilingTransforms?.isNotEmpty() == true

  override val dependenciesClassesAreInstrumented: Boolean
    get() =
      registeredDependenciesClassesVisitors.isNotEmpty() ||
    ...
    if (projectClassesAreInstrumented) {
      if (asmFramesComputationMode == FramesComputationMode.COMPUTE_FRAMES_FOR_ALL_CLASSES) {
        internalServices.fileCollection(
          component.artifacts.get(InternalArtifactType.FIXED_STACK_FRAMES_ASM_INSTRUMENTED_PROJECT_CLASSES),
          internalServices
            .fileCollection(component.artifacts.get(InternalArtifactType.FIXED_STACK_FRAMES_ASM_INSTRUMENTED_PROJECT_JARS))
            .asFileTree,
        )
      } else {
        internalServices.fileCollection(
          component.artifacts.get(InternalArtifactType.ASM_INSTRUMENTED_PROJECT_CLASSES),
          internalServices.fileCollection(component.artifacts.get(InternalArtifactType.ASM_INSTRUMENTED_PROJECT_JARS)).asFileTree,
        )
      }
    } else {
      component.artifacts.forScope(ScopedArtifacts.Scope.PROJECT).getFinalArtifacts(ScopedArtifact.CLASSES)
    }
  }

  override fun getDependenciesClassesJarsPostInstrumentation(scope: AndroidArtifacts.ArtifactScope): FileCollection {
  ...
  override fun configureAndLockAsmClassesVisitors(objectFactory: ObjectFactory) {
    instrumentation.configureAndLockAsmClassesVisitors(objectFactory, component.global.asmApiVersion)
  }
}
