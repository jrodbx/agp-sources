 ...
 *
 * If [payloadIndirection] is false, then header files and library files are also copied into the structure.
 *
 * If [payloadIndirection] is true, then header files and library files are NOT copied into the structure but a list of payload mappings is
 * returned. These mappings describe the copies that would have happened.
 *
 * For reference, this is the folder layout when this function is finished: [installDir]/prefab.json [installDir]/modules/foo1/module.json
 * [installDir]/modules/foo1/include/... [installDir]/modules/foo1/libs/android.x86/abi.json [installDir]/modules/foo2/module.json
 * [installDir]/modules/foo2/include/... [installDir]/modules/foo2/libs/android.x86/abi.json
 *
 * The modules foo1 and foo2 are established in build.gradle by, for example: android { prefab { foo1 { headers "src/main/cpp/include"
 * libraryName "libfoo1" } } }
 */
fun buildPrefabPackage(
  ...
  return PrefabPackageBuilder(fileOperations, payloadIndirection).configurePrefab(publication)
}

/**
 * Describes a mapping of origin payload folder and file (.so, .a, .h, etc) paths to the analog path in the package that is being generated.
 */
...
private class PrefabPackageBuilder(private val fileOperations: FileSystemOperations?, private val payloadIndirection: Boolean) {
  ...
  private fun createPrefabJson(installDir: File, packageInfo: PrefabPackagePublication) {
  ...
  /** Create, for example, the modules/foo1 and modules/foo2 folders and their contents. */
  private fun createModule(module: PrefabModulePublication, packageDir: File) {
  ...
  private fun createModuleJson(module: PrefabModulePublication, moduleDir: File) {
    writeJsonFileIfDifferent(
      moduleDir.resolve("module.json"),
      ModuleMetadataV1(exportLibraries = module.moduleExportLibraries, libraryName = module.moduleLibraryName),
    )
  }

  /** Create the per-module includes folder and sync content into it. */
  private fun installHeaders(headers: File?, moduleDir: File) {
    ...
    if (payloadIndirection) {
      addPayloadMapping(originModulePath = headers, builtPackagePath = includeDir)
      // Prefab requires a file to be in the include folder.
      if (headers.isDirectory) {
        includeDir.resolve("placeholder.txt").writeTextIfDifferent("")
      }
      return
    }
  ...
  private fun createLibs(moduleDir: File, module: PrefabModulePublication) {
  ...
  private fun createAbiJson(libDir: File, abi: PrefabAbiPublication) {
    libDir.mkdirs()
    writeJsonFileIfDifferent(
    ...
    if (payloadIndirection) {
      addPayloadMapping(originModulePath = source, builtPackagePath = destination)
      return
    }
    infoln("Installing $source to $destination")
    synchronizeFile(source = source, destination = destination)
  }

  private fun addPayloadMapping(originModulePath: File, builtPackagePath: File) {
    payloadMappings.add(
      PayloadMapping(originModulePath = originModulePath.platformSlashes(), builtPackagePath = builtPackagePath.platformSlashes())
    )
  }

  private fun File.platformSlashes() =
