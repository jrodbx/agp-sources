 ...
 *
 * The function 'createConfigurationInvalidationState' decides whether a C/C++ configure is needed. It does this by checking various file
 * sizes and timestamps.
 *
 * There are 4 kinds of files:
 * 1) input files: These files are inputs to the configuration phase. When they are known, changes to them will trigger a rerun of
 *    configuration. However, not all inputs are known until after the first configure is run (This is important, and it complicates the
 *    logic).
 * 2) optional output files: If these files exist, changes to them since the last configuration run will trigger a new configuration run.
 * 3) required output files: same as (2), but also a new configuration run will be triggered if these files don't exist.
 * 4) hard configure files: changes to these files will cause the configuration folder to be deleted and a new configuration will be
 *    triggered.
 *
 * Additionally, a support file called 'lastConfigureFingerPrintFile' is used to remember the existence, size, and timestamp of files as
 * they were after the last successful configuration run.
 *
 * The logic of 'createConfigurationInvalidationState' is complicated by the fact that some input files are not discovered until after the
 * first configure has executed.
 * ------------------------------------------------------------------------------------
 */
fun createConfigurationInvalidationState(
  ...
  val result =
    ConfigureInvalidationState.newBuilder()
  ...
  val lastConfigureFingerPrint =
    tryReadFingerPrintFile(lastConfigureFingerPrintFile) ?: return result.setConfigureType(HARD_CONFIGURE).build()

  val compareLastFingerPrintFilesToCurrent = lastConfigureFingerPrint.map { it to it.compareToCurrent() }
  val (unchangedFingerPrintFiles, changedFingerPrintFiles) = compareLastFingerPrintFilesToCurrent.partition { it.second == null }
  ...
  val softConfigureReasons = (changedInputFiles + changedRequiredOutputFiles + changedOptionalOutputFiles).distinct()
  val configureType =
    when {
      forceConfigure || changedHardConfigureFiles.isNotEmpty() -> HARD_CONFIGURE
  ...
  val compareLastFingerPrintFilesToCurrentAgain = lastConfigureFingerPrint.map { it.compareToCurrent() }
  for (i in compareLastFingerPrintFilesToCurrentAgain.indices) {
    ...
    if (firstCompareToCurrent != secondCompareToCurrent) {
      ...
      return result.setConfigureType(HARD_CONFIGURE).build()
    }
  }
...
/** Read the fingerprint file, return null if it doesn't exist or if there are other problems reading it. */
private fun tryReadFingerPrintFile(lastConfigureFingerPrintFile: File): List<FileFingerPrint>? {
  val lastConfigureFingerPrint =
    if (lastConfigureFingerPrintFile.isFile)
      try {
        readStructuredLog(lastConfigureFingerPrintFile, ::decodeFileFingerPrint)
      } catch (e: Exception) {
        errorln(FINGER_PRINT_FILE_CORRUPTED, "Could not read '$lastConfigureFingerPrintFile': ${e.message}")
        null
      }
    else null
 ...
 *
 * The fingerprint file contains all files that were known the last time configure ran (even if those files didn't exist on disk). If there
 * is a file that isn't known to the fingerprint file then it is a new input (for example, CMakeLists.txt changed to pull in a new
 * dependency.cmake).
 *
 * A new file appearing this way alone doesn't mean that we should redo C++ configure, instead we compare unknown files like this to the
 * timestamp of the fingerprint file itself (which was written after the last successful C++ configure).
 */
private fun computeChangedFiles(
  ...
  return files.mapNotNull { file ->
    ...
    }?.let { type -> ChangedFile.newBuilder().setFileName(file.path).setType(type).build() }
  }
}

/** Whether configure should be executed or not. */
val ConfigureInvalidationState.shouldConfigure: Boolean
  get() = configureType == HARD_CONFIGURE || configureType == SOFT_CONFIGURE

/** Whether it's okay to *not* delete the configuration folder before configuration is executed. */
val ConfigureInvalidationState.softConfigureOkay: Boolean
  get() = configureType == SOFT_CONFIGURE
...
val ConfigureInvalidationState.shouldConfigureReasonMessages: List<String>
  get() {
    ...
    val softRegenerateMessage = if (softConfigureOkay) "" else ", will remove stale configuration folder"
    ...
    if (hardConfigureReasonsList.isNotEmpty()) {
      ...
      hardConfigureReasonsList.forEach { messages += "  - ${it.fileName} (${it.type})" }
      return messages
    }

    if (softConfigureReasonsList.isNotEmpty()) {
      messages += "- a file changed$softRegenerateMessage"
      softConfigureReasonsList.forEach { messages += "  - ${it.fileName} (${it.type})" }
      return messages
    }
...
fun ConfigureInvalidationState.recordConfigurationFingerPrint() {
  val fingerPrints =
    (inputFilesList + requiredOutputFilesList + optionalOutputFilesList + hardConfigureFilesList).distinct().sorted().map {
      File(it).fingerPrint
    }
  File(fingerPrintFile).delete()
  CxxStructuredLogEncoder(File(fingerPrintFile)).use { encoder ->
    fingerPrints.forEach { fingerPrint -> encoder.write(fingerPrint.encode(encoder)) }
  }
}

/** Check whether the file has changed and report the kind of change that occurred. Returns null if there was no change. */
