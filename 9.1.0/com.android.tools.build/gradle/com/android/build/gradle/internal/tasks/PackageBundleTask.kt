...
abstract class PackageBundleTask : NonIncrementalTask() {
  ...
  @get:Input abstract val bundleType: Property<Config.BundleConfig.BundleType>

  @get:InputFiles @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val baseModuleZip: DirectoryProperty
  ...
  @get:InputFiles @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val assetPackZips: DirectoryProperty

  @get:InputFile @get:Optional @get:PathSensitive(PathSensitivity.NONE) abstract val bundleDeps: RegularFileProperty

  @get:InputFile @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val mainDexList: RegularFileProperty

  @get:InputFiles @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val obsfuscationMappingFile: RegularFileProperty

  @get:InputFiles @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val integrityConfigFile: RegularFileProperty

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val nativeDebugMetadataFiles: ConfigurableFileCollection

  @get:Input abstract val abiFilters: SetProperty<String>

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val appMetadata: RegularFileProperty

  @get:InputFile @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val d8Metadata: RegularFileProperty

  @get:InputFile @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val r8Metadata: RegularFileProperty
  ...
  @get:Input abstract val compressNativeLibs: Property<Boolean>

  @get:Input @get:Optional abstract val pageSize: Property<Long>

  @get:Nested @get:Optional abstract val assetPackOptionsForAssetPackBundle: Property<AssetPackOptionsForAssetPackBundle>
  ...
  @get:Input abstract val bundleNeedsFusedStandaloneConfig: Property<Boolean>

  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val binaryArtProfile: RegularFileProperty

  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val binaryArtProfileMetadata: RegularFileProperty

  @get:Nested abstract val metadataFiles: ListProperty<MetadataRecord>

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val deviceTargetingConfig: RegularFileProperty
  ...
  abstract class BundleToolWorkAction : ProfileAwareWorkAction<Params>() {
    override fun run() {
      ...
      val noCompressGlobsForBundle = PackagingUtils.getNoCompressGlobsForBundle(parameters.aaptOptionsNoCompress.get())

      val splitsConfig =
        Config.SplitsConfig.newBuilder()
          .splitBy(Config.SplitDimension.Value.ABI, parameters.bundleOptions.get().enableAbi)
          .splitBy(Config.SplitDimension.Value.SCREEN_DENSITY, parameters.bundleOptions.get().enableDensity)
      ...
      parameters.bundleOptions.get().enableDeviceTier?.let {
        splitsConfig.addSplitDimension(
          Config.SplitDimension.newBuilder()
            ...
            .setSuffixStripping(
              Config.SuffixStripping.newBuilder().setEnabled(true).setDefaultSuffix(parameters.bundleOptions.get().defaultDeviceTier ?: "")
            )
            .setNegate(!it)
        )
      }
      parameters.bundleOptions.get().enableDeviceGroup?.let {
        splitsConfig.addSplitDimension(
          Config.SplitDimension.newBuilder()
            ...
            .setSuffixStripping(
              Config.SuffixStripping.newBuilder().setEnabled(true).setDefaultSuffix(parameters.bundleOptions.get().defaultDeviceGroup ?: "")
            )
            .setNegate(!it)
        )
      }
      parameters.bundleOptions.get().enableCountrySet?.let {
        splitsConfig.addSplitDimension(
          Config.SplitDimension.newBuilder()
            ...
            .setSuffixStripping(
              Config.SuffixStripping.newBuilder().setEnabled(true).setDefaultSuffix(parameters.bundleOptions.get().defaultCountrySet ?: "")
            )
            .setNegate(!it)
        )
      }
      ...
      val pageAlignment =
        when (parameters.pageSize.orNull) {
      ...
      val uncompressNativeLibrariesConfig =
        Config.UncompressNativeLibraries.newBuilder().setEnabled(!parameters.compressNativeLibs.get()).setAlignment(pageAlignment)

      val bundleOptimizations =
      ...
      if (parameters.bundleNeedsFusedStandaloneConfig.get()) {
        bundleOptimizations.setStandaloneConfig(
          Config.StandaloneConfig.newBuilder()
            .addSplitDimension(Config.SplitDimension.newBuilder().setValue(Config.SplitDimension.Value.ABI).setNegate(true))
            .addSplitDimension(Config.SplitDimension.newBuilder().setValue(Config.SplitDimension.Value.SCREEN_DENSITY).setNegate(true))
            .addSplitDimension(Config.SplitDimension.newBuilder().setValue(Config.SplitDimension.Value.LANGUAGE).setNegate(true))
            .addSplitDimension(
              Config.SplitDimension.newBuilder().setValue(Config.SplitDimension.Value.TEXTURE_COMPRESSION_FORMAT).setNegate(true)
            )
      ...
      val bundleConfig =
        Config.BundleConfig.newBuilder()
          ...
          .setCompression(Config.Compression.newBuilder().addAllUncompressedGlob(noCompressGlobsForBundle))
      ...
      if (parameters.assetPackOptionsForAssetPackBundle.isPresent) {
        bundleConfig.assetModulesConfigBuilder
          .setAssetVersionTag(parameters.assetPackOptionsForAssetPackBundle.get().versionTag)
          .addAllAppVersion(parameters.assetPackOptionsForAssetPackBundle.get().versionCodes.map { it.toLong() })
      }
      ...
      if (parameters.binaryArtProfiler.isPresent && parameters.binaryArtProfiler.get().asFile.exists()) {
      ...
      if (parameters.binaryArtProfilerMetadata.isPresent && parameters.binaryArtProfilerMetadata.get().asFile.exists()) {
        command.addMetadataFile(
          SdkConstants.FN_BINART_ART_PROFILE_FOLDER_IN_AAB,
      ...
      parameters.mainDexList.asFile.orNull?.let { command.setMainDexListFile(it.toPath()) }
      ...
      parameters.deviceTargetingConfig.asFile.orNull?.let {
        if (it.isFile) {
          command.addMetadataFile("com.android.tools.build.bundletool", "DeviceGroupConfig.pb", it.toPath())
        }
      }
      ...
      parameters.nativeDebugMetadataFiles.forEach { file ->
        ...
        if (abiFilters.isEmpty() || abiFilters.contains(abi)) {
          command.addMetadataFile("com.android.tools.build.debugsymbols", "$abi/${file.name}", file.toPath())
        }
      }

      command.addMetadataFile("com.android.tools.build.gradle", APP_METADATA_FILE_NAME, parameters.appMetadata.asFile.get().toPath())

      parameters.d8Metadata.orNull?.let { d8Metadata ->
        command.addMetadataFile("com.android.tools", "d8.json", d8Metadata.asFile.toPath())
      }
      ...
      parameters.metadataFiles.get().forEach { metadataFile ->
        metadataFile.asFile.let { command.addMetadataFile(directories.next(), it.name, it.toPath()) }
      }
  ...
  data class BundleOptions(
    @get:Input @get:Optional val enableAbi: Boolean?,
    @get:Input @get:Optional val enableDensity: Boolean?,
    @get:Input @get:Optional val enableLanguage: Boolean?,
    @get:Input @get:Optional val enableTexture: Boolean?,
    @get:Input @get:Optional val textureDefaultFormat: String?,
    @get:Input @get:Optional val enableDeviceTier: Boolean?,
    @get:Input @get:Optional val defaultDeviceTier: String?,
    @get:Input @get:Optional val enableDeviceGroup: Boolean?,
    @get:Input @get:Optional val defaultDeviceGroup: String?,
    @get:Input val enableStoreArchive: Boolean,
    @get:Input @get:Optional val enableCountrySet: Boolean?,
    @get:Input @get:Optional val defaultCountrySet: String?,
    @get:Input @get:Optional val enableAiModelVersion: Boolean?,
    @get:Input @get:Optional val defaultAiModelVersion: String?,
  ) : Serializable

  data class AssetPackOptionsForAssetPackBundle(@get:Input val versionCodes: List<Int>, @get:Input var versionTag: String?) : Serializable
  ...
  class CreationForAssetPackBundleAction(
    ...
    override fun handleProvider(taskProvider: TaskProvider<PackageBundleTask>) {
      super.handleProvider(taskProvider)
      artifacts.setInitialProvider(taskProvider, PackageBundleTask::bundleFile).on(InternalArtifactType.INTERMEDIARY_BUNDLE)
    }
    ...
    override fun configure(task: PackageBundleTask) {
      ...
      artifacts.setTaskInputToFinalProduct(InternalArtifactType.ASSET_PACK_BUNDLE, task.assetPackZips)
      ...
      task.assetPackOptionsForAssetPackBundle.set(
        AssetPackOptionsForAssetPackBundle(versionCodes = assetPackBundle.versionCodes.toList(), versionTag = assetPackBundle.versionTag)
      )
      task.bundleNeedsFusedStandaloneConfig.set(false)
      artifacts.setTaskInputToFinalProduct(InternalArtifactType.APP_METADATA, task.appMetadata)
  ...
  class CreationAction(private val componentProperties: ApplicationCreationConfig) :
    VariantTaskCreationAction<PackageBundleTask, ApplicationCreationConfig>(componentProperties) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<PackageBundleTask>) {
      ...
      creationConfig.artifacts.setInitialProvider(taskProvider, PackageBundleTask::bundleFile).on(InternalArtifactType.INTERMEDIARY_BUNDLE)
    }
    ...
    override fun configure(task: PackageBundleTask) {
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.MODULE_BUNDLE, task.baseModuleZip)
      ...
      if (creationConfig.needAssetPackTasks) {
        creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.ASSET_PACK_BUNDLE, task.assetPackZips)
      }

      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.BUNDLE_DEPENDENCY_REPORT, task.bundleDeps)

      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.APP_INTEGRITY_CONFIG, task.integrityConfigFile)

      if (creationConfig.services.projectOptions[BooleanOption.PACKAGE_NATIVE_DEBUG_METADATA_IN_APP_BUNDLE]) {
        task.nativeDebugMetadataFiles.fromDisallowChanges(MergeNativeDebugMetadataTask.getNativeDebugMetadataFiles(creationConfig))
      } else {
        task.nativeDebugMetadataFiles.disallowChanges()
      }

      task.abiFilters.setDisallowChanges(creationConfig.nativeBuildCreationConfig?.supportedAbis ?: emptyList())
      ...
      if (!creationConfig.services.projectOptions[BooleanOption.ENABLE_DEVICE_TARGETING_CONFIG_API]) {
        if (task.bundleOptions.enableDeviceGroup != null || task.bundleOptions.defaultDeviceGroup != null) {
          throw IllegalStateException("Feature deviceGroup splits is not enabled by default.")
        }
      }

      task.compressNativeLibs.set(componentProperties.packaging.jniLibs.useLegacyPackagingFromBundle)
      ...
      if (creationConfig.dexing.needsMainDexListForBundle) {
        creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.MAIN_DEX_LIST_FOR_BUNDLE, task.mainDexList)
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(SingleArtifact.OBFUSCATION_MAPPING_FILE, task.obsfuscationMappingFile)
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.APP_METADATA, task.appMetadata)
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.D8_METADATA, task.d8Metadata)
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.R8_METADATA, task.r8Metadata)

      if (!creationConfig.debuggable) {
        creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.BINARY_ART_PROFILE, task.binaryArtProfile)
        creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.BINARY_ART_PROFILE_METADATA, task.binaryArtProfileMetadata)
      }
      ...
      if (creationConfig.global.bundleOptions.deviceTargetingConfig.isPresent) {
        if (creationConfig.services.projectOptions[BooleanOption.ENABLE_DEVICE_TARGETING_CONFIG_API]) {
          creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.DEVICE_TARGETING_CONFIG, task.deviceTargetingConfig)
        } else {
 ...
 * @param value if true, split is enabled for the given flag. If null, no change is made and the bundle-tool will decide the value.
 */
private fun Config.SplitsConfig.Builder.splitBy(flag: Config.SplitDimension.Value, value: Boolean?): Config.SplitsConfig.Builder {
  value?.let { addSplitDimension(Config.SplitDimension.newBuilder().setValue(flag).setNegate(!it)) }
  return this
}
