...
abstract class AarToClassTransform : TransformAction<AarToClassTransform.Params> {
  ...
  interface Params : TransformParameters {
     ...
     */
    @get:Input val generateRClassJar: Property<Boolean>

    /** If set, return the compile classpath, otherwise return the runtime classpath. */
    @get:Input val forCompileUse: Property<Boolean>
  }
  ...
  override fun transform(transformOutputs: TransformOutputs) {
    ...
    ZipFile(inputAarFile.get().asFile).use { inputAar ->
      ...
      val outputFileName = "${inputAarFile.get().asFile.nameWithoutExtension}-$useSuffix$DOT_JAR"
      val outputJar = transformOutputs.file(outputFileName).toPath()
      mergeJars(outputJar, inputAar, parameters.forCompileUse.get(), parameters.generateRClassJar.get())
    }
  }

  companion object {
    @VisibleForTesting
    internal fun mergeJars(outputJar: Path, inputAar: ZipFile, forCompileUse: Boolean, generateRClassJar: Boolean) {
      val ignoreFilter =
        if (forCompileUse) {
    ...
    internal fun ZipFile.copyAllClassesJarsTo(outputApiJar: JarFlinger) {
      entries().asSequence().filter(::isClassesJar).forEach { copyEntryTo(it, outputApiJar) }
    }
    ...
    internal fun generateRClassJarFromRTxt(outputApiJar: JarFlinger, inputAar: ZipFile) {
      val manifest = inputAar.getEntry(FN_ANDROID_MANIFEST_XML)
      val pkg = inputAar.getInputStream(manifest).use { AndroidManifestParser.parse(it).`package` }
    ...
    private fun isClassesJar(entry: ZipEntry): Boolean {
      val name = entry.name
      return name == FN_CLASSES_JAR || (name.startsWith(LIBS_FOLDER_SLASH) && name.endsWith(DOT_JAR))
    }
  }
}
