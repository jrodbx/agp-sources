...
class BuildConfigByteCodeGenerator(private val data: BuildConfigData) : GeneratedCodeFileCreator {

  private val fullyQualifiedBuildConfigClassName: String by lazy { "${data.namespace.replace('.', '/')}/${data.buildConfigName}" }

  override val folderPath: File = data.outputPath.toFile().also { it.mkdirs() }

  override val generatedFilePath: File = File(folderPath, "${data.buildConfigName}${SdkConstants.DOT_JAR}")

  /** Creates a JAR file within the genFolder containing a build config .class which is generated based on the current class attributes. */
  override fun generate() =
    writeToJar(
      generatedFilePath.toPath(),
      """${data.namespace.replace('.', '/')}/${data
                    .buildConfigName}${SdkConstants.DOT_CLASS}"""
        .trimMargin(),
      generateByteCode(),
    )
  ...
  private fun generateUsingAsm(): ClassWriter {
    ...
    // Class Signature
    cw.visit(V1_8, ACC_PUBLIC + ACC_FINAL + ACC_SUPER, fullyQualifiedBuildConfigClassName, null, "java/lang/Object", null)

    // Field Attributes
    data.buildConfigFields.forEach { it.value.emit(it.key, cw) }
  ...
  private fun writeToJar(outputPath: Path, buildConfigPackage: String, bytecodeBuildConfig: ByteArray) {
