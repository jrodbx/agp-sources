...
/** A string and list-of-string to ordinal int table class. Strings and lists share the same incremented ID counter. */
data class StringTable(
  ...
  /**
   * Given a [String] return its integer ID value. A new ID will be created if necessary and, in this case, the [createAction] function will
   * be called.
   */
  ...
  /**
   * Given a list of [Int] return its integer ID value. If this is the first time this list was seen, then [createAction] will be invoked
   * with its new ID.
   */
  ...
  /** Return the [String] corresponding to the given integer [id]. Will throw an exception if [id] is out of range. */
  ...
  /** Return the id corresponding to the given [string]. Will throw an exception if [string] is not known */
