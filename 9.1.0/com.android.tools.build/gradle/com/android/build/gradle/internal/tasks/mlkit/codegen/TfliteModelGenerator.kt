...
class TfliteModelGenerator(private val modelFile: File, private val packageName: String, private val localModelPath: String) :
  ModelGenerator {
  ...
  override fun generateBuildClass(outputDirProperty: DirectoryProperty) {
    val classBuilder = TypeSpec.classBuilder(className).addModifiers(Modifier.PUBLIC, Modifier.FINAL)
    ...
    if (modelInfo.isMetadataExisted) {
      ...
      classBuilder.addJavadoc("This model doesn't have metadata, so no javadoc can be generated.")
    }
    ...
    // Final steps.
    try {
      JavaFile.builder(packageName, classBuilder.build()).build().writeTo(outputDirProperty.asFile.get())
    } catch (e: IOException) {
  ...
  private fun buildConstructor(classBuilder: TypeSpec.Builder) {
    val context = ParameterSpec.builder(ClassNames.CONTEXT, "context").addAnnotation(ClassNames.NON_NULL).build()
    val options = ParameterSpec.builder(ClassNames.MODEL_OPTIONS, "options").addAnnotation(ClassNames.NON_NULL).build()
    val constructorBuilder =
      MethodSpec.constructorBuilder()
        ...
        .addStatement("\$L = \$T.createModel(context, \$S, options)", FIELD_MODEL, ClassNames.MODEL, localModelPath)
        .addStatement("\$T extractor = new \$T(model.getData())", ClassNames.METADATA_EXTRACTOR, ClassNames.METADATA_EXTRACTOR)
  ...
  private fun buildProcessMethod(classBuilder: TypeSpec.Builder, isGeneric: Boolean = false) {
    val outputType: TypeName = ClassName.get(packageName, className).nestedClass(MlNames.OUTPUTS)
    val localOutputs = "outputs"
    val methodBuilder =
      MethodSpec.methodBuilder("process").addModifiers(Modifier.PUBLIC).addAnnotation(ClassNames.NON_NULL).returns(outputType)
    ...
    for (tensorInfo in modelInfo.inputs) {
      ...
      val parameterSpec = ParameterSpec.builder(parameterType, tensorInfo.identifierName).addAnnotation(ClassNames.NON_NULL).build()
  ...
  private fun buildCloseMethod(classBuilder: TypeSpec.Builder) {
    val methodBuilder = MethodSpec.methodBuilder("close").addModifiers(Modifier.PUBLIC).addStatement("\$L.close()", FIELD_MODEL)
    classBuilder.addMethod(methodBuilder.build())
  }
