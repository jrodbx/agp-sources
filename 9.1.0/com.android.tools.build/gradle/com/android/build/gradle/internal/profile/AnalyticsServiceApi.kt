...
interface AnalyticsServiceApi {
  ...
  /** Records the time elapsed while executing a [Recorder.VoidBlock] and append the span to the build profile */
  fun recordBlock(
  ...
  /**
   * In non-configuration cached run, we records time for configuring project and creating base extension & tasks. Those spans are passed
   * from [AnalyticsConfiguratorService] to [AnalyticsService] at the end of configuration when [AnalyticsService] is instantiated.
   */
  fun setConfigurationSpans(spans: ConcurrentLinkedQueue<GradleBuildProfileSpan>)

  /**
   * In non-configuration cached run, the initial memory sample is collected at the beginning of configuration phase and passed from
   * [AnalyticsConfiguratorService] to [AnalyticsService] at the end of configuration when [AnalyticsService] is instantiated.
   */
