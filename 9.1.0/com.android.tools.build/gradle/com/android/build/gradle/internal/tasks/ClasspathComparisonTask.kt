...
/**
 * Abstract class used to compare two configurations, and to report differences in versions of artifacts. This is useful to warn user about
 * potential issues that could arise from such differences. E.g. for application, differences in runtime and compile classpath could result
 * in runtime failure.
 *
...
abstract class ClasspathComparisonTask : NonIncrementalTask() {
  ...
  @get:Input abstract val runtimeVersionMap: MapProperty<Info, String>

  @get:Input abstract val compileVersionMap: MapProperty<Info, String>
  ...
  protected abstract fun onDifferentVersionsFound(group: String, module: String, runtimeVersion: String, compileVersion: String)
  ...
  override fun doTaskAction() {
    ...
    for (compileEntry in compileVersionMap.get().entries) {
      ...
      if (runtimeVersion != compileEntry.value) {
        onDifferentVersionsFound(compileEntry.key.group, compileEntry.key.module, runtimeVersion, compileEntry.value)
      }
    }
  }
}

data class Info(val group: String, val module: String) : Serializable {
  companion object {
