...
abstract class BaseVariantData(
  ...
  /**
   * If true, variant outputs will be considered signed. Only set if you manually set the outputs to point to signed files built by other
   * tasks.
   */
  @JvmField var outputsAreSigned = false
  @JvmField var applicationIdTextResource: TextResource = services.projectInfo.createTestResources("")

  fun getGeneratedBytecode(generatorKey: Any?): FileCollection {
    val fileCollection =
      if (generatorKey == null) {
        allPreJavacGeneratedBytecode
      } else preJavacGeneratedBytecodeMap?.get(generatorKey) ?: throw RuntimeException("Bytecode generator key not found")
  ...
  fun addJavaSourceFoldersToModel(vararg generatedSourceFolders: File) {
    Collections.addAll(extraGeneratedSourceFoldersOnlyInModel, *generatedSourceFolders)
  }
  ...
  fun registerPreJavacGeneratedBytecode(fileCollection: FileCollection): Any {
    val map = preJavacGeneratedBytecodeMap ?: mutableMapOf<Any, FileCollection>().also { preJavacGeneratedBytecodeMap = it }
  ...
  /**
   * Calculates the filters for this variant. The filters can either be manually specified by the user within the build.gradle or can be
   * automatically discovered using the variant specific folders.
   *
  ...
  /**
   * Returns the filters values (as manually specified or automatically discovered) for a particular
   ...
   * @throws IllegalStateException if [.calculateFilters] has not been called prior to invoking this method.
   */
  fun getFilters(filterType: VariantOutput.FilterType): Set<String> {
    check(::abiFilters.isInitialized) { "calculateFilters method not called" }
  ...
  override fun toString(): String {
    return MoreObjects.toStringHelper(this).addValue(componentIdentity.name).toString()
  }
  ...
  companion object {
     ...
     */
    private fun getFilters(filterType: DiscoverableFilterType, splits: Splits): Set<String> {
