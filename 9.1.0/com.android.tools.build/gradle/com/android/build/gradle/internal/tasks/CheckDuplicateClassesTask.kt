...
/**
 * A task that checks that project external dependencies do not contain duplicate classes. Without this task in case duplicate classes exist
 * the failure happens during dexing stage and the error is not especially user friendly. Moreover, we would like to fail fast.
 *
...
abstract class CheckDuplicateClassesTask : NonIncrementalTask() {
  ...
  @get:InputFiles
  val enumeratedClasses: FileCollection
    get() = enumeratedClassesArtifacts.artifactFiles
  ...
  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<CheckDuplicateClassesTask, ComponentCreationConfig>(creationConfig) {

    override val type = CheckDuplicateClassesTask::class.java

    override val name = computeTaskName("check", "DuplicateClasses")

    override fun handleProvider(taskProvider: TaskProvider<CheckDuplicateClassesTask>) {
      super.handleProvider(taskProvider)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, CheckDuplicateClassesTask::dummyOutputDirectory)
    ...
    override fun configure(task: CheckDuplicateClassesTask) {
      ...
      task.enumeratedClassesArtifacts =
        creationConfig.variantDependencies.getArtifactCollection(RUNTIME_CLASSPATH, EXTERNAL, ENUMERATED_RUNTIME_CLASSES)
    }
  }
}
