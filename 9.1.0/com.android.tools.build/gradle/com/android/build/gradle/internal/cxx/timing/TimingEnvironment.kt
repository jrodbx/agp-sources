...
/**
 * Time an operation. The result is written to a file defined by the current [TimingEnvironment] scope. Usage, time("operation") { // Long
 * running operation }
 */
 ...
 *
 * Usage, TimingEnvironment(folder, "generate-cxx-metadata").use { time("run-cmake") { // Log to run CMake for this configuration. } }
 *
 * Timing environments can be nested and the inner-most timing environment determines where the result of 'time()' calls goes.
 *
 * @param timingFolder is the output folder for the timings file.
 * @param outerTimingOperation is the opening timing scope inside the timings file and also the prefix used for the timings file name.
 * @param currentTimeMillis is a lambda used to return the current time in milliseconds. It's a lambda so that tests can replace it with
...
internal class TimingEnvironment(
  ...
  private class TimingScope(val operation: String, val start: Long) {
  ...
  /**
   * Close this [TimingEnvironment]. This closes the implicit 'time {...}' call started in the constructor. At the end, it pops this
   * [TimingEnvironment] from the stack of environments.
   */
  override fun close() {
  ...
  /**
   * The name of the current output file. Because [TimingEnvironment] is thread-local, the name of file needs to be unique for all current
   * threads.
   */
  internal fun getTimingFile() = timingFolder.resolve("${outerTimingOperation}_${Thread.currentThread().id}_timing.txt")

  /**
   * When a time() { } call is first made, we shouldn't yet output the name of the operation because we don't know yet whether it's going to
   * be a parent or leaf timing. We defer opening until the point this is known.
   */
  private fun openParentsIfNeeded(index: Int) {
    if (index >= timingScopeList.size || timingScopeList[index].openHasBeenReported) return
  ...
  companion object {
    /**
     * Singly-linked list where null is used as empty list. The purpose of using this structure rather than, say,
     * MutableList<TimingEnvironment> is that we need to leave zero allocated memory when the last environment scope is exited. If we leave
     * memory allocated it will have a GC root in thread local storage and this will be reported as a memory leak for tools that check
     * whether all instances allocated by a class loader are destroyed.
     */
    private data class TimingStack(val environment: TimingEnvironment, val next: TimingStack?)

    private val stack: ThreadLocal<TimingStack?> = ThreadLocal.withInitial { null }

    /** Push a new timing environment onto the stack of environments. */
    private fun push(timing: TimingEnvironment) = stack.set(TimingStack(timing, stack.get()))
    ...
    /** Return the depth of the current timer stack. This because the indentation level of the output report. */
    private val timerStackDepth: Int
      get() {
    ...
    internal fun openTimingScope(operation: String) {
      ...
      environment.timingScopeList.add(0, TimingScope(operation, environment.currentTimeMillis()))
    }
