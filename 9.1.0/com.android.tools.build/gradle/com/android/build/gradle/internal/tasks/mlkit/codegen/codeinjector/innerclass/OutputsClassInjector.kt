...
class OutputsClassInjector(private val metadata: ClassMetadata) : CodeInjector<TypeSpec.Builder, ModelInfo> {
  override fun inject(classBuilder: TypeSpec.Builder, modelInfo: ModelInfo) {
    ...
    for (tensorInfo in tensorInfos) {
      val fieldSpec = FieldSpec.builder(getParameterType(tensorInfo), tensorInfo.identifierName).addModifiers(Modifier.PRIVATE).build()
      builder.addField(fieldSpec)
    }

    // Add constructor.
    val constructorBuilder = MethodSpec.constructorBuilder().addParameter(ClassNames.MODEL, "model").addModifiers(Modifier.PRIVATE)
  ...
  companion object {
    private fun buildGetBufferMethod(classBuilder: TypeSpec.Builder, tensorInfos: List<TensorInfo>) {
      val mapType: TypeName = ParameterizedTypeName.get(ClassNames.MAP, ClassNames.INTEGER, ClassNames.OBJECT)
      val getterBuilder =
      ...
      for ((index, tensorInfo) in tensorInfos.withIndex()) {
        getterBuilder.addStatement("outputs.put(\$L, \$L.getBuffer())", index, tensorInfo.identifierName)
      }
