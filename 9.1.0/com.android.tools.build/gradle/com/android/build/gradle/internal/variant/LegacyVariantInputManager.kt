...
/** Implementation of [AbstractVariantInputManager] with the legacy types for build types, flavors, etc... */
class LegacyVariantInputManager(dslServices: DslServices, componentType: ComponentType, sourceSetManager: SourceSetManager) :
  AbstractVariantInputManager<DefaultConfig, BuildType, ProductFlavor, SigningConfig>(dslServices, componentType, sourceSetManager) {
  ...
  override val buildTypeContainer: NamedDomainObjectContainer<BuildType> =
    if (dslServices.projectOptions[BooleanOption.USE_DECLARATIVE_INTERFACES]) {
      dslServices.domainObjectContainer(DeclarativeBuildType::class.java, DeclarativeBuildTypeFactory(dslServices, componentType))
        as NamedDomainObjectContainer<BuildType>
    } else {
      dslServices.domainObjectContainer(BuildType::class.java, BuildTypeFactory(dslServices, componentType))
    }
  ...
  override val productFlavorContainer: NamedDomainObjectContainer<ProductFlavor> =
    if (dslServices.projectOptions[BooleanOption.USE_DECLARATIVE_INTERFACES]) {
      ...
      dslServices.domainObjectContainer(ProductFlavor::class.java, ProductFlavorFactory(dslServices))
    }
  ...
  override val signingConfigContainer: NamedDomainObjectContainer<SigningConfig> =
    dslServices.domainObjectContainer(
      ...
      SigningConfigFactory(
        dslServices,
        getBuildService(dslServices.buildServiceRegistry, AndroidLocationsBuildService::class.java).get().getDefaultDebugKeystoreLocation(),
      ),
    )
  ...
  init {
    ...
    if (componentType.hasTestComponents) {
      androidTestSourceSet = sourceSetManager.setUpSourceSet(ComponentType.ANDROID_TEST_PREFIX, ComponentTypeImpl.ANDROID_TEST)
      unitTestSourceSet = sourceSetManager.setUpSourceSet(ComponentType.UNIT_TEST_PREFIX, ComponentTypeImpl.UNIT_TEST)
      testFixturesSourceSet = sourceSetManager.setUpSourceSet(ComponentType.TEST_FIXTURES_PREFIX, ComponentTypeImpl.TEST_FIXTURES)
      screenshotTestSourceSet =
        if (dslServices.projectOptions[BooleanOption.ENABLE_SCREENSHOT_TEST]) {
          sourceSetManager.setUpSourceSet(ComponentType.SCREENSHOT_TEST_PREFIX, ComponentTypeImpl.SCREENSHOT_TEST)
        } else null
    }
    ...
    signingConfigContainer.whenObjectRemoved { throw UnsupportedOperationException("Removing signingConfigs is not supported.") }

    buildTypeContainer.whenObjectAdded(this::addBuildType)
    buildTypeContainer.whenObjectRemoved { throw UnsupportedOperationException("Removing build types is not supported.") }

    productFlavorContainer.whenObjectAdded(this::addProductFlavor)
    productFlavorContainer.whenObjectRemoved { throw UnsupportedOperationException("Removing product flavors is not supported.") }
  }
}
