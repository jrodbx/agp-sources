...
/** This is a CMakeSettings.json file that is equivalent to the environment CMakeServerJsonGenerator traditionally has run. */
fun getCmakeDefaultEnvironment(buildIsPrefabCapable: Boolean): Settings {
  ...
  variables.addAll(
    Macro.values().toList().flatMap { macro -> macro.cmakeProperties.map { cmake -> SettingsConfigurationVariable(cmake.name, macro.ref) } }
  )
...
/** A placeholder environment for ndk-build and Ninja. It doesn't do anything except declare the name of the inherited environment "ndk" */
...
fun CxxAbiModel.getNdkMetaSettingsJson(): Settings {
  ...
  environments[NDK.environment] = NameTable(lookup(NDK_MIN_PLATFORM), lookup(NDK_MAX_PLATFORM), lookup(NDK_CMAKE_TOOLCHAIN))
  ...
  for (abiValue in Abi.values()) {
    ...
    environments[Environment.NDK_ABI.environment.replace(NDK_ABI.ref, abiValue.tag)] =
      NameTable(
  ...
  for (potentialPlatform in NdkMetaPlatforms.potentialPlatforms) {
    val environmentName = Environment.NDK_PLATFORM.environment.replace(NDK_PLATFORM_SYSTEM_VERSION.ref, potentialPlatform.toString())
    environments[environmentName] =
      NameTable(
        NDK_PLATFORM_SYSTEM_VERSION to "$potentialPlatform",
        NDK_PLATFORM to "android-$potentialPlatform",
        NDK_PLATFORM_CODE to (metaPlatformAliases?.lastOrNull { (_, platform) -> platform == potentialPlatform }?.first ?: ""),
      )
  }
  val settingsEnvironments = environments.map { (name, nameTable) -> nameTable.environments().single().copy(environment = name) }
  return Settings(environments = settingsEnvironments, configurations = listOf())
}
...
fun CxxAbiModel.getAndroidGradleSettings(): Settings {
  ...
  nameTable.addAll(
    Macro.values()
      .filter { it.environment == GRADLE || it.environment == MICROSOFT_BUILT_IN || it.environment == NDK_EXPOSED_BY_HOST }
  ...
  nameTable.addAll(
    ...
    NDK_SO_REPUBLISH_DIR to
      join(
        NDK_MODULE_BUILD_INTERMEDIATES_BASE_DIR.ref,
        legacyConfigurationSegment,
        variant.module.intermediatesParentDirSuffix,
        NDK_ABI.ref,
      ),
  )

  return Settings(environments = nameTable.environments(), configurations = listOf())
}
...
fun CxxAbiModel.getSettingsFromCommandLine(arguments: List<CommandLineArgument>): Settings {
  ...
  when (variant.module.buildSystem) {
    CMAKE ->
      arguments.forEach { argument ->
        when (argument) {
          ...
          is DefineProperty -> Macro.withCMakeProperty(argument.propertyName).forEach { macro -> nameTable[macro] = argument.propertyValue }
          else -> {}
        }
      }
    NDK_BUILD ->
    ...
    else -> infoln("No Macro values redefined from command-line arguments " + "for '${variant.module.buildSystem}' build system")
  }

  val stl = variant.module.determineUsedStlFromArguments(arguments)

  val stlLibraryFile = variant.module.stlSharedObjectMap[stl]?.get(name)?.toString() ?: ""

  nameTable.addAll(NDK_VARIANT_STL_TYPE to stl.argumentName, NDK_STL_LIBRARY_FILE to stlLibraryFile)

  return Settings(environments = nameTable.environments(), configurations = listOf())
}

/** Gather CMake settings from different locations. */
fun CxxAbiModel.gatherSettingsFromAllLocations(providers: ProviderFactory, layout: ProjectLayout): Settings {
