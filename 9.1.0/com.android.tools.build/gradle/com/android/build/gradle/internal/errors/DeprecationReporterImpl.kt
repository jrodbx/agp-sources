...
class DeprecationReporterImpl(
  ...
  private val suppressedOptionWarnings: Set<String> =
    projectOptions[StringOption.SUPPRESS_UNSUPPORTED_OPTION_WARNINGS]?.splitToSequence(',')?.toSet() ?: setOf()

  override fun reportDeprecatedUsage(newDslElement: String, oldDslElement: String, deprecationTarget: DeprecationTarget) {
  ...
  override fun reportDeprecatedApi(
    ...
    if (requiresOptIn || !checkAndSet(oldApiElement)) {
      ...
      if (debugApi) {
        ...
        val kotlinAndroidPluginPresent =
          Thread.currentThread().stackTrace.any { it.className == "org.jetbrains.kotlin.gradle.plugin.KotlinAndroidPlugin" }

        messageEnd =
          if (kotlinAndroidPluginPresent) {
            """
        ...
        messageEnd =
          "To determine what is calling $oldApiElement, use -P${BooleanOption.DEBUG_OBSOLETE_API.propertyName}=true on the command line to display more information."
      }
  ...
  override fun reportRemovedApi(oldApiElement: String, url: String, deprecationTarget: DeprecationTarget) {
    if (!checkAndSet(oldApiElement)) {
      ...
      issueReporter.reportError(Type.REMOVED_API, "$messageStart\n$messageEnd")
    }
  }
  ...
  private fun getCallingSite(traces: Array<StackTraceElement>): String {
      ...
      traces
        ...
        .filter {
          it?.fileName?.let { fileName ->
            ...
            file.isAbsolute && file.isFile && (fileName.endsWith(".gradle") || fileName.endsWith(".gradle.kts"))
          } ?: false
        }
        .map { "${it.fileName}:${it.lineNumber}" }
        .firstOrNull()
    ...
    return if (gradleFile != null) {
      ...
      "REASON: It is currently called from the following trace:\n" + formattedTraces.joinToString(separator = "", prefix = "", postfix = "")
    }
  }

  override fun reportObsoleteUsage(oldDslElement: String, deprecationTarget: DeprecationTarget) {
    issueReporter.reportWarning(
      Type.DEPRECATED_DSL,
  ...
  override fun reportRenamedConfiguration(newConfiguration: String, oldConfiguration: String, deprecationTarget: DeprecationTarget) {
    issueReporter.reportWarning(
  ...
  override fun reportDeprecatedConfiguration(newDslElement: String, oldConfiguration: String, deprecationTarget: DeprecationTarget) {
    issueReporter.reportWarning(
      Type.USING_DEPRECATED_CONFIGURATION,
  ...
  override fun reportDeprecatedValue(dslElement: String, oldValue: String, newValue: String?, deprecationTarget: DeprecationTarget) {
    issueReporter.reportWarning(
      Type.USING_DEPRECATED_DSL_VALUE,
      "DSL element '$dslElement' has a value '$oldValue' which is obsolete " +
        (if (newValue != null) "and has been replaced with '$newValue'.\n" else "and has not been replaced.\n") +
        deprecationTarget.getDeprecationTargetMessage(),
    )
  }
  ...
  override fun reportOptionIssuesIfAny(option: Option<*>, value: Any) {
    ...
    val status = if (useSimulatedVersionBehavior) futureStage.stage.status else option.status

    val defaultValue = if (useSimulatedVersionBehavior) futureStage.defaultValue else option.defaultValue

    val defaultValueMessage = defaultValue?.let { "\nThe current default is '$it'." } ?: ""
    ...
    when (status) {
      is Option.Status.EXPERIMENTAL -> {
        ...
        if (defaultValue != value) {
          issueReporter.reportWarning(
            ...
            "The option setting '${option.propertyName}=$value' is experimental." + defaultValueMessage,
            option.propertyName,
          )
        }
      }
      ...
      is Option.Status.Deprecated -> {
        ...
        val useRecommendedValue = option is OptionalBooleanOption && option.recommendedValue != null && option.recommendedValue == value
      ...
      is Option.Status.Removed -> {
        if (defaultValue == value) {
          issueReporter.reportWarning(
            Type.UNSUPPORTED_PROJECT_OPTION_USE,
            "The option '${option.propertyName}' is deprecated." + defaultValueMessage + "\n" + status.getRemovedVersionMessage(),
            option.propertyName,
          )
        } else {
  ...
  companion object {
     ...
     */
    fun checkAndSet(api: String): Boolean =
     ...
     */
    fun checkAndSet(option: Any, value: Any): Boolean =
    ...
    fun clean() {
      synchronized(obsoleteApis) { obsoleteApis.clear() }
      synchronized(options) { options.clear() }
    }
  }
}

data class OptionInfo(val option: Any, val value: Any)
