...
class VariantModelImpl(
   ...
   *
   * Given user preferences, this attempts to respect them in the presence of the variant filter.
   *
   ...
   *
   * @return the name of a variant that exists under the presence of the variant filter. Only returns null if all variants are removed.
   */
   ...
   *
   * @param syncIssueHandler any configuration issues will be added here, e.g. if multiple flavors in one dimension are marked as default.
   * @return map from flavor dimension to the user-specified default flavor for that dimension, with entries missing for flavors without
 ...
 *
 * Note: this comparator imposes orderings that are inconsistent with equals, as variants that do not match the default will compare the
 * same.
 */
private class BuildAuthorSpecifiedDefaultBuildTypeComparator constructor(private val chosen: String?) :
...
/**
 * Compares variants prioritizing those that match the given default flavors over those that do not.
 *
 * The best match is the *minimum* element.
 *
 * Note: this comparator imposes orderings that are inconsistent with equals, as variants that do not match the default will compare the
 * same.
 */
private class BuildAuthorSpecifiedDefaultsFlavorComparator constructor(private val defaultFlavors: Map<String, String>) :
  Comparator<ComponentCreationConfig> {
  override fun compare(v1: ComponentCreationConfig, v2: ComponentCreationConfig): Int {
    var f1Score = 0
    var f2Score = 0
 ...
 * The best match is the *minimum* element.
 */
private class DefaultBuildTypeComparator constructor(private val preferredBuildType: String) : Comparator<ComponentCreationConfig> {
  override fun compare(v1: ComponentCreationConfig, v2: ComponentCreationConfig): Int {
    val b1 = v1.buildType
    val b2 = v2.buildType
    return if (b1 == b2) {
