...
/**
 * Ninja dependencies file format coder/decoder. This is a compact representation of C/C++ file dependencies.
 *
 * Everything is aligned to four bytes, everything is little endian.
 *
 * Header 16 bytes: MAGIC (12 bytes) "# ninjadeps\n" Version (u32): The number 3 or 4 Sequence of Record: Record Type and Size (u32): High
 * Bit: The number 0 for 'Path Record' or 1 for 'Dependencies Record'. Remaining: Size of the record that follows. Path Record: File Path
 * (UTF-8 bytes, not zero terminated, back slashes on Windows, forward elsewhere). 0 to 3 NUL byt to align on four byte boundary. Checksum
 * (u32): One's complement of file index. Dependencies Record: Target Path (u32): Path to some .o file (or other output) relative to
 * .ninja_deps file. One of Timestamp: v1 (u32): seconds since epoch (2000 for windows, 1970 for others). v2 (u64): nanoseconds since epoch
 * (2000 for windows, 1970 for others). (See: https://github.com/ninja-build/ninja/blob/master/src/disk_interface.cc) Sequence of
 * Dependency: Dependency (u32): Path to .cpp and .h. When relative, relative to .ninja_deps file.
 *
 * Example, CMakeFiles/main.dir/WiiUtils.cpp.o|1617220563|WiiUtils.cpp,<toolchain>sysroot/usr/include/c++/v1/string,etc.
 ...
 */
class NinjaDepsEncoder
private constructor(private val stringTable: StringTable, private val schemaVersion: Int, private val output: DataOutputStream) :
  AutoCloseable {
  ...
  fun writeTarget(target: String, targetTimestamp: Long?, dependencies: List<String>) {
    ...
    dependencyIds.forEach { writeLittleEndianUInt(it.toLong()) }
  }
  ...
  private fun writePath(path: String): Int {
    return stringTable.getIdCreateIfAbsent(path) { id ->
      ...
      repeat(padding) { output.writeByte(0) }
  ...
  /** Writes a 32-bit int to the [DataOutputStream] in little-endian format with the high bit set. */
  ...
  /** Helper function writes an a 64-bit int to the [DataOutputStream] in little-endian format. */
  ...
  companion object {
    ...
    fun open(ninjaDepsFile: File): NinjaDepsEncoder {
      ...
      return NinjaDepsEncoder(
        deps.pathTable,
        deps.schemaVersion,
        DataOutputStream(BufferedOutputStream(FileOutputStream(ninjaDepsFile, true))),
      )
    }
  }
}
...
data class NinjaDepsInfo(
  ...
  /**
   * Dependencies of a particular target file. Later dependencies for the same target file have replaced earlier dependencies. Returns null
   * if [target] is not known.
   */
  fun getDependencies(target: String): List<String>? {
    if (pathTable.containsString(target)) {
      return dependencies.getValue(pathTable.getId(target)).map { id -> pathTable.decode(id) }
    }
    return null
  }

  companion object {
    /** Read a .ninja_deps file into memory. Only retain the latest dependencies if they are restated in the file. */
...
/**
 * Stream all ninja dependencies inside the file then close the file at the end. Note that the way the ninja dependencies format works is
 * that dependencies for the same file may be repeated multiple times. In this case, the last list of dependencies is the true value.
 */
@VisibleForTesting
