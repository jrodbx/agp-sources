...
/** SdkLocation is the result of calling {@code SdkLocator.getSdkLocation(sourceSet) and contains the located SDK directory and its type. */
data class SdkLocation(val directory: File?, val type: SdkType)

/**
 * A SdkLocationSourceSet represents all the variables and information necessary for locating the SDK to be used based on all the possible
 * ways that the user can set their preferences.
 *
 * <p>Create a new instance using {@code SdkLocationSourceSet(gradleProject.rootDir)} and the constructor will collect the proper variables
 * from the local.properties file, the environment variables and system properties.
 *
 ...
 *
 * <p>Use {@link SdkLocator.getSdkLocation(sourceSet)} to fetch a {@code SdkLocation} object based on the user preferences represented by
 * the passed {@code sourceSet}. If this sourceSet was used before than the result will be cached.
 *
 * <p>To clear the cache, at the end of the build for example, use {@link SdkLocator.resetCache()}.
 */
object SdkLocator {

  @JvmStatic @VisibleForTesting var sdkTestDirectory: File? = null

  @VisibleForTesting const val ANDROID_HOME_SYSTEM_PROPERTY = "android.home"
  ...
  internal class ValidationResult<T>(val value: T?, val failures: List<ValidationFailure> = listOf()) {
  ...
  internal data class ValidationFailure(val locationDescription: PropertyLocation, val problem: ValidationProblem)
  ...
  internal enum class SdkLocationSource(val sdkType: SdkType) {
    TEST_SDK_DIRECTORY(SdkType.TEST) {
      override fun getSdkPathProperty(sourceSet: SdkLocationSourceSet): ValidationResult<File>? {
        return sdkTestDirectory?.absolutePath?.let { validateSdkPath(it, sourceSet.projectRoot, SDK_TEST_DIRECTORY_LOCATION) }
      }
    },
    ...
    INJECTED_SDK_HOME(SdkType.REGULAR) {
      ...
      override fun getSdkPathProperty(sourceSet: SdkLocationSourceSet): ValidationResult<File>? {
        ...
        sourceSet.environmentProperties.getProperty(SdkConstants.ANDROID_SDK_ROOT_ENV)?.let { path ->
        ...
        sourceSet.environmentProperties.getProperty(SdkConstants.ANDROID_HOME_ENV)?.let { path ->
...
"""
    ...
    fun getSdkLocation(sourceSet: SdkLocationSourceSet): ValidationResult<SdkLocation>? {
      return getSdkPathProperty(sourceSet)?.let { it.map { value -> value?.let { SdkLocation(value, sdkType) } ?: null } }
    }
    ...
    protected fun validateSdkPath(path: String, rootDir: File, locationDescription: PropertyLocation): ValidationResult<File> {
      if (path == "") return ValidationResult(null, listOf(ValidationFailure(locationDescription, ValidationProblem.EMPTY_VALUE)))
      var sdk = File(path)
      if (!sdk.isAbsolute) {
      ...
      return if (sdk.isDirectory) ValidationResult(sdk)
        ...
        ValidationResult(
          ...
          listOf(
            ValidationFailure(
              locationDescription,
              if (sdk.exists()) ValidationProblem.NOT_A_DIRECTORY else ValidationProblem.DOES_NOT_EXIST,
            )
          ),
        )
    }
  ...
  @JvmStatic
  fun getSdkDirectory(projectRootDir: File, issueReporter: IssueReporter, sdkLocationSourceSet: SdkLocationSourceSet): File {
    val sdkLocation = getSdkLocation(sdkLocationSourceSet, issueReporter)
    return if (sdkLocation.type == SdkType.MISSING) {
      ...
      sdkLocation.directory ?: error("Directory must not be null when type = ${sdkLocation.type}")
    }
  }

  @JvmStatic
  @Synchronized
  fun getSdkLocation(sourceSet: SdkLocationSourceSet, issueReporter: IssueReporter): SdkLocation {
    ...
    SdkLocation(null, SdkType.MISSING).let {
      ...
      val filePath = File(sourceSet.projectRoot, SdkConstants.FN_LOCAL_PROPERTIES).absolutePath
  ...
  private fun reportWarnings(validationFailures: List<ValidationFailure>, issueReporter: IssueReporter) {
    if (validationFailures.isNotEmpty()) {
      issueReporter.reportWarning(IssueReporter.Type.GENERIC, createWarningMessage(validationFailures))
    }
  }
  ...
  private fun createWarningMessage(failures: List<ValidationFailure>) =
    failures.fold("The following problems were found when resolving the SDK location:\n") { acc, e ->
      acc + "Where: " + e.locationDescription.description + " Problem: " + e.problem.message + "\n"
    }
...
private object GradleLocalPropertiesFactory {
  ...
  internal fun get(projectRoot: File, providers: ProviderFactory): Properties {
    ...
    val propertiesContent = providers.of(PropertiesValueSource::class.java) { it.parameters.projectRoot.set(projectRoot) }.get()
...
abstract class PropertiesValueSource : ValueSource<String, PropertiesValueSource.Params> {
  ...
  override fun obtain(): String {
    val localPropertiesFile = File(parameters.projectRoot.get().asFile, SdkConstants.FN_LOCAL_PROPERTIES)
...
private object EnvironmentVariablesPropertiesFactory {
  internal fun get(): Properties {
    val properties = Properties()
    System.getenv(SdkConstants.ANDROID_HOME_ENV)?.let { properties.setProperty(SdkConstants.ANDROID_HOME_ENV, it) }
    System.getenv(SdkConstants.ANDROID_SDK_ROOT_ENV)?.let { properties.setProperty(SdkConstants.ANDROID_SDK_ROOT_ENV, it) }
    return properties
  }
}
...
private object SystemPropertiesFactory {
  internal fun get(): Properties {
    val properties = Properties()
    System.getProperty(SdkLocator.ANDROID_HOME_SYSTEM_PROPERTY)?.let { properties.setProperty(SdkLocator.ANDROID_HOME_SYSTEM_PROPERTY, it) }
    return properties
  }
}
