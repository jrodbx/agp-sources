...
abstract class ExtractNativeDebugMetadataTask : NonIncrementalTask() {
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val inputDir: DirectoryProperty
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val strippedNativeLibs: DirectoryProperty

  @get:OutputDirectory abstract val outputDir: DirectoryProperty

  @get:Inject abstract val execOperations: ExecOperations

  @get:Nested abstract val ndkHandlerInput: NdkHandlerInput
  ...
  override fun doTaskAction() {
    workerExecutor.noIsolation().submit(ExtractNativeDebugMetadataWorkAction::class.java) {
      ...
      it.objcopyExecutableMap.set(
        sdkBuildService.flatMap { buildService -> buildService.versionedNdkHandler(ndkHandlerInput).objcopyExecutableMapProvider }
      )
  ...
  abstract class CreationAction(creationConfig: VariantCreationConfig) :
    VariantTaskCreationAction<ExtractNativeDebugMetadataTask, VariantCreationConfig>(creationConfig) {
    ...
    override fun configure(task: ExtractNativeDebugMetadataTask) {
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.STRIPPED_NATIVE_LIBS, task.strippedNativeLibs)
  ...
  class FullCreationAction(creationConfig: VariantCreationConfig) : CreationAction(creationConfig) {

    override val name: String
      get() = computeTaskName("extract", "NativeDebugMetadata")

    override fun handleProvider(taskProvider: TaskProvider<ExtractNativeDebugMetadataTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ExtractNativeDebugMetadataTask::outputDir)
        .withName("out")
        .on(NATIVE_DEBUG_METADATA)
    }
  ...
  class SymbolTableCreationAction(creationConfig: VariantCreationConfig) : CreationAction(creationConfig) {
...
@VisibleForTesting
abstract class ExtractNativeDebugMetadataWorkAction : ProfileAwareWorkAction<ExtractNativeDebugMetadataWorkAction.Parameters>() {
  ...
  override fun run() {
    ...
    for (inputFile in FileUtils.getAllFiles(inputDir)) {
      ...
      val strippedNativeLib = File(strippedNativeLibs, inputDir.toPath().relativize(inputFile.toPath()).toString())
      ...
      when (parameters.debugSymbolLevel.get()) {
        DebugSymbolLevel.FULL -> {
          outputFile = File(outputDir, "${inputFile.parentFile.name}/${inputFile.name}$DOT_DBG")
          objcopyArgs = listOf("--only-keep-debug")
        }
        DebugSymbolLevel.SYMBOL_TABLE -> {
          outputFile = File(outputDir, "${inputFile.parentFile.name}/${inputFile.name}$DOT_SYM")
          objcopyArgs = listOf("--strip-debug")
        }
        DebugSymbolLevel.NONE -> throw RuntimeException("NativeDebugMetadataMode.NONE not supported in ${this.javaClass.name}")
      }
    ...
    var ord = 0
    allRequests
      .groupBy { (ord++) % parameters.maxWorkerCount.get() }
      .values
      .forEach { requests ->
        if (requests.isNotEmpty()) {
          workerExecutor.noIsolation().submit(ExtractNativeDebugMetadataRunnable::class.java) {
...
abstract class ExtractNativeDebugMetadataRunnable : ProfileAwareWorkAction<ExtractNativeDebugMetadataRunnable.Params>() {
  ...
  override fun run() {
    ...
    parameters.requests.get().forEach { processSingle(it.inputFile, it.outputFile, it.objcopyExecutable, it.objcopyArgs, processExecutor) }
  }
  ...
  private fun processSingle(
    ...
    val result =
      processExecutor.execute(
        ...
        LoggedProcessOutputHandler(LoggerWrapper(Logging.getLogger(ExtractNativeDebugMetadataTask::class.java))),
      )
    if (result.exitValue != 0) {
      logger.warning(
        "Unable to extract native debug metadata from ${inputFile.absolutePath} " + "because of non-zero exit value from objcopy."
      )
    }
  }

  data class SingleRequest(val inputFile: File, val outputFile: File, val objcopyExecutable: File, val objcopyArgs: List<String>) :
    Serializable
