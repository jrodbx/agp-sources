 ...
 * @param excludedDirectoryFiles [FileCollection] containing text files which content are a list of filtered artifacts.
 */
class FilteringSpec(
  ...
  @Transient private var excluded: Lazy<Set<String>> = lazy { initExcluded() }

  @Transient private var filteredFileCollection: Lazy<FileCollection> = lazy { initFilteredFileCollection() }

  /** Keep lazy, as invoking getArtifacts() is quite costly with configuration caching. */
  @Transient private var filteredArtifacts = lazy { initFilteredArtifacts() }
  ...
  private fun initExcluded(): Set<String> =
    excludedDirectoryFiles.files.asSequence().filter { it.isFile }.flatMapTo(HashSet()) { it.readLines(Charsets.UTF_8).asSequence() }

  private fun initFilteredFileCollection() =
    objectFactory
      .fileCollection()
      .from(artifacts.artifactFiles.filter(this))
      .builtBy(artifacts.artifactFiles.buildDependencies)
      .builtBy(excludedDirectoryFiles.buildDependencies)

  private fun initFilteredArtifacts() = artifacts.artifacts.asSequence().filter { !excluded.value.contains(it.toIdString()) }.toMutableSet()
