...
/**
 * A logging environment that will report errors and warnings to an [IssueReporter]. Messages are also logger to a standard
...
private constructor(
  ...
  init {
    ...
    structuredLogEncoder =
      if (allowStructuredLogging && structuredLogFolder.isDirectory) {
        val log = structuredLogFolder.resolve("log_${System.currentTimeMillis()}_${Thread.currentThread().id}.bin")
  ...
  constructor(
    ...
    analyticsService.getVariantBuilder(variant.module.gradleModulePathName, variant.variantName),
  )
  ...
  override fun log(message: LoggingMessage) {
    logStructured { strings ->
      val encoded =
        EncodedLoggingMessage.newBuilder()
    ...
    when (message.level) {
      ...
      WARN -> {
        ...
        issueReporter.reportWarning(EXTERNAL_NATIVE_BUILD_CONFIGURATION, message.text())
        logger.warn(message.text())
      }
      ERROR -> {
        metrics?.addCxxDiagnosticCodes(message.diagnosticCode)
        issueReporter.reportError(EXTERNAL_NATIVE_BUILD_CONFIGURATION, message.text())
      ...
      BUG -> {
        ...
        issueReporter.reportError(EXTERNAL_NATIVE_BUILD_CONFIGURATION, sb.toString())
