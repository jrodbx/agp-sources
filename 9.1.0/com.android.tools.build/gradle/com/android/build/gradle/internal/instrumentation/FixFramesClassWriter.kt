...
open class FixFramesClassWriter : ClassWriter {
  ...
  private fun isAssignableFrom(
    ...
    return type == otherType || otherTypeInterfaces.contains(type) || otherTypeSuperClasses.contains(type)
  }
  ...
  private fun getCommonSuperClassForArrayTypes(firstType: String, secondType: String): String {
    ...
    val firstComponentType = firstType.substring(firstTypeArrayNestingDepth + 1, firstType.length - 1)
    val secondComponentType = secondType.substring(firstTypeArrayNestingDepth + 1, secondType.length - 1)

    val firstTypeInterfaces = classesHierarchyResolver.getAllInterfacesInInternalForm(firstComponentType)
    val firstTypeSuperClasses = classesHierarchyResolver.getAllSuperClassesInInternalForm(firstComponentType)
    val secondTypeInterfaces = classesHierarchyResolver.getAllInterfacesInInternalForm(secondComponentType)
    val secondTypeSuperClasses = classesHierarchyResolver.getAllSuperClassesInInternalForm(secondComponentType)

    if (isAssignableFrom(firstComponentType, secondComponentType, secondTypeInterfaces, secondTypeSuperClasses)) {
      return firstType
    }

    if (isAssignableFrom(secondComponentType, firstComponentType, firstTypeInterfaces, firstTypeSuperClasses)) {
      return secondType
    }
  ...
  override fun getCommonSuperClass(firstType: String, secondType: String): String {
    ...
    val firstTypeInterfaces = classesHierarchyResolver.getAllInterfacesInInternalForm(firstType)
    val firstTypeSuperClasses = classesHierarchyResolver.getAllSuperClassesInInternalForm(firstType)
    val secondTypeInterfaces = classesHierarchyResolver.getAllInterfacesInInternalForm(secondType)
    val secondTypeSuperClasses = classesHierarchyResolver.getAllSuperClassesInInternalForm(secondType)
    ...
    firstTypeSuperClasses.forEach { firstTypeSuperClass ->
      if (isAssignableFrom(firstTypeSuperClass, secondType, secondTypeInterfaces, secondTypeSuperClasses)) {
