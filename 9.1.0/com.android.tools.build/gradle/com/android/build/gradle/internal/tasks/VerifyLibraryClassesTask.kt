 ...
 *
 * This currently doesn't handle compileOnly dependencies, they will lead to validation errors.
 */
...
abstract class VerifyLibraryClassesTask : NonIncrementalTask() {
  ...
  @get:OutputDirectory abstract val verifiedOutputDirectory: DirectoryProperty

  @get:Classpath abstract val aarMainJar: RegularFileProperty

  @get:InputDirectory @get:PathSensitive(PathSensitivity.RELATIVE) abstract val aarLibsDirectory: DirectoryProperty

  @get:Classpath abstract val runtime: ConfigurableFileCollection

  @get:Classpath abstract val bootclasspath: ConfigurableFileCollection

  @get:Classpath abstract val runtimeRSymbolLists: ConfigurableFileCollection
  ...
  abstract class TraceReferencesWorkAction : ProfileAwareWorkAction<Params>() {
    override fun run() {
      val symbolTables: List<SymbolTable> = parameters.symbolTableBuildService.get().loadClasspath(parameters.runtimeSymbolTables)
      val inMemoryClasses =
        mutableMapOf<String, ByteArray>().apply {
          exportToCompiledJava(symbolTables, false, null) { key, value -> this["L${key.removeSuffix(".class")};"] = value }
        }
  ...
  class CreationAction(creationConfig: VariantCreationConfig) :
    VariantTaskCreationAction<VerifyLibraryClassesTask, ComponentCreationConfig>(creationConfig) {

    override val name: String = computeTaskName("verify", "Classes")
    override val type: Class<VerifyLibraryClassesTask>
      get() = VerifyLibraryClassesTask::class.java

    override fun handleProvider(taskProvider: TaskProvider<VerifyLibraryClassesTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider = taskProvider, property = VerifyLibraryClassesTask::verifiedOutputDirectory)
