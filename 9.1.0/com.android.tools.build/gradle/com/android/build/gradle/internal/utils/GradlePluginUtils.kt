...
/**
 * Fails the build if the given [requiredPlugin] has not been applied after this project's evaluation.
 *
 ...
 *
 * If the [incompatiblePlugin] has not been applied but will be applied later, then this will make the build fail later at the point when
 * the [incompatiblePlugin] has been applied (unless that plugin fails the build before that point -- see b/397373580).
 *
...
fun getBuildscriptDependencies(project: Project): List<ModuleComponentIdentifier> {
  ...
  return buildScriptClasspath.incoming.resolutionResult.getModuleComponents().map { it.id as ModuleComponentIdentifier }
}

/**
 * Enumerates through the gradle plugin jars existing in the given [classLoader], finds the buildSrc jar and loads the id of each plugin
 * from the `META-INF/gradle-plugins/${id}.properties` file name.
 *
...
val MUTUALLY_EXCLUSIVE_ANDROID_GRADLE_PLUGINS =
  mapOf(
