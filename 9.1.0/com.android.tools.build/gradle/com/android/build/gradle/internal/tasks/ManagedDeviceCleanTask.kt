...
abstract class ManagedDeviceCleanTask : NonIncrementalGlobalTask() {
  ...
  @get:Input abstract val dslDevices: ListProperty<String>

  @get:Input abstract val preserveDefined: Property<Boolean>

  @Option(
    option = "unused-only",
    description =
      "Remove only the avds that are not defined in the dsl for this project." +
  ...
  abstract class ManagedDeviceCleanRunnable : ProfileAwareWorkAction<ManagedDeviceCleanParams>() {
    override fun run() {
      ...
      val avdsRemoved = parameters.avdService.get().deleteAvds(allAvds.filterNot { parameters.ignoredDevices.get().contains(it) })
      Logging.getLogger(ManagedDeviceCleanTask::class.java).lifecycle("Successfully deleted ${avdsRemoved.size} managed devices.")
  ...
  constructor(
    ...
    override fun configure(task: ManagedDeviceCleanTask) {
      ...
      task.dslDevices.setDisallowChanges(definedDevices.map { computeAvdName(it) })
    }
  }
}
