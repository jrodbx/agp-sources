 ...
 *
 * This is well above the time it takes to delete a snapshot entry. So if we manage to hit this timeout it will clearly be the result of a
 * failure in qemu-img
 */
...
class QemuExecutor(
   ...
   *
   * Runs the qemu command: `qemu-img snapshot -d $deviceDir <qcow-file>` on every qcow file in the device directory, then deletes the
   * snapshot folder.
   *
   ...
   */
  fun deleteSnapshot(avdName: String, deviceDir: File, snapshotName: String, logger: ILogger) {
    ...
    for (qcowFile in deviceDir.listFiles { pathname -> pathname.isFile() && pathname.name.endsWith(".qcow2") }) {

      val process = processFactory(listOf(qemuImageExecutable.absolutePath, "snapshot", "-d", snapshotName, qcowFile.absolutePath)).start()
    ...
    if (snapshotDir.exists() && snapshotDir.isDirectory) {
      try {
        ...
        logger.error(ioException, "Could not delete snapshot folder at location: ${snapshotDir.absolutePath}.")
        failedToDelete = true
      }
    }
