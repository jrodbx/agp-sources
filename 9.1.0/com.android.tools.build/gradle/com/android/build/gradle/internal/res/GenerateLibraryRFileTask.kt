...
abstract class GenerateLibraryRFileTask : ProcessAndroidResources() {
  ...
  @get:OutputFile abstract val rClassOutputJar: RegularFileProperty
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val dependencies: ConfigurableFileCollection

  @get:Input abstract val namespace: Property<String>

  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val platformAttrRTxt: ConfigurableFileCollection

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val localResourcesFile: RegularFileProperty

  @get:Input abstract val nonTransitiveRClass: Property<Boolean>

  @get:Input abstract val compileClasspathLibraryRClasses: Property<Boolean>

  @get:Input abstract val useConstantIds: Property<Boolean>

  @Suppress("UnstableApiUsage") @get:ServiceReference abstract val symbolTableBuildService: Property<SymbolTableBuildService>

  @get:Internal abstract val mergedManifestFile: RegularFileProperty
  ...
  abstract class GenerateLibRFileRunnable @Inject constructor() : ProfileAwareWorkAction<GenerateLibRFileParams>() {
    override fun run() {
      ...
      val depSymbolTables: List<SymbolTable> = parameters.symbolTableBuildService.get().loadClasspath(parameters.dependencies)
    ...
    private fun getAndroidAttrSymbols() =
      if (parameters.androidJar.singleFile.exists()) SymbolIo.readFromAapt(parameters.androidJar.singleFile, "android")
      else SymbolTable.builder().tablePackage("android").build()
  }

  internal class CreationAction(creationConfig: ComponentCreationConfig) :
    ...
    override fun handleProvider(taskProvider: TaskProvider<GenerateLibraryRFileTask>) {
      super.handleProvider(taskProvider)
      creationConfig.taskContainer.processAndroidResTask = taskProvider

      creationConfig.artifacts
        .setInitialProvider(taskProvider, GenerateLibraryRFileTask::rClassOutputJar)
        .withName("R.jar")
        .on(InternalArtifactType.COMPILE_R_CLASS_JAR)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, GenerateLibraryRFileTask::textSymbolOutputFileProperty)
        .withName(SdkConstants.FN_RESOURCE_TEXT)
        .on(InternalArtifactType.COMPILE_SYMBOL_LIST)

      // Synthetic output for AARs (see SymbolTableWithPackageNameTransform), and created in
      // process resources for local subprojects.
      creationConfig.artifacts
        .setInitialProvider(taskProvider, GenerateLibraryRFileTask::symbolsWithPackageNameOutputFile)
    ...
    override fun configure(task: GenerateLibraryRFileTask) {
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(SingleArtifact.MERGED_MANIFEST, task.mergedManifestFile)
  ...
  internal class TestRuntimeStubRClassCreationAction(creationConfig: HostTestCreationConfig) :
    VariantTaskCreationAction<GenerateLibraryRFileTask, HostTestCreationConfig>(creationConfig) {
    ...
    override fun configure(task: GenerateLibraryRFileTask) {
      ...
      creationConfig.onTestedVariant {
        it.artifacts.setTaskInputToFinalProduct(SingleArtifact.MERGED_MANIFEST, task.mergedManifestFile)

        it.artifacts.setTaskInputToFinalProduct(InternalArtifactType.LOCAL_ONLY_SYMBOL_LIST, task.localResourcesFile)
      }
    }
  }
}
