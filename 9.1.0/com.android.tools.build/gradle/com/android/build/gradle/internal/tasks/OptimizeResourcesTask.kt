...
/**
 * OptimizeResourceTask attempts to use AAPT2's optimize sub-operation to reduce the size of the final apk. There are a number of potential
 * optimizations performed such as resource obfuscation, path shortening and sparse encoding. If the optimized apk file size is less than
 * before, then the optimized resources content is made identical to [InternalArtifactType.LINKED_RESOURCES_BINARY_FORMAT].
 */
@CacheableTask
...
abstract class OptimizeResourcesTask : NonIncrementalTask() {
  ...
  @get:InputDirectory @get:PathSensitive(PathSensitivity.RELATIVE) abstract val inputProcessedRes: DirectoryProperty

  @get:Nested abstract val aapt2: Aapt2Input

  @get:Input abstract val enableResourceObfuscation: Property<Boolean>

  @get:Internal abstract val transformationRequest: Property<ArtifactTransformationRequest<OptimizeResourcesTask>>

  @get:Nested abstract val outputsHandler: Property<MultiOutputHandler>

  @get:OutputFile abstract val resourceConfigFile: RegularFileProperty

  @get:OutputDirectory abstract val optimizedProcessedRes: DirectoryProperty

  override fun doTaskAction() {
    transformationRequest.get().submit(this, workerExecutor.noIsolation(), Aapt2OptimizeWorkAction::class.java) {
      builtArtifact,
  ...
  abstract class Aapt2OptimizeWorkAction @Inject constructor(private val params: OptimizeResourcesParams) :
    WorkActionAdapter<OptimizeResourcesParams> {
    override fun doExecute() = doFullTaskAction(params)
  }

  class CreateAction(creationConfig: ApkCreationConfig) :
    ...
    override fun handleProvider(taskProvider: TaskProvider<OptimizeResourcesTask>) {
      ...
      val operationRequest =
        creationConfig.artifacts
          .use(taskProvider)
          .wiredWithDirectories(OptimizeResourcesTask::inputProcessedRes, OptimizeResourcesTask::optimizedProcessedRes)

      transformationRequest =
        if (resourceShrinkingEnabled) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, OptimizeResourcesTask::resourceConfigFile)
...
internal fun doFullTaskAction(params: OptimizeResourcesTask.OptimizeResourcesParams) {
  ...
  val aaptInputFile =
    if (inputFile.isDirectory) {
      inputFile.listFiles()?.filter { it.extension == SdkConstants.EXT_RES || it.extension == SdkConstants.EXT_ANDROID_PACKAGE }?.get(0)
    } else {
      inputFile
    }

  aaptInputFile?.let {
    invokeAapt(params.aapt2Executable.get().asFile, "optimize", it.path, *optimizeFlags.toTypedArray(), "-o", outputFile.path)
...
internal fun invokeAapt(aapt2: File, vararg args: String): List<String> {
  ...
  val processInfoBuilder = ProcessInfoBuilder().setExecutable(aapt2).addArgs(args)
  val processExecutor = DefaultProcessExecutor(StdLogger(StdLogger.Level.ERROR))
  processExecutor.execute(processInfoBuilder.createProcess(), processOutputHeader).rethrowFailure()
