...
class WorkerProfilingRecord(
  ...
  @Synchronized fun isStarted() = startTime != Instant.MIN

  fun isFinished() = endTime != Instant.MIN

  fun waitTime(): Duration = if (isStarted()) Duration.between(submissionTime, startTime) else Duration.ZERO

  fun duration(): Duration = if (isStarted() && isFinished()) Duration.between(startTime, endTime) else Duration.ZERO
