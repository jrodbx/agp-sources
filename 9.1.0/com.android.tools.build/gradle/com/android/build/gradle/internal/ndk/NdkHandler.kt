...
sealed class NdkInstallStatus {
   ...
   *
   * An NDK is invalid if the directory exists but is otherwise determined to be not usable. We may have been unable to determine the NDK's
   * revision, it could be an unsupported NDK, or the NDK-specific validation may have failed (commonly because the install was corrupt and
   * was missing necessary components).
   *
   ...
   *
   * If the NDK directory was not able to be determined or the directory does not exist, the NDK has not yet been installed.
   */
...
open class NdkHandler(private val ndkLocator: NdkLocator) {
  ...
  private fun getNdkInfo(ndk: NdkLocatorRecord) =
    when {
  ...
  private fun getNdkStatus(downloadOkay: Boolean): NdkInstallStatus {
    val ndk = ndkLocator.findNdkPath(downloadOkay) ?: return NotInstalled
  ...
  fun getNdkPlatform(downloadOkay: Boolean): NdkInstallStatus {
    if (ndkInstallStatus == null || (downloadOkay && ndkInstallStatus == NotInstalled)) {
