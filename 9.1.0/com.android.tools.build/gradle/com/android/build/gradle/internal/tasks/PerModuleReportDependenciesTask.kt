...
abstract class PerModuleReportDependenciesTask : NonIncrementalTask() {
  ...
  @get:Internal abstract val runtimeClasspathArtifacts: Property<ArtifactCollection>

  @Input abstract fun getRootComponent(): Property<ResolvedComponentResult>
  ...
  @get:OutputFile abstract val dependencyReport: RegularFileProperty

  @get:Input abstract val moduleName: Property<String>

  @get:Nested abstract val projectRepositories: ListProperty<InternalRepositoryMetadata>
  ...
  override fun doTaskAction() {
    val componentDigestMap =
      runtimeClasspathArtifacts
        .get()
        .groupBy(keySelector = { artifact -> artifact.id.componentIdentifier }, valueTransform = { artifact -> artifact.file })
        .mapValues { (id, fileList) ->
          val fileSet = fileList.toSet()
          if (fileSet.size > 1) logger.warn("Component ${id.displayName} maps to multiple files")
          getFileDigest(fileSet.first())
        }

    // Declare local classes used for intermediate data representations
    class InternalGraphNode(val id: ComponentIdentifier, val neighborIdList: List<ComponentIdentifier>, val repoIndex: Int?)
    class InternalLibraryMetadata(id: ComponentIdentifier, val sha256: ByteString?, val repoIndex: Int?) {
      val mavenLibrary: MavenLibraryData? =
        (id as? ModuleComponentIdentifier)?.let { MavenLibraryData(groupId = it.group, artifactId = it.module, version = it.version) }

      inner class MavenLibraryData(val groupId: String, val artifactId: String, val version: String)
    }
    class InternalLibraryDependenciesMetadata(val libIndex: Int, val libDepIndices: List<Int>)

    // Build a map of repoName->Index
    val repoNameToIndexMap = projectRepositories.get().associateIndexed { index, it -> it.name to index }
    ...
    val (projectNodes, libraryNodesPartialList) =
      allComponents
        .asSequence()
        .map { component ->
          InternalGraphNode(
    ...
    val libraryNodes = libraryNodesPartialList + unusedIds.asSequence().map { unusedId -> InternalGraphNode(unusedId, listOf(), null) }

    // Build an intermediate sequence of InternalLibraryMetadata objects
    val intermediateLibraries =
      libraryNodes.asSequence().map { node ->
        InternalLibraryMetadata(id = node.id, sha256 = componentDigestMap[node.id], repoIndex = node.repoIndex)
      }
    ...
    val intermediateLibraryDependencies =
      libraryNodes
        .asSequence()
        .map { node ->
    ...
    val appDependenciesProto =
      AppDependencies.newBuilder()
        .apply {
          ...
          projectRepositories.get().forEach { repoObj ->
            addRepositoriesBuilder().apply {
              when (repoObj) {
                is InternalRepositoryMetadata.Maven -> mavenRepoBuilder.url = repoObj.url.toString()
                is InternalRepositoryMetadata.Ivy -> ivyRepoBuilder.url = repoObj.url.toString()
              }
            }
          }
  ...
  private fun collectAll(node: ResolvedComponentResult, allComponents: MutableSet<ResolvedComponentResult>) {
  ...
  class CreationAction(creationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<PerModuleReportDependenciesTask, ApkCreationConfig>(creationConfig) {

    companion object {

      internal fun Iterable<ArtifactRepository>.toInternalRepoMetadataList() = mapNotNull { repo ->
        when (repo) {
          is MavenArtifactRepository -> InternalRepositoryMetadata.Maven(repo.name, repo.url).takeUnless { it.url.scheme == "file" }
          is IvyArtifactRepository -> InternalRepositoryMetadata.Ivy(repo.name, repo.url).takeUnless { it.url.scheme == "file" }
          else -> return@mapNotNull null // Drop any other types of repositories, such as flatDirectory
        }
      }
    }
    ...
    override fun handleProvider(taskProvider: TaskProvider<PerModuleReportDependenciesTask>) {
      super.handleProvider(taskProvider)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, PerModuleReportDependenciesTask::dependencyReport)
        .withName("dependencies.pb")
        .on(SingleArtifact.METADATA_LIBRARY_DEPENDENCIES_REPORT)
    }
    ...
    override fun configure(task: PerModuleReportDependenciesTask) {
      ...
      val repositoryHandlerProvider: Provider<RepositoryHandler> =
        try {
          // this is using private Gradle APIs, so it may break in the future Gradle versions
          val dependencyResolutionManagement = (task.project.gradle as GradleInternal).settings.dependencyResolutionManagement

          // If repositoriesMode == PREFER_PROJECT, prioritize projectRepositories
          dependencyResolutionManagement.repositoriesMode.map {
            if (it == RepositoriesMode.PREFER_PROJECT && projectRepositories.isNotEmpty()) projectRepositories
            else dependencyResolutionManagement.repositories
          }
        } catch (ignored: Throwable) {
          // fall back to using projectRepositories
          creationConfig.services.provider { projectRepositories }
        }
