...
/**
 * Enable Ninja build system through experimental flags in Android Gradle Plugin. Mimics the final DSL.
 *
 * Example usage in build.gradle: externalNativeBuild { experimentalProperties["ninja.abiFilters"] = ["x86", "arm64-v8a" ]
 * experimentalProperties["ninja.path"] = "../Teapot.sln" experimentalProperties["ninja.configure"] = "msbuild"
 * experimentalProperties["ninja.arguments"] =
 * [ "\${ndk.moduleMakeFile}", "-p:Configuration=\${ndk.variantName}", "-p:Platform=Android-\${ndk.abi}", "-p:NinjaBuildLocation=\${ndk.buildRoot}", "-p:NinjaProject=GameApplication", "-t:GenerateBuildNinja" ]
 * }
 */

/** Per-module Ninja information. Planned as class, of same name, in com.android.build.api.dsl. */
data class Ninja(
...
/** Per-variant Ninja information. Planned as class, of same name, in com.android.build.gradle.internal.dsl */
...
val NativeBuildCreationConfig.ninja: Ninja
  get() = externalNativeExperimentalProperties.ninja

val Map<String, Any>.ninja: Ninja
  get() =
    Ninja(
...
val NativeBuildCreationConfig.externalNativeNinjaOptions
  get() =
    CoreExternalNativeNinjaOptions(
...
data class PrefabExperimentalPackagingOptions(
  /**
   * export_libraries may specify either literal arguments to be used as-is, intra-package references, or inter-package references. This
   * field is optional.
   */
...
fun NativeBuildCreationConfig.getPrefabExperimentalPackagingOptions(module: String): PrefabExperimentalPackagingOptions {
  ...
  return PrefabExperimentalPackagingOptions(exportLibraries = exportLibraries)
}
