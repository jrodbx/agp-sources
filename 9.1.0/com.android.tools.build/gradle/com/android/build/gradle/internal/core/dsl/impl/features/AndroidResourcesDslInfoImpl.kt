  ...
  override fun getResValues(): Map<ResValue.Key, ResValue> {
    ...
    fun addToListIfNotAlreadyPresent(classField: ClassField, comment: String) {
      ...
      if (!resValueFields.containsKey(key)) {
        resValueFields[key] = ResValue(value = classField.value, comment = comment)
      }
    }
    ...
    defaultConfig.resValues.values.forEach { classField -> addToListIfNotAlreadyPresent(classField, "Value from default config.") }
