...
abstract class JetifyTransform : TransformAction<JetifyTransform.Parameters> {
  ...
  interface Parameters : GenericTransformParameters {
    ...
    @get:Input val ignoreListOption: Property<String>
  }
  ...
  @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:InputArtifact abstract val inputArtifact: Provider<FileSystemLocation>

  /**
   * Computes the Jetifier ignore list of type [Regex] from a string containing a comma-separated list of regular expressions. The string
   * may be empty.
   *
   ...
   *
   * For example, if the regular expression is "doNot.*\.jar", then "/path/to/doNotJetify.jar" won't be jetified.
   */
  ...
  override fun transform(transformOutputs: TransformOutputs) {
    ...
    check(inputFile.name.endsWith(".aar", ignoreCase = true) || inputFile.name.endsWith(".jar", ignoreCase = true)) {
      "Transform's input file is not .aar or .jar: ${inputFile.path}"
    }
    check(inputFile.isFile) {
      "Transform's input file does not exist: ${inputFile.path}." + " (See https://issuetracker.google.com/issues/158753935)"
    }
