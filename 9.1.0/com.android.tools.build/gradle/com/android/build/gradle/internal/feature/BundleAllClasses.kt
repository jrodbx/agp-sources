...
/**
 * Task to jar all classes in a project. This includes pre/post java classes, and compiled namespaced R class (if it exists).
 *
 * It is used for e.g.:
 ...
 *
 * Caching disabled by default for this task because the task does very little work. The task moves files from Inputs, unchanged, into a Jar
 * file. Calculating cache hit/miss and fetching results is likely more expensive than simply executing the task.
 */
...
abstract class BundleAllClasses : NonIncrementalTask() {
  ...
  @get:Classpath abstract val inputDirs: ConfigurableFileCollection
  ...
  abstract class BundleAllClassesWorkAction : ProfileAwareWorkAction<BundleAllClassesWorkAction.Parameters>() {
    ...
    override fun run() {
      ...
      JarFlinger(parameters.outputJar.asFile.get().toPath(), null).use { out ->
        ...
        parameters.inputJars.forEach { out.addJar(it.toPath()) }
      }
    }
  }

  class CreationAction(creationConfig: ComponentCreationConfig, private val publishedType: AndroidArtifacts.PublishedConfigType) :
    VariantTaskCreationAction<BundleAllClasses, ComponentCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<BundleAllClasses>) {
      ...
      creationConfig.artifacts.setInitialProvider(taskProvider, BundleAllClasses::outputJar).withName(FN_CLASSES_JAR).let {
        if (publishedType == AndroidArtifacts.PublishedConfigType.API_ELEMENTS) {
    ...
    override fun configure(task: BundleAllClasses) {
      ...
      if (publishedType == AndroidArtifacts.PublishedConfigType.RUNTIME_ELEMENTS) {
        val projectClasses = creationConfig.artifacts.forScope(ScopedArtifacts.Scope.PROJECT).getFinalArtifacts(ScopedArtifact.CLASSES)

        task.inputJars.from(projectClasses.getRegularFiles(creationConfig.services.projectInfo.projectDirectory))
        task.inputDirs.from(projectClasses.getDirectories(creationConfig.services.projectInfo.projectDirectory))
      } else {
        task.inputDirs.from(
          listOfNotNull(
        ...
        task.inputJars.fromDisallowChanges(creationConfig.artifacts.get(InternalArtifactType.COMPILE_AND_RUNTIME_R_CLASS_JAR))
      }
