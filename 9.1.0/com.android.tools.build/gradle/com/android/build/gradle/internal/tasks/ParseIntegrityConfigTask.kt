...
abstract class ParseIntegrityConfigTask : NonIncrementalTask() {
  ...
  @get:InputDirectory @get:Optional @get:PathSensitive(PathSensitivity.NONE) abstract val integrityConfigDir: DirectoryProperty

  @get:OutputFile abstract val appIntegrityConfigProto: RegularFileProperty
  ...
  abstract class ParseIntegrityConfigRunnable : ProfileAwareWorkAction<Params>() {
    override fun run() {
      ...
      } ?: run { Files.deleteIfExists(parameters.appIntegrityConfigProto.asFile.get().toPath()) }
    }
  ...
  class CreationAction(creationConfig: VariantCreationConfig) :
    VariantTaskCreationAction<ParseIntegrityConfigTask, VariantCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ParseIntegrityConfigTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ParseIntegrityConfigTask::appIntegrityConfigProto)
        .withName("AppIntegrityConfig.pb")
        .on(InternalArtifactType.APP_INTEGRITY_CONFIG)
    }
