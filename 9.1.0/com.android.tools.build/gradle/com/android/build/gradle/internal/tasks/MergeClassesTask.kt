 ...
 *
 * This task is necessary in a feature or base module when minification is enabled in the base because the base needs to know which classes
 * came from which modules to eventually split the classes to the correct APKs via the Dex Splitter.
 */
...
abstract class MergeClassesTask : NonIncrementalTask() {
  ...
  @get:Classpath abstract val inputFiles: ConfigurableFileCollection
  ...
  abstract class MergeClassesWorkAction : ProfileAwareWorkAction<MergeClassesWorkAction.Parameters>() {
    ...
    override fun run() {
      JarFlinger(parameters.outputFile.asFile.get().toPath(), CLASS_MATCHER).use { out ->
  ...
  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<MergeClassesTask, ComponentCreationConfig>(creationConfig) {
    ...
    private val inputFiles =
      creationConfig.artifacts.forScope(ScopedArtifacts.Scope.ALL).getFinalArtifacts(InternalScopedArtifact.FINAL_TRANSFORMED_CLASSES)

    override fun handleProvider(taskProvider: TaskProvider<MergeClassesTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, MergeClassesTask::outputFile)
        .withName(
