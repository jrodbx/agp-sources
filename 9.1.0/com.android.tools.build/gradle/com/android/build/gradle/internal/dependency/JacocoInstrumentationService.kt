...
*/
@ThreadSafe
abstract class JacocoInstrumentationService : BuildService<BuildServiceParameters.None>, AutoCloseable {

  protected open val instrumenterCache =
    CacheBuilder.from(CacheBuilderSpec.parse("softValues"))
      .build(
        object : CacheLoader<ClassLoaderCacheKey, Instrumenter>() {
          override fun load(key: ClassLoaderCacheKey): Instrumenter {
            return Instrumenter(createClassLoader(key.classpath))
          }
        }
      )

  fun instrument(input: InputStream, name: String, classpaths: Iterable<File>, jacocoVersion: String): ByteArray {
  ...
  class Instrumenter(val classLoader: URLClassLoader) {
    ...
    private val offlineInstrumentationAccessGenerator =
      loadClass("$jacocoCorePackage.runtime.OfflineInstrumentationAccessGenerator").getConstructor().newInstance()
    private val iExecutionDataAccessorGeneratorInterface = loadClass("$jacocoCorePackage.runtime.IExecutionDataAccessorGenerator")
    private val instrumenterClass = loadClass("$jacocoCorePackage.instr.Instrumenter")
    private val instrumenter =
      instrumenterClass.getConstructor(iExecutionDataAccessorGeneratorInterface).newInstance(offlineInstrumentationAccessGenerator)
    private val instrumentMethod = instrumenterClass.getMethod("instrument", InputStream::class.java, String::class.java)

    fun instrument(input: InputStream, name: String): ByteArray {
      return instrumentMethod.invoke(instrumenter, input, name) as ByteArray
    }
