...
abstract class ProcessNavigationXmlTask : NonIncrementalTask() {
  ...
  @get:Optional @get:Classpath abstract val runtimeDependenciesNavigationFiles: ConfigurableFileCollection
  ...
  override fun doTaskAction() {
    ...
    resources.get().forEach { it.sourceDirectories.files.forEach { processNavigationResources(it, placeholders) } }
  }
  ...
  private fun validateResourceFileNames(files: Array<File>) {
    files.forEach { file -> FileResourceNameValidator.validate(file, ResourceFolderType.XML) }
  }

  private fun getPlaceholders() =
    manifestPlaceholders.get().let {
  ...
  private fun transformFile(original: File, placeholdersValues: Map<String, String>) {
    ...
    original.inputStream().use { inputStream ->
      ...
      val updatedXml = XmlPrettyPrinter.prettyPrint(root, XmlFormatPreferences.defaults(), XmlFormatStyle.get(root), null, false)

      val toBeGenerated = FileUtils.join(outputDir.get().asFile, FD_RES_NAVIGATION, original.name)
  ...
  private fun performPlaceHolderSubstitution(node: Node, placeholdersValues: Map<String, String>) {
    if (node.nodeType == Node.ELEMENT_NODE && node.nodeName == "deepLink") {
      node.attributes?.forEach { attribute ->
        var content = attribute.nodeValue
        placeholdersValues.forEach { (key, value) -> content = content.replace("\${$key}", value) }
    ...
    node.attributes?.forEach { performPlaceHolderSubstitution(it, placeholdersValues) }

    // Now fix the children.
    node.childNodes.forEach { if (it.nodeType != Node.TEXT_NODE) performPlaceHolderSubstitution(it, placeholdersValues) }
  }

  abstract class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<ProcessNavigationXmlTask, ComponentCreationConfig>(creationConfig) {
    override val name: String
      get() = creationConfig.computeTaskNameInternal("process", "NavigationResources")
    ...
    override fun handleProvider(taskProvider: TaskProvider<ProcessNavigationXmlTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ProcessNavigationXmlTask::outputDir)
  ...
  open class LibraryCreationAction(creationConfig: ComponentCreationConfig) : CreationAction(creationConfig) {
  ...
  class ApplicationCreationAction(creationConfig: ComponentCreationConfig) : CreationAction(creationConfig) {
