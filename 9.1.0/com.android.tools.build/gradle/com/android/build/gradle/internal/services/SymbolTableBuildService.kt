 ...
 *
 * @param cacheBuilderSpec The configuration of the cache to use. Gradle calls the `@`[Inject] annotated constructor which uses
 *   [SOFT_VALUES].
 */
@ThreadSafe
abstract class SymbolTableBuildService @VisibleForTesting internal constructor(cacheBuilderSpec: CacheBuilderSpec) :
  ...
  fun loadClasspath(files: Iterable<File>): List<SymbolTable> {
    ...
    getSymbolTablesCached(files).also { result ->
      logger.log(logLevel, "SymbolTableBuildService: loadClasspath took {} to return {} tables", stopwatch.elapsed(), result.size)
      return result
    }
  }
   ...
   *
   * This is to avoid having a very large number of soft references, but still being able to drop symbols from the intern table under memory
   * pressure.
   */
  ...
  private val symbolInterner: Interner<Symbol>
    @Synchronized
    get() {
      return symbolInternerReference?.get()
        ?: Interners.newStrongInterner<Symbol>().also {
  ...
  private data class FileCacheKey(
    ...
    override fun equals(other: Any?): Boolean = other is FileCacheKey && this.key == other.key
      ...
      .build(
        object : CacheLoader<FileCacheKey, SymbolTable>() {
          override fun load(key: FileCacheKey): SymbolTable {
            val result = SymbolIo(symbolInterner).readSymbolListWithPackageName(key.file)
            logger.log(logLevel, "SymbolTableBuildService: cache miss - loaded table '{}' from disk", result.tablePackage)
            return result
          }
        }
      )
   ...
   *
   * [SymbolTable]s will be returned from the in-memory cache if present, and only loaded from disk if they are not already cached. [Symbol]
   * instances will be interned and shared across invocations.
   */
  ...
  class RegistrationAction(project: Project) :
    ServiceRegistrationAction<SymbolTableBuildService, BuildServiceParameters.None>(project, SymbolTableBuildService::class.java) {
