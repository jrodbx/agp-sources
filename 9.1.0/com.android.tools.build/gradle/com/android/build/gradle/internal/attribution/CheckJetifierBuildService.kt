...
/** [BuildService] to aggregate [CheckJetifierResult]s from [CheckJetifierTask]s and write the final result to a file. */
@Suppress("UnstableApiUsage")
abstract class CheckJetifierBuildService : BuildService<CheckJetifierBuildService.Parameters>, AutoCloseable {
  ...
  class RegistrationAction(project: Project, private val projectOptions: ProjectOptions) :
    ServiceRegistrationAction<CheckJetifierBuildService, Parameters>(project, CheckJetifierBuildService::class.java) {
    ...
    override fun execute(): Provider<CheckJetifierBuildService> {
      ...
      if (resultFilePath != null) {
        ...
        check(resultFile.isAbsolute) {
          "${StringOption.IDE_CHECK_JETIFIER_RESULT_FILE.propertyName} must be an absolute path." + " Current value is: $resultFilePath"
        }
    ...
    override fun configure(parameters: Parameters) {
      parameters.resultFile.fileValue(projectOptions.get(StringOption.IDE_CHECK_JETIFIER_RESULT_FILE)?.let { File(it) })
    }
  }
