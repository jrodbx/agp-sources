...
class DimensionCombinator(
  ...
  private fun computeFlavorlessVariants(): List<DimensionCombination> {
    return if (variantInputModel.buildTypes.isEmpty()) {
      ...
      for (buildType in variantInputModel.buildTypes.keys) {
        builder.add(DimensionCombinationImpl(buildType = buildType))
      }
  ...
  private fun computeVariantsWithFlavors(): List<DimensionCombination> {
    ...
    val flavorMap = variantInputModel.productFlavors.values.asSequence().map { it.productFlavor }.groupBy({ it.dimension!! }, { it.name })

    // get the flavor combos and combine them with build types.
    val builder = ImmutableList.builder<FlavorCombinationBuilder>()
    createProductFlavorCombinations(flavorDimensionList, flavorMap, builder, errorReporter)

    return combineFlavorsAndBuildTypes(builder.build())
  }

  /** Computes [ComponentIdentity] from a list of [FlavorCombinationBuilder] by combining them with build types. */
  private fun combineFlavorsAndBuildTypes(flavorCombos: List<FlavorCombinationBuilder>): List<DimensionCombination> {
    if (variantInputModel.buildTypes.isEmpty()) {
      // just convert the Accumulators to VariantConfiguration with no build type info
      return flavorCombos.map { DimensionCombinationImpl(productFlavors = it.flavorPairs) }
    } else {
      val builder = ImmutableList.builder<DimensionCombination>()

      for (buildType in variantInputModel.buildTypes.keys) {
        builder.addAll(flavorCombos.map { DimensionCombinationImpl(buildType = buildType, productFlavors = it.flavorPairs) })
      }
   ...
   *
   * This checks that there's at least a dimension declared, and for the case of a single dimension, assign that dimension to all flavors.
   * This is to facilitate migrating from AGP versions that did not required explicitly declared dimension name (for single dimension case).
   */
 ...
 *
 * This runs through the flavor dimensions list and for the current dimension, gathers the list of flavors. It then loops on these and for
 * each, add the current (dimension, value) pair to a new list and then recursively calls into the next dimension for it to loop on its own
 * flavors and do the same (fill the list.) This goes on until there's no dimension left.
 *
 * The recursion is handled by 2 objects:
 * - `dimensionIndex`: this is the dimensionIndex of the current dimension in the dimension list. Each recursive call increases this to go
 *   to the next dimension.
 * - `currentFlavorCombo`: this accumulates the list of (dimension, value) pairs for the dimensions already processed. Each new dimension
 *   adds its new pairs to it (or rather to a new copy of it for each pair)
 *
 * At the end of the recursion (based on `dimensionIndex` reaching the end of the list), `currentFlavorCombo` contains a pair for each
 * dimension. This is then added to `comboList`
 *
 * This function is out of the main class because its `flavorDimensionList` param shadows [DimensionCombinator.flavorDimensionList],
 * therefore it's clearer and more obvious that this is not the same value.
 *
...
private fun createProductFlavorCombinations(
  ...
  return if (flavorList == null || flavorList.isEmpty()) {
    errorReporter.reportError(IssueReporter.Type.EMPTY_FLAVOR_DIMENSION, "No flavor is associated with flavor dimension '$dimension'.")
  } else {
    for (flavor in flavorList) {
      val newCombo = currentFlavorCombo.add(dimension, flavor)

      createProductFlavorCombinations(flavorDimensionList, flavorMap, comboList, errorReporter, newCombo, dimensionIndex + 1)
    }
  }
}
 ...
 *
 * This class is immutable. To add a new (dimension, value) pair to the list, use [add] that will return a new instance.
 */
private class FlavorCombinationBuilder(
  /** The list of (dimension, value) pairs. The order is important and match the dimension priority order. */
