...
private val publicArtifactMap: Map<String, KClass<out Artifact<*>>> =
  SingleArtifact::class.sealedSubclasses.associateBy { it.objectInstance?.name() ?: throw RuntimeException("No instance") }

private val sourceArtifactMap: Map<String, KClass<out Artifact<*>>> =
  SourceArtifactType::class.sealedSubclasses.associateBy { it.objectInstance?.name() ?: "class" }
private val buildArtifactMap: Map<String, KClass<out Artifact<*>>> =
  BuildArtifactType::class.sealedSubclasses.associateBy { it.objectInstance?.name() ?: "class" }
private val internalArtifactMap: Map<String, KClass<out Artifact<*>>> =
  InternalArtifactType::class.sealedSubclasses.associateBy { it.objectInstance?.name() ?: throw RuntimeException("No instance") }

/**
 * Return the enum of [Artifact] base on the name.
 *
 * The typical implementation of valueOf in an enum class cannot be used because there are multiple implementations of [Artifact]. For this
 * to work, the name of all [Artifact] must be unique across all implementations.
 */
