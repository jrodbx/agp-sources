...
class ManagedDeviceRegistry(private val testOptions: DeviceTestOptionsDslInfo) :
   ...
   *
   * Since the actual classes in the [Managed Device Block][ManagedDevices.allDevices] are decorated Gradle classes, we have to check for
   * the registration via ```instanceof``` the implementation. This is not ideal, so we can at least cache the registrations based on the
   * decorated classes as the key for future calls to [get]
   */
  private val decoratedDeviceCache: MutableMap<Class<out Device>, Registration<*>> = mutableMapOf()

  /** Tracks the api and implementation classes that are being used, to ensure the same class is not used twice. */
  private val registeredDSLClasses: MutableSet<Class<out Device>> = mutableSetOf()

  /**
   * Gets the Registration associated with the given Decorated Device class. If no such class is registered, then null is returned.
   *
   * @param DecoratedDeviceT the class of given Decorated Device
   * @param clazz the Class object for DecoratedDeviceT
   * @return the registration associated with the decorated class, or null if no such registration exists.
   */
  @Suppress("UNCHECKED_CAST")
  fun <DecoratedDeviceT : Device> get(clazz: Class<DecoratedDeviceT>): Registration<in DecoratedDeviceT>? {
    if (decoratedDeviceCache.containsKey(clazz)) {
      return decoratedDeviceCache[clazz] as Registration<in DecoratedDeviceT>
    }
    return registrationList.firstOrNull { it.deviceImpl.isAssignableFrom(clazz) }?.apply { decoratedDeviceCache[clazz] = this }
      as Registration<in DecoratedDeviceT>?
  }

  override fun <DeviceT : Device> registerDeviceType(dslInterface: Class<DeviceT>, setupBlock: DeviceDslRegistration<DeviceT>.() -> Unit) {
    ...
    if (registeredDSLClasses.contains(implementationClass)) {
      error("Custom Device Implementation Class: $implementationClass " + "is already registered with the Managed Device Registry.")
    }
    if (registeredDSLClasses.contains(apiClass)) {
      error("Custom Device Api Class: $apiClass " + "is already registered with the Managed Device Registry.")
    }
    // Ensure that an implementation is not a subclass of another implementation or vice versa.
    registrationList
      .firstOrNull { it.deviceImpl.isAssignableFrom(implementationClass) }
      ?.apply {
        error(
    ...
    registrationList
      .firstOrNull { implementationClass.isAssignableFrom(it.deviceImpl) }
      ?.apply {
        error(
          "Cannot register Implementation Class: $implementationClass " +
    ...
    testOptions.managedDevices.allDevices.registerBinding(apiClass, implementationClass)
  ...
  class DeviceDslRegistrationBuilder<DeviceT : Device>(val dslInterface: Class<DeviceT>) : DeviceDslRegistration<DeviceT> {
    ...
    fun toRegistration(): Registration<DeviceT> {
      return Registration(dslInterface, dslImplementationClass, setupConfigAction, setupTaskAction, testRunConfigAction, testRunTaskAction)
    }
  }
