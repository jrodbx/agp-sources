...
/**
 * Given a user-generated custom build.ninja, produce the build information needed to integrate with Android Gradle Plugin's build system.
 *
...
fun adaptNinjaToCxxBuild(
  ...
  val adapter = NinjaToCxxBuildAdapter(abi, cxxBuildFolder, createNinjaCommand, buildFileFilter)
  ...
  writeCompileCommandsJsonBin(ninjaBuildFile, cxxBuildFolder, compileCommandsJsonBin, platform)
...
/**
 * Helper class holds state for Ninja adaption including the string table [strings] and [commandLineBuffer] which is a shared space for
...
private class NinjaToCxxBuildAdapter(
  ...
  /** Make a single pass over build.ninja building the graph of inputs to outputs and gathering the IDs of various interesting targets */
  ...
  fun createAndroidGradleBuildMini(graph: BuildGraph) =
    with(graph) {
      // Build a map where:
      ...
      val passthroughMap =
        passthroughs
          .flatMap { passthrough -> edges.breadthFirst(removeExtension(passthrough, "passthrough")).map { child -> child to passthrough } }
      ...
      NativeBuildConfigValueMini().apply {
        ...
        buildFiles = buildFileIds.map { resolveFile(it) }.filter { buildFile -> buildFileFilter(buildFile) }
        libraries =
          edges
            .ancestors(packageableIds + archiveIds)
            .filter { (_, libraries) ->
            ...
            .map { (ancestor, library) -> assignTargetName(stringOf(ancestor)) to (ancestor to library.first()) }
            ...
            .map { (name, targets) ->
              val (targetId, outputId) =
                targets.singleOrNull()
                  ...
                  ?: targets.last()
              createLibrary(name = name, targetId = targetId, outputId = outputId, passthroughId = passthroughMap[outputId])
            }
  ...
  private fun BuildGraph.createLibrary(name: String, targetId: Int, outputId: Int, passthroughId: Int?) =
    NativeLibraryValueMini().apply {
      artifactName = name
      abi = targetAbi
      output = resolveFile(outputId)
      buildCommandComponents = ninjaCommand(passthroughId ?: targetId)
      runtimeFiles = edges.breadthFirst(targetId).filter { it != targetId && packageableIds.contains(it) }.map { resolveFile(it) }.toList()
    }
  ...
  private fun removeExtension(value: Int, ext: String) = idOf(stringOf(value).removeExtensionIfPresent(ext))
...
/** Heuristic to check whether name could be a packageable library (like .so or executable) that is *not* an Android system library. */
@VisibleForTesting
fun isPackageable(name: String): Boolean {
  return name.hasExtensionIgnoreCase("") ||
    (name.hasExtensionIgnoreCase("so") && (!name.contains("ndk") || !NDK_SYSTEM_LIBS.any { name.endsWith(it) }))
}
...
/** Assign a name to a target. Strip "lib" from front and ".a" or ".so" from the end. Preserve case. */
@VisibleForTesting
fun assignTargetName(target: String): String {
  ...
  return if (target.filenameStartsWithIgnoreCase("lib") && !target.filenameStartsWithIgnoreCase("lib.")) name.substring(3) else name
}
