...
/**
 * Define built-in macros for CMakeSettings.json. Built-in means they're defined by the host (Android Studio) and their names can be known
 * in advance.
 *
...
enum class Macro(
  ...
  NDK_PLATFORM_SYSTEM_VERSION(
    description = "The currently targeted Android system version, suitable for passing to " + "CMake in CMAKE_SYSTEM_VERSION.",
  ...
  NDK_PLATFORM(
    description = "The currently targeted Android platform string, that can be passed to " + "CMake in ANDROID_PLATFORM.",
    environment = Environment.NDK_PLATFORM,
    tag = "platform",
    example = "android-19",
  ...
  NDK_MODULE_NINJA_EXECUTABLE(
    description =
      "Path to Ninja executable if one was found by Gradle. Otherwise, it expands" +
  ...
  NDK_MODULE_HAS_BUILD_TIME_INFORMATION(
    description = "Whether build-time information is available. If false, then only " + "configuration-time information is available.",
    environment = GRADLE,
  ...
  NDK_NINJA_BUILD_LOCATION_FILE(
    description =
      "Path to a file that contains the location of build.ninja. Written by custom" +
  ...
  NDK_SO_REPUBLISH_DIR(
    description =
      "A folder with a predictable name where final build outputs (mainly .so) are " +
  ...
  /**
   * The literal value of this macro in CMakeSettings.json. For example, "${ndk.minPlatform}". Macros in the namespace "env" are reduced to
   * just the name. For example, "${thisFile}".
   */
  val ref = if (environment.namespace == "env") "\${$tag}" else "\${${environment.namespace}.$tag}"

  /** The namespace-qualified name of this macro. */
  val qualifiedName
    get() = "${environment.namespace}.$tag"

  /** Placeholder to use for build-time values during configuration time. */
  val configurationPlaceholder: String
    get() {
      if (available != BUILD) {
  ...
  companion object {
    /**
     * Look up the [Macro] enum for [name]. If [name] doesn't have a namespace then 'env' is used. If [name] cannot be found then null is
     * returned.
     */
    @JvmStatic
    ...
    fun withCMakeProperty(cmakeProperty: String) = CMAKE_PROPERTY_STRING_TO_MACRO_MAP[cmakeProperty] ?: listOf()
    ...
    private val BINDING_PROPERTY_TO_MACRO = values().filter { macro -> macro.bind != null }.groupBy { macro -> macro.bind!! }

    private val CMAKE_PROPERTY_STRING_TO_MACRO_MAP =
      values()
 ...
 *
 * In CMakeSettings.json environments may inherit other environments. Inheritance means values from parent environments exist in the child
 * environment and can be replaced in the child environment.
 */
enum class Environment(
  ...
  /**
   * Environment for holding the built-in macros exposed by Microsoft. For example, ${thisFile}. Even though the names of the macros are
   * defined by Microsoft, the values in this particular environment come from us. Therefore, the name of the environment contains
   * "from-android-ndk".
   */
  ...
  /**
   * Environment for macros exposed by Android Gradle Plugin. For example, ${ndk.ABI} is the name of the ABI that is currently being built.
   */
   ...
   *
   * NDK is first (for global NDK values), then the more specific NDK environments NDK_PLATFORM and NDK_ABI. Next GRADLE so that Android
   * Gradle Plugin can replace NDK values if needed. Lastly, there is MICROSOFT_BUILT_IN which shouldn't be replaceable.
   *
   * It exposes a single macro ${ndk.androidGradleIsHosting} that can be used to detect whether CMakeSettings.json is being evaluated by
   * Android Gradle Plugin.
   */
  NDK_EXPOSED_BY_HOST("ndk", "ndk", NDK, NDK_PLATFORM, NDK_ABI, GRADLE, MICROSOFT_BUILT_IN);
