...
class SymbolStripExecutableFinder(val stripExecutables: Map<String, File>) : Serializable {
  ...
  /**
   * This method finds the path to the strip executable for a given ABI. Some basic checks are done and if a problem is find then report
   * will be invoked with an appropriate message.
   *
   * @return The path to the exe or null if it couldn't be located.
   */
  fun stripToolExecutableFile(input: File, abi: String?, reportAndFallback: (String) -> File?): File? {
    if (abi == null) {
      return reportAndFallback("Unable to strip library '${input.absolutePath}' due to " + "unknown ABI.")
    }
    return stripExecutables[abi]
      ?: return reportAndFallback("Unable to strip library '${input.absolutePath}' due to missing strip " + "tool for ABI '$abi'.")
  }
}
