...
/** Lint [NativeBuildConfigValueMini]. Emphasis is on detecting problematic information early because it may be easier to diagnose here. */
fun NativeBuildConfigValueMini.lint(json: File, defaultAbis: List<String>, supportedAbis: List<String>) {
  ...
  PassThroughPrefixingLoggingEnvironment(file = json).use { logger ->
    for (buildFile in buildFiles) {
      if (!buildFile.isFile) {
        errorln(BUILD_FILE_DID_NOT_EXIST, "expected buildFiles file '$buildFile' to exist")
      }
    }
    ...
    if (hasTopLevelBuildCommand && !File(buildCommand[0]).isFile) {
      errorln(BUILD_TARGET_COMMAND_COMPONENTS_DID_NOT_EXIST, "expected buildTargetsCommandComponents command '${buildCommand[0]}' to exist")
    ...
    for ((name, library) in libraries) {
      ...
      if (artifactName.isEmpty()) {
        errorln(LIBRARY_ARTIFACT_NAME_DID_NOT_EXIST, "expected ${name}.artifactName to exist")
      }
      for (runtimeFile in library.runtimeFiles) {
        ...
        checkCanonicalize("$name.runtimeFiles", runtimeFile)
      }
    ...
    val allAbis = libraries.values.mapNotNull { it.abi.toString() }.filter { supportedAbis.contains(it) }.distinct()
    if (allAbis.size > 1) {
...
/**
 * Canonicalize a file name. If there is an IOException from the file system then an error is logged with the name of the file that caused
 * the problem. A regular IOException doesn't always contain the path of the file and that's usually what we need to diagnose a bug.
 */
private fun checkCanonicalize(location: String, file: File) {
  try {
    file.canonicalFile
  } catch (e: IOException) {
    val cause = e.cause?.let { " ($it)" } ?: ""
    errorln(COULD_NOT_CANONICALIZE_PATH, "Could not canonicalize '$file' in $location due to ${e.javaClass.simpleName}$cause")
  }
}
