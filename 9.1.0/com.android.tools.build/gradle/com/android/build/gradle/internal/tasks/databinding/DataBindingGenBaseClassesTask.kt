 ...
 *
 * This class takes the output of XML processor which generates binding info files (binding information in layout files). Then it generates
 * base classes which are the classes accessed by the user code.
 *
 * Generating these classes in gradle instead of annotation processor avoids showing too many errors to the user if the compilation fails
 * before annotation processor output classes are compiled.
 */
@CacheableTask
...
abstract class DataBindingGenBaseClassesTask : NewIncrementalTask() {
  ...
  @get:InputFiles @get:Incremental @get:PathSensitive(PathSensitivity.RELATIVE) abstract val layoutInfoDirectory: DirectoryProperty

  // the package name for the module / app
  @get:Input abstract val namespace: Property<String>

  // list of artifacts from dependencies
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val dependencyClassLogDirectories: ConfigurableFileCollection

  @get:InputFiles @get:Optional @get:Classpath abstract val dependenciesFileCollection: ConfigurableFileCollection
  ...
  @get:InputFile @get:Optional @get:PathSensitive(PathSensitivity.NONE) abstract val localResourcesFile: RegularFileProperty

  @get:Internal abstract val symbolTableBuildService: Property<SymbolTableBuildService>

  // list of v1 artifacts from dependencies
  @get:Optional @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val v1Artifacts: DirectoryProperty

  // where to keep the log of the task
  @get:OutputDirectory
  lateinit var logOutFolder: File
    private set
  ...
  private fun collectResources(): List<SymbolTable> {
    ...
    val depSymbolTables: List<SymbolTable> = symbolTableBuildService.get().loadClasspath(dependenciesFileCollection.files)
    val localTable =
      when (val localR = localResourcesFile.orNull) {
  ...
  private fun getRPackageProvider(): ((String, String) -> String)? {
    ...
    val getRPackage: (String, String) -> String =
      fun(type: String, name: String): String {
  ...
  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<DataBindingGenBaseClassesTask, ComponentCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<DataBindingGenBaseClassesTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, DataBindingGenBaseClassesTask::classInfoBundleDir)
        .withName("out")
        .on(InternalArtifactType.DATA_BINDING_BASE_CLASS_LOG_ARTIFACT)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, DataBindingGenBaseClassesTask::sourceOutFolder)
        .withName("out")
        .on(InternalArtifactType.DATA_BINDING_BASE_CLASS_SOURCE_OUT)
    }
    ...
    override fun configure(task: DataBindingGenBaseClassesTask) {
      ...
      artifacts.setTaskInputToFinalProduct(InternalArtifactType.DATA_BINDING_DEPENDENCY_ARTIFACTS, task.v1Artifacts)
      ...
      task.encodeErrors = creationConfig.services.projectOptions[BooleanOption.IDE_INVOKED_FROM_IDE]
      ...
      //  view binding.
      task.enableRPackageLookup = creationConfig !is DeviceTestCreationConfig && isNonTransitiveR

      if (task.enableRPackageLookup) {
        artifacts.setTaskInputToFinalProduct(InternalArtifactType.LOCAL_ONLY_SYMBOL_LIST, task.localResourcesFile)
  ...
  constructor(
    ...
    override fun run() {
      try {
        initLogger()
        BaseDataBinder(LayoutInfoInput(args), getRPackage).generateAll(DataBindingBuilder.GradleFileWriter(sourceOutFolder.absolutePath))
      } finally {
        clearLogger()
      }
    }
    ...
    private fun initLogger() {
      ...
      L.setClient { kind, message, _ -> printMessage(kind, message) }
    }
