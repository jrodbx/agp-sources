 ...
 *
 * This task outputs a directory of single-entry jars (instead of a single jar) so that the downstream packaging task doesn't have to manage
 * its incremental state via a [com.android.builder.files.ZipSnapshotRepository].
 *
 * Each single-entry jar file's relative path in the output directory is equal to "assets/" + the corresponding asset's relative path in the
 * input directory + ".jar".
 */
@CacheableTask
...
abstract class CompressAssetsTask : NewIncrementalTask() {
  ...
  @get:Incremental @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val inputDirs: DirectoryProperty

  @get:Input abstract val noCompress: ListProperty<String>

  @get:Input abstract val compressionLevel: Property<Int>
  ...
  class CreationAction(creationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<CompressAssetsTask, ApkCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<CompressAssetsTask>) {
      super.handleProvider(taskProvider)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, CompressAssetsTask::outputDir)
        .withName("out")
        .on(InternalArtifactType.COMPRESSED_ASSETS)
    }

    override fun configure(task: CompressAssetsTask) {
  ...
  fun run() {
    for (change in changes) {
      ...
      val entryCompressionLevel =
        if (noCompressPredicate.test(entryPath)) {
...
abstract class CompressAssetsWorkAction @Inject constructor(private val compressAssetsWorkParameters: CompressAssetsWorkParameters) :
