...
class DefaultAndroidSourceDirectorySet(
  ...
  /**
   * Set once the sourceset is read to produce the android components, so that subsequent additions (e.g. during the applicationVariants
   * API) are not ignored.
   *
  ...
  private val disallowProviderInAndroidSourceSet =
    ProjectOptionService.RegistrationAction(project)
      .execute()
      .get()
      .projectOptions
      .get(BooleanOption.DISALLOW_PROVIDER_IN_ANDROID_SOURCE_SET)
  ...
  override fun srcDir(srcDir: Any): AndroidSourceDirectorySet {
    if (srcDir is Iterable<*> && srcDir !is Path) {
      srcDir.forEach { src -> src?.let { srcDir(it) } }
      return this
    }
    ...
    if (lateAdditionsDelegates.isNotEmpty()) {
      val entry =
        when (srcDir) {
          ...
          is File -> FileBasedDirectoryEntryImpl(sourceSetName, srcDir, filter, isUserAdded = false, shouldBeAddedToIdeModel = true)
          is Directory ->
  ...
  override fun setSrcDirs(srcDirs: Iterable<*>): AndroidSourceDirectorySet {
    if (lateAdditionsDelegates.isNotEmpty()) {
      /**
       * Filter out potential duplicates to avoid re-registering things that have already been registered. (Note that actually removing
       * things that have already been added is not supported after AGP DSL finalization)
       *
       * If a build author writes in groovy `srcDirs += "othersrc"` this becomes something like setSrcDirs(mutableSetOf().also
       * {addAll(getSrcDirs()); add("otherSrc") })
       *
       * So if this sourceDirectorySet contained ["src/main/java"], `srcDirs += "othersrc"` will call
       * setSrcDirs(listOf(File("/absolute/path/src/main/java"), "othersrc")).
       *
       * And we want to avoid registering src/main/java twice to avoid duplicate definition errors when compiling.
       */
