...
fun CxxAbiModel.toJsonString(): String {
  return StringWriter().also { writer -> GSON.toJson(this, writer) }.toString()
}
 ...
 *
 * Prefab configuration state needs to be persisted to disk because changes in configuration require model regeneration.
 */
data class PrefabConfigurationState(val enabled: Boolean, val prefabPath: File?, val packages: List<File>) {
