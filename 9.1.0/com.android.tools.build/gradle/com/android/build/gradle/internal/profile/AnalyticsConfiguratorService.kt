 ...
 *
 * [AnalyticsConfiguratorService] is paired with [AnalyticsService] which means there will always be one and only one
 * [AnalyticsConfiguratorService] instance for each [AnalyticsService] instance.
 */
abstract class AnalyticsConfiguratorService : BuildService<AnalyticsConfiguratorService.Params> {
  ...
  /**
   * When configure on demand is enabled, undeclared dependency resolution would cause projects to be configured during execution phase. In
   * this case, we don't throw error for accessing [getVariantBuilder] during execution time.
   *
   * Side note: Undeclared dependency resolution at execution time would be deprecated by Gradle in 8.X.
   */
  private fun configureOnDemandDisabled(): Boolean {
    return !parameters.configureOnDemand.get()
  }

  /** Records the time elapsed while executing a void block and saves the resulting [GradleBuildProfileSpan]. */
  ...
  open fun createAnalyticsService(
    ...
    if (project.gradle.startParameter.taskNames.isEmpty()) {
      project.gradle.projectsEvaluated {
        resourcesManager.recordGlobalProperties(project, configurationCacheActive, projectIsolationActive)
      ...
      project.gradle.taskGraph.whenReady {
        ...
        resourcesManager.configureAnalyticsService(parameters)
        instantiateAnalyticsService(project)
        registry.onTaskCompletion(getBuildService(project.gradle.sharedServices, AnalyticsService::class.java))
      }
    }
  }
  ...
  private fun instantiateAnalyticsService(project: Project) {
    ...
    analyticsService.get().setInitialMemorySampleForConfiguration(resourcesManager.initialMemorySample)
  ...
  class RegistrationAction(project: Project) :
    ServiceRegistrationAction<AnalyticsConfiguratorService, Params>(project, AnalyticsConfiguratorService::class.java) {
