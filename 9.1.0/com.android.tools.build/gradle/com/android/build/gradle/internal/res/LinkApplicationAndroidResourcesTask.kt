...
abstract class LinkApplicationAndroidResourcesTask : ProcessAndroidResources() {
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) @get:Incremental abstract val manifestFiles: DirectoryProperty
  ...
  @get:OutputDirectory @get:Optional abstract val sourceOutputDirProperty: DirectoryProperty

  @get:OutputFile @get:Optional abstract val textSymbolOutputFileProperty: RegularFileProperty

  @get:OutputFile @get:Optional abstract val symbolsWithPackageNameOutputFile: RegularFileProperty

  @get:OutputFile @get:Optional abstract val proguardOutputFile: RegularFileProperty

  @get:Optional @get:OutputFile abstract val rClassOutputJar: RegularFileProperty
  ...
  @get:Optional @get:Input abstract val resOffset: Property<Int>

  @get:Internal abstract val componentType: Property<ComponentType>

  @get:Input abstract val resourceConfigs: SetProperty<String>

  @get:Input @get:Optional abstract val noCompress: ListProperty<String>

  @get:Input @get:Optional abstract val aaptAdditionalParameters: ListProperty<String>

  // Not an input as it is only used to rewrite exceptions and doesn't affect task output
  @get:Internal abstract val mergeBlameLogFolder: DirectoryProperty

  // No effect on task output, used for generating absolute paths for error messaging.
  @get:Internal abstract val sourceSetMaps: ConfigurableFileCollection

  @get:Classpath @get:Optional abstract val featureResourcePackages: ConfigurableFileCollection

  @get:Input abstract val namespace: Property<String>

  @get:Input abstract val useConditionalKeepRules: Property<Boolean>

  @get:Input abstract val useMinimalKeepRules: Property<Boolean>

  @get:OutputDirectory abstract val linkedResourcesOutputDir: DirectoryProperty

  @get:Input abstract val linkedResourcesArtifactType: Property<InternalArtifactType<Directory>>

  @get:Input abstract val projectBaseName: Property<String>

  @get:Input abstract val namespaced: Property<Boolean>

  @get:Input abstract val applicationId: Property<String>

  @get:Classpath @get:Optional @get:Incremental abstract val inputResourcesDir: DirectoryProperty

  @get:Input abstract val library: Property<Boolean>

  @get:Nested abstract val androidJarInput: AndroidJarInput

  @get:Input abstract val useFinalIds: Property<Boolean>

  @get:Input abstract val useStableIds: Property<Boolean>
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val aarMetadataCheck: ConfigurableFileCollection

  @get:Nested abstract val aapt2: Aapt2Input

  @get:Internal abstract val symbolTableBuildService: Property<SymbolTableBuildService>

  // Not an input as it is only used to rewrite exception and doesn't affect task output
  @get:Internal abstract val manifestMergeBlameFile: RegularFileProperty

  @get:Classpath @get:Optional abstract val compiledDependenciesResources: ConfigurableFileCollection

  @get:Internal abstract val incrementalDirectory: DirectoryProperty

  @get:Input abstract val localeFilters: SetProperty<String>

  @get:Input abstract val pseudoLocalesEnabled: Property<Boolean>

  @get:Classpath @get:Optional @get:Incremental abstract val navigationUpdatedFolder: DirectoryProperty
  ...
  override fun getManifestFile(): File? {
    ...
    return if (mainSplit != null) {
      FileUtils.join(manifestDirectory, mainSplit.dirName(), SdkConstants.ANDROID_MANIFEST_XML)
    } else {
      FileUtils.join(manifestDirectory, SdkConstants.ANDROID_MANIFEST_XML)
    }
  }
  ...
  abstract class TaskAction : ProfileAwareWorkAction<TaskWorkActionParameters>() {
    ...
    override fun run() {
      ...
      val manifestBuiltArtifacts =
        BuiltArtifactsLoaderImpl().load(parameters.manifestFiles)
      ...
      val mainOutput =
        outputsHandler.getOutputs { it.getFilter(FilterConfiguration.FilterType.DENSITY) == null }.firstOrNull()
          ?: throw RuntimeException("No non-density apk found")

      val manifestOutput =
        outputsHandler.extractArtifactForSplit(artifacts = manifestBuiltArtifacts, config = mainOutput.variantOutputConfiguration)
      ...
      outputsHandler
        .getOutputs { true }
        .minus(mainOutput)
        .let { unprocessedOutputs ->
          val workQueue = workerExecutor.noIsolation()
          for (variantOutput in unprocessedOutputs) {

            val splitManifestOutput =
              outputsHandler.extractArtifactForSplit(artifacts = manifestBuiltArtifacts, config = variantOutput.variantOutputConfiguration)
  ...
  abstract class BaseCreationAction(
    ...
    VariantTaskCreationAction<LinkApplicationAndroidResourcesTask, ComponentCreationConfig>(creationConfig),
    ...
    override fun handleProvider(taskProvider: TaskProvider<LinkApplicationAndroidResourcesTask>) {
      super.handleProvider(taskProvider)
      creationConfig.taskContainer.processAndroidResTask = taskProvider
      creationConfig.artifacts
        .setInitialProvider(taskProvider, LinkApplicationAndroidResourcesTask::linkedResourcesOutputDir)
        .on(linkedResourcesArtifactType)

      if (generatesProguardOutputFile(creationConfig)) {
        creationConfig.artifacts
          .setInitialProvider(taskProvider, LinkApplicationAndroidResourcesTask::proguardOutputFile)
      ...
      if (generateLegacyMultidexMainDexProguardRules) {
        creationConfig.artifacts
          .setInitialProvider(taskProvider, LinkApplicationAndroidResourcesTask::mainDexListProguardOutputFile)
          .withName("manifest_keep.txt")
          .on(InternalArtifactType.LEGACY_MULTIDEX_AAPT_DERIVED_PROGUARD_RULES)
      }

      creationConfig.artifacts
        .setInitialProvider(taskProvider, LinkApplicationAndroidResourcesTask::stableIdsOutputFileProperty)
        .withName("stableIds.txt")
        .on(InternalArtifactType.STABLE_RESOURCE_IDS_FILE)
    }
    ...
    override fun configure(task: LinkApplicationAndroidResourcesTask) {
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.PACKAGED_MANIFESTS, task.manifestFiles)

      creationConfig.artifacts.setTaskInputToFinalProduct(
        InternalArtifactType.AAPT_FRIENDLY_MERGED_MANIFESTS,
        task.aaptFriendlyManifestFiles,
      )
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.MERGED_MANIFESTS, task.mergedManifestFiles)
      ...
      task.useConditionalKeepRules.setDisallowChanges(projectOptions.getProvider(BooleanOption.CONDITIONAL_KEEP_RULES))
      task.useMinimalKeepRules.setDisallowChanges(projectOptions.getProvider(BooleanOption.MINIMAL_KEEP_RULES))
      ...
      if (!componentType.isForTesting) {
        task.featureResourcePackages.fromDisallowChanges(
          creationConfig.variantDependencies.getArtifactFileCollection(COMPILE_CLASSPATH, PROJECT, FEATURE_RESOURCE_PKG)
        )
      } else {
      ...
      task.useFinalIds.setDisallowChanges(projectOptions.getProvider(BooleanOption.USE_NON_FINAL_RES_IDS).map { !it })

      task.manifestMergeBlameFile.setDisallowChanges(creationConfig.artifacts.get(InternalArtifactType.MANIFEST_MERGE_BLAME_FILE))
      ...
      task.useStableIds.setDisallowChanges(projectOptions.getProvider(BooleanOption.ENABLE_STABLE_IDS))

      task.aarMetadataCheck.from(creationConfig.artifacts.get(InternalArtifactType.AAR_METADATA_CHECK))
  ...
  internal class CreationAction(
    ...
    isLibrary: Boolean,
  ) : BaseCreationAction(creationConfig, generateLegacyMultidexMainDexProguardRules, baseName, isLibrary) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<LinkApplicationAndroidResourcesTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, LinkApplicationAndroidResourcesTask::rClassOutputJar)
        .withName(FN_R_CLASS_JAR)
        .on(InternalArtifactType.COMPILE_AND_RUNTIME_R_CLASS_JAR)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, LinkApplicationAndroidResourcesTask::textSymbolOutputFileProperty)
        .withName(SdkConstants.FN_RESOURCE_TEXT)
        .on(SingleArtifact.RUNTIME_SYMBOL_LIST)
      ...
      if (!creationConfig.services.projectOptions[BooleanOption.ENABLE_APP_COMPILE_TIME_R_CLASS]) {
        ...
        creationConfig.artifacts
          .setInitialProvider(taskProvider, LinkApplicationAndroidResourcesTask::symbolsWithPackageNameOutputFile)
          .withName("package-aware-r.txt")
          .on(InternalArtifactType.SYMBOL_LIST_WITH_PACKAGE_NAME)
      }
    }
    ...
    override fun configure(task: LinkApplicationAndroidResourcesTask) {
      ...
      if (creationConfig.services.projectOptions[BooleanOption.NON_TRANSITIVE_R_CLASS]) {
        ...
        creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.LOCAL_ONLY_SYMBOL_LIST, task.localResourcesFile)
      }

      task.dependenciesFileCollection.fromDisallowChanges(
        creationConfig.variantDependencies.getArtifactFileCollection(RUNTIME_CLASSPATH, ALL, ArtifactType.SYMBOL_LIST_WITH_PACKAGE_NAME)
      )
      creationConfig.artifacts.setTaskInputToFinalProduct(sourceArtifactType.outputType, task.inputResourcesDir)
      creationConfig.artifacts.setTaskInputToFinalProduct(COMPILED_NAVIGATION_RES, task.navigationUpdatedFolder)
      ...
      if (creationConfig.services.projectOptions[BooleanOption.SUPPORT_OEM_TOKEN_LIBRARIES]) {
        task.sharedLibraryDependencies.fromDisallowChanges(
          creationConfig.variantDependencies.getArtifactFileCollection(RUNTIME_CLASSPATH, ALL, ArtifactType.RES_SHARED_OEM_TOKEN_LIBRARY)
        )
      }
      ...
      if (androidResourcesCreationConfig.isPrecompileDependenciesResourcesEnabled) {
        task.compiledDependenciesResources.fromDisallowChanges(
          creationConfig.variantDependencies.getArtifactFileCollection(RUNTIME_CLASSPATH, ALL, ArtifactType.COMPILED_DEPENDENCIES_RESOURCES)
        )
  ...
  companion object {
    ...
    private fun invokeAaptForSplit(
      ...
      for (featurePackage in parameters.featureResourcePackages.files) {
        ...
        if (buildElements?.elements != null && buildElements.elements.isNotEmpty()) {
          ...
          if (mainBuildOutput != null) {
            ...
            throw IOException("Cannot find PROCESSED_RES output for $variantOutput")
          }
        }
      }

      val variantName =
        if (variantOutput.fullName.isNotEmpty()) {
          RES_QUALIFIER_SEP + variantOutput.fullName
        } else ""

      val linkedResourcesOutputFile =
        File(
      ...
      val generateProtos =
        when (parameters.linkedResourcesArtifactType.get()) {
      ...
      try {
        ...
        run {
          val configBuilder =
            AaptPackageConfig.Builder()
              ...
              .setAllowReservedPackageId(parameters.packageId.isPresent && parameters.packageId.get() < FeatureSetMetadata.BASE_ID)
              ...
              .apply {
                val compiledDependencyResourceFiles = parameters.compiledDependenciesResources.files
          ...
          configBuilder.setIdentifiedSourceSetMap(mergeIdentifiedSourceSetFiles(parameters.sourceSetMaps.files.filterNotNull()))
        ...
        throw BuildException("Failed to process resources, see aapt output above for details.", e)
      }
    }
