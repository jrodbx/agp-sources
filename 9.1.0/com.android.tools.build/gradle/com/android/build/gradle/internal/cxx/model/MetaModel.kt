...
/**
 * Lighter-weight reflection over the CXX model rooted at [CxxAbiModel]. The reason for not using Kotlin reflection is in b/233948259.
 *
 * This enum is a flattened list representing various fields in the model. Fields may either be actual values in the model or extension
 * properties.
 *
 * The naming pattern is like this: CXX_ABI_MODEL_IS_64_BITS represents CxxAbiModel::is64Bits ^ ^ | +----- is64Bits +-------------------
 * CxxAbiModel
 *
...
/**
 * Lookup up a [ModelField] value from this [CxxAbiModel]. Value is converted to [String]. Not all values in [ModelField] have an equivalent
 * here. We only need the ones that are consumed by [Macro]. The others defined in [ModelField] are still needed for the rewriting logic in
