...
/**
 * Task responsible for publishing this module metadata (like its application ID) for other modules to consume.
 *
 * If the module is an application module, it publishes the value coming from the variant config.
 *
 * If the module is a base feature, it consumes the value coming from the (installed) application module and republishes it.
 *
 * Both dynamic-feature and feature modules consumes it, from the application module and the base feature module respectively.
 */
...
abstract class ModuleMetadataWriterTask : NonIncrementalTask() {
  ...
  @get:Input abstract val abiFilters: ListProperty<String>
  ...
  override fun doTaskAction() {
    val declaration =
      ModuleMetadata(
        ...
        ignoreFromAllExternalDependenciesInKeepRules = ignoreFromAllExternalDependenciesInKeepRules.get(),
  ...
  internal class CreationAction(creationConfig: ApplicationCreationConfig) :
    VariantTaskCreationAction<ModuleMetadataWriterTask, ApplicationCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ModuleMetadataWriterTask>) {
      super.handleProvider(taskProvider)
      // publish the ID for the dynamic features (whether it's hybrid or not) to consume.
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ModuleMetadataWriterTask::outputFile)
    ...
    override fun configure(task: ModuleMetadataWriterTask) {
      ...
      task.abiFilters.setDisallowChanges((creationConfig.nativeBuildCreationConfig?.userDefinedAbis ?: emptyList()).sorted())
      task.ignoreFromInKeepRules.setDisallowChanges(creationConfig.optimizationCreationConfig.ignoreFromInKeepRules)
