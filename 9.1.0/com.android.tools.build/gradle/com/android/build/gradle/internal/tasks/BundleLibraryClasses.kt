...
interface BundleLibraryClassesInputs {
  ...
  @get:Classpath @get:Optional val classes: ConfigurableFileCollection

  @get:Input val packageRClass: Property<Boolean>

  @get:Nested @get:Optional val dataBindingExcludeDelegate: Property<DataBindingExcludeDelegate>
}
...
private fun BundleLibraryClassesInputs.configure(
  ...
  // dependencies .
  val disableMinifyLocalDeps = creationConfig.services.projectOptions.get(BooleanOption.DISABLE_MINIFY_LOCAL_DEPENDENCIES_FOR_LIBRARIES)
  ...
  if (
    ...
    classes.from(creationConfig.artifacts.forScope(ScopedArtifacts.Scope.PROJECT).getFinalArtifacts(ScopedArtifact.CLASSES))
  }
...
private fun BundleLibraryClassesInputs.configureWorkerActionParams(
  ...
  val incrementalChanges =
    if (inputChanges?.isIncremental == true) {
      inputChanges.getFileChanges(classes)
    } else {
      emptyList()
    }
  params.namespace.set(namespace)
  params.toIgnore.set(dataBindingExcludeDelegate.orNull?.getExcludedClassList(namespace.get()) ?: listOf())
...
abstract class BundleLibraryClassesDir : NewIncrementalTask(), BundleLibraryClassesInputs {
  ...
  @get:Classpath @get:Incremental abstract override val classes: ConfigurableFileCollection

  override fun doTaskAction(inputChanges: InputChanges) {
    workerExecutor.noIsolation().submit(BundleLibraryClassesWorkAction::class.java) {
  ...
  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<BundleLibraryClassesDir, ComponentCreationConfig>(creationConfig) {
    ...
    override fun configure(task: BundleLibraryClassesDir) {
      ...
      task.configure(creationConfig, packageRClass = false, publishRuntimeElement = true)
    }
  }

  class KotlinMultiplatformCreationAction(creationConfig: KmpCreationConfig) :
    VariantTaskCreationAction<BundleLibraryClassesDir, KmpCreationConfig>(creationConfig) {
    ...
    override fun configure(task: BundleLibraryClassesDir) {
      super.configure(task)
      task.configure(creationConfig, packageRClass = false, publishRuntimeElement = false)
    }
  }
}
...
abstract class BundleLibraryClassesJar : NonIncrementalTask(), BundleLibraryClassesInputs {
  ...
  class CreationAction(creationConfig: ComponentCreationConfig, private val publishedType: PublishedConfigType) :
    VariantTaskCreationAction<BundleLibraryClassesJar, ComponentCreationConfig>(creationConfig) {
    override val name: String =
      creationConfig.computeTaskNameInternal(
    ...
    override fun handleProvider(taskProvider: TaskProvider<BundleLibraryClassesJar>) {
      super.handleProvider(taskProvider)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, BundleLibraryClassesJar::output)
        .withName(creationConfig.getArtifactName(FN_CLASSES_JAR))
        .let {
          if (publishedType == PublishedConfigType.API_ELEMENTS) {
    ...
    override fun configure(task: BundleLibraryClassesJar) {
      super.configure(task)
      val packageRClass = publishedType == PublishedConfigType.API_ELEMENTS && creationConfig.buildFeatures.androidResources

      task.configure(creationConfig, packageRClass, publishedType != PublishedConfigType.API_ELEMENTS)
    }
  }

  class KotlinMultiplatformCreationAction(component: KmpCreationConfig, private val publishedType: PublishedConfigType) :
    VariantTaskCreationAction<BundleLibraryClassesJar, KmpCreationConfig>(component) {

    init {
      check(publishedType == PublishedConfigType.API_ELEMENTS || publishedType == PublishedConfigType.RUNTIME_ELEMENTS) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<BundleLibraryClassesJar>) {
      ...
      creationConfig.artifacts.setInitialProvider(taskProvider, BundleLibraryClassesJar::output).withName(FN_CLASSES_JAR).let {
        if (publishedType == PublishedConfigType.API_ELEMENTS) {
          it.on(InternalArtifactType.COMPILE_LIBRARY_CLASSES_JAR)
        } else {
          it.on(InternalArtifactType.RUNTIME_LIBRARY_CLASSES_JAR)
...
abstract class BundleLibraryClassesWorkAction : ProfileAwareWorkAction<BundleLibraryClassesWorkAction.Params>() {
  ...
  override fun run() {
    val ignorePatterns =
      (LibraryAarJarsTask.getDefaultExcludes(packageR = parameters.packageRClass.get()) + parameters.toIgnore.get()).map {
        Pattern.compile(it)
      }

    val predicate =
      Predicate<String> { relativePath ->
        val normalizedPath = FileUtils.toSystemIndependentPath(relativePath)
        (CLASS_PATTERN.matcher(normalizedPath).matches() || META_INF_PATTERN.matcher(normalizedPath).matches()) &&
    ...
    if (isJarFile(parameters.output.get())) {
      ...
      when (getClassesDirFormat(parameters.input.files)) {
        CONTAINS_SINGLE_JAR -> {
          ...
          zipFilesNonIncrementally(parameters.input.files, parameters.output.get().resolve(FN_CLASSES_JAR), predicate)
        }
  ...
  /**
   * Returns the format of the directory with artifact type [CLASSES_DIR]. The format depends on whether the input classes contain jars nor
   * not. See [AndroidArtifacts.ClassesDirFormat] for more details.
   */
  ...
  private fun zipFilesNonIncrementally(input: Set<File>, outputJar: File, filter: Predicate<String>) {
  ...
  private fun copyFilesNonIncrementally(input: Set<File>, outputDir: File, filter: Predicate<String>) {
  ...
  private fun copyFilesIncrementally(inputChanges: SerializableFileChanges, outputDir: File, filter: Predicate<String>) {
