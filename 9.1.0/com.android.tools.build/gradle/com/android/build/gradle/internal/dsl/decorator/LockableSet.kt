 ...
 *
 * This is intentionally not serializable, as model classes should take copies e.g. [com.google.common.collect.ImmutableList.copyOf]
 */
class LockableSet<T> @Inject @JvmOverloads constructor(private val name: String, private val delegate: MutableSet<T> = mutableSetOf()) :
  ...
  override fun add(element: T): Boolean = check { delegate.add(element) }
  ...
  private inner class LockableIterator<T>(private val delegate: MutableIterator<T>) : MutableIterator<T> {
    ...
    override fun remove() = check { delegate.remove() }
  }
}
