...
class FullDependencyGraphBuilder(
  private val artifactsProvider: (AndroidArtifacts.ConsumedConfigType, root: ResolvedComponentResult) -> Set<ResolvedArtifact>,
  ...
  fun build(): ArtifactDependencies =
    ArtifactDependenciesImpl(
      buildGraph(AndroidArtifacts.ConsumedConfigType.COMPILE_CLASSPATH, buildAdjacencyList = false).graphItems(),
      if (dontBuildRuntimeClasspath) null
      else buildGraph(AndroidArtifacts.ConsumedConfigType.RUNTIME_CLASSPATH, buildAdjacencyList = false).graphItems(),
      unresolvedDependencies.values.toList(),
    )

  fun buildWithAdjacencyList() =
    ArtifactDependenciesAdjacencyListImpl(
      buildGraph(AndroidArtifacts.ConsumedConfigType.COMPILE_CLASSPATH, buildAdjacencyList = true).edges(),
      if (dontBuildRuntimeClasspath) null
      else buildGraph(AndroidArtifacts.ConsumedConfigType.RUNTIME_CLASSPATH, buildAdjacencyList = true).edges(),
      unresolvedDependencies.values.toList(),
    )

  private fun buildGraph(configType: AndroidArtifacts.ConsumedConfigType, buildAdjacencyList: Boolean): Graph {
    ...
    val javadocArtifacts =
      if (addAdditionalArtifactsInModel) {
        resolutionResultProvider.getAdditionalArtifacts(configType, AdditionalArtifactType.JAVADOC).associate {
    ...
    for (dependency in roots.filter { !it.isConstraint }) {
      handleDependency(dependency, visited, artifactMap, javadocArtifacts, sourceArtifacts)?.let { items.add(it) }
    }
    ...
    return if (buildAdjacencyList) {
      ...
      GraphItemList(graphItems = items.toList())
    }
  }

  private fun buildAdjacencyList(items: MutableList<GraphItem>, graphEdgeCache: GraphEdgeCache?): MutableList<Edge> {
    ...
    while (queue.isNotEmpty()) {
      ...
      if (seen.add(from.key)) {
        ...
        edges.addAll(from.dependencies.map { to -> graphEdgeCache.getEdge(from.key, to.key) })
  ...
  private fun handleDependency(
    ...
    if (dependency !is ResolvedDependencyResult) {
      (dependency as? UnresolvedDependencyResult)?.let {
        ...
        if (!unresolvedDependencies.containsKey(name)) {
          unresolvedDependencies[name] = UnresolvedDependencyImpl(name, it.failure.cause?.message)
        }
      }
      return null
    }
    ...
    val variantDependencies by lazy { dependency.selected.getDependenciesForVariant(variant) }

    val library =
      try {
        getLibrary(projectPath, libraryService, variant, variantDependencies, artifactMap, javadocArtifacts, sourceArtifacts)
      } catch (e: Exception) {
        val name = dependency.requested.toString()
        if (!unresolvedDependencies.containsKey(name)) {
          unresolvedDependencies[name] =
    ...
    if (library != null) {
      ...
      val libraryGraphItem = GraphItemImpl(library.key, null).also { visited[variant] = it }

      // Now visit children, and add them as dependencies
      variantDependencies.forEach {
        handleDependency(it, visited, artifactMap, javadocArtifacts, sourceArtifacts)?.let { childGraphItem ->
...
fun getLibrary(
  ...
  fun androidProjectDependency(owner: ProjectComponentIdentifier) =
    libraryService.getLibrary(
      ResolvedArtifact(
        owner,
        variant,
        variantName = variant.attributes.getAttribute(VariantAttr.ATTRIBUTE)?.toString() ?: "unknown",
        artifactFile = null,
        isTestFixturesArtifact = variant.capabilities.any { it.isProjectTestFixturesCapability(owner.projectName) },
  ...
  return if (artifact == null) {
    ...
    when {
      ...
      owner is ProjectComponentIdentifier &&
        variantDependencies.isNotEmpty() &&
        getCategoryAttribute(variant.attributes) == Category.REGULAR_PLATFORM -> dependencyWithoutArtifactButTransitiveDependencies()
      // AARs for Android projects aren't fetched as an optimization, but we still need them
      // and their transitive dependencies
      owner is ProjectComponentIdentifier && variant.isAndroidProjectDependency() -> androidProjectDependency(owner)
...
private fun getCategoryAttribute(attributes: AttributeContainer): String? {
  ...
  val categoryAttr = attributes.keySet().firstOrNull { it.name == Category.CATEGORY_ATTRIBUTE.name } ?: return null
