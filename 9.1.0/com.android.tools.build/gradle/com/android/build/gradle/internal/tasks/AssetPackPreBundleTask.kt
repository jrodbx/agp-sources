...
/**
 * Task that collects the processed manifest file for the asset pack and the asset files to be included and uses a JarCreator to package
 * them together for the base app to include in the Android App Bundle.
 */
...
abstract class AssetPackPreBundleTask : NonIncrementalTask() {
  /**
   * Where to put the final archive containing all the files for the asset pack, to be included in the bundle build task for the base
   * module.
   */
  @get:OutputDirectory abstract val outputDir: DirectoryProperty
  /** The processed manifest file to be included in the app bundle for the asset pack. */
  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val manifestFiles: DirectoryProperty
  /** The asset files to be included in the app bundle as the contents of the asset pack. */
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val assetsFiles: ConfigurableFileCollection
  ...
  override fun doTaskAction() {
    for (manifestFile: File in manifestFiles.asFileTree.files) {
      ...
      workerExecutor.noIsolation().submit(AssetPackPreBundleTaskRunnable::class.java) {
        ...
        it.assetsFilesPath.set(
          assetsFiles
            .filter { assetPack ->
              assetPack.absolutePath.contains(assetPackName + File.separator + "src" + File.separator + "main" + File.separator + "assets")
            }
            .asPath
        )
  ...
  class CreationForAssetPackBundleAction(private val artifacts: ArtifactsImpl, private val assetFileCollection: FileCollection) :
    ...
    override fun handleProvider(taskProvider: TaskProvider<AssetPackPreBundleTask>) {
      super.handleProvider(taskProvider)
      artifacts.setInitialProvider(taskProvider, AssetPackPreBundleTask::outputDir).on(InternalArtifactType.ASSET_PACK_BUNDLE)
    }

    override fun configure(task: AssetPackPreBundleTask) {
      super.configure(task)
      artifacts.setTaskInputToFinalProduct(InternalArtifactType.LINKED_RES_FOR_ASSET_PACK, task.manifestFiles)
      task.assetsFiles.from(assetFileCollection)
    }
  }

  class CreationAction(creationConfig: VariantCreationConfig, private val assetFileCollection: FileCollection) :
    VariantTaskCreationAction<AssetPackPreBundleTask, VariantCreationConfig>(creationConfig) {
    override val type = AssetPackPreBundleTask::class.java
    ...
    override fun handleProvider(taskProvider: TaskProvider<AssetPackPreBundleTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, AssetPackPreBundleTask::outputDir)
        .on(InternalArtifactType.ASSET_PACK_BUNDLE)
    }

    override fun configure(task: AssetPackPreBundleTask) {
      super.configure(task)
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.LINKED_RES_FOR_ASSET_PACK, task.manifestFiles)
...
abstract class AssetPackPreBundleTaskRunnable : ProfileAwareWorkAction<AssetPackPreBundleTaskRunnable.Params>() {
  override fun run() {
    ...
    jarCreator.use {
      if (parameters.assetsFilesPath.get().isNotEmpty()) {
        it.addDirectory(Paths.get(parameters.assetsFilesPath.get()), null, null) { entryPath -> "$FD_ASSETS/$entryPath" }
      }
