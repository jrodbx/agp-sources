...
abstract class FeatureSetMetadataWriterTask : NonIncrementalTask() {
  ...
  abstract class FeatureSetRunnable : ProfileAwareWorkAction<Params>() {
    override fun run() {
      ...
      val featureMetadata = FeatureSetMetadata.Builder(parameters.minSdkVersion.get(), parameters.maxNumberOfFeaturesBeforeOreo.get())
      ...
      for (feature in features) {
        featureMetadata.addFeatureSplit(feature.modulePath, featureNameMap[feature.modulePath]!!, feature.namespace)
      }
  ...
  class CreationAction(creationConfig: VariantCreationConfig) :
    VariantTaskCreationAction<FeatureSetMetadataWriterTask, VariantCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<FeatureSetMetadataWriterTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, FeatureSetMetadataWriterTask::outputFile)
        .withName(FeatureSetMetadata.OUTPUT_FILE_NAME)
        .on(InternalArtifactType.FEATURE_SET_METADATA)
    }
    ...
    override fun configure(task: FeatureSetMetadataWriterTask) {
      ...
      val maxNumberOfFeaturesBeforeOreo = creationConfig.services.projectOptions.get(IntegerOption.PRE_O_MAX_NUMBER_OF_FEATURES)
      if (maxNumberOfFeaturesBeforeOreo != null) {
        task.maxNumberOfFeaturesBeforeOreo = Integer.min(100, maxNumberOfFeaturesBeforeOreo)
      }
    }
  }
}
    ...
    val module =
      modules.singleOrNull()
        ?: throw RuntimeException(
          modules
            .joinTo(
              StringBuilder("Module name '$leaf' is used by multiple modules. All dynamic features must have a unique name."),
              separator = "\n\t-> ",
              prefix = "\n\t-> ",
            )
            .toString()
