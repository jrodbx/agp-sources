  ...
  destConfiguration.incoming.afterResolve { dest ->
    val moduleToProjectSubstitutionMap =
      srcConfiguration.incoming.resolutionResult.allDependencies
        ...
        .filter { it.selected.id is ProjectComponentIdentifier && it.requested is ModuleComponentSelector }
        .associate {
          (it.requested as ModuleComponentSelector).moduleIdentifier to (it.selected.id as ProjectComponentIdentifier).projectPath
        }
    // In that case assert the substitution was also done in the aligned configuration
    val neededAndroidTestProjectDeps =
      dest.resolutionResult.allDependencies
        .filterIsInstance<ResolvedDependencyResult>()
        .filter { dependency ->
          ...
          componentIdentifier is ModuleComponentIdentifier && componentIdentifier.moduleIdentifier in moduleToProjectSubstitutionMap
        }
        .map { dependency ->
    ...
    val dependencyNumber = if (neededAndroidTestProjectDeps.size > 1) "dependencies" else "dependency"
    issueReporter.reportError(
      IssueReporter.Type.DEPENDENCY_INTERNAL_CONFLICT,
      RuntimeException(
        "Unable to align dependencies in configurations " +
