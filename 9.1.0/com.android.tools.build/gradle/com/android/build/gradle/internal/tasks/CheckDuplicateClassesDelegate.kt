...
/**
 * A class that checks for duplicate classes within an ArtifactCollection. Classes are assumed to be duplicate if they have the same name
 * and they are positioned within the same package (this is possible if they are in different artifacts).
 */
class CheckDuplicateClassesDelegate {
  ...
  fun run(enumeratedClasses: Map<String, File>) {
    ...
    classesMap.forEach {
      ...
      it.value.forEach { className -> classes.getOrPut(className) { mutableListOf() }.add(artifactName) }
    }

    val duplicatesMap = classes.filter { it.value.size > 1 }.toSortedMap()
    if (!duplicatesMap.isEmpty()) {
      val lineSeparator = System.lineSeparator()
      val duplicateMessages = duplicatesMap.map { duplicateClassMessage(it.key, it.value) }.joinToString(lineSeparator)
...
abstract class CheckDuplicatesRunnable @Inject constructor() : WorkAction<CheckDuplicatesParams> {
  override fun execute() {
    CheckDuplicateClassesDelegate().run(parameters.enumeratedClasses.get())
  }
}
