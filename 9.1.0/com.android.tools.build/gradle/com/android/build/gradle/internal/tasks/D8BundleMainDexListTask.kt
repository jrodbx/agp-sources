...
abstract class D8BundleMainDexListTask : NonIncrementalTask() {
  ...
  @get:Input abstract val errorFormat: Property<SyncOptions.ErrorFormatMode>

  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val aaptGeneratedRules: RegularFileProperty

  @get:Optional @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val userMultidexProguardRules: ListProperty<RegularFile>
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val baseDexDirs: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val featureDexDirs: ConfigurableFileCollection

  @get:Classpath abstract val libraryClasses: ConfigurableFileCollection

  @get:OutputFile abstract val output: RegularFileProperty

  override fun doTaskAction() {
    workerExecutor.noIsolation().submit(MainDexListWorkerAction::class.java) { params ->
  ...
  abstract class MainDexListWorkerAction : ProfileAwareWorkAction<MainDexListWorkerAction.Params>() {
    ...
    override fun run() {
      ...
      parameters.userMultidexKeepFile.orNull?.let { mainDexClasses.addAll(it.asFile.readLines()) }

      parameters.output.asFile.get().writeText(mainDexClasses.joinToString(separator = System.lineSeparator()))
    }
  }

  class CreationAction(creationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<D8BundleMainDexListTask, ApkCreationConfig>(creationConfig),
    ...
    override fun handleProvider(taskProvider: TaskProvider<D8BundleMainDexListTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, D8BundleMainDexListTask::output)
    ...
    override fun configure(task: D8BundleMainDexListTask) {
      ...
      task.userMultidexProguardRules.setDisallowChanges(creationConfig.artifacts.getAll(MultipleArtifact.MULTIDEX_KEEP_PROGUARD))
      task.userMultidexKeepFile.setDisallowChanges(dexingCreationConfig.multiDexKeepFile)
      task.bootClasspath.from(creationConfig.global.bootClasspath).disallowChanges()
      task.errorFormat.setDisallowChanges(SyncOptions.getErrorFormatMode(creationConfig.services.projectOptions))
      ...
      task.baseDexDirs.from(creationConfig.artifacts.getAll(InternalMultipleArtifactType.DEX))
...
internal fun getPlatformRules(): List<String> =
  listOf(
