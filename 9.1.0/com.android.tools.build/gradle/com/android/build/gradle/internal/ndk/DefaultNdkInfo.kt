...
open class DefaultNdkInfo(protected val rootDirectory: File) : NdkInfo {
  ...
  protected val hostTag: String by lazy {
    ...
    checkTags.find { prebuiltBase.resolve(it).isDirectory }
      ?: throw InvalidUserDataException(
        "Could not determine NDK host architecture. None of the following directories " + "exist: ${checkDirs.joinToString(",")}"
      )
  }
  ...
  private fun getToolchainPath(abi: String): File {
    ...
    val version =
      getDefaultToolchainVersion(toolchainAbi).let {
    ...
    val prebuiltFolder = rootDirectory.resolve("toolchains/${getToolchainPrefix(toolchainAbi)}$version/prebuilt/$hostTag")
  ...
  override fun getStripExecutable(abi: String): File {
    ...
    return FileUtils.join(getToolchainPath(toolchainAbi), "bin", "$triple-strip")
  }
  ...
  override fun getObjcopyExecutable(abi: String): File {
    val toolchainAbi = getToolchainAbi(abi)
    val triple = abiInfoList.single { info -> info.name == toolchainAbi }.triple
    return FileUtils.join(getToolchainPath(toolchainAbi), "bin", "$triple-objcopy")
  }

  /**
   * Return the default version of the specified toolchain for a target abi.
   *
   * The default version is the highest version found in the NDK for the specified toolchain and ABI. The result is cached for performance.
   */
  private fun getDefaultToolchainVersion(abi: String): String {
    ...
    val toolchainsForAbi = toolchains.listFiles { _, filename -> filename.startsWith(toolchainPrefix) }
    if (toolchainsForAbi == null || toolchainsForAbi.isEmpty()) {
      throw RuntimeException("No toolchains found in the NDK toolchains folder for ABI with prefix: $toolchainPrefix")
    }
  ...
  override val defaultAbis
    get() = abiInfoList.stream().filter { abiInfo -> abiInfo.isDefault && !abiInfo.isDeprecated }.map { it.name }.toList()

  override val supported32BitsAbis
    get() = abiInfoList.stream().filter { abiInfo -> abiInfo.bitness == 32 }.map { it.name }.toList()

  override val supportedAbis
    get() = abiInfoList.stream().map { it.name }.toList()
