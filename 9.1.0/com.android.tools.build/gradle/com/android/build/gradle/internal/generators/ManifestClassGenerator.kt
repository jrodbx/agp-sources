...
/** Creates a jar file containing a compiled manifest.class file based on the properties provided a [ManifestClassData] instance. */
class ManifestClassGenerator(private val manifestClassData: ManifestClassData) : GeneratedCodeFileCreator {
  ...
  val fullyQualifiedManifestClassName by lazy {
    if (manifestClassData.namespace.isEmpty()) "Manifest" else "${manifestClassData.namespace.replace('.', '/')}/Manifest"
  }

  val customPermissions by lazy { getCustomPermissions(manifestClassData.manifestFile) }

  override fun generate() = generateManifestJar(customPermissions, manifestClassData.outputFilePath)
  ...
  private fun generateManifestJar(permissions: List<String>, outputJar: File) {
    JarFlinger(outputJar.toPath()).use {
      ...
      it.addEntry("$fullyQualifiedManifestClassName${SdkConstants.DOT_CLASS}", generateManifestClass().inputStream())
    }
  }
