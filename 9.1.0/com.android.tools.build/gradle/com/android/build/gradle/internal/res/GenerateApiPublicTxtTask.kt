 ...
 *
 * The artifact in the AAR has the challenging-for-consumers attribute (They can;t ) of sometimes not existing, so this tasks
 *
...
abstract class GenerateApiPublicTxtTask : NonIncrementalTask() {
  ...
  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val localOnlyResourceSymbols: RegularFileProperty

  @get:OutputFile abstract val externalPublicTxt: RegularFileProperty
  ...
  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<GenerateApiPublicTxtTask, ComponentCreationConfig>(creationConfig) {

    override val name: String = computeTaskName("generate", "ExternalPublicTxt")
    override val type: Class<GenerateApiPublicTxtTask>
      get() = GenerateApiPublicTxtTask::class.java

    override fun handleProvider(taskProvider: TaskProvider<GenerateApiPublicTxtTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, GenerateApiPublicTxtTask::externalPublicTxt)
        .withName("public.txt")
  ...
  companion object {
    @VisibleForTesting
    internal fun writeFile(internalPublicTxt: Path, symbols: Path, externalPublicTxt: Path) {
      if (Files.exists(internalPublicTxt)) {
        FileUtils.copyFile(internalPublicTxt, externalPublicTxt)
      } else {
        Files.newBufferedWriter(externalPublicTxt).use { writer -> writePublicTxtFile(SymbolIo.readRDef(symbols), writer) }
      }
    }
  }
}
