...
/**
 * Returns module components in this [ResolutionResult] ([ResolvedComponentResult]s that have a [ModuleComponentIdentifier]) which satisfy
 * the given filter.
 *
 * Note that there is no guarantee on the order of this list, but in practice Gradle usually makes sure that it is deterministic.
 */
...
/**
 * Returns a [ComponentPath] starting from the root of this [ResolutionResult] to the given component.
 *
 * Note that there can be multiple paths, and this method returns only one of the paths. The result is stable across invocations.
 */
fun ResolutionResult.getPathToComponent(component: ResolvedComponentResult): ComponentPath {
  ...
  checkNotNull(foundPathToRoot) { "Unable to find a path from root (${root.id.displayName}) to ${component.id.displayName}" }
  return ComponentPath(foundPathToRoot!!.reversed())
}

/** A path consisting of [ResolvedComponentResult]s starting from the root of a [ResolutionResult] to a component. */
class ComponentPath(val components: List<ResolvedComponentResult>) {

  /**
   * Returns a string describing this [ComponentPath].
   *
   * Note that the root of the [ResolutionResult] may not be in a user-friendly format (e.g., ":<project-name>:unspecified"), so the caller
   * of this method can provide a `rootDisplayNameOverride` (e.g., "Project <name>" or "Configuration <name>") to substitute it.
   *
   * @param rootDisplayNameOverride a display name substitute for the root of the [ResolutionResult]
   */
  fun getPathString(rootDisplayNameOverride: String? = null, separator: String = " -> "): String {
    val path = this.components.map { it.id.displayName }
    val adjustedPath =
      if (rootDisplayNameOverride != null) {
