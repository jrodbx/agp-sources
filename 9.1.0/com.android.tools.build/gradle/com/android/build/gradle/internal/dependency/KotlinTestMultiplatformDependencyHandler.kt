...
/**
 * Configure capability of the `org.jetbrains.kotlin:kotlin-test` artifact to resolve to the proper artifact depending on the test platform
 * configured. So when users declare a dependency on `kotlin-test` from their common sourceSets, the unitTest configurations would be able
 * to resolve it to `junit` or `testng`.
 *
 * @note: This is a workaround until the kotlin plugin provides a proper API for us to either declare out test tasks, or to configure the
...
internal fun configureKotlinTestDependencyForUnitTestCompilation(
  ...
  configureKotlinTestDependency(
    ...
    testFrameworkCapabilityProvider =
      project.tasks.named(unitTestComponent.computeTaskNameInternal(ComponentType.UNIT_TEST_PREFIX)).map {
...
/**
 * Configure capability of the `org.jetbrains.kotlin:kotlin-test` artifact to resolve to the proper artifact depending on the test platform
 * configured. So when users declare a dependency on `kotlin-test` from their common sourceSets, the instrumentedTest configurations would
 * be able to\ resolve it to `kotlin-test-junit`.
 *
 * @note: This is a workaround until the kotlin plugin provides a proper API for us to either declare out test tasks, or to configure the
 * test framework directly.
 */
internal fun configureKotlinTestDependencyForInstrumentedTestCompilation(
  ...
  configureKotlinTestDependency(
    ...
    kotlinMultiplatformExtension,
    testFrameworkCapabilityProvider = instrumentedTestComponent.services.provider { testFrameworkCapabilityOf("junit") },
  )
}
...
private fun configureKotlinTestDependency(
  ...
  listOf(
    ...
    .forEach {
      project.configurations.getByName(it).maybeAddTestDependencyCapability(project.dependencies, testFrameworkCapabilityProvider) {
  ...
  withDependencies { dependencies ->
    val testRootDependency =
      allDependencies
        ...
        .singleOrNull { dependency -> dependency.group == KOTLIN_MODULE_GROUP && dependency.name == KOTLIN_TEST_ROOT_MODULE_NAME }
    ...
    if (testRootDependency != null) {
      ...
      dependencies.addLater(
        testFrameworkCapabilityProvider.map { capability ->
          dependencyHandler.create("$KOTLIN_MODULE_GROUP:$KOTLIN_TEST_ROOT_MODULE_NAME:$depVersion").apply {
            (this as ExternalDependency).capabilities { it.requireCapability(capability) }
          }
        }
      )
    }
...
private fun testFrameworkOf(testTask: Test?): String =
  when (testTask?.options) {
...
private fun testFrameworkCapabilityOf(testFramework: String) = "$KOTLIN_MODULE_GROUP:$KOTLIN_TEST_ROOT_MODULE_NAME-framework-$testFramework"
