...
abstract class AndroidLintWorkAction : WorkAction<AndroidLintWorkAction.LintWorkActionParameters> {
  ...
  companion object {
     ...
     *
     * Use a soft reference so the cache doesn't end up keeping a classloader that isn't actually reachable.
     *
    ...
    /**
     * Classloaders used during this build that will have disposeApplicationEnvironment called at the end of the build.
     *
     * This is cleared at the end of each build
     */
    @GuardedBy("this") private val toDispose = mutableMapOf<String, URLClassLoader>()

    private val logger
      get() = Logging.getLogger(AndroidLintWorkAction::class.java)
    ...
    private fun getClassloader(key: String, classpath: FileCollection): ClassLoader {
      ...
      return cachedClassloader.executeCallableSynchronously {
        ...
        val classloader =
          map[key]?.get()?.also { logger.info("Android Lint: Reusing lint classloader {}", key) }
      ...
      if (toDispose.isNotEmpty()) {
        ...
        try {
          toDispose.values.forEach { classloader ->
            classloader.loadClass("com.android.tools.lint.UastEnvironment").getDeclaredMethod("disposeApplicationEnvironment").invoke(null)
          }
        } finally {
          toDispose.clear()
        }
    ...
    fun maybeThrowException(
      ...
      when (execResult) {
        ERRNO_SUCCESS -> {}
        ERRNO_ERRORS -> {
          throw RuntimeException(getErrorMessage(android, fatalOnly, abbreviatedLintOutput, hasBaseline))
        }
        ...
        ERRNO_APPLIED_SUGGESTIONS ->
          throw RuntimeException("Aborting build since sources were modified to apply quickfixes after compilation")
        else -> throw IllegalStateException("Internal error: unexpected lint return value $execResult")
      }
    }

    private fun getErrorMessage(android: Boolean, fatalOnly: Boolean, abbreviatedLintOutput: String?, hasBaseline: Boolean): String {
      val prefix =
      ...
      val suggestion =
        when {
          hasBaseline -> "Fix the issues identified by lint, or add the issues to the lint baseline via `gradlew updateLintBaseline`."
          android ->
            """
      ...
      val moreInfo = "For more details, see https://developer.android.com/studio/write/lint#snapshot"
