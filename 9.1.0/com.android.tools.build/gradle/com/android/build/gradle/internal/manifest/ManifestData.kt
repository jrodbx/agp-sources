...
class ManifestData(
  ...
  /** The value of the `isolatedSplits` attribute of the `application` tag, or `null` if it is not specified. */
  var isolatedSplits: Boolean? = null,
) {

  /**
   * Temporary holder of api/codename to store the output of the minSdkVersion value. This is not final and will be replaced by the final
   * API class. // FIXME b/150290704
   */
  data class AndroidTarget(val apiLevel: Int?, val codeName: String?)
...
fun parseManifest(
  ...
  if (!manifestParsingAllowed) {
    ...
    val stackTrace =
      Thread.currentThread()
        .stackTrace
        .map { it.toString() }
        .filter { !it.startsWith("com.android.build.gradle.internal.manifest.") && !it.startsWith("org.gradle.") }
        .subList(1, 10)
  ...
  if (manifestFileContent.isEmpty()) {
    if (manifestFileRequired) {
      issueReporter.reportError(IssueReporter.Type.MISSING_ANDROID_MANIFEST, "Manifest file does not exist: $manifestFilePath")
        ...
        if (uri.isNullOrEmpty()) {
          when {
            SdkConstants.TAG_MANIFEST == localName -> {
              ...
              data.versionCode = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_VERSION_CODE)?.toInt()

              data.versionName = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_VERSION_NAME)
            }
            SdkConstants.TAG_INSTRUMENTATION == localName -> {
              data.testLabel = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_LABEL)

              data.functionalTest = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_FUNCTIONAL_TEST)?.toBoolean()

              data.instrumentationRunner = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_NAME)

              data.handleProfiling = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_HANDLE_PROFILING)?.toBoolean()
            }
            SdkConstants.TAG_USES_SDK == localName -> {

              data.minSdkVersion = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_MIN_SDK_VERSION)?.toAndroidTarget()

              data.targetSdkVersion = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_TARGET_SDK_VERSION)?.toAndroidTarget()
            }
            SdkConstants.TAG_APPLICATION == localName -> {
              data.extractNativeLibs = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_EXTRACT_NATIVE_LIBS)?.toBoolean()

              data.useEmbeddedDex = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_USE_EMBEDDED_DEX)?.toBoolean()

              data.isolatedSplits = attributes.getValue(SdkConstants.ANDROID_URI, SdkConstants.ATTR_ISOLATED_SPLITS)?.toBoolean()
            }
          }
        }
      }
...
private val PARSER_FACTORY =
  SAXParserFactory.newInstance(XmlUtils.SAX_PARSER_FACTORY, null).also { XmlUtils.configureSaxFactory(it, true, false) }
