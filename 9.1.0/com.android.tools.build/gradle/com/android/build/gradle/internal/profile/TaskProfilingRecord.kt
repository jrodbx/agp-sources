 ...
 *
 * Information contained in each instance will be use to upload our performance data per task once optional workers profiling information
 * has been gathered.
 */
open class TaskProfilingRecord
/** Construct a new task profiling record with the [GradleBuildProfileSpan] and decorations like project path and variant name. */
(
  ...
  @get:GuardedBy("this") val taskSpans = mutableListOf<GradleBuildProfileSpan>()
  ...
  internal enum class Status {
    ...
    RUNNING,
    /**
     * Task invoked [com.android.ide.common.workers.WorkerExecutorFacade.await] method and has blocked waiting for spawned workers to be
     * completed before resuming its activities. Since we do not intercept when the task resumes, it is unclear if the task is still blocked
     * waiting or running.
     */
    AWAIT,

    /** Gradle invoked the [org.gradle.tooling.events.OperationCompletionListener.onFinish] method when the associated task is finished. */
    FINISHED,
  ...
  open fun addWorker(key: String, type: GradleBuildProfileSpan.ExecutionType) {
    val workerRecord = WorkerProfilingRecord(taskPath, type, clock.instant())
    workerRecordList[key] = workerRecord
  }

  @Synchronized open fun get(key: String): WorkerProfilingRecord? = workerRecordList[key]

  @Synchronized
  fun allWorkersFinished(): Boolean {
    return workerRecordList.isEmpty() || workerRecordList.values.stream().allMatch(WorkerProfilingRecord::isFinished)
  }

  /**
   * Notification that a worker item has finished executing.
   *
   * Add a new span for the worker item and possibly close the task span if all spawned worker item have finished executing.
   */
  @Synchronized
  fun workerFinished(workerRecord: WorkerProfilingRecord) {

    // create the span for the worker item itself
    val workerSpan = GradleBuildProfileSpan.newBuilder().setId(resourceManager.allocateRecordId()).setParentId(spanBuilder.id)
  ...
  fun writeTaskSpan() {
    ...
    resourceManager.writeRecord(projectPath, variant, spanBuilder, taskSpans)
  }

  fun minimumWaitTime(): Duration = workerRecordList.values.asSequence().map { it.waitTime() }.minOrNull() ?: Duration.ZERO
  ...
  companion object {
    ...
    @VisibleForTesting var clock: Clock = Clock.systemDefaultZone()
  }
}
