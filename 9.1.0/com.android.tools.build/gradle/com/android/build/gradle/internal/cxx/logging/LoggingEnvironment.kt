...
/**
 * This file exposes functions for logging where the logger is held in a stack on thread-local storage.
 *
 * Example usage,
 *
 ...
 *       }
 *
 * The purpose is to separate the concerns of other classes and functions from the need to log and warn.
 */

/**
 * Report a bug in Android Gradle Plugin.
 *
 * Unlike [errorln], this function requires a diagnostic code [CxxBugDiagnosticCode] and that diagnostic code is the Issue Tracker bug
 * number.
 */
fun bugln(bugCode: CxxBugDiagnosticCode, format: String, vararg args: Any) =
  ThreadLoggingEnvironment.reportFormattedBugToCurrentLogger(checkedFormat(format, args), bugCode)
...
fun warnln(format: String, vararg args: Any) =
  ThreadLoggingEnvironment.reportFormattedWarningToCurrentLogger(checkedFormat(format, args), null)
...
/** Report a non-error/non-warning message that should be displayed during normal gradle build without --info flag. */
...
fun infoln(format: String, vararg args: Any) = ThreadLoggingEnvironment.reportFormattedInfoToCurrentLogger(checkedFormat(format, args))

/** Log a structured message. The function [message] will only be called if there is a logger that accepts structured messages attached. */
fun logStructured(message: (StringEncoder) -> GeneratedMessageV3) = ThreadLoggingEnvironment.logStructuredMessageToCurrentLogger(message)

/**
 * If caller from Java side misuses %s-style formatting (too many %s for example), the exception from String.format can be concealed by
 * Gradle's logging system. It will appear as "Invalid format (%s)" with no other indication about the source of the problem. This is not
 * necessarily a code bug (for example a user string might be passed in format and that user string has an invalid %)
 */
...
/**
 * Logger base class. When used from Java try-with-resources or Kotlin use() function it will automatically register and deregister with the
 * thread-local stack of loggers.
 */
abstract class ThreadLoggingEnvironment : LoggingEnvironment {
  init {
    ...
    @Suppress("LeakingThis") push(this)
  }
  ...
  companion object {
    /**
     * Singly-linked list where null is used as empty list. The purpose is that Thread Local has zero allocations when there are no loggers
     * so that the class loader that creates the loggers won't leak.
     */
    private data class LoggerStack(val logger: LoggingEnvironment, val next: LoggerStack?)

    /** Stack of logger environments. */
    private val loggerStack: ThreadLocal<LoggerStack?> = ThreadLocal.withInitial { null }

    /**
     * The logger environment to use if there is no other environment. There should always be an intentional logging environment. This
     * logging environment does not register itself on with a thread-local (to avoid leaking class loader). It is stateless the call to
     * close() is a no-op.
     */
    ...
    private fun push(logger: ThreadLoggingEnvironment) = loggerStack.set(LoggerStack(logger, loggerStack.get()))
    ...
    /** Report a bug. */
    fun reportFormattedBugToCurrentLogger(message: String, bugCode: CxxBugDiagnosticCode) = logger.log(bugRecordOf(message, bugCode))
    ...
    /** Report a non-error/non-warning message that should be displayed during normal gradle build without --info flag. */
    fun reportFormattedLifecycleToCurrentLogger(message: String) = logger.log(lifecycleRecordOf(message))

    /** Report diagnostic/informational message. */
    fun reportFormattedInfoToCurrentLogger(message: String) = logger.log(infoRecordOf(message))

    /** Log a structured (ProtoBuf) message */
    fun logStructuredMessageToCurrentLogger(message: (StringEncoder) -> GeneratedMessageV3) = logger.logStructured(message)

    /** Throw an exception if the currently registered logger is the default one. */
    fun requireExplicitLogger() {
      if (logger == BOTTOM_LOGGING_ENVIRONMENT) throw Exception("Non-default logger is required")
    }

    /** Produce an ILogger over the current logger. */
    fun getILogger(warningDiagnosticCode: CxxDiagnosticCode, errorDiagnosticCode: CxxDiagnosticCode) =
      object : ILogger {
