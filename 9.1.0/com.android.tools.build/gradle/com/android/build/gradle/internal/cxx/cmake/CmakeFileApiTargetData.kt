...
/** Closed messages for stream from target-{hash}.json file. This is where source files and flags arrive. */
sealed class TargetDataItem {
   ...
   * "defines" : [{ "define" : "hello_jni_EXPORTS" }], "language" : "C", "sourceIndexes" : [0], "sysroot" : { "path" :
   * "/sdk/ndk/21.3.6528147/toolchains/llvm/prebuilt/darwin-x86_64/sysroot" }
   */
  data class CompileGroups(val compileGroups: List<TargetCompileGroupData>) : TargetDataItem()

  /** "backtrace" : 2, "id" : "foo::@6890427a1f51a3e7e1df" */
  data class Dependencies(val targetId: String, val dependencyIds: List<String>) : TargetDataItem()

  /** "name" : "Source Files", "sourceIndexes" : [0,2,4,6,9,11,13,15,17] */
  data class SourceGroups(val sourceGroups: List<String>) : TargetDataItem()

  /** "build" : "Externals/cpp-optparse", "source" : "Externals/cpp-optparse" */
  data class Paths(val build: String, val source: String) : TargetDataItem()

  /**
   * "backtrace" : 1, "compileGroupIndex" : 0, "path" : "src/main/cxx/hello-jni.c", "sourceGroupIndex" : 0
   *
   * compileGroupIndex can be missing (null). This usually indicates a .h source file.
   */
  data class Source(val compileGroupIndex: Int?, val sourceGroupIndex: Int, val path: String) : TargetDataItem()
  ...
  /**
   * "commandFragments" :
   * [ { "fragment" : "-Wl,--exclude-libs,libgcc.a -Wl,--exclude-libs,libgcc_real.a ...", "role" : "flags" }, { "fragment" : "-llog", "role" : "libraries" }, { "backtrace" : 2, "fragment" : "/root/jetified-curl/prefab/modules/curl/libs/android.x86_64/libcurl.so", "role" : "libraries" } ],
   * "language" : "CXX", "sysroot" : { "path" : "/ndk/21.3.6528147/toolchains/llvm/prebuilt/darwin-x86_64/sysroot }
   */
  data class Link(val targetId: String, val compileCommandFragments: List<CommandFragmentData>, val language: String, val sysroot: String) :
    TargetDataItem()
}
