...
/**
 * A build service that is responsible for configuring the [BuildAnalyzerService] as the build analyzer service requires data that is
 * collected through [org.gradle.api.execution.TaskExecutionGraph.whenReady] which is evaluated after configuration. Once configured, use
 * the [BuildAnalyzerService] directly to add any additional data at execution time.
 *
...
abstract class BuildAnalyzerConfiguratorService : BuildService<BuildServiceParameters.None> {
  ...
  fun initBuildAnalyzerService(
    ...
    project.gradle.taskGraph.whenReady { taskGraph ->
      ...
      val buildscriptDependenciesInfo =
        if (isProjectIsolationActive.not()) {
          // Accessing root project is not supported in project isolation mode
          getBuildscriptDependencies(project.rootProject).map { "${it.group}:${it.module}:${it.version}" }
        } else emptyList()

      parameters.attributionFileLocation.set(attributionFileLocation)
      parameters.tasksSharingOutputs.set(outputFileToTasksMap.filter { it.value.size > 1 })
  ...
  private fun collectAllTaskInfo(
    ...
    taskGraph.allTasks.forEach { task ->
      task.outputs.files.forEach { outputFile ->
        outputFileToTasksMap.computeIfAbsent(outputFile.absolutePath) { ArrayList() }.add(task.path)
      }
      ...
      val taskCategoryInfo =
        if (task::class.java.isAnnotationPresent(BuildAnalyzer::class.java)) {
          ...
          val primaryTaskCategory = taskCategoryConverter.convert(annotation.primaryTaskCategory.toString())!!
          val secondaryTaskCategories = annotation.secondaryTaskCategories.map { taskCategoryConverter.convert(it.toString())!! }
        ...
        } else AndroidGradlePluginAttributionData.TaskCategoryInfo(primaryTaskCategory = TaskCategory.UNCATEGORIZED)

      taskNameToTaskInfoMap[task.name] =
        AndroidGradlePluginAttributionData.TaskInfo(className = getTaskClassName(task.javaClass.name), taskCategoryInfo = taskCategoryInfo)
    }
  }
