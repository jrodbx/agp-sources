...
abstract class ManagedDeviceTestTask : NonIncrementalTask(), AndroidTestTask {
  ...
  @get:Input abstract val testAction: Property<Class<out DeviceTestRunTaskAction<out DeviceTestRunInput>>>

  @get:Nested abstract val deviceInput: Property<DeviceTestRunInput>

  @get:Nested abstract val testData: Property<TestData>

  @get:Input abstract val deviceDslName: Property<String>

  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val setupResult: DirectoryProperty
  ...
  public override fun doTaskAction() {
    checkForNonApks(buddyApks.files) { message: String -> throw InvalidUserDataException(message) }
    ...
    val additionalTestOutputDir =
      if (getAdditionalTestOutputEnabled().get()) {
        getAdditionalTestOutputDir().get().also { FileUtils.cleanOutputDir(it.asFile) }
      } else {
        null
      }

    workerExecutor.noIsolation().submit(ManagedDeviceTestRunnable::class.java) { params ->
      val testRunTaskAction = objectFactory.newInstance(testAction.get()) as DeviceTestRunTaskAction<DeviceTestRunInput>
      ...
      params.testLibraries.setDisallowChanges(gatherTestLibraries(dependencies))
  ...
  abstract class ManagedDeviceTestRunnable : ProfileAwareWorkAction<ManagedDeviceTestRunnableParameters>() {

    val logger: Logger = Logging.getLogger("ManagedDeviceTestRunnable")

    override fun run() {
      val success =
        if (!parameters.testsFound.get()) {
          logger.info("No tests found, nothing to do.")
          true
        } else {
          try {
            val testRunParams =
              object : DeviceTestRunParameters<DeviceTestRunInput> {
                override val deviceInput = parameters.deviceInput.get()
                override val setupResult = parameters.setupResult
                override val testRunData =
                  TestRunData(
      ...
      val report = TestReport(ReportType.SINGLE_FLAVOR, parameters.resultsOutDir.get().asFile, reportOutDir)
  ...
  class CreationAction<DeviceT : Device>(
    ...
    nameSuffix: String = "",
  ) : VariantTaskCreationAction<ManagedDeviceTestTask, InstrumentedTestCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ManagedDeviceTestTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ManagedDeviceTestTask::getCoverageDirectory)
      ...
      if (isAdditionalAndroidTestOutputEnabled) {
        creationConfig.artifacts
          .setInitialProvider(taskProvider, ManagedDeviceTestTask::getAdditionalTestOutputDir)
    ...
    override fun configure(task: ManagedDeviceTestTask) {
      ...
      task.deviceInput.setDisallowChanges(task.objectFactory.newInstance(testRunConfigAction).configureTaskInput(device))
