...
/**
 * Methods in this class convert between decoded and encoded versions of C/C++ build cache protocol buffers. Encoded means strings and lists
 * of strings have been converted to their int counterparts.
 */

/** String-encode an [ExecuteProcess] structure into an [EncodedExecuteProcess] structure. */
...
/** String-decode an [EncodedExecuteProcess] structure from an [ExecuteProcess] structure. */
fun EncodedExecuteProcess.decode(decoder: StringDecoder): ExecuteProcess {
  val result =
    ExecuteProcess.newBuilder()
...
/** Call EncodedExecuteProcess#decode in a pattern that can be used directly with [readStructuredLogs]. */
fun decodeExecuteProcess(encoded: EncodedExecuteProcess, decoder: StringDecoder) = encoded.decode(decoder)
