...
private constructor(
  ...
  private class FeatureInfo(val modulePath: String, val featureName: String, val resOffset: Int, val namespace: String)
  ...
  private constructor(
    ...
    ) : this(minSdkVersion, maxNumberOfSplitsBeforeO, mutableSetOf<FeatureInfo>())

    constructor(
      featureSetMetadata: FeatureSetMetadata
    ) : this(featureSetMetadata.minSdkVersion, featureSetMetadata.maxNumberOfSplitsBeforeO, featureSetMetadata.featureSplits.toMutableSet())

    fun addFeatureSplit(modulePath: String, featureName: String, packageName: String): Int {
      val id: Int =
        if (minSdkVersion < AndroidVersion.VersionCodes.O) {
          if (featureSplits.size >= maxNumberOfSplitsBeforeO) {
            throw RuntimeException("You have reached the maximum number of feature splits : " + maxNumberOfSplitsBeforeO)
          }
          ...
          if (featureSplits.size >= MAX_NUMBER_OF_SPLITS_STARTING_IN_O) {
            throw RuntimeException("You have reached the maximum number of feature splits : " + MAX_NUMBER_OF_SPLITS_STARTING_IN_O)
          }
    ...
    fun save(outputFile: File) {
      JsonWriter(outputFile.bufferedWriter()).use { writer -> FeatureSetMetadataTypeAdapter(outputFile).write(writer, build()) }
    }
  ...
  companion object {
    ...
    @VisibleForTesting const val OUTPUT_FILE_NAME = "feature-metadata.json"

    /** Base module or application module resource ID */
    @VisibleForTesting const val BASE_ID = 0x7F
