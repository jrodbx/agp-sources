...
abstract class BaseTask : DefaultTask(), UsesAnalytics {
  @get:Internal("only for task execution") abstract val projectPath: Property<String>
   ...
   *
   * The task execution will use [GradleBuildProfileSpan.ExecutionType.TASK_EXECUTION_ALL_PHASES] as the span type to record the
   * [AndroidVariantTask.recordedTaskAction].
   */
  protected inline fun recordTaskAction(crossinline block: () -> Unit) {
    Blocks.recordSpan<Exception>(path, GradleBuildProfileSpan.ExecutionType.TASK_EXECUTION_ALL_PHASES, analyticsService.get()) { block() }
  }
