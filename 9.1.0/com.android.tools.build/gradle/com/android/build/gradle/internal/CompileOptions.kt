...
abstract class CompileOptions : CompileOptions {
   ...
   *
   * Note that even if this option is set to `true`, Java compilation may still be non-incremental (e.g., if incremental annotation
   * processing is not yet possible in the project).
   */
  ...
  /**
   * Computes the final sourceCompatibility and targetCompatibility versions based on the following precedence order (first one wins):
  ...
  fun finalizeSourceAndTargetCompatibility(project: Project, globalConfig: GlobalTaskCreationConfig) {
    val toolchainVersion =
      project.extensions.getByType(JavaPluginExtension::class.java).toolchain.languageVersion.let {
