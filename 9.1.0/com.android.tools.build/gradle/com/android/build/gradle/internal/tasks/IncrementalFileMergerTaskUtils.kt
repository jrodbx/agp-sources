...
/**
 * Creates an [IncrementalFileMergerInput] from a file (either a directory or jar file). All children of the file will be reported in the
 * incremental input.
 *
...
/**
 * Creates an [IncrementalFileMergerInput] from a [File]. This method assumes the input does not contain incremental information. All files
 * will be reported as new.
 *
 * @param input the file input
 * @param name the display name for the file in case of errors
 * @param zipCache the zip cache; the cache will not be modified
 * @param cacheUpdates will receive actions to update the cache for the next iteration, if file.isFile is true, in which case it's assumed
 *   to be a jar file.
  ...
  if (jar in changedInputs) {
    ...
    try {
      return when (fileStatus) {
        ...
        FileStatus.REMOVED -> {
          val cached =
            zipCache.getLastSnapshotOfZip(jar) ?: throw RuntimeException("File '$jar' was deleted, but previous version not found in cache")
 ...
 */
private fun computeUpdatesFromDir(dir: File, changedInputs: Map<File, FileStatus>): ImmutableMap<RelativeFile, FileStatus> {
...
internal data class InputData(val file: File, val priority: JavaResMergingPriority, val source: String)
 ...
 * @param changes map of files to file status, passed from the incremental task, or null if the task is not incremental
 * @param zipCache the zip cache; the cache will not be modified
 * @param cacheUpdates receives updates to the cache
 * @param full is this a full build? If not, then it is an incremental build; in full builds the output is not cleaned, it is the
 *   responsibility of the caller to ensure the output is properly set up; `full` cannot be `false` if changedInputs is null
 */
internal fun toInputs(
  ...
  for (input in inputs) {
    val fileMergerInput: IncrementalFileMergerInput? =
      if (full) {
    ...
    fileMergerInput?.let { builder.put(it, input.priority) }
  }
 ...
 *
 * When inputs are reordered with @Classpath, that leads to sequential 'add' and 'remove' change being reported. These can be re-combined in
 * to changes as the cache is keyed by file path. See JavaResPackagingTest.testAppProjectWithReorderedDeps
 *
 * TODO(b/225872980): Unify with IncrementalChanges.classpathToRelativeFileSet Currently the IncrementalFileMerger goes through each input
 *   and attaches the changes to each input whereas with the new API, that is not necessary as the changes are already associated with each
 *   input.
 */
