...
/**
 * Makes [destination] content the same as [source] either by hard linking or by physically copying the file when hard linking isn't
 * available.
 */
fun hardLinkOrCopyFile(source: File, destination: File) {
  if (!source.isFile) error("Could not hard link or copy '$source' because it did not exist.")
  synchronizeFile(source, destination)
}

/**
 * Makes [destination] content the same as [source]. If [source] exists, it will attempt to hard link to [destination]. If hard linking
 * doesn't exist on this platform then it does a copy instead. If [source] doesn't exist, it will delete destination.
 */
fun synchronizeFile(source: File, destination: File, @VisibleForTesting createLink: (Path, Path) -> Unit = ::realCreateLink) {
...
/** Returns true if the two files are the same file (including through hard links) or if they have the same content. */
fun isSameFileOrContent(source: File, destination: File) = compareFileContents(source, destination).areSameFileOrContent

/** Returns true if the two files are the same file (including through hard links) or if they have the same content. */
@VisibleForTesting
fun compareFileContents(source: File, destination: File, compareBufferSize: Int = 8192): SynchronizeFile.Comparison {
  when {
...
/** Write a file only if that file's contents is different. Return true iff the content of the file changed. */
...
/** Create a finger print record for this file. It contains the size, last modified stamp, and whether the file exist */
val File.fingerPrint: FileFingerPrint
  get() {
