...
class ResValueGenerator(genFolder: File, private val requests: Map<ResValue.Key, ResValue>) {
  ...
  fun generate() {
    ...
    for ((key, value) in requests) {
      ...
      val hasResourceTag = resourceType != null && RESOURCES_WITH_TAGS.contains(resourceType)
      val itemNode: Node = document.createElement(if (hasResourceTag) type else SdkConstants.TAG_ITEM)
      ...
      if (!hasResourceTag) {
        val typeAttr = document.createAttribute(SdkConstants.ATTR_TYPE)
        typeAttr.value = type
    ...
    try {
      ...
      ?.also { Files.asCharSink(resFile, Charsets.UTF_8).write(it) }
  }
