...
fun createSettingsFromJsonString(
  ...
  val settings =
    try {
      Gson().getAdapter(object : TypeToken<Settings>() {}).read(reader)
    } catch (e: Throwable) {
  ...
  return Settings(
    ...
    configurations =
      settings.configurations.filterNotNull().map { configuration ->
        configuration.copy(variables = configuration.variables.filterNotNull())
      },
  )
}

/** Given a file with json construct [Settings]. */
fun createSettingsFromJsonFile(json: File, providers: ProviderFactory, layout: ProjectLayout): Settings {
  PassThroughPrefixingLoggingEnvironment(file = json).use {
    val jsonContent = providers.fileContents(layout.file(providers.provider { json })).asText.get()
...
fun Settings.toJsonString(): String {
  return StringWriter().also { writer -> GsonBuilder().setPrettyPrinting().create().toJson(this, writer) }.toString()
}
