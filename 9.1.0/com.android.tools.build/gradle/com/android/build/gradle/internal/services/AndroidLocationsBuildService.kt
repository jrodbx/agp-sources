...
/**
 * A build service around [AndroidLocations] in order to make this basically a singleton while not using static fields.
 *
 * Using static fields (like we used to do) means that the cached value is tied to the loaded class and can survive across different builds,
 * even if the computed value would have changed (due to injected environment value for instance), as long as the gradle daemon is re-used.
 */
abstract class AndroidLocationsBuildService @Inject constructor(providerFactory: ProviderFactory) :
  ...
  /** Use [ConfigPhaseFileCreator] to create android folder during configuration phase to avoid configuration cache miss. */
  private val androidLocations =
    AndroidLocations(
      ...
      .also { androidLocations ->
        providerFactory
          .of(AndroidDirectoryCreator::class.java) {
            it.parameters.androidDir.set(File(androidLocations.computeAndroidFolder().absolutePathString()))
          }
          .get()
      }

  abstract class AndroidDirectoryCreator : ConfigPhaseFileCreator<String, AndroidDirectoryCreator.Params> {
  ...
  private class AndroidLocations(environmentProvider: EnvironmentProviderImpl, logger: ILogger) :
