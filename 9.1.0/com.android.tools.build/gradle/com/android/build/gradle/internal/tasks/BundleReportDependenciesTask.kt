...
abstract class BundleReportDependenciesTask : NonIncrementalTask() {
  ...
  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val baseDeps: RegularFileProperty
  ...
  @get:OutputFile abstract val dependenciesList: RegularFileProperty

  public override fun doTaskAction() {

    val baseAppDeps = BufferedInputStream(FileInputStream(baseDeps.get().asFile)).use { AppDependencies.parseFrom(it) }

    val featureAppDeps = LinkedList<AppDependencies>()

    featureDeps.files.forEach { file -> file.inputStream().buffered().use { featureAppDeps.add(AppDependencies.parseFrom(it)) } }
    ...
    for (featureAppDep in featureAppDeps) {
      ...
      featureAppDep.repositoriesList.forEachIndexed { origIndex, repo ->
        val newIndex =
          repoToIndexMap.computeIfAbsent(repo) {
            repoList += it
            repoList.size - 1
          }
      ...
      for ((origIndex, lib) in featureLibraryDeps.withIndex()) {
        ...
        if (newIndex == null) {
          ...
          if (lib.hasRepoIndex())
            libraryList.add(
              lib.toBuilder().also { libCopy -> libCopy.repoIndexBuilder.value = repoIndexDict[lib.repoIndex.value]!! }.build()
            )
          else libraryList.add(lib)
        }
      ...
      for (libraryDep in featureAppDep.libraryDependenciesList) {
        val transformedDepBuilder = LibraryDependencies.newBuilder().setLibraryIndex(libIndexDict[libraryDep.libraryIndex]!!.toInt())
        for (depIndex in libraryDep.libraryDepIndexList) {
          transformedDepBuilder.addLibraryDepIndex(libIndexDict[depIndex]!!.toInt())
        }
      ...
      for (moduleDep in featureAppDep.moduleDependenciesList) {
        val moduleDepBuilder = ModuleDependencies.newBuilder().setModuleName(moduleDep.moduleName)
        for (depIndex in moduleDep.dependencyIndexList) {
          moduleDepBuilder.addDependencyIndex(libIndexDict[depIndex]!!.toInt())
        }
    ...
    FileOutputStream(dependenciesList.get().asFile).use { appDeps.writeTo(it) }
  }

  class CreationAction(creationConfig: VariantCreationConfig) :
    VariantTaskCreationAction<BundleReportDependenciesTask, VariantCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<BundleReportDependenciesTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, BundleReportDependenciesTask::dependenciesList)
        .withName("dependencies.pb")
        .on(InternalArtifactType.BUNDLE_DEPENDENCY_REPORT)
    }

    override fun configure(task: BundleReportDependenciesTask) {
      super.configure(task)
      creationConfig.artifacts.setTaskInputToFinalProduct(SingleArtifact.METADATA_LIBRARY_DEPENDENCIES_REPORT, task.baseDeps)
      task.featureDeps =
