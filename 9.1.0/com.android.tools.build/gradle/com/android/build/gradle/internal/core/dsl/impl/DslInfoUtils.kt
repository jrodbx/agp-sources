...
private const val DEPRECATED_MULTIDEX_TEST_RUNNER = "com.android.test.runner.MultiDexTestRunner"
private const val ANDROIDX_TEST_RUNNER = "androidx.test.runner.AndroidJUnitRunner"

internal fun computeInstrumentationTestRunner(
  manifestData: Provider<ManifestData>,
...
internal fun getInstrumentationRunner(
  ...
  val fromFlavor =
    productFlavorList.asSequence().map { it.testInstrumentationRunner }.firstOrNull { it != null } ?: baseFlavor.testInstrumentationRunner
...
/**
 * Returns the full, unique name of the variant in camel case (starting with a lower case), including BuildType, Flavors and Test (if
 * applicable).
 *
 * This is to be used for the normal variant name. In case of Feature plugin, the library side will be called the same as for library
 * plugins, while the feature side will add 'feature' to the name.
 *
...
fun computeName(
  ...
  val flavorName =
    if (dimensionCombination.productFlavors.isEmpty()) {
      ""
    } else {
...
fun computeSourceSetName(baseName: String, componentType: ComponentType): String {
  var name = baseName
 ...
 *
 * It is assumed that merged option type with a method to reset and append is created for the option being merged.
 *
 * The order of priority is BuildType, ProductFlavors, and default config. ProductFlavor added earlier has higher priority than
 * ProductFlavor added later.
 *
 ...
 * @param getBuildTypeOption A Function to return the option from a BuildType. takes priority and overwrite option in the first input
 *   argument.
 * @param <CoreOptionsT> The core type of the option being merge.
 * @param <MergedOptionsT> The merge option type. </MergedOptionsT></CoreOptionsT>
 */
internal fun <CoreOptionsT, MergedOptionsT : MergedOptions<CoreOptionsT>> computeMergedOptions(
  ...
  computeMergedOptions(defaultConfig, buildTypeObj, productFlavorList, getFlavorOption, getBuildTypeOption) { mergedOption.append(it) }
}
...
internal fun <CoreOptionsT> computeMergedOptions(
  ...
  defaultConfig.getFlavorOption()?.let { mergeAction(it) }
  // reverse loop for proper order
  for (i in productFlavorList.indices.reversed()) {
    productFlavorList[i].getFlavorOption()?.let { mergeAction(it) }
  }
  buildTypeObj.getBuildTypeOption()?.let { mergeAction(it) }
}
