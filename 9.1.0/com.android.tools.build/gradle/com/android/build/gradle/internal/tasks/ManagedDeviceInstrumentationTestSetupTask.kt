 ...
 *
 * Expands the dsl from a [ManagedVirtualDevice] definition in [TestOptions.devices] to a functional Android Virtual Device to be used with
 * the emulator. This includes the downloading of required system images, configuring the AVD, and creating an AVD snapshot.
 *
 * This task is required as a dependency for all Unified Testing Platform Tasks that require this device.
 */
@DisableCachingByDefault
@BuildAnalyzer(primaryTaskCategory = TaskCategory.TEST)
abstract class ManagedDeviceInstrumentationTestSetupTask : NonIncrementalGlobalTask() {

  @get:Internal abstract val sdkService: Property<SdkComponentsBuildService>
  ...
  @get:Input @get:Optional abstract val testedAbi: Property<String>

  @get:Input abstract val sdkVersion: Property<Int>

  @get:Input abstract val sdkMinorVersion: Property<Int>

  @get:Input abstract val systemImageVendor: Property<String>

  @get:Input @get:Optional abstract val sdkExtensionVersion: Property<Int>

  @get:Input abstract val pageAlignmentSuffix: Property<String>

  @get:Input abstract val hardwareProfile: Property<String>

  // Used in error messaging
  @get:Internal abstract val managedDeviceName: Property<String>

  // Used in error messaging.
  @get:Internal abstract val require64Bit: Property<Boolean>
  ...
  private fun assertTestedAbiCompatible() {
    val translationEnabled = canSourcePerformNdkTranslation(systemImageVendor.get(), sdkVersion.get())
    ...
    when (abi.get()) {
      ABI_ARM ->
        when (testedAbi.orNull) {
          ...
          ABI_X86,
          ABI_X86_64 ->
            error(generateTestedAbiNotCompatibleError(deviceName = managedDeviceName.get(), testedAbi = testedAbi.orNull, abi = abi.get()))
          else -> error(generateUnrecognizedTestedAbiError(deviceName = managedDeviceName.get(), testedAbi = testedAbi.orNull))
        }
      ABI_X86 ->
        when (testedAbi.orNull) {
          ABI_X86 -> return
          ABI_ARM -> error(generateArmTranslationOnX86Error(deviceName = managedDeviceName.get(), abi = abi.get()))
          ABI_X86_64 ->
            error(generateTestedAbiNotCompatibleError(deviceName = managedDeviceName.get(), testedAbi = testedAbi.orNull, abi = abi.get()))
          null -> {
            logger.warn(generateUnspecifiedAbiWarningWithNdkTranslationUnsupported(deviceName = managedDeviceName.get(), abi = abi.get()))
            return
          }
          else -> error(generateUnrecognizedTestedAbiError(deviceName = managedDeviceName.get(), testedAbi = testedAbi.orNull))
        }
        ...
        when (testedAbi.orNull) {
          ...
          null -> {
            if (!translationEnabled) {
              ...
              logger.warn(generateUnspecifiedAbiWarningWithNdkTranslation(deviceName = managedDeviceName.get(), abi = abi.get()))
            }
            return
          }
          else -> error(generateUnrecognizedTestedAbiError(deviceName = managedDeviceName.get(), testedAbi = testedAbi.orNull))
  ...
  abstract class ManagedDeviceSetupRunnable : ProfileAwareWorkAction<ManagedDeviceSetupParams>() {
    override fun run() {
      val versionedSdkLoader =
        parameters.sdkService
          .get()
          .sdkLoader(compileSdkVersion = parameters.compileSdkVersion, buildToolsRevision = parameters.buildToolsRevision)
      ...
      if (!sdkImageProvider.isPresent) {
        error(
          generateSystemImageErrorMessage(
      ...
      parameters.avdService
        .get()
        .avdProvider(sdkImageProvider, imageHash, parameters.deviceName.get(), parameters.hardwareProfile.get())
        .get()

      parameters.avdService.get().ensureLoadableSnapshot(parameters.deviceName.get())
    }
  ...
  class CreationAction(
    ...
    override fun configure(task: ManagedDeviceInstrumentationTestSetupTask) {
      ...
      task.sdkService.setDisallowChanges(getBuildService(creationConfig.services.buildServiceRegistry))
  ...
  companion object {
    ...
    private fun generateTestedAbiNotCompatibleError(deviceName: String, testedAbi: String?, abi: String) =
      """
    ...
    private fun generateUnspecifiedAbiWarningWithNdkTranslationUnsupported(deviceName: String, abi: String) =
      """
    ...
    // TODO(b/477334887): Implement this behavior change in AGP 10.0.
    private fun generateUnspecifiedAbiWarningWithNdkTranslation(deviceName: String, abi: String) =
      """
                The device "$deviceName" does not specify a "testedAbi".
                This currently defaults to "$abi", but will change to "$ABI_ARM" in AGP 10.0.
