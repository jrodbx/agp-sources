...
/**
 * Convert from compile_commands.json [json] to compile_commands.json.bin [bin]. Only convert if [bin] is out-of-date with respect to
 * [json].
 *
 * This function only works for CMake-generated compile_commands.json because it uses the .o output path to figure out the corresponding
 * build target name. See [extractCMakeTargetFromObjectFile] for details on how that is done.
 */
fun convertCMakeToCompileCommandsBin(
  ...
  JsonReader(json.reader(StandardCharsets.UTF_8)).use { reader ->
    CompileCommandsEncoder(bin).use { encoder ->
      val tokenMap =
        TokenizedCommandLineMap<Triple<String, List<String>, String?>>(raw = false, platform = platform) { tokens, sourceFile ->
      ...
      while (reader.hasNext()) {
        ...
        reader.endObject()
        val (compiler, flags, output) =
          tokenMap.computeIfAbsent(command, sourceFile) {
        ...
        if (output == null) {
          errorln(COULD_NOT_EXTRACT_OUTPUT_FILE_FROM_CLANG_COMMAND, "Expected to find output file in command-line: ${flags.joinToString()}")
          return
        }
...
/**
 * This function extracts a build target from a CMake object file path.
 *
 * CMake .o files are stored in a directory structure like:
 *
 * BasePath/CMakeFiles/hello-jni.dir/src/main/cxx/hello-jni.c.o
 *
 * Where "hello-jni" from "hello-jni.dir" is the associated build target.
 */
private fun extractCMakeTargetFromObjectFile(objectFile: File): String {
  ...
  errorln(OBJECT_FILE_CANT_BE_CONVERTED_TO_TARGET_NAME, "Could not determine target from [$objectFile]")
