...
fun createCxxAbiModel(
  ...
  if (!variant.module.ndkMetaAbiList.map { it.name }.contains(abiName)) {
    errorln(
      ABI_IS_UNSUPPORTED,
      "ABI $abiName was not recognized. Valid ABIs are: " +
  ...
  with(variant) {
    val variantSoFolder = join(module.intermediatesBaseFolder, module.buildSystemTag, variantName, module.intermediatesParentDirSuffix)
    val variantCxxBuildFolder = join(module.cxxFolder, module.buildSystemTag, variantName)
    val variantIntermediatesFolder = join(module.intermediatesFolder, module.buildSystemTag, variantName)
    return CxxAbiModel(
      variant = this,
      info = info,
      ...
      cmake =
        when (module.buildSystem) {
          NativeBuildSystem.CMAKE -> CxxCmakeAbiModel(buildCommandArgs = null, effectiveConfiguration = SettingsConfiguration())
          else -> null
        },
      ...
      stlLibraryFile =
        Stl.fromArgumentName(variant.stlType)?.let { module.stlSharedObjectMap[it]?.get(info.name)?.toString() }?.let { File(it) },
