...
abstract class LinkAndroidResForBundleTask : NonIncrementalTask() {
  ...
  @get:OutputFile abstract val linkedResourcesOutputFile: RegularFileProperty
  ...
  @get:Input @get:Optional abstract val resOffset: Property<Int>

  @get:OutputDirectory
  lateinit var incrementalFolder: File
    private set

  @get:Input abstract val excludeResSourcesForReleaseBundles: Property<Boolean>
  ...
  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val stableIdsFile: RegularFileProperty
  ...
  override fun doTaskAction() {
    ...
    val compiledDependenciesResourcesDirs = getCompiledDependenciesResources()?.reversed()?.toImmutableList() ?: emptyList<File>()

    val identifiedSourceSetMap = mergeIdentifiedSourceSetFiles(sourceSetMaps.files.toList())
      ...
      AaptPackageConfig(
        ...
        resourceDirs =
          ImmutableList.Builder<File>()
            .addAll(compiledDependenciesResourcesDirs)
            .add(checkNotNull(getCompiledNavigationResources().orNull?.asFile))
            .add(checkNotNull(getInputResourcesDir().orNull?.asFile))
            .build(),
  ...
  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val manifestFile: RegularFileProperty

  @InputFiles @Optional @PathSensitive(PathSensitivity.RELATIVE) abstract fun getInputResourcesDir(): DirectoryProperty

  @get:Input
  lateinit var resConfig: Collection<String>
    private set

  @get:Input abstract val localeFilters: SetProperty<String>

  @get:Input abstract val noCompress: ListProperty<String>

  @get:Input abstract val aaptAdditionalParameters: ListProperty<String>

  @get:Input abstract val pseudoLocalesEnabled: Property<Boolean>

  /** Returns a file collection of the directories containing the compiled remote libraries resource files. */
  @Optional
  @InputFiles
  ...
  @Optional @InputFiles @PathSensitive(PathSensitivity.RELATIVE) abstract fun getCompiledNavigationResources(): DirectoryProperty
  ...
  class CreationAction(apkCreationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<LinkAndroidResForBundleTask, ApkCreationConfig>(apkCreationConfig),
    AndroidResourcesTaskCreationAction by AndroidResourcesTaskCreationActionImpl(apkCreationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<LinkAndroidResForBundleTask>) {
      super.handleProvider(taskProvider)

      val artifactType = InternalArtifactType.LINKED_RESOURCES_FOR_BUNDLE_PROTO_FORMAT
      val fileName =
        when {
          creationConfig.componentType.isBaseModule -> artifactType.name().lowercase().replace("_", "-") + SdkConstants.DOT_RES
    ...
    override fun configure(task: LinkAndroidResForBundleTask) {
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.BUNDLE_MANIFEST, task.manifestFile)

      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.MERGED_RES, task.getInputResourcesDir())
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.STABLE_RESOURCE_IDS_FILE, task.stableIdsFile)
      ...
      task.aaptAdditionalParameters.setDisallowChanges(creationConfig.androidResources.aaptAdditionalParameters)

      task.excludeResSourcesForReleaseBundles.setDisallowChanges(
      ...
      val resSourceSetMap = creationConfig.artifacts.get(InternalArtifactType.ANDROID_RES_SOURCE_SET_PATH_MAP)
