...
/**
 * Task to check whether Jetifier is needed for the current project. This is done by checking whether the dependency graphs contain any
 * legacy support libraries.
 *
 * Note: This task is currently not compatible with configuration caching as it requires resolving dependency graphs and as of version 7.1
 * Gradle does not yet support that use case (https://github.com/gradle/gradle/issues/12871).
 */
...
abstract class CheckJetifierTask : NonIncrementalGlobalTask() {
  ...
  @get:Internal abstract val checkJetifierBuildService: Property<CheckJetifierBuildService>

  /**
   * Because of b/204421586, we need to resolve AGP's configurations first. This is required as AGP tries to sync versions between
   * configurations in [ConstraintHandler]. If an external configuration extends both main and test configurations, those two may be
   * resolved w/o [ConstraintHandler] running. Following that, when we resolve test configuration on its own, [ConstraintHandler] will run
   * and it may try to add dependencies to it, which is not legal and Gradle will fail the build.
   */
  @get:Internal abstract val configurationToResolveFirst: ListProperty<String>

  @get:Nested abstract val resolutionUnit: ListProperty<ResolutionUnit>
  ...
  class CreationAction(
    ...
    private val configurationsToResolveFirst =
      (variants.map { it.variant.variantDependencies } +
          ...
          testFixturesComponents.map { it.variantDependencies })
        .flatMap { listOf(it.compileClasspath.name, it.runtimeClasspath.name) }
  ...
  data class ResolutionUnit(
    @get:Internal val configurationName: String,
    @get:Internal val rootComponent: Provider<ResolvedComponentResult>,
  ) : Serializable
  ...
  private fun detect(): CheckJetifierProjectResult {
    ...
    val (handleFirst, theRest) = resolutionUnit.get().partition { resolveFirstNames.contains(it.configurationName) }
...
private class ConfigurationAnalyzer(private val rootComponent: ResolvedComponentResult) {
  ...
  /**
   * Finds direct dependencies of the given configuration that directly/transitively depend on support libraries, or are support libraries
   * themselves
   *
   * @return A list of [DependencyPath] from a direct dependency to a support library. There may be multiple paths from the direct
   *   dependency to a support library, but this method retains only one path.
   */
  fun findPathsToSupportLibs(): List<DependencyPath> {
    val directDependencies =
      rootComponent.dependencies.filter { dependency ->
        ...
        return@filter (dependency is ResolvedDependencyResult) && (dependency.selected.id is ModuleComponentIdentifier)
      }
  ...
  /**
   * Computes a [DependencyPath] from the given dependency to a support library. There may be multiple paths, but this method retains only
   * one path.
   *
  ...
  private fun computePathToSupportLib(dependency: DependencyResult): DependencyPath? {
    ...
    if (
      AndroidXDependencySubstitution.isLegacySupportLibDependency(dependency.requested.displayName)
      ...
      for (childDependency in dependency.selected.dependencies) {
        ...
        if (childDependencyPath != null) {
          dependencyPath = DependencyPath(listOf(dependency.selected.id.displayName) + childDependencyPath.elements)
