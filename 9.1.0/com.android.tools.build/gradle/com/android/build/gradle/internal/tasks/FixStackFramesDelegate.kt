...
/**
 * When running Desugar, unit tests on instrumented classes or packaging instrumented classes, we need to make sure stack frames information
 * is valid in the class files. This is due to fact that classes may be loaded in the JVM, and if stack frame information is invalid for
 * bytecode 1.7 and above, [VerifyError] is thrown. Also, if stack frames are broken, ASM might be unable to read those classes.
 *
 * This delegate uses ASM to recalculate the stack frames information. In order to obtain new stack frames, types need to be resolved, which
 * is done using [ClassesHierarchyResolver].
 */
class FixStackFramesDelegate(
  ...
  companion object {
    ...
    fun transformJar(inputJar: File, outputJar: File, classesHierarchyResolver: ClassesHierarchyResolver) {
      ZipFile(inputJar).use { inputZip ->
        ZipOutputStream(outputJar.outputStream().buffered()).use { outputZip ->
          ...
          while (inEntries.hasMoreElements()) {
            ...
            val newEntryContent =
              inputZip.getInputStream(entry).buffered().use {
    ...
    private fun getFixedClass(originalFile: InputStream, classesHierarchyResolver: ClassesHierarchyResolver): ByteArray {
      val bytes = ByteStreams.toByteArray(originalFile)
      return try {
        val classReader = ClassReader(bytes)
        val classWriter = FixFramesClassWriter(ClassWriter.COMPUTE_FRAMES, classesHierarchyResolver)
  ...
  private fun processJars(changedInput: Map<File, ChangeType>) {
    changedInput.forEach { (inputJar, changeType) ->
      ...
      if (changeType == ChangeType.ADDED || changeType == ChangeType.MODIFIED) {
        workers.noIsolation().submit(FixJarStackFramesRunnable::class.java) { params ->
          ...
          params.classesHierarchyBuildService.set(classesHierarchyBuildServiceProvider)
          val jars = jarsDir?.listFiles()?.toSet() ?: emptySet()

          params.classpath.set(listOf(bootClasspath, jars, referencedClasses).flatten())
        }
      }
    }
  }

  private fun processClasses(changedInput: Map<File, ChangeType>) {
    changedInput
      .filterValues { it == ChangeType.REMOVED }
      .forEach { (inputFile, _) -> FileUtils.deleteIfExists(classesOutDir.resolve(inputFile.relativeTo(classesDir))) }

    val filesToProcess = changedInput.filterValues { it == ChangeType.ADDED || it == ChangeType.MODIFIED }.keys

    if (filesToProcess.isNotEmpty()) {
      workers.noIsolation().submit(FixClassesStackFramesRunnable::class.java) { params ->
        params.initializeFromBaseTask(task)
        params.inputDir.set(classesDir)
        params.inputFiles.set(filesToProcess)
        params.outputDir.set(classesOutDir)
        params.classesHierarchyBuildService.set(classesHierarchyBuildServiceProvider)
        params.classpath.set(listOf(bootClasspath, listOf(classesDir), referencedClasses).flatten())
      }
    }
  }
  ...
  fun doFullRun() {
    ...
    jarsDir?.listFiles()?.associate { it to ChangeType.ADDED }?.let { processJars(it) }
    processClasses(FileUtils.getAllFiles(classesDir).toMap { ChangeType.ADDED })
  }

  fun doIncrementalRun(jarChanges: Iterable<FileChange>, classesChanges: Iterable<FileChange>) {
  ...
  abstract class FixClassesStackFramesRunnable : ProfileAwareWorkAction<ClassesParams>() {
    ...
    override fun run() {
      ...
      parameters.inputFiles.get().forEach { inputFile ->
        val outputFile = parameters.outputDir.get().resolve(inputFile.relativeTo(parameters.inputDir.get()))
