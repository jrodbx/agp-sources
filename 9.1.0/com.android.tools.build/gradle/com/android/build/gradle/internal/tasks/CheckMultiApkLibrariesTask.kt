...
abstract class CheckMultiApkLibrariesTask : NonIncrementalTask() {
  ...
  override fun doTaskAction() {
    ...
    for (artifact in featureTransitiveDeps) {
      ...
      if (artifact.id.componentIdentifier !is ProjectComponentIdentifier) {
        throw GradleException(artifact.id.componentIdentifier.displayName + " is not a Gradle project.")
      }
  ...
  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<CheckMultiApkLibrariesTask, ComponentCreationConfig>(creationConfig, dependsOnPreBuildTask = false) {
    ...
    override fun configure(task: CheckMultiApkLibrariesTask) {
      ...
      task.fakeOutputDir =
        creationConfig.services.projectInfo.intermediatesDirectory.map { it.dir("check-libraries").dir(creationConfig.dirName) }
    }
  }

  private fun checkAndUpdateLibraryMap(file: File, projectPath: String, map: MutableMap<String, MutableList<String>>): Boolean {
    var found = false
