 ...
 */
class ClassesHierarchyResolver(classesDataCache: ClassesDataCache, sources: Map<File, ClassesDataSourceCache.SourceType>) {
  ...
  private fun maybeLoadClassData(className: String): ClassesDataSourceCache.ClassData? {
    ...
    classesDataCaches.forEach {
      it.getClassDataIfLoaded(className)?.let {
        return it
      }
    }
    ...
    classesDataCaches.forEach {
      it.maybeLoadClassData(className)?.let {
        return it
      }
    }
  ...
  /**
   * Loads the class data for a class requested by a class visitor. If the requested class is a project class, the request is saved as it
   * will be saved later as a part of the incremental state.
   *
  ...
  fun loadClassDataForVisitor(className: String): ClassData? {
    return maybeLoadClassDataForClass(className)?.also {
      classesDataCaches
        .filter { it.sourceType == ClassesDataSourceCache.SourceType.PROJECT }
  ...
  fun getAnnotations(className: String): List<String> {
    return maybeLoadClassData(className)?.annotations?.map { it.replace('/', '.') } ?: emptyList()
  }
