...
/**
 * Merges BR and Adapter artifacts from dependencies and serves it back to the annotation processor. <b> To account for V1 dependencies, we
 * still copy their layout-info files from the compile classpath.
 *
 * Caching disabled by default for this task because the task does very little work. Some Input files are copied to the OutputDirectory and
 * no computation is required. Calculating cache hit/miss and fetching results is likely more expensive than simply executing the task.
 */
...
abstract class DataBindingMergeDependencyArtifactsTask : NonIncrementalTask() {
  /**
   * Classes available at Runtime. We extract BR files from there so that even if there is no compile time dependency on a particular
   * artifact, we can still generate the BR file for it.
   */
  @get:InputFiles
  ...
  /**
   * Classes that are available at Compile time. We use setter-store files in there so that code access between dependencies is scoped to
  ...
  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<DataBindingMergeDependencyArtifactsTask, ComponentCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<DataBindingMergeDependencyArtifactsTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, DataBindingMergeDependencyArtifactsTask::outFolder)
  ...
  override fun run() {
    parameters.run {
      ...
      compileTimeDependencies
        .get()
        .filter { file -> DataBindingBuilder.RESOURCE_FILE_EXTENSIONS.any { ext -> file.name.endsWith(ext) } }
