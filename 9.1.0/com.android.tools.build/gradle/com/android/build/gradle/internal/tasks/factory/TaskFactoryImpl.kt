...
class TaskFactoryImpl(private val taskContainer: TaskContainer) : TaskFactory {
  ...
  ): TaskProvider<T> = taskContainer.registerTask(creationAction, secondaryPreConfigAction, secondaryAction, secondaryProviderCallback)
  ...
  ): TaskProvider<Task> = taskContainer.registerTask(taskName, Task::class.java, preConfigAction, action, providerCallback)

  override fun <T : Task> register(taskName: String, taskType: Class<T>, action: Action<in T>): TaskProvider<T> {
    return taskContainer.register(taskName, taskType, action)
  }

  override fun register(taskName: String, action: Action<in Task>): TaskProvider<Task> {
