...
fun generateChromeTrace(file: File, outputFile: File = File(file.parentFile, file.nameWithoutExtension + ".json.gz")) {
  val allAttributions =
    try {
      readZipContent(file)
    } catch (e: Throwable) {
      warnln("Cannot parse native build attribution zip file. " + "Exception: ${Throwables.getStackTraceAsString(e)}")
      return
    }
  generateChromeTrace(outputFile, allAttributions)
}

fun generateChromeTrace(allTasks: BuildTaskAttributions, abiModel: CxxAbiModel, buildStartTime: Long, extraChromeTraceDir: File) {
  val attributionKey = abiModel.createAttributionKey()
  extraChromeTraceDir.mkdirs()
  generateChromeTrace(
    extraChromeTraceDir.resolve("external_native_build-$buildStartTime-${attributionKey.filename}.json.gz"),
    mapOf(attributionKey to allTasks),
  )
}

fun generateChromeTrace(outputFile: File, allAttributions: Map<AttributionKey, BuildTaskAttributions>) = run {
  ...
  allAttributions.entries
    ...
    .forEachIndexed { pid, (key, tasksForAnAbi) ->
      ...
      squashTasks(tasksForAnAbi).forEachIndexed { tid, tasks ->
        ...
        tasks.attributionList.forEach { task -> allEvents.addTaskEvent(pid, tid, tasksForAnAbi.buildStartTimeMs, task) }
      }
    }
        ...
        BuildTaskAttributions.newBuilder().apply {
          InputStreamReader(zipFile.getInputStream(entry), StandardCharsets.UTF_8).use { inputStreamReader ->
            for (line in inputStreamReader.readLines()) {
              if (line.startsWith('#')) {
                buildStartTimeMs = line.dropWhile { c -> !Character.isDigit(c) }.takeWhile { c -> Character.isDigit(c) }.toLong()
              } else {
                collectTask(line)
              }
            }
...
private fun BuildTaskAttributions.Builder.collectTask(line: String) {
  ...
  addAttribution(
    BuildTaskAttribution.newBuilder().setStartTimeOffsetMs(start.toInt()).setEndTimeOffsetMs(end.toInt()).setOutputFile(output).build()
  )
}

/** Create [BuildTaskAttributions] from a .ninja_log file starting at [linesToSkip]. */
fun generateNinjaSourceFileAttribution(abi: CxxAbiModel, linesToSkip: Int, buildStartTime: Long): BuildTaskAttributions {
  val allTasks =
...
private fun MutableList<TraceEventJson>.addProcessNameMetaEvent(pid: Int, key: AttributionKey) {
  add(TraceEventJson(pid, 0, 0, "M", name = "process_name", args = mapOf("name" to key.describe)))
}
...
private fun MutableList<TraceEventJson>.addThreadNameMetaEvent(pid: Int, tid: Int) {
  add(TraceEventJson(pid, tid, 0, "M", name = "thread_name", args = mapOf("name" to "ninja->clang $tid")))
}
 ...
 */
private fun MutableList<TraceEventJson>.addTaskEvent(pid: Int, tid: Int, buildStartTimeMs: Long, task: BuildTaskAttribution) {
  add(
    TraceEventJson(
      pid,
      tid,
...
/**
 * Tries to fit tasks into some number of tracks compactly. The idea is to present a view of how tasks are executed concurrently on a
 * multi-core machine. There is no way to reconstruct which CPU thread did what since that information is lost.
 */
private fun squashTasks(tasks: BuildTaskAttributions): List<BuildTaskAttributions> {
  ...
  return result.map { BuildTaskAttributions.newBuilder().addAllAttribution(it).build() }
}
...
private fun createAttributionKey(module: String, variant: String, abi: String): AttributionKey {
  return AttributionKey.newBuilder().setModule(module).setVariant(variant).setAbi(abi).build()
}

private fun CxxAbiModel.createAttributionKey(): AttributionKey {
  return AttributionKey.newBuilder().setModule(variant.module.gradleModulePathName).setVariant(variant.variantName).setAbi(name).build()
}
...
val BuildTaskAttribution.type: OperationType
  get() = if (outputFile.endsWith(".o")) OperationType.COMPILE else OperationType.LINK

val BuildTaskAttribution.name: String
  get() = File(outputFile).name
