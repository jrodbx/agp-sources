...
class SdkFullLoadingStrategy(
  ...
  private fun init(): Boolean {
    ...
    val platformHash = checkNotNull(platformTargetHashSupplier) { "Extension not initialized yet, couldn't access compileSdkVersion." }
    val buildToolRevision = checkNotNull(buildToolRevisionSupplier) { "Extension not initialized yet, couldn't access buildToolsVersion." }
  ...
  fun getApiVersionsFile(): File? =
    if (init()) {
      getFileFromTarget(IAndroidTarget.DATA)?.resolve(API_VERSIONS_FILE_NAME)?.takeIf { it.exists() }
    } else null
  ...
  fun getSystemImageLibFolder(imageHash: String) = sdkHandler.installSystemImage(imageHash)

  fun getEmulatorLibFolder() = sdkHandler.localEmulator
