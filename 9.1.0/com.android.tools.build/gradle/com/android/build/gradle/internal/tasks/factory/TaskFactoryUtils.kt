...
/**
 * Extension function for [TaskContainer] to add a way to create a task with our [VariantTaskCreationAction] without having a [TaskFactory]
 */
fun <T : Task> TaskContainer.registerTask(
  ...
  val actionWrapper = TaskConfigurationActions(creationAction, secondaryPreConfigAction, secondaryAction, secondaryProviderCallback)
  return this.register(creationAction.name, creationAction.type, actionWrapper).also { provider ->
    actionWrapper.postRegisterHook(provider)
  }
}

/**
 * Extension function for [TaskContainer] to add a way to create a task with our [PreConfigAction] and [TaskConfigAction] without having a
 * [TaskFactory]
 */
fun <T : Task> TaskContainer.registerTask(
  taskName: String,
  ...
  val actionWrapper =
    TaskConfigurationActions(preConfigAction = preConfigAction, configureAction = action, providerHandler = providerCallback)
  return this.register(taskName, taskType, actionWrapper).also { provider -> actionWrapper.postRegisterHook(provider) }
}

/**
 * Wrapper for the [VariantTaskCreationAction] as a simple [Action] that is passed to [TaskContainer.register].
 *
 * If the task is configured during the register then [VariantTaskCreationAction.preConfigure] is called right away.
 *
 * After register, if it has not been called then it is called, alongside [VariantTaskCreationAction.handleProvider]
 */
...
fun <T : Task> TaskProvider<out T>?.dependsOn(task: TaskProvider<out T>?): TaskProvider<out T>? {
  this?.letIfPresent { nonNullThis -> task?.letIfPresent { nonNullTask -> nonNullThis.configure { it.dependsOn(nonNullTask) } } }
...
fun <T : Task> TaskProvider<out T>?.dependsOn(taskName: String): TaskProvider<out T>? {
  this?.letIfPresent { nonNullThis -> nonNullThis.configure { it.dependsOn(taskName) } }
...
fun <T : Task> TaskProvider<out T>?.dependsOn(buildable: Buildable): TaskProvider<out T>? {
  this?.letIfPresent { nonNullThis -> nonNullThis.configure { it.dependsOn(buildable) } }
 ...
 *
 * @deprecated This is meant to be replaced with the version using 2 [TaskProvider] as [Task] get replaced with [TaskProvider]
 */
@Deprecated("Use TaskProvider.dependsOn(TaskProvider)")
fun <T : Task> TaskProvider<out T>?.dependsOn(task: Task?): TaskProvider<out T>? {
  this?.letIfPresent { nonNullThis -> task?.let { nonNullTask -> nonNullThis.configure { it.dependsOn(nonNullTask) } } }
...
fun <T : Task> Task?.dependsOn(task: TaskProvider<out T>?): Task? {
  this?.let { nonNullThis -> task?.letIfPresent { nonNullTask -> nonNullThis.dependsOn(nonNullTask) } }
