...
data class CxxAbiModel(
  ...
  /** The .cxx build folder ex, $moduleRootFolder/.cxx/Debug/{hashcode}/x86_64 */
  val cxxBuildFolder: File,

  /** Folder for .so files ex, $moduleRootFolder/build/intermediates/cxx/Debug/{hashcode}/obj/x86_64 */
  val soFolder: File,

  /**
   * An extra .cxx folder where build outputs are copied or symlinked too. This is also the old location where actual builds happened before
   * configuration folding was implemented. ex, $moduleRootFolder/build/intermediates/cmake/debug/obj/x86_64
   */
  ...
  /** Ninja/gnu make build settings specified by BuildSettings.json. Returns an empty model if the file is absent. */
  ...
  /**
   * A SHA-256 of the configuration parameters for CMake or ndk-build. This value is invariant of ABI, variant name, and output locations.
  ...
  /**
   * This ABI's intermediates folder but without the ABI at the end. This can't be stored on the variant because {hashcode} may be different
   * between two ABIs in the same variant. ex, $moduleRootFolder/build/intermediates/cxx/Debug/{hashcode}
   */
...
/** Location of model generation metadata ex, $moduleRootFolder/.cxx/Debug/{hashcode}/x86_64 */
private val CxxAbiModel.modelMetadataFolder: File
  get() = cxxBuildFolder

/** Location of spawned process logs file ex, $moduleRootFolder/build/intermediates/cxx/Debug/{hashcode}/log */
val CxxAbiModel.logsFolder: File
  get() = join(intermediatesParentFolder, "logs", name)

/** A predictable location to republish files like compile_commands.json. ex, $moduleRootFolder/.cxx/tools/debug/x86 */
...
/** The ninja log file ex, $moduleRootFolder/.cxx/cmake/debug/x86/.ninja_log */
val CxxAbiModel.ninjaLogFile: File
  get() = join(redirectedCxxBuildFolder, ".ninja_log")

/**
 * .ninja_deps file for this ABI. Only applies to CMake builds. This file contains the source -> header dependencies discovered during the
 * last build. For example, $moduleRootFolder/.cxx/ndkBuild/debug/armeabi-v7a/.ninja_deps
 */
val CxxAbiModel.ninjaDepsFile: File
  get() = join(redirectedCxxBuildFolder, ".ninja_deps")

/** Path to the expected build.ninja for this ABI. For example, $moduleRootFolder/.cxx/Debug/3c254s6s/arm64-v8a/build.ninja */
val CxxAbiModel.ninjaBuildFile: File
  get() = join(redirectedCxxBuildFolder, "build.ninja")

/**
 * The location of [cxxBuildFolder] after considering [ninjaBuildLocationFile]. The purpose is to allow [cxxBuildFolder] to be overridden in
 * the presence of [ninjaBuildLocationFile].
 */
private val CxxAbiModel.redirectedCxxBuildFolder: File
  get() {
    if (cxxBuildFolder.resolve("build.ninja").isFile || !ninjaBuildLocationFile.isFile) return cxxBuildFolder
    return File(ninjaBuildLocationFile.readText().lineSequence().first()).parentFile
  }

/**
 * Path to the expected build.ninja redirect file which is a file that contains the full path to the real build.ninja. For example,
 * $moduleRootFolder/.cxx/Debug/3c254s6s/arm64-v8a/build.ninja.txt
 */
val CxxAbiModel.ninjaBuildLocationFile: File
  get() = join(cxxBuildFolder, "build.ninja.txt")

/** Folder for .o files ex, $moduleRootFolder/build/intermediates/ndkBuild/debug/obj/local/armeabi-v7a/objs-debug */
...
/** Output file of the Cxx*Model structure ex, $moduleRootFolder/build/intermediates/cxx/Debug/{hashcode}/x86_64/build_model.json */
val CxxAbiModel.modelOutputFile: File
  get() = join(logsFolder, "build_model.json")

/** Json Generation logging record ex, $moduleRootFolder/build/intermediates/cxx/Debug/{hashcode}/x86_64/metadata_generation_record.json */
val CxxAbiModel.jsonGenerationLoggingRecordFile: File
  get() = join(logsFolder, "metadata_generation_record.json")

/**
 * Text file containing command run to generate C/C++ metadata. needs to have the same clean semantics as compile_commands.json, so place in
 * that folder.
 */
...
/**
 * Text file containing STDOUT for the process run to generate C/C++ metadata. ex,
 * $moduleRootFolder/.cxx/Debug/{hashcode}/x86_64/metadata_generation_stdout.txt
 */
val CxxAbiModel.metadataGenerationStdoutFile: File
  get() = join(modelMetadataFolder, "metadata_generation_stdout.txt")

/**
 * Text file containing STDERR for the process run to generate C/C++ metadata. ex,
 * $moduleRootFolder/.cxx/Debug/{hashcode}/x86_64/metadata_generation_stderr.txt
 */
...
/**
 * When CMake server is used, this is the log of the interaction with it. ex,
 * $moduleRootFolder/build/intermediates/cxx/Debug/{hashcode}/x86_64/cmake_server_log.txt
 */
val CxxAbiModel.cmakeServerLogFile: File
  get() = join(logsFolder, "cmake_server_log.txt")

/** The prefab configuration used when building this project ex, $moduleRootFolder/.cxx/ndkBuild/debug/armeabi-v7a/prefab_config.json */
...
/** compile_commands.json file for this ABI. For example, $moduleRootFolder/.cxx/{hashcode}/armeabi-v7a/compile_commands.json */
val CxxAbiModel.compileCommandsJsonFile: File
  get() = join(cxxBuildFolder, "compile_commands.json")

/**
 * compile_commands.json.bin file for this ABI. This is equivalent to a compile_commands.json file but more compact. Note: Should be in the
 * same folder as compile_commands.json because it is derived from that file.
 */
val CxxAbiModel.compileCommandsJsonBinFile: File
  get() = compileCommandsJsonFile.resolveSibling("compile_commands.json.bin")

/**
 * additional_project_files.txt file for this ABI. This file contains a newline separated list of filenames that are known by the build
 * system and considered to be part of the project. For example, $moduleRootFolder/.cxx/Debug/{hashcode}/x86_64/additional_project_files.txt
 */
val CxxAbiModel.additionalProjectFilesIndexFile: File
  get() = join(modelMetadataFolder, "additional_project_files.txt")

/**
 * Text file containing absolute paths to folders containing the generated symbols, one per line. For example,
 * $moduleRootFolder/.cxx/Debug/{hashcode}/x86_64/symbol_folder_index.txt
 */
val CxxAbiModel.symbolFolderIndexFile: File
  get() = join(modelMetadataFolder, "symbol_folder_index.txt")

/**
 * Text file containing absolute paths to native build files (For example, CMakeLists.txt for CMake). One per line. For example,
 * $moduleRootFolder/.cxx/Debug/{hashcode}/x86_64/meta/build_file_index.txt
 */
val CxxAbiModel.buildFileIndexFile: File
  get() = join(modelMetadataFolder, "build_file_index.txt")

/** The CMake file API query folder. ex, $moduleRootFolder/.cxx/cmake/debug/armeabi-v7a/.cmake/api/v1/query/client-agp */
val CxxAbiModel.clientQueryFolder: File
  get() = join(cxxBuildFolder, ".cmake/api/v1/query/client-agp")

/** The CMake file API reply folder. ex, $moduleRootFolder/.cxx/cmake/debug/armeabi-v7a/.cmake/api/v1/reply */
...
// time.
fun CxxAbiModel.buildIsPrefabCapable(): Boolean =
  variant.module.project.isPrefabEnabled
...
fun CxxAbiModel.shouldGeneratePrefabPackages(): Boolean =
  buildIsPrefabCapable() && variant.prefabPackages != null && !variant.prefabPackages.isEmpty

/** Returns the Ninja build commands from CMakeSettings.json. Returns an empty string if it does not exist. */
...
/**
 * The name of the CPU architecture (like "arm") for use in android triplet naming that is compatible with vcpkg. The difference is that
 * X86_64 is represented by "x64" not "x86_64"
 */
...
val CxxAbiModel.platformCode
  get() =
    variant.module.ndkMetaPlatforms?.let {
      it.aliases.toList().filter { (_, code) -> code == abiPlatformVersion }.minByOrNull { (alias, _) -> alias.length }?.first
    } ?: ""

/** File that holds the key for the hashed subfolder. ex, $moduleRootFolder/.cxx/Debug/{hashcode}/hash_key.txt */
val CxxAbiModel.cxxBuildHashKeyFile: File
  get() = cxxBuildFolder.parentFile.resolve("hash_key.txt")
