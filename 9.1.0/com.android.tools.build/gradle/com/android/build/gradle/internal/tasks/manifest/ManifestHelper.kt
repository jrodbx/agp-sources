 ...
 *
 * @param packageOverride the value used for the merged manifest's package attribute, which is the applicationId for apps and the namespace
 *   for libraries.
 ...
 *   null. Even if not null, the attribute will not be modified if it's already present in the main manifest or an overlay manifest.
 */
fun mergeManifests(
  mainManifest: File,
  ...
  try {
    ...
    if (checkIfPackageInMainManifest) {
      manifestMergerInvoker.withFeatures(ManifestMerger2.Invoker.Feature.CHECK_IF_PACKAGE_IN_MAIN_MANIFEST)
    }
    ...
    if (outMergedManifestLocation != null) {
      save(mergingReport.getMergedDocument(MergingReport.MergedManifestKind.MERGED), File(outMergedManifestLocation))
    }

    if (outAaptSafeManifestLocation != null) {
      save(
        mergingReport.getMergedDocument(if (mergingReport.isAaptSafeManifestUnchanged) MERGED else AAPT_SAFE),
 ...
 *
 * The manifest merge blame file is formatted as follow <lineNumber>--><filePath>:<startLine>:<startColumn>-<endLine>:<endColumn>
 */
fun findOriginalManifestFilePosition(manifestMergeBlameContents: List<String>, mergedFilePosition: SourceFilePosition): SourceFilePosition {
  if (
    mergedFilePosition.file == SourceFile.UNKNOWN ||
      mergedFilePosition.file.sourceFile?.absolutePath?.let { it.contains("merged_manifests") || it.contains("packaged_manifests") } ==
        false
  ) {
...
/** Sets the [ManifestSystemProperty] that can be injected in the manifest file. */
private fun setInjectableValues(
  ...
  if (extractNativeLibs != null) {
    ...
    if (compileSdk == null || compileSdk >= 23) {
      invoker.setOverride(ManifestSystemProperty.Application.EXTRACT_NATIVE_LIBS, extractNativeLibs.toString())
    }
  }
}
