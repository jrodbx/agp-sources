...
class SigningConfigDataProvider(
  ...
  @get:Nested @get:Optional val signingConfigData: Provider<out SigningConfigData>,

  /** When not `null`, the signing config information can be obtained from a file. */
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) @get:Optional val signingConfigFileCollection: FileCollection?,

  /**
   * The result of validating the signing config information. It may be `null` if the validation is already taken care of elsewhere (e.g.,
   * by a different module/task).
   */
  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) @get:Optional val signingConfigValidationResultDir: Provider<Directory>?,
) {
  ...
  fun convertToParams(): SigningConfigProviderParams {
    return SigningConfigProviderParams(signingConfigData.orNull, signingConfigFileCollection?.let { it.singleFile })
  }
    ...
    fun create(creationConfig: ApkCreationConfig): SigningConfigDataProvider {
      ...
      return if (!isInDynamicFeature) {
        ...
        SigningConfigDataProvider(
          ...
          signingConfigValidationResultDir = creationConfig.artifacts.get(InternalArtifactType.VALIDATE_SIGNING_CONFIG),
        )
      } else {
