...
object BootClasspathBuilder {
    ...
    return targetBootClasspath.flatMap { bootClasspath ->
      ...
      services.provider {
        classpathCache.getOrPut(key) {
          ...
          files.addAll(bootClasspath.map { objects.fileProperty().fileValue(it).get() })
          ...
          if (target.androidApiLevel.majorVersion <= 15) {
            files.add(annotationsJar.flatMap { it: File -> services.projectInfo.buildDirectory.file(it.absolutePath) })
          }
   ...
   * @param addAllOptionalLibraries overrides {@code libraryRequestsArg} and add all optional libraries available.
   * @param libraryRequestsArg the list of requested optional libraries
   * @param issueReporter the issueReporter which is written to if a requested library is not found
  ...
  fun computeAdditionalAndRequestedOptionalLibraries(
      ...
      .map<RegularFile> { lib ->
        services
          .fileProvider(
            services.provider {
              val jar = lib.jar
              Verify.verify(jar != null, "Jar missing from additional library %s.", lib.name)
          ...
          .fileProvider(
            services.provider {
              ...
              if (addAllOptionalLibraries || libraryRequested) {
                val jar = lib.jar
                Verify.verify(jar != null, "Jar missing from optional library %s.", lib.name)
                if (libraryRequested) {
                  libraryRequests.remove(lib.name)
                }
                jar?.toFile()
    ...
    for (library in libraryRequests) {
      issueReporter.reportError(IssueReporter.Type.OPTIONAL_LIB_NOT_FOUND, "Unable to find optional library: $library", library)
    }
    return files.build()
  }
}
