...
/**
 * Designates which artifacts are to be used by consumers according to whether AAR/JAR processors are enabled.
 *
 * Currently only considers jetifier as a processor, but more of the logic from things like namespaced android resources can be pulled here.
 *
 * TODO(b/288263843): Pull more logic around namespaced resources and artifact types such as JACOCO_ASM_INSTRUMENTED_JARS here and consider
 *   renaming the intermediate types used below.
 */
 ...
 *
 * It will also return the "jetifier enabled" case if namespaced resources are enabled, as that path is not optimized and still uses the
 * transforms.
 */
fun getAarOrJarTypeToConsume(projectOptions: ProjectOptions) =
  ...
  if (projectOptions[BooleanOption.ENABLE_JETIFIER]) {
    ...
    AarOrJarTypeToConsume(aar = ArtifactType.PROCESSED_AAR, jar = ArtifactType.PROCESSED_JAR)
  } else {
    // In any other case, consumers should use the original artifact directly.
    AarOrJarTypeToConsume(aar = ArtifactType.AAR, jar = ArtifactType.JAR)
  }
