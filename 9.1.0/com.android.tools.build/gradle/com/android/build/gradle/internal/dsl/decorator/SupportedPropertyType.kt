...
sealed class SupportedPropertyType {
  ...
  /**
   * This property type may only be used with 'var', e.g. primitives and Strings. These are not explicitly initialized, so use the default
   * values (e.g. null for Strings, false for booleans 0 for ints) are used.
   */
  ...
  sealed class Collection(
     ...
     *
     * The type here must have a 1-argument constructor that takes a name as argument, and must implement Lockable.
     */
    ...
    object List :
      Collection(
        ...
        bridgeTypes = listOf(Type.getType(kotlin.collections.Collection::class.java), Type.getType(Iterable::class.java)),
      )
   ...
   * ```
   */
  data class Block(override val type: Type, override val implementationType: Type) : SupportedPropertyType() {
    constructor(type: Class<*>, implementationType: Class<*>) : this(Type.getType(type), Type.getType(implementationType))
