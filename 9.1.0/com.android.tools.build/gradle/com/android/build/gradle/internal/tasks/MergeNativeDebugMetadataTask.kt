...
/**
 * Task that merges the .so.dbg or .so.sym native debug metadata files into a zip to be published in the outputs folder.
 *
 * Caching disabled by default for this task because the task does very little work. The task moves files from Inputs, unchanged, into a Zip
...
abstract class MergeNativeDebugMetadataTask : NonIncrementalTask() {
  ...
  override fun doTaskAction() {
    workerExecutor.noIsolation().submit(MergeNativeDebugMetadataWorkAction::class.java) {
  ...
  abstract class MergeNativeDebugMetadataWorkAction : ProfileAwareWorkAction<MergeNativeDebugMetadataWorkAction.Parameters>() {
  ...
  companion object {
    ...
    internal fun mergeFiles(inputFiles: Collection<File>, outputFile: File) {
      ...
      JarFlinger(outputFile.toPath(), null).use { zipCreator ->
        inputFiles.forEach { zipCreator.addFile("${it.parentFile.name}/${it.name}", it.toPath()) }
      }
    }
    ...
    private fun collectDebugSymbols(
      ...
      nativeDebugMetadataDirs.from(variant.artifacts.get(extractedDebugSymbolsArtifact))
      nativeDebugMetadataDirs.from(variant.artifacts.getAll(externalDebugSymbolArtifacts))
    ...
    fun getNativeDebugMetadataFiles(variant: ApplicationCreationConfig): FileCollection {
  ...
  class CreationAction(componentProperties: ApplicationCreationConfig) :
    VariantTaskCreationAction<MergeNativeDebugMetadataTask, ApplicationCreationConfig>(componentProperties) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<MergeNativeDebugMetadataTask>) {
      super.handleProvider(taskProvider)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, MergeNativeDebugMetadataTask::outputFile)
        .withName("native-debug-symbols.zip")
        .on(InternalArtifactType.MERGED_NATIVE_DEBUG_METADATA)
    }

    override fun configure(task: MergeNativeDebugMetadataTask) {
