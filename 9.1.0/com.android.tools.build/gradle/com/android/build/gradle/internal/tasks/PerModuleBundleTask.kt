...
/** Task that zips a module's bundle elements into a zip file. This gets published so that the base app can package into the bundle. */
@DisableCachingByDefault
@BuildAnalyzer(primaryTaskCategory = TaskCategory.BUNDLE_PACKAGING)
abstract class PerModuleBundleTask : NonIncrementalTask() {

  @get:Optional @get:OutputDirectory abstract val outputDir: DirectoryProperty

  @get:Optional @get:OutputFile abstract val outputFile: RegularFileProperty

  /** Whether this is a base module or a dynamic feature module. */
  @get:Input abstract val baseModule: Property<Boolean>

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val dexDirectories: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val featureDexDirectories: ConfigurableFileCollection

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val javaResJar: RegularFileProperty

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val featureJavaResFiles: ConfigurableFileCollection
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val nativeLibsFiles: ConfigurableFileCollection

  @get:Input abstract val abiFilters: SetProperty<String>

  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val baseModuleMetadata: ConfigurableFileCollection

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val appMetadata: RegularFileProperty

  @get:Optional @get:Input abstract val fileName: Property<String>

  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val versionControlInfoMetadata: RegularFileProperty

  public override fun doTaskAction() {
    val outputPath = (outputFile.orNull?.asFile ?: File(outputDir.get().asFile, fileName.get())).toPath()
    JarFlinger(outputPath, null).use { jarCreator ->
      ...
      val filters = baseModuleMetadata.singleOrNull()?.let { ModuleMetadata.load(it).abiFilters.toSet() } ?: abiFilters.get()

      val abiFilter = filters?.let { NativeLibraryAbiPredicate(it, false) }

      // https://b.corp.google.com/issues/140219742
      val excludeJarManifest = Predicate { path: String -> !path.uppercase(Locale.US).endsWith("MANIFEST.MF") }

      jarCreator.addDirectory(assetsFilesDirectory.get().asFile.toPath(), null, null, Relocator(FD_ASSETS))
      ...
      addHybridFolder(
        jarCreator,
        versionControlInfoMetadata.orNull?.asFile?.let { metadataFile -> setOf(metadataFile) } ?: setOf(),
  ...
  /**
   * Whether this is a dynamic feature module and the base module has shrinking enabled (see
   * [ApplicationCreationConfig.shrinkingWithDynamicFeatures]).
   *
   * Note: From a dynamic feature module, we don't have this info at task configuration time, so we need to check it at task execution time,
   * and check it indirectly through the presence of [featureDexDirectories].
   */
  ...
  private fun getShrunkResourcesFileInDirectory(shrunkResourcesDirectory: File): File {
    val shrunkResourcesFiles = shrunkResourcesDirectory.walk().filter { it.path.endsWith(SdkConstants.DOT_RES) }.toList()
  ...
  class CreationAction(creationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<PerModuleBundleTask, ApkCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<PerModuleBundleTask>) {
      ...
      creationConfig.artifacts.setInitialProvider(taskProvider, PerModuleBundleTask::outputDir).on(InternalArtifactType.MODULE_BUNDLE)
    }
    ...
    override fun configure(task: PerModuleBundleTask) {
      ...
      if (componentType.isBaseModule) {
        task.runResourceShrinking.setDisallowChanges((creationConfig as ApplicationCreationConfig).runResourceShrinking())
        if (creationConfig.runResourceShrinking()) {
          task.shrunkResourcesDirectory.setDisallowChanges(artifacts.get(InternalArtifactType.SHRUNK_RESOURCES_PROTO_FORMAT))
        } else {
          task.linkedResourcesFile.setDisallowChanges(artifacts.get(InternalArtifactType.LINKED_RESOURCES_FOR_BUNDLE_PROTO_FORMAT))
        }
      } else {
        ...
        task.featureLinkedResourcesFile.setDisallowChanges(artifacts.get(InternalArtifactType.LINKED_RESOURCES_FOR_BUNDLE_PROTO_FORMAT))
      }
      ...
      if (creationConfig.componentType.isDynamicFeature) {
        ...
        task.abiFilters.set(creationConfig.nativeBuildCreationConfig?.supportedAbis ?: emptyList())
      }
      ...
      if (creationConfig.componentType.isBaseModule) {
        ...
        artifacts.setTaskInputToFinalProduct(SingleArtifact.VERSION_CONTROL_INFO_FILE, task.versionControlInfoMetadata)
      }
    }
  }
}
...
/**
 * Relocate the dex files into a single folder which might force renaming the dex files into a consistent scheme : classes.dex,
 * classes2.dex, classes4.dex, ...
 *
 * <p>Note that classes1.dex is not a valid dex file name</p>
 *
 * When dealing with native multidex, we can have several folders each containing 1 to many dex files (with the same naming scheme). In that
 * case, merge and rename accordingly, note that only one classes.dex can exist in the merged location so the others will be renamed.
 *
 * When dealing with a single feature (base) in legacy multidex, we must not rename the main dex file (classes.dex) as it contains the
...
private class ResRelocator : JarFlinger.Relocator {
  override fun relocate(entryPath: String) =
    when (entryPath) {
