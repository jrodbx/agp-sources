...
abstract class AppMetadataTask : NonIncrementalTask() {
  ...
  override fun doTaskAction() {
    ...
    writeAppMetadataFile(appMetadataFile, appMetadataVersion.get(), agpVersion.get(), agdeVersion.orNull)
  }
  ...
  class CreationForAssetPackBundleAction(private val artifacts: ArtifactsImpl, private val projectOptions: ProjectOptions) :
...
private fun writeAppMetadataFile(file: File, appMetadataVersion: String, agpVersion: String, agdeVersion: String?) {
...
private fun TaskProvider<AppMetadataTask>.configureTaskOutputs(artifacts: ArtifactsImpl) {
  artifacts.setInitialProvider(this, AppMetadataTask::outputFile).withName(APP_METADATA_FILE_NAME).on(InternalArtifactType.APP_METADATA)
}
