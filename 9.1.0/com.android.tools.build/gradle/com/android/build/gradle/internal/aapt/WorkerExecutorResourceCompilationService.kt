...
class WorkerExecutorResourceCompilationService(
  ...
  /** Temporary workaround for b/73804575 / https://github.com/gradle/gradle/issues/4502 Only submit a small number of worker actions */
  ...
  override fun compileOutputFor(request: CompileResourceRequest): File {
    return File(request.outputDirectory, Aapt2RenamingConventions.compilationRename(request.inputFile))
  }
  ...
  override fun close() {
    ...
    val jvmRequests = requests.filter { canCompileResourceInJvm(it.inputFile, it.isPngCrunching) }
    requests.removeAll(jvmRequests)

    // Split all requests into buckets, giving each worker the same number of files to process
    var ord = 0
    val jvmBuckets = jvmRequests.groupByTo(HashMap(maxWorkersCount)) { (ord++) % maxWorkersCount }
    ...
    for (bucket in 0 until buckets) {
      val bucketRequests = requests.filterIndexed { i, _ -> i.rem(buckets) == bucket }
      // b/73804575
      val workQueue = workerExecutor.noIsolation()
