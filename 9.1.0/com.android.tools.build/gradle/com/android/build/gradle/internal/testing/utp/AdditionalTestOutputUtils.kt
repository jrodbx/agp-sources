...
fun findAdditionalTestOutputDirectoryOnDevice(device: DeviceConnector, testData: StaticTestData): String? {
  ...
  if (additionalTestOutputLocation == null) {
    logger.warn(
      "additionalTestOutput is not supported on this device running API level ${device.getApiLevel()} because the additional test output directory could not be found"
    )
    return null
  }
...
fun findAdditionalTestOutputDirectoryOnManagedDevice(device: UtpManagedDevice, testData: StaticTestData): String? {
...
private fun queryAdditionalTestOutputLocation(device: DeviceConnector): String? {
  ...
  val receiver: MultiLineReceiver =
