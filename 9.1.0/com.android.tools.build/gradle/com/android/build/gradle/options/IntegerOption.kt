...
enum class IntegerOption(override val propertyName: String, stage: ApiStage, override val defaultValue: Int? = null) : Option<Int> {
   ...
   *
   * The model-only mode is triggered when the IDE does a sync, and therefore we do things a bit differently (don't throw exceptions for
   * instance). Things evolved a bit over versions and the behavior changes. This reflects the mode to use.
   *
   ...
   *
   * <p>For preview versions that is the last stable version, and the {@link StringOption#IDE_TARGET_DEVICE_CODENAME} will also be set.
   */
  IDE_TARGET_DEVICE_API(PROPERTY_BUILD_API, ApiStage.Stable),

  /** Size of the buffers in kilobytes used to read .class files and storage for writing .dex files translations into. */
  ...
  /**
   * Maximum number of R8 tasks that can run in parallel (see [org.gradle.api.services.BuildServiceSpec.getMaxParallelUsages]).
   *
   * The [defaultValue] is 1, meaning that if the user doesn't specify a different value, only 1 R8 task can run at a time.
   *
   * Note:
   * - The property was named "maxWorkers" because each R8 task launches 1 Gradle worker action. "maxParallelTasks" might be a better name,
   *   but it is a bit late to change the name at this point.
   * - The task/worker action runs R8 in a thread pool. The thread pool's size is controlled by another property ([R8_THREAD_POOL_SIZE]).
   *
  ...
  /**
   * The size of the thread pool ([java.util.concurrent.ExecutorService]) that runs R8 tasks. If no value is given, a default
   * heuristics-based value will be used.
   */
  R8_THREAD_POOL_SIZE("android.r8.threadPoolSize", ApiStage.Experimental, R8D8ThreadPoolBuildService.defaultThreadPoolSize()),

  /**
   * Flags for Android Test Retention >=1: enable Android Test Retention and set it as maximum number of snapshots. 0: disable Android Test
   * Retention. <0: enable Android Test Retention with unlimited number of snapshots.
   */
  ...
  /**
   * The timeout duration in minute for Gradle Managed Device setup steps (booting AVD and creating snapshot image) If the value is 0 or
   * negative value, it will never time out.
   */
  ...
  /**
   * Timeout in minutes for acquiring a device lock for a Gradle Managed Device. A value of 0 or less disables the timeout, causing it to
   * wait indefinitely.
   */
  GRADLE_MANAGED_DEVICE_LOCK_TIMEOUT_MINUTES(
    "android.experimental.testOptions.managedDevices.deviceLockTimeoutMinutes",
    ApiStage.Experimental,
  ),

  /** The timeout duration in seconds for install apk If the value is 0 or negative value, it will be set to a default value by UTP. */
  INSTALL_APK_TIMEOUT("android.experimental.testOptions.installApkTimeout", ApiStage.Experimental),
  /* ------------
   * REMOVED APIS
   */

  THREAD_POOL_SIZE("android.threadPoolSize", ApiStage.Removed(VERSION_BEFORE_4_0, "The android.threadPoolSize property has no effect"));
  ...
  override fun parse(value: Any): Int {
    ...
    throw IllegalArgumentException(
      "Cannot parse project property " + this.propertyName + "='" + value + "' of type '" + value.javaClass + "' as integer."
    )
  }
}
