...
/**
 * The purpose of this class is to take the raw output of an ndk-build -n call and to produce a NativeBuildConfigValue instance to pass
 * upstream through gradle.
 *
 * This involves several stages of processing:
 *
 * (1) CommandLineParser.parse accepts a single string which is the ndk-build -n output. It tokenizes each command in the string according
 * to shell parsing rules on Windows or bash (includes mac). The result is a list of CommandLine.
 *
 * (2) CommandClassifier.classify accepts the output of (1). It looks at each command for something it recognizes. This will typically be
 * calls to clang, gcc or gcc-ar. Once a command is recognized, its file inputs and outputs are recorded. The result is a list of
 * Classification.
 *
 * (3) FlowAnalyzer.analyze accepts the output of (2). It traces the flow of inputs and outputs. This flow tracing will involve intermediate
 * steps through linking and possibly archiving (gcc-ar). Files involved are typically .c, .cpp, .o, .a and .so. The result of this step is
 * a map from terminal outputs (.so) to original inputs (.c and .cpp).
 *
 * (4) NativeBuildConfigValueBuilder.build accepts the output of (3). It examines the terminal outputs and input information to build up an
 * instance of NativeBuildConfigValue.
 */
class NativeBuildConfigValueBuilder
internal constructor(
  ...
  /** Skips processing compiler flags so that the generated [NativeBuildConfigValue] won't contain any native compiler flags. */
  var skipProcessingCompilerFlags = false

  /**
   * Notes on implementation details: This is an implicit writer that generates the compile_commands.json.bin. This is used by
  ...
  fun setCommands(
    ...
    buildTargetsCommand = buildCommand + listOf(CxxRegularBuilder.BUILD_TARGETS_PLACEHOLDER)
    return this
  }
  ...
  private fun extractWorkingDirectoryFromDryRunOutputIfAvailable(dryRunOutput: String) {
    if (dryRunOutput.contains("make: Entering directory")) {
      ...
      val workingDirPattern = Regex("^make: Entering directory [`'](.*?)'$", RegexOption.MULTILINE)
  ...
  private fun findLibraryNames() {
    for (output in outputs) {
      ...
      output.artifactName = NdkUtils.getTargetNameFromBuildOutputFileName(fileConventions.getFileName(output.outputFileName))
  ...
  private fun findToolChainCompilers() {
    for (output in outputs) {
      ...
      val cCompilers: MutableSet<String> = HashSet()
      val cppCompilers: MutableSet<String> = HashSet()
      val compilerToWeirdExtensions: MutableMap<String, MutableSet<String>> = HashMap()
  ...
  private fun generateCleanCommands(): List<List<String>> {
    val cleanCommands: MutableSet<List<String>> = Sets.newHashSet()
    for (output in outputs) {
  ...
  private fun generateLibraries(): Map<String?, NativeLibraryValue> {
    ...
    for (output in outputs) {
      ...
      value.abi = fileConventions.getFileName(fileConventions.getFileParent(output.outputFileName))
      ...
      for (input in output.commandInputs) {
        ...
        file.flags = stripArgsForIde(input.onlyInput, input.command.escapedFlags).map { rawFlagsLookUpMap[it] }.joinToString(" ")
      }
    }
    return librariesMap
  }
  ...
  private fun generateToolchains(): Map<String?, NativeToolchainValue> {
    ...
    for (toolchain in toolchains) {
      ...
      if (toolChainToCCompiler.containsKey(toolchain)) {
        toolchainValue.cCompilerExecutable = fileConventions.toFile(toolChainToCCompiler[toolchain]!!)
      }
      if (toolChainToCppCompiler.containsKey(toolchain)) {
        toolchainValue.cppCompilerExecutable = fileConventions.toFile(toolChainToCppCompiler[toolchain]!!)
      }
    }
