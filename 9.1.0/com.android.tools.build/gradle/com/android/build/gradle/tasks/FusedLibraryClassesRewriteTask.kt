...
abstract class FusedLibraryClassesRewriteTask : NonIncrementalGlobalTask() {
  ...
  @get:Optional @get:OutputDirectory abstract val rewrittenClassesDirectory: DirectoryProperty

  @get:OutputFile abstract val fusedLibraryRClass: RegularFileProperty

  @get:Input abstract val fusedLibraryNamespace: Property<String>

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val librariesSymbolLists: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val mergedClasses: ConfigurableFileCollection

  @get:ServiceReference abstract val symbolTableBuildService: Property<SymbolTableBuildService>

  override fun doTaskAction() {
    val dependencyTables = symbolTableBuildService.get().loadClasspath(librariesSymbolLists.files).filterNot { it.symbols.isEmpty }
    ...
    rewriteRClassReferencesToFusedLibraryNamespace(mergedClassFiles, rewrittenClassesDirectory.get().asFile, namespaceRewriter)
  }
  ...
  private fun rewriteRClassReferencesToFusedLibraryNamespace(
    ...
    val mergedClassFiles = incomingClassesFiles.filter { it.extension == SdkConstants.EXT_CLASS }
    for (incomingClass in mergedClassFiles) {
      val incomingClasspath = incomingClass.relativeTo(mergedClasses.singleFile)
      val rewriteOutputPath = FileUtils.join(outgoingClassesDir, incomingClasspath.toString())
      FileUtils.createFile(rewriteOutputPath, "")
      namespaceRewriter.rewriteClass(incomingClass.toPath(), rewriteOutputPath.toPath())
    }
  }

  private fun getFusedSymbolTable(fusedLibraryNamespace: String, dependencySymbolTables: List<SymbolTable>): SymbolTable {
    return SymbolTable.builder()
      .tablePackage(fusedLibraryNamespace)
      .addAll(dependencySymbolTables.flatMap { it.symbols.values() }.toSet())
      .build()
  }

  class CreationAction(val creationConfig: FusedLibraryGlobalScope) : GlobalTaskCreationAction<FusedLibraryClassesRewriteTask>() {
    ...
    override fun handleProvider(taskProvider: TaskProvider<FusedLibraryClassesRewriteTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, FusedLibraryClassesRewriteTask::rewrittenClassesDirectory)
        .on(FusedLibraryInternalArtifactType.CLASSES_WITH_REWRITTEN_R_CLASS_REFS)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, FusedLibraryClassesRewriteTask::fusedLibraryRClass)
    ...
    override fun configure(task: FusedLibraryClassesRewriteTask) {
      ...
      task.fusedLibraryNamespace.setDisallowChanges(creationConfig.namespace)
      ...
      task.mergedClasses.setFrom(creationConfig.artifacts.get(FusedLibraryInternalArtifactType.MERGED_CLASSES))
      task.symbolTableBuildService.setDisallowChanges(SymbolTableBuildService.RegistrationAction(task.project).execute())
    }
  }
}
