...
abstract class MergeResources : NewIncrementalTask() {
  ...
  @get:Input abstract val dataBindingEnabled: Property<Boolean>

  @get:Input abstract val viewBindingEnabled: Property<Boolean>

  @get:Input abstract val namespace: Property<String>

  @get:Optional @get:Input abstract val useAndroidX: Property<Boolean>

  /**
   * Set of absolute paths to resource directories that are located outside the root project directory when data binding / view binding is
   * enabled.
   *
   * These absolute paths appear in the layout info files generated by data binding, so we have to mark them as @Input to ensure build
   * correctness. (If this set is not empty, this task is still cacheable but not relocatable.)
   *
   * If data binding / view binding is not enabled, this set is empty as it won't be used.
   */
  @get:Input abstract val resourceDirsOutsideRootProjectDir: SetProperty<String>

  @get:Input abstract val pseudoLocalesEnabled: Property<Boolean>

  @get:PathSensitive(PathSensitivity.RELATIVE) @get:Optional @get:InputFiles abstract val renderscriptGeneratedResDir: DirectoryProperty

  @get:Nested abstract val aapt2: Aapt2Input

  @get:Nested abstract val resourcesComputer: DependencyResourcesComputer

  /** Directory to write the generated vector drawable resources to */
  @get:Optional @get:OutputDirectory abstract val generatedPngsOutputDir: DirectoryProperty

  @get:Optional @get:OutputDirectory abstract val dataBindingLayoutInfoOutFolder: DirectoryProperty

  /** Directory to write the merged resources to */
  @get:OutputDirectory abstract val outputDir: DirectoryProperty

  @get:OutputFile @get:Optional abstract val publicFile: RegularFileProperty

  /** Optional blame output folder for the case where the task generates it. */
  @get:Optional @get:OutputDirectory abstract val blameLogOutputFolder: DirectoryProperty

  @get:Optional @get:OutputDirectory abstract val incrementalFolder: DirectoryProperty

  @get:Optional @get:OutputDirectory abstract val mergedNotCompiledResourcesOutputDirectory: DirectoryProperty

  @get:Internal abstract val aaptEnv: Property<String>
  ...
  @get:Internal abstract val aapt2ThreadPoolBuildService: Property<Aapt2ThreadPoolBuildService>
  ...
  protected fun doFullTaskAction() {
    ...
    if (dataBindingLayoutInfoOutFolder.isPresent) {
      FileUtils.deleteDirectoryContents(dataBindingLayoutInfoOutFolder.get().asFile)
    }
    ...
    try {
      aaptWorkerFacade.use { workerExecutorFacade ->
        getResourceProcessor(this, flags, processResources, aapt2).use { resourceCompiler ->
          val dataBindingLayoutProcessor = maybeCreateLayoutProcessor()
          Blocks.recordSpan<MergingException>(path, GradleBuildProfileSpan.ExecutionType.TASK_EXECUTION_PHASE_1, analyticsService.get()) {
          ...
          Blocks.recordSpan<MergingException>(path, GradleBuildProfileSpan.ExecutionType.TASK_EXECUTION_PHASE_2, analyticsService.get()) {
            merger.mergeData(writer, false /*doCleanUp*/)
          }
          Blocks.recordSpan<JAXBException>(path, GradleBuildProfileSpan.ExecutionType.TASK_EXECUTION_PHASE_3, analyticsService.get()) {
            dataBindingLayoutProcessor?.end()
          }

          // No exception? Write the known state.
          Blocks.recordSpan<MergingException>(path, GradleBuildProfileSpan.ExecutionType.TASK_EXECUTION_PHASE_4, analyticsService.get()) {
  ...
  /**
   * Check if the changed file is filtered out from the input to be compiled in compile library resources task, if it is then it should be
   * ignored.
   */
  ...
  override fun doTaskAction(inputChanges: InputChanges) {
    ...
    for (input in resourcesComputer.resources.get().values) {
      val changes: Iterable<FileChange> = inputChanges.getFileChanges(input.sourceDirectories)
      Iterables.addAll(thisProjectResourceChanges, changes)
    }
    val libraryResourceChanges = inputChanges.getFileChanges(resourcesComputer.librarySourceSets)
    if (!thisProjectResourceChanges.iterator().hasNext() && !libraryResourceChanges.iterator().hasNext()) {
      return
    }
    // create a merger and load the known state.
    val merger = ResourceMerger(minSdk.get())
    try {
      if (!merger.loadFromBlob(incrementalFolder, true /*incrementalState*/, aaptEnv.orNull)) {
        logger.info("[MergeResources] Blob cannot be loaded causing a full task action.")
        doFullTaskAction()
        return
      }
      ...
      for (entry in thisProjectResourceChanges) {
        if (!precompileDependenciesResources || !isFilteredOutLibraryResource(entry.file)) {
      ...
      val mergingLog = if (blameLogOutputFolder.isPresent) MergingLog(blameLogOutputFolder.get().asFile) else null
      aaptWorkerFacade.use { workerExecutorFacade ->
        getResourceProcessor(this, flags, processResources, aapt2).use { resourceCompiler ->
          val dataBindingLayoutProcessor = maybeCreateLayoutProcessor()
          val publicFile = if (publicFile.isPresent) publicFile.get().asFile else null
  ...
  private fun tryUpdateResourceSetsWithChangedFile(merger: ResourceMerger, entry: FileChange): Boolean {
    val changedFile = entry.file
    merger.findDataSetContaining(changedFile, fileValidity)
    if (fileValidity.status == FileValidity.FileStatus.UNKNOWN_FILE) {
      logger.info("[MergeResources] " + changedFile.absolutePath + " has an unknown file status, requiring full task run.")
      return false
    } else if (fileValidity.status == FileValidity.FileStatus.VALID_FILE) {
      if (
        !fileValidity.dataSet.updateWith(fileValidity.sourceFile, changedFile, entry.changeType.toSerializable(), LoggerWrapper(logger))
      ) {
        logger.info(String.format("[MergeResources] Failed to process %s event! " + "Requires full task run", entry.changeType))
        return false
      }
    }
    return true
  ...
  private fun maybeCreateLayoutProcessor(): SingleFileProcessor? {
    ...
    val fileLookup: OriginalFileLookup =
      if (blameLogOutputFolder.isPresent) {
    ...
    return object : SingleFileProcessor {
      ...
      override fun processSingleFile(inputFile: File, outputFile: File, inputFileIsFromDependency: Boolean?): Boolean {
        ...
        if (FileUtils.isFileInDirectory(inputFile, rootProjectDir)) {
          ...
          check(!resourceIsInResourceDirs(inputFile, resourceDirsOutsideRootProjectDir.get())) {
            "Layout file ${inputFile.canonicalPath} is not expected be located in " +
              resourceDirsOutsideRootProjectDir.get().joinToString(", ", "[", "]") { File(it).canonicalPath }
          }
          ...
          // 128643036.
          normalizedInputFile = RelativizableFile.fromAbsoluteFile(inputFile.canonicalFile, rootProjectDir)
          ...
          normalizedInputFile = RelativizableFile.fromAbsoluteFile(inputFile.canonicalFile, null)
          Preconditions.checkState(normalizedInputFile.relativeFile == null)
        }
        return processor.processSingleFile(normalizedInputFile, outputFile, viewBindingEnabled.get(), dataBindingEnabled.get())
      }

      /** Returns `true` if the given resource file is located inside one of the resource directories. */
      private fun resourceIsInResourceDirs(resFile: File, resDirs: Set<String>): Boolean {
        return resDirs.any { resDir: String -> FileUtils.isFileInDirectory(resFile, File(resDir)) }
      }
      ...
      override fun end() {
        processor.writeLayoutInfoFiles(dataBindingLayoutInfoOutFolder.get().asFile)
      }
    }
  }
  ...
  ) : VectorDrawableRenderer(minSdk, supportLibraryIsUsed, outputDir, densities, loggerSupplier!!) {
  ...
  private fun getConfiguredResourceSets(preprocessor: ResourcePreprocessor, aaptEnv: String?): List<ResourceSet> {
    ...
    if (processedInputs.isEmpty()) {
      processedInputs.addAll(resourcesComputer.compute(precompileDependenciesResources, ignoreFilesPattern, renderscriptGeneratedResDir))
      val generatedSets: MutableList<ResourceSet> = ArrayList(processedInputs.size)
  ...
  private fun prepareIgnoreFilesPattern(aaptEnv: String?): String = buildString {
    if (aaptEnv != null) {
      append(aaptEnv)
    } else {
  ...
  /**
   * Releases resource sets not needed anymore, otherwise they will waste heap space for the duration of the build.
   *
  ...
  class KotlinMultiplatformCreationAction(creationConfig: KmpCreationConfig, private val flags: Set<Flag> = emptySet()) :
    ...
    override fun handleProvider(taskProvider: TaskProvider<MergeResources>) {
      ...
      artifacts.setInitialProvider(taskProvider) { obj: MergeResources -> obj.outputDir }.on(InternalArtifactType.PACKAGED_RES)

      artifacts.use(taskProvider).wiredWith { obj: MergeResources -> obj.incrementalFolder }.toCreate(MERGED_RES_INCREMENTAL_FOLDER)
    }

    override fun configure(task: MergeResources) {
      super.configure(task)
      task.namespace.setDisallowChanges(creationConfig.namespace)
      task.minSdk.setDisallowChanges(task.project.provider { creationConfig.minSdk.apiLevel })
      task.useAndroidX.setDisallowChanges(creationConfig.services.projectOptions[BooleanOption.USE_ANDROID_X])
      task.flags = flags
      task.resourcesComputer.initFromVariantScope(creationConfig = creationConfig, libraryDependencies = null)
      task.aapt2ThreadPoolBuildService.setDisallowChanges(
        getBuildService(creationConfig.services.buildServiceRegistry, Aapt2ThreadPoolBuildService::class.java)
      )
      ...
      task.resourceDirsOutsideRootProjectDir.setDisallowChanges(task.project.provider { emptySet() })
  ...
  class CreationAction(
    ...
    override fun handleProvider(taskProvider: TaskProvider<MergeResources>) {
      ...
      creationConfig.taskContainer.mergeResourcesTask = taskProvider
      val artifacts = creationConfig.artifacts
      artifacts.setInitialProvider(taskProvider) { obj: MergeResources -> obj.outputDir }.on(mergeType.outputType)
      if (mergedNotCompiledOutputDirectory != null) {
        artifacts
          .setInitialProvider(taskProvider) { obj: MergeResources -> obj.mergedNotCompiledResourcesOutputDirectory }
          .atLocation(mergedNotCompiledOutputDirectory)
          .on(MERGED_NOT_COMPILED_RES)
      }
      artifacts
        .setInitialProvider(taskProvider) { obj: MergeResources -> obj.dataBindingLayoutInfoOutFolder }
        .withName("out")
        .on(if (mergeType === TaskManager.MergeType.MERGE) DATA_BINDING_LAYOUT_INFO_TYPE_MERGE else DATA_BINDING_LAYOUT_INFO_TYPE_PACKAGE)

      // only the full run with dependencies generates the blame folder
      if (includeDependencies) {
        artifacts
          .setInitialProvider(taskProvider) { obj: MergeResources -> obj.blameLogOutputFolder }
          .withName("out")
      ...
      if (enableVectorDrawables) {
        artifacts
          .setInitialProvider(taskProvider) { obj: MergeResources -> obj.generatedPngsOutputDir }
          .atLocation(creationConfig.paths.getGeneratedResourcesDir("pngs").get().asFile.absolutePath)
    ...
    override fun configure(task: MergeResources) {
      ...
      task.isVectorSupportLibraryUsed = java.lang.Boolean.TRUE == vectorDrawablesOptions.useSupportLibrary
      val libraryArtifacts =
        if (includeDependencies)
      ...
      task.renderscriptGeneratedResDir.setDisallowChanges(creationConfig.artifacts.get(InternalArtifactType.RENDERSCRIPT_GENERATED_RES))

      task.resourcesComputer.initFromVariantScope(creationConfig = creationConfig, libraryDependencies = libraryArtifacts)
      ...
      task.pseudoLocalesEnabled.setDisallowChanges(androidResourcesCreationConfig.pseudoLocalesEnabled)
      task.flags = flags
      task.errorFormatMode = SyncOptions.getErrorFormatMode(creationConfig.services.projectOptions)
      task.precompileDependenciesResources =
        (mergeType == TaskManager.MergeType.MERGE && !isLibrary && androidResourcesCreationConfig.isPrecompileDependenciesResourcesEnabled)
      task.resourceDirsOutsideRootProjectDir.setDisallowChanges(
        task.project.provider { getResourcesDirsOutsideRoot(task, isDataBindingEnabled, isViewBindingEnabled) }
      )
