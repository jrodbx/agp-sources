...
abstract class ProcessLibraryManifest : ManifestProcessorTask() {
  ...
  @get:OutputFile abstract val manifestOutputFile: RegularFileProperty
  ...
  /**
   * The aapt friendly processed Manifest. In case we are processing a library manifest, some placeholders may not have been resolved (and
   * will be when the library is merged into the importing application). However, such placeholders keys are not friendly to aapt which
   * flags some illegal characters. Such characters are replaced/encoded in this version.
   */
  @get:Optional @get:OutputDirectory abstract val aaptFriendlyManifestOutputDirectory: DirectoryProperty
  /**
   * The aapt friendly processed Manifest. In case we are processing a library manifest, some placeholders may not have been resolved (and
   * will be when the library is merged into the importing application). However, such placeholders keys are not friendly to aapt which
   * flags some illegal characters. Such characters are replaced/encoded in this version.
  ...
  val aaptFriendlyManifestOutputFile: File?
    get() =
      if (aaptFriendlyManifestOutputDirectory.isPresent)
        FileUtils.join(aaptFriendlyManifestOutputDirectory.get().asFile, SdkConstants.ANDROID_MANIFEST_XML)
      else null
  ...
  abstract class ProcessLibWorkAction : ProfileAwareWorkAction<ProcessLibParams>() {
    override fun run() {
      ...
      try {
        outputMergeBlameContents(mergingReport, parameters.mergeBlameFile.asFile.get())
      ...
      if (parameters.aaptFriendlyManifestOutputDirectory.isPresent) {
        BuiltArtifactsImpl(
            ...
            listOf(BuiltArtifactImpl.make(outputFile = parameters.aaptFriendlyManifestOutputFile.asFile.get().absolutePath)),
          )
  ...
  @get:Optional @get:Input abstract val namespace: Property<String>

  @get:Internal abstract val tmpDir: DirectoryProperty

  @get:Optional @get:Input abstract val disableMinSdkVersionCheck: Property<Boolean>
  ...
  class CreationAction(
  ...
  ) : VariantTaskCreationAction<ProcessLibraryManifest, ComponentCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ProcessLibraryManifest>) {
      super.handleProvider(taskProvider)
      creationConfig.taskContainer.processManifestTask = taskProvider
      val artifacts = creationConfig.artifacts
      artifacts
        .setInitialProvider(taskProvider, ProcessLibraryManifest::aaptFriendlyManifestOutputDirectory)
        .withName("aapt")
        .on(InternalArtifactType.AAPT_FRIENDLY_MERGED_MANIFESTS)

      artifacts.setInitialProvider(taskProvider, ProcessLibraryManifest::manifestOutputFile).on(SingleArtifact.MERGED_MANIFEST)

      artifacts
        .setInitialProvider(taskProvider, ProcessLibraryManifest::mergeBlameFile)
        .withName("manifest-merger-blame-" + creationConfig.baseName + "-report.txt")
        .on(InternalArtifactType.MANIFEST_MERGE_BLAME_FILE)

      artifacts
        .setInitialProvider(taskProvider, ProcessLibraryManifest::reportFile)
    ...
    override fun configure(task: ProcessLibraryManifest) {
      super.configure(task)
      task.minSdkVersion.setDisallowChanges(creationConfig.minSdk.getApiString())
      task.targetSdkVersion.setDisallowChanges(
        if (targetSdkVersion == null || targetSdkVersion.apiLevel < 1) null else targetSdkVersion.getApiString()
      )
      task.maxSdkVersion.setDisallowChanges(maxSdkVersion)
      creationConfig.manifestPlaceholdersCreationConfig?.placeholders?.let { task.manifestPlaceholders.setDisallowChanges(it) }
      ...
      task.tmpDir.setDisallowChanges(creationConfig.paths.intermediatesDir("tmp", "ProcessLibraryManifest", creationConfig.dirName))
      task.disableMinSdkVersionCheck.setDisallowChanges(creationConfig.services.projectOptions[BooleanOption.DISABLE_MINSDKLIBRARY_CHECK])
