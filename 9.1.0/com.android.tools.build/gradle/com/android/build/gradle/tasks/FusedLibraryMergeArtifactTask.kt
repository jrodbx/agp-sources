...
/**
 * Responsible for merging multiple library artifacts of the same artifact type into a single artifact in the fused library aar, for future
 * consumption. The task is intended to be configured for each artifact type. Eventually these operations can be migrated to artifact
 * transforms once this feature is supported in the fused library plugin.
 */
@CacheableTask
...
abstract class FusedLibraryMergeArtifactTask : NonIncrementalGlobalTask() {
  ...
  @get:Input abstract val artifactType: Property<ArtifactType>

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val artifactFiles: ConfigurableFileCollection

  @get:Nested abstract val aarMetadataInputs: AarMetadataInputs

  @get:Optional @get:Input abstract val jniExcludes: SetProperty<String>

  @get:Optional @get:Input abstract val jniKeepDebugSymbols: SetProperty<String>

  @get:Optional @get:Input abstract val jniPickFirst: SetProperty<String>
  ...
  override fun doTaskAction() {
    workerExecutor.noIsolation().submit(FusedLibraryMergeArtifactWorkAction::class.java) {
      ...
      it.output.setDisallowChanges(if (outputDir.isPresent) outputDir else outputFile)
    }
  }
  ...
  abstract class FusedLibraryMergeArtifactWorkAction : ProfileAwareWorkAction<FusedLibraryMergeArtifactParams>() {
          ...
          ArtifactType.JNI -> {
            ...
            val relativeInputFiles =
              inputFiles
                ...
                .filter { it.isFile }
                .map { MergeNativeLibsTask.InputFile(it, it.toString().substringAfter("$separator$FD_JNI$separator")) }
            mergeJavaNativeLibs(
            ...
            JarFlinger(output.get().asFile.toPath()).use { jarFlinger ->
              input.files.forEach {
                jarFlinger.addJar(
                  ...
                  { it.endsWith(DOT_JAVA) || it.endsWith(DOT_KT) },
                  null,
                )
              }
            }
          ...
          ArtifactType.UNFILTERED_PROGUARD_RULES -> {
            val consumerProguardFiles =
              inputFiles
                ...
                .onEach { ConsumerRuleGlobalGuardian.validateConsumerRulesHasNoBannedGlobals(it, false, { error(it.errorMessage) }) }
                .toList()
            ...
            if (currentArtifactType !in supportedArtifacts) {
              error("${currentArtifactType.type} is not a supported artifact type for " + "fused library artifact republishing.")
            }
          }
        }
      }
      ...
      when (fusedArtifact) {
        is FusedArtifact.Directory ->
          creationConfig.artifacts
            .setInitialProvider(taskProvider, FusedLibraryMergeArtifactTask::outputDir)
        ...
        is FusedArtifact.File ->
          creationConfig.artifacts
            .setInitialProvider(taskProvider, FusedLibraryMergeArtifactTask::outputFile)
    ...
    override fun configure(task: FusedLibraryMergeArtifactTask) {
      ...
      task.artifactFiles.setFrom(
        when (androidArtifactType) {
          ArtifactType.SOURCES_JAR -> {
            val fusedSources = task.project.configurations.getByName(FusedLibraryConstants.FUSED_SOURCES_CONFIGURATION_NAME)
            fusedSources.incoming
              .artifactView { config -> config.attributes { it.attribute(AndroidArtifacts.ARTIFACT_TYPE, ArtifactType.SOURCES_JAR.type) } }
              .files
          }
          else -> {
      ...
      when (androidArtifactType) {
        ArtifactType.AAR_METADATA -> {
          creationConfig.aarMetadata.minAgpVersion?.let { task.aarMetadataInputs.minAgpVersion.setDisallowChanges(it) }
        ...
        ArtifactType.JNI -> {
          task.jniExcludes.setDisallowChanges(creationConfig.packaging.jniLibs.excludes)
          task.jniPickFirst.setDisallowChanges(creationConfig.packaging.jniLibs.pickFirsts)

          // Only added to the task for error messaging, as debug stripping
          // should be configured in the dependencies.
          task.jniKeepDebugSymbols.setDisallowChanges(creationConfig.packaging.jniLibs.keepDebugSymbols)
        }
        else -> {}
      }
    }
  ...
  abstract class AarMetadataInputs {
    ...
    @get:Input @get:Optional abstract val minCompileSdkVersion: Property<CompileSdkVersion>

    @get:Input @get:Optional abstract val minAgpVersion: Property<String>
  }
  ...
  companion object {
    ...
    fun getCreationActions(creationConfig: FusedLibraryGlobalScope): List<CreateActionFusedLibrary> {
...
sealed class FusedArtifact(val artifactType: Artifact.Single<*>) {
  internal class Directory(artifactType: Artifact.Single<org.gradle.api.file.Directory>) : FusedArtifact(artifactType)

  internal class File(artifactType: Artifact.Single<RegularFile>, val filename: String) : FusedArtifact(artifactType)
}
