...
abstract class AidlCompile : NonIncrementalTask() {
  ...
  @get:Internal abstract val sourceDirs: ListProperty<Directory>
  ...
  @InputFile @PathSensitive(PathSensitivity.NONE) fun getAidlFrameworkProvider(): Provider<File> = buildTools.aidlFrameworkProvider()
  ...
  override fun doTaskAction() {
    ...
    val aidlExecutable = buildTools.aidlExecutableProvider().get().absoluteFile
  ...
  class CreationAction(creationConfig: ConsumableCreationConfig) :
    VariantTaskCreationAction<AidlCompile, ConsumableCreationConfig>(creationConfig) {

    override val name: String = computeTaskName("compile", "Aidl")

    override val type: Class<AidlCompile> = AidlCompile::class.java

    override fun handleProvider(taskProvider: TaskProvider<AidlCompile>) {
      super.handleProvider(taskProvider)
      creationConfig.taskContainer.aidlCompileTask = taskProvider
      creationConfig.artifacts
        .setInitialProvider(taskProvider, AidlCompile::sourceOutputDir)
        .withName("out")
        .on(InternalArtifactType.AIDL_SOURCE_OUTPUT_DIR)

      if (creationConfig.componentType.isAar) {
        creationConfig.artifacts
          .setInitialProvider(taskProvider, AidlCompile::packagedDir)
          .withName("out")
          .on(InternalArtifactType.AIDL_PARCELABLE)
      }
    }
  ...
  abstract class AidlCompileRunnable : ProfileAwareWorkAction<AidlCompileRunnable.Params>() {
    ...
    override fun run() {
      ...
      val collector = FileAction { root: Path, file: Path -> processingRequests.add(ProcessingRequest(root.toFile(), file.toFile())) }

      try {
        DirectoryWalker.builder().root(parameters.dir.get().toPath()).extensions("aidl").action(collector).build().walk()
      } catch (e: IOException) {
      ...
      val logger = LoggedProcessOutputHandler(LoggerWrapper.getLogger(AidlCompileRunnable::class.java))
