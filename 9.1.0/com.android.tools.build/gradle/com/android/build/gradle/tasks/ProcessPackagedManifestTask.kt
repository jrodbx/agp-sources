...
abstract class ProcessPackagedManifestTask @Inject constructor(objects: ObjectFactory, workers: WorkerExecutor) : NonIncrementalTask() {

  @get:OutputDirectory abstract val packageManifests: DirectoryProperty

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val mergedManifests: DirectoryProperty

  @get:Internal abstract val transformationRequest: Property<ArtifactTransformationRequest<ProcessPackagedManifestTask>>

  // Use a property to hold the [WorkerExecutor] so unit tests can reset it if necessary.
  @get:Internal val workersProperty: Property<WorkerExecutor> = objects.property(WorkerExecutor::class.java)
  ...
  override fun doTaskAction() {
    ...
    transformationRequest.get().submit(this, workersProperty.get().noIsolation(), WorkItem::class.java) {
      builtArtifact: BuiltArtifact,
      directory: Directory,
      parameters: WorkItemParameters ->
      parameters.inputXmlFile.set(File(builtArtifact.outputFile))
      parameters.outputXmlFile.set(File(directory.asFile, FileUtils.join(builtArtifact.dirName(), SdkConstants.ANDROID_MANIFEST_XML)))
  ...
  abstract class WorkItem @Inject constructor(private val workItemParameters: WorkItemParameters) : WorkActionAdapter<WorkItemParameters> {
    ...
    override fun doExecute() {
      ...
      val xmlDocument = BufferedInputStream(FileInputStream(inputFile)).use { PositionXmlParser.parse(it) }
  ...
  class CreationAction(creationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<ProcessPackagedManifestTask, ApkCreationConfig>(creationConfig = creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ProcessPackagedManifestTask>) {
      ...
      transformationRequest =
        creationConfig.artifacts
          .use(taskProvider)
          .wiredWithDirectories(ProcessPackagedManifestTask::mergedManifests, ProcessPackagedManifestTask::packageManifests)
          .toTransformMany(InternalArtifactType.MERGED_MANIFESTS, InternalArtifactType.PACKAGED_MANIFESTS)
    }
