...
/**
 * Creates a Manifest Class as a compiled JAR file as [InternalArtifactType.COMPILE_MANIFEST_JAR]. This manifest class is used for accessing
 * Android Manifest custom permission names.
 */
@CacheableTask
...
abstract class GenerateManifestJarTask : NonIncrementalTask() {
  ...
  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val mergedManifests: RegularFileProperty
  ...
  override fun doTaskAction() {
    ManifestClassGenerator(
        ManifestClassData(manifestFile = mergedManifests.get().asFile, namespace = namespace.get(), outputFilePath = outputJar.get().asFile)
      )
      .apply {
  ...
  class CreationAction(creationConfig: ApkCreationConfig) :
    ...
    override fun handleProvider(taskProvider: TaskProvider<GenerateManifestJarTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, GenerateManifestJarTask::outputJar)
    ...
    override fun configure(task: GenerateManifestJarTask) {
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(SingleArtifact.MERGED_MANIFEST, task.mergedManifests)
