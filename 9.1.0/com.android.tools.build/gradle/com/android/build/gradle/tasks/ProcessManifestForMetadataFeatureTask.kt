...
abstract class ProcessManifestForMetadataFeatureTask : NonIncrementalTask() {
  ...
  @get:OutputFile abstract val metadataFeatureManifest: RegularFileProperty

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val bundleManifest: RegularFileProperty
  ...
  abstract class WorkItem @Inject constructor(private val workItemParameters: WorkItemParameters) : WorkAction<WorkItemParameters> {
    override fun execute() {
      val xmlDocument =
        BufferedInputStream(FileInputStream(workItemParameters.inputXmlFile.get().asFile)).use { PositionXmlParser.parse(it) }
  ...
  class CreationAction(creationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<ProcessManifestForMetadataFeatureTask, ApkCreationConfig>(creationConfig = creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ProcessManifestForMetadataFeatureTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ProcessManifestForMetadataFeatureTask::metadataFeatureManifest)
    ...
    override fun configure(task: ProcessManifestForMetadataFeatureTask) {
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.BUNDLE_MANIFEST, task.bundleManifest)
