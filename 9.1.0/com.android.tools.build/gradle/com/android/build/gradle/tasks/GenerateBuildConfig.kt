...
abstract class GenerateBuildConfig : NonIncrementalTask() {
  ...
  @get:OutputDirectory @get:Optional abstract val sourceOutputDir: DirectoryProperty

  @get:OutputFile @get:Optional abstract val bytecodeOutputFile: RegularFileProperty

  // ----- PRIVATE TASK API -----

  @get:Input @get:Optional abstract val buildTypeName: Property<String>
  ...
  @get:Input abstract val flavorName: Property<String>

  @get:Input abstract val flavorNamesWithDimensionNames: ListProperty<String>
  ...
  // and null value.
  @get:Internal abstract val hasVersionInfo: Property<Boolean>

  @get:Input abstract val items: MapProperty<String, BuildConfigField<out Serializable>>

  @get:InputFiles @get:Optional @get:PathSensitive(PathSensitivity.RELATIVE) abstract val mergedManifests: DirectoryProperty

  override fun doTaskAction() {
    val itemsToGenerate = items.get()

    val buildConfigData =
      BuildConfigData.Builder().setNamespace(namespace.get()).apply {
        ...
        buildTypeName.orNull?.let { addStringField("BUILD_TYPE", it) }
        flavorName.get().let {
          if (it.isNotEmpty()) {
    ...
    val generator: GeneratedCodeFileCreator =
      if (bytecodeOutputFile.isPresent) {
        ...
        val sourceCodeBuildConfigData =
          buildConfigData
            .setOutputPath(sourceOutputDir.get().asFile.toPath())
            .apply {
              // user generated items, order them by field name so generation
              // is stable.
              itemsToGenerate.toSortedMap().forEach { (name, buildConfigField) -> addItem(name, buildConfigField) }
            }
            .build()
  ...
  internal class CreationAction(creationConfig: ConsumableCreationConfig) :
    VariantTaskCreationAction<GenerateBuildConfig, ConsumableCreationConfig>(creationConfig),
    BuildConfigTaskCreationAction by BuildConfigTaskCreationActionImpl(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<GenerateBuildConfig>) {
      super.handleProvider(taskProvider)
      val outputBytecode = creationConfig.services.projectOptions.get(BooleanOption.ENABLE_BUILD_CONFIG_AS_BYTECODE)
      ...
      if (outputBytecode && !generateItems) {
        creationConfig.artifacts
          .setInitialProvider(taskProvider, GenerateBuildConfig::bytecodeOutputFile)
          .withName("BuildConfig.jar")
          .on(InternalArtifactType.COMPILE_BUILD_CONFIG_JAR)
      } else {
        creationConfig.artifacts
          .setInitialProvider(taskProvider, GenerateBuildConfig::sourceOutputDir)
    ...
    override fun configure(task: GenerateBuildConfig) {
      ...
      task.flavorName.setDisallowChanges(services.provider { creationConfig.flavorName ?: "" })

      task.flavorNamesWithDimensionNames.setDisallowChanges(services.provider { creationConfig.getFlavorNamesWithDimensionNames() })

      task.items.set(buildConfigCreationConfig.buildConfigFields)

      if (creationConfig.componentType.isTestComponent) {
        creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.PACKAGED_MANIFESTS, task.mergedManifests)
      }
