...
abstract class FusedLibraryBundle : Jar(), GlobalTask {
  ...
  @get:InputDirectory @get:PathSensitive(PathSensitivity.NONE) abstract val validationDir: DirectoryProperty
  ...
  abstract class CreationAction<T : FusedLibraryBundle>(
    ...
    override fun handleProvider(taskProvider: TaskProvider<T>) {
      ...
      creationConfig.artifacts.setInitialProvider(taskProvider, FusedLibraryBundle::outputFile).on(artifactType)
    }
    ...
    override fun configure(task: T) {
      ...
      task.validationDir.set(creationConfig.artifacts.get(FusedLibraryInternalArtifactType.DEPENDENCY_VALIDATION))
    }
  }
}
...
abstract class FusedLibraryBundleAar : FusedLibraryBundle() {
  ...
  class CreationAction(creationConfig: FusedLibraryGlobalScope) :
    FusedLibraryBundle.CreationAction<FusedLibraryBundleAar>(creationConfig, FusedLibraryInternalArtifactType.BUNDLED_LIBRARY) {
    ...
    override fun configure(task: FusedLibraryBundleAar) {
      ...
      task.destinationDirectory.set(creationConfig.projectLayout.buildDirectory.dir("$FD_OUTPUTS/$EXT_AAR"))
      ...
      task.from(creationConfig.artifacts.get(FusedLibraryInternalArtifactType.MERGED_AAR_METADATA)) {
        it.rename("aar-metadata.properties", AarMetadataTask.AAR_METADATA_ENTRY_PATH)
      }
      task.from(creationConfig.artifacts.get(FusedLibraryInternalArtifactType.MERGED_ASSETS)) { it.into(SdkConstants.FD_ASSETS) }

      task.from(creationConfig.getLocalJars()) { it.into(FD_AAR_LIBS) }
    }
  }
}
...
abstract class FusedLibraryBundleClasses : NonIncrementalGlobalTask() {
  ...
  @get:InputFiles @get:Classpath abstract val include: ConfigurableFileCollection

  @get:Optional @get:OutputFile abstract val jar: RegularFileProperty
  ...
  class CreationActionClassesJar(val creationConfig: FusedLibraryGlobalScope) : GlobalTaskCreationAction<FusedLibraryBundleClasses>() {
    ...
    override fun handleProvider(taskProvider: TaskProvider<FusedLibraryBundleClasses>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, FusedLibraryBundleClasses::jar)
        .withName("classes.jar")
        .on(FusedLibraryInternalArtifactType.CLASSES_JAR)
    }
  ...
  class CreationActionLintJar(val creationConfig: FusedLibraryGlobalScope) : GlobalTaskCreationAction<FusedLibraryBundleClasses>() {
    ...
    override fun handleProvider(taskProvider: TaskProvider<FusedLibraryBundleClasses>) {
      ...
      creationConfig.artifacts
        ...
        .on(FusedLibraryInternalArtifactType.LINT_JAR)
    }

    override fun configure(task: FusedLibraryBundleClasses) {
      super.configure(task)
      task.include.from(creationConfig.artifacts.get(FusedLibraryInternalArtifactType.MERGED_PUBLISHED_LINT_CLASSES))
    }
  }
}
