...
/** Compile navigation XMLs for APK taken from application and libraries. Simplified version of MergeResource task. */
@CacheableTask
...
abstract class CompileNavigationXmlTask : NewIncrementalTask() {
  ...
  @get:Internal abstract val resSourceSetMap: RegularFileProperty

  @get:OutputDirectory abstract val outputDir: DirectoryProperty

  @get:OutputDirectory @get:Optional abstract val partialRDirectory: DirectoryProperty

  @get:Nested abstract val aapt2: Aapt2Input

  override fun doTaskAction(inputChanges: InputChanges) {
    workerExecutor.noIsolation().submit(CompileNavigationResourcesAction::class.java) { parameters ->
  ...
  protected abstract class CompileNavigationResourcesAction : ProfileAwareWorkAction<CompileNavigationResourcesParams>() {
    ...
    private fun handleFullRun(processor: WorkerExecutorResourceCompilationService) {
      ...
      for (inputDirectory in parameters.inputDirectories) {
        ...
        inputDirectory
          ...
          .forEach { dir -> dir.listFiles()!!.forEach { file -> submitFileToBeCompiled(file, processor) } }
      }
    }

    /** Given an input resource file return the partial R file to generate, or null if partial R generation is not enabled */
    ...
    private fun submitFileToBeCompiled(file: File, compilationService: WorkerExecutorResourceCompilationService) {
    ...
    private fun handleModifiedFile(file: File, changeType: FileStatus, compilationService: WorkerExecutorResourceCompilationService) {
      if (changeType == FileStatus.CHANGED || changeType == FileStatus.REMOVED) {
        FileUtils.deleteIfExists(File(parameters.outputDirectory.asFile.get(), Aapt2RenamingConventions.compilationRename(file)))
    ...
    private fun handleIncrementalChanges(
      ...
      fileChanges.changes.forEach { fileChange -> handleModifiedFile(fileChange.file, fileChange.fileStatus, compilationService) }
    }
  }

  open class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<CompileNavigationXmlTask, ComponentCreationConfig>(creationConfig) {

    override val name: String
      get() = creationConfig.computeTaskNameInternal("compile", "NavigationResources")
    ...
    override fun handleProvider(taskProvider: TaskProvider<CompileNavigationXmlTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, CompileNavigationXmlTask::outputDir)
    ...
    override fun configure(task: CompileNavigationXmlTask) {
      ...
      val navigationResources = creationConfig.artifacts.get(InternalArtifactType.UPDATED_NAVIGATION_XML)
      task.inputDirectories.setFrom(creationConfig.services.fileCollection(navigationResources))
      task.pseudoLocalesEnabled.setDisallowChanges(creationConfig.androidResourcesCreationConfig!!.pseudoLocalesEnabled)
