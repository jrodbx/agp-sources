...
class DependencyUsageFinder(
  ...
  val requiredDependencies: Set<String> = variantClasses.getUsedClasses().mapNotNull { classFinder.find(it) }.toSet()
  /** Dependencies we direct declare and are being used. */
  val usedDirectDependencies: Set<String> = variantDependencies.all.intersect(requiredDependencies)

  /** Dependencies we direct declare and are not being used. */
  val unusedDirectDependencies: Set<String> = variantDependencies.all.minus(requiredDependencies)
}

/** Find required dependencies that are being included indirectly and would be unreachable if we remove unused direct dependencies. */
class DependencyGraphAnalyzer(
  ...
  fun findIndirectRequiredDependencies(): Set<String> {
    ...
    val requiredRenderableDependencies =
      depsUsageFinder.requiredDependencies
        .asSequence()
        .mapNotNull { renderableDependencies[it] }
        .filterNot { it.id is ProjectComponentIdentifier }
        .map { (it.id as ComponentIdentifier).displayName }
        .toSet()
...
class ClassFinder(private val classesArtifactCollection: ArtifactCollection) {
  ...
  private fun getClassDependencyMap(): Map<String, String> {
    ...
    for (artifact in classesArtifactCollection) {
      ...
      if (file.name != SdkConstants.FN_CLASS_LIST) {
        throw IllegalArgumentException("${file.name} should have name ${SdkConstants.FN_CLASS_LIST}.")
      }
      file.forEachLine { artifactFileLine -> map[artifactFileLine] = artifact.id.componentIdentifier.displayName }
    }
    return map
  }
  ...
  fun findClassesInDependency(dependencyId: String) = classToDependency.filterValues { it == dependencyId }.keys
}
...
class ResourcesFinder(
  ...
  private val resourceToDependency: Map<String, ResourceDependencyHolder> by lazy(this::getResourceDependencyMap)
  ...
  private fun getResourceDependencyMap(): Map<String, ResourceDependencyHolder> {
    ...
    val manifestResources: List<String> =
      if (manifest != null) {
    ...
    val usedResources: List<String> = resourceSets.flatMap(this::parseResourceSourceSet).plus(manifestResources)

    usedResources.forEach { resourceToDependency[it] = ResourceDependencyHolder(it, true) }
    ...
    for (artifact in resourceSymbolsArtifactCollection) {
      ...
      if (resSymbols.name != SdkConstants.FN_RESOURCE_SYMBOLS) {
        throw IllegalArgumentException("${resSymbols.name} should have name ${SdkConstants.FN_RESOURCE_SYMBOLS}.")
      }
      resSymbols.readLines().forEach { resourceName ->
        ...
        val resDeps =
          resourceToDependency.getOrDefault(resourceName, ResourceDependencyHolder(resourceName, false)).apply {
  ...
  fun find(resourceId: String): Set<String> = resourceToDependency[resourceId]?.dependencyUsages ?: emptySet()

  /** Returns a list of resources which are declared or referenced by the dependency. */
  fun findResourcesInDependency(dependencyId: String): Set<ResourceDependencyHolder> =
    resourceToDependency.filterValues { it.dependencyUsages.contains(dependencyId) }.values.toSet()

  /** Returns a list of dependencies which contains an identical resource in another dependency. */
  fun findUsedDependencies(): Set<String> =
    resourceToDependency
      .filter { it.value.isUsedbyAppModule || checkDependenciesForAppUsages(it.value.dependencyUsages) }
      .flatMap { it.value.dependencyUsages }
      .toSet()

  /** Returns a list of dependencies which do not contain an identical resource in another dependency. */
  fun findUnUsedDependencies(): Set<String> =
    resourceSymbolsArtifactCollection.map { it.id.componentIdentifier.displayName }.minus(findUsedDependencies()).toSet()

  /** Gets a list of declared and referenced resources from all files in the given resource set directory. */
  private fun parseResourceSourceSet(resourceSourceSet: File): List<String> = getResourcesFromDirectory(resourceSourceSet)

  /** Check if a collection of dependencies have a resource used by an app module. */
  private fun checkDependenciesForAppUsages(dependencies: Collection<String>) =
    dependencies.flatMap { findResourcesInDependency(it) }.asSequence().filter { it.isUsedbyAppModule }.any()
}

data class DependenciesUsageReport(@SerializedName("add") val add: List<String>, @SerializedName("remove") val remove: List<String>)
...
class DependencyUsageReporter(
  ...
  fun writeUnusedDependencies(destinationFile: File) {
    val toRemove =
      depsUsageFinder.unusedDirectDependencies
  ...
  fun writeMisconfiguredDependencies(destinationFile: File) {
    val apiDependencies =
      variantDependencies.api?.let { variantClasses.getPublicClasses().mapNotNull { classFinder.find(it) }.intersect(it) }

    val misconfiguredDependencies = variantDependencies.api?.minus(apiDependencies) ?: emptySet()
