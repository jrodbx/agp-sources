...
abstract class ExtractSupportedLocalesTask : NonIncrementalTask() {
  ...
  @get:OutputFile abstract val supportedLocales: RegularFileProperty
  ...
  @get:Input abstract val fromAppModule: Property<Boolean>
  ...
  public override fun doTaskAction() {
    ...
    if (fromAppModule.get()) {
      ...
      val noResourcesPropertiesMessage =
        "No resources.properties file found. " + "See https://developer.android.com/r/studio-ui/build/automatic-per-app-languages"
      ...
      if (defaultLocale == null) {
        val noDefaultLocaleMessage =
          "No locale is set for unqualified res. " + "See https://developer.android.com/r/studio-ui/build/automatic-per-app-languages"
        logger.error(noDefaultLocaleMessage)
        throw RuntimeException(noDefaultLocaleMessage)
      }

      validatedDefaultLocale =
        validateLocale(defaultLocale)
          ?: throw RuntimeException(
            "The default locale \"$defaultLocale\" from the file " +
  ...
  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<ExtractSupportedLocalesTask, ComponentCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ExtractSupportedLocalesTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ExtractSupportedLocalesTask::supportedLocales)
        .withName("supported_locales.txt")
        .on(InternalArtifactType.SUPPORTED_LOCALE_LIST)
    }

    override fun configure(task: ExtractSupportedLocalesTask) {
      super.configure(task)
      creationConfig.sources.res { resSources ->
        val localeSources = resSources.getVariantSourcesWithFilter()
        task.nonMainResSet.from(*(localeSources.filter { it.key != FD_MAIN }.values.toTypedArray()))
