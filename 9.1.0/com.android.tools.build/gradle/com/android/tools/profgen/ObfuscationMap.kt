...
/**
 * A data structure representing a subset of the information encoded in a Proguard Mapping file. This file provides the information
 * necessary to deobfuscate method named and class names. This file also contains a lot of information about line mappings and inlining
 * which is not attempted to be preserved here, as it is not important for profile generation
 */
class ObfuscationMap internal constructor(classMappings: List<ClassMapping>) {
  private val obfToOrig: Map<String, List<String>> = classMappings.groupBy({ it.obfuscated }, { it.original })

  private val origToObf = TypeMap(classMappings.associate { it.original to it.obfuscated })

  private val methodObfToOrig: Map<DexMethod, DexMethod> =
    classMappings
      .flatMap { mapping ->
        mapping.originalToObfuscated.map { (original, obfName) ->
          val obfuscated =
            DexMethod(
              ...
              name = obfName,
              prototype =
                DexPrototype(returnType = origToObf[original.returnType], parameters = original.prototype.parameters.map { origToObf[it] }),
            )
...
/**
 * A small abstraction on top of a Map<String, String> where the mapping is understood to be a map of JVM type signatures to other type
 * signatures. In this case, this map will optimize the case where the passed in value is a primitive type (ie, does NOT start with `L`) and
 * will just return identity in that case. This avoids unnecessary map lookups.
 */
...
internal class ClassMapping(val original: String, val obfuscated: String) {
  ...
  internal fun addMemberFromParser(parser: MemberParser) {
    ...
    val method =
      DexMethod(
        ...
        prototype = DexPrototype(returnType = parser.returnType, parameters = parser.parameters.toList()),
      )
