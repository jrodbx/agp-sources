...
class HumanReadableProfile
internal constructor(
  /** The map of method descriptors to flags */
  private val exactMethods: Map<DexMethod, Int>,
  /**
   * The set of string descriptors that are included in the rules, specified to be "startup" classes. This is modeled as a Set because the
   * classes are either specified or aren't. These are "exact" in the sense that these classes were specified in the file without any
   * wildcards.
   *
  ...
  private fun fuzzyMatch(type: String): Int {
    val fuzzy = fuzzyTypes.firstOrNull(type) { it.target.matches(type) }
  ...
  fun expandWildcards(classFileResources: Collection<ClassFileResource>): HumanReadableProfile {
    ...
    for (classFileResource in classFileResources) {
      expandWildcards(classFileResource, classFileResource.getBytes(), newExactMethods, newExactTypes)
    }

    return HumanReadableProfile(newExactMethods, newExactTypes, MutablePrefixTree(), MutablePrefixTree())
  }
  ...
  private fun expandWildcards(
    ...
    if (hasFuzzyMethods(classDescriptor)) {
      val classVisitor =
        object : ClassVisitor(Opcodes.ASM9) {
          override fun visitMethod(
            ...
            val method = DexMethod(classDescriptor, name, DexPrototype(returnTypeDescriptor, splitParameters(parameters)))
      ...
      val parsingOptions: Int = ClassReader.SKIP_CODE or ClassReader.SKIP_DEBUG or ClassReader.SKIP_FRAMES
  ...
  fun printExact(os: Appendable) {
    ...
    for (method in exactMethods.keys) {
      app.getOrPut(method.parent) { mutableListOf() }.add(method)
    }
...
fun HumanReadableProfile(src: InputStreamReader, onError: (Int, Int, String) -> Unit): HumanReadableProfile? {
  var failed = false
  val fragmentParser = RuleFragmentParser(80)
  val rules =
    src.readLines().mapIndexedNotNull { lineNumber, line ->
  ...
  return HumanReadableProfile(exactMethods, exactTypes, fuzzyMethods, fuzzyTypes)
}
...
internal fun ProfileRule.toDexMethod(): DexMethod {
  ...
  return DexMethod(
    ...
    name = method.prefix,
    prototype = DexPrototype(returnType = returnType.prefix, parameters = splitParameters(params.prefix)),
  )
}
...
fun HumanReadableProfile(file: File, diagnostics: Diagnostics): HumanReadableProfile? {
  return file.reader().use {
    HumanReadableProfile(it) { line, column, error -> diagnostics.onError("${file.name}:${line + 1}:${column + 1} error: $error") }
  }
}

internal fun String.withSnippet(line: String, column: Int) = "$this\n$line\n${" ".repeat(column)}^"
...
internal class RuleFragmentParser(capacity: Int, private var parts: MutableList<Part> = mutableListOf()) : Parseable(capacity) {
  ...
  fun build(): RuleFragment {
    ...
    for (i in parts.indices) {
      ...
      when (val part = parts[i]) {
        is Part.Exact -> sb.append(if (exact) part.value else Regex.escape(part.value))
        is Part.Pattern -> {
          if (exact) {
            exact = false
            prefix = sb.toString()
    ...
    sb.clear()
    parts.clear()
    return RuleFragment(empty, exact, prefix, pattern)
  }
}

internal class RuleFragment(val isEmpty: Boolean, val isExact: Boolean, val prefix: String, private val pattern: Regex) {
...
internal fun parseRule(line: String, onError: (Int, String) -> Unit, fragmentParser: RuleFragmentParser): ProfileRule? {
  if (line.isBlank()) {
  ...
  try {
    ...
    if (i == line.length || line[i].isWhitespace()) {
      ...
      return ProfileRule(MethodFlags.STARTUP, target, RuleFragment.Empty, RuleFragment.Empty, RuleFragment.Empty)
    }
    ...
    return ProfileRule(flags = flags.flags, target = target, method = method, params = parameters, returnType = returnType)
