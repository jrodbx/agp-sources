 ...
 *
 * LEB128 ("Little-Endian Base 128") is a variable-length encoding for arbitrary signed or unsigned integer quantities. The format was
 * borrowed from the DWARF3 specification. In a .dex file, LEB128 is only ever used to encode 32-bit quantities.
 *
 * Each LEB128 encoded value consists of one to five bytes, which together represent a single 32-bit value. Each byte has its most
 * significant bit set except for the final byte in the sequence, which has its most significant bit clear. The remaining seven bits of each
 * byte are payload, with the least significant seven bits of the quantity in the first byte, the next seven in the second byte and so on.
 * In the case of a signed LEB128 (sleb128), the most significant payload bit of the final byte in the sequence is sign-extended to produce
 * the final value. In the unsigned case (uleb128), any bits not explicitly represented are interpreted as 0.
 */
 ...
 *
 * Decodes bytes from the ByteBuffer until a delimiter 0x00 is encountered. Returns a new string containing the decoded characters.
 */
...
internal fun Long.toIntSaturated(): Int =
  when {
...
internal enum class Endian(val number: Int, val order: ByteOrder) {
  LITTLE(0x12345678, ByteOrder.LITTLE_ENDIAN),
  BIG(0x78563412, ByteOrder.BIG_ENDIAN);

  companion object {
    fun forNumber(value: Int) =
      when (value) {
...
/**
 * Compresses data the using [DeflaterOutputStream] before writing it to the stream. The method will write the size of the compressed data
 * (32 bits, [.writeUInt32]) before the actual compressed bytes.
 *
...
internal fun ByteArray.compressed(): ByteArray {
  ...
  deflater.use { deflater.write(this) }
  return out.toByteArray()
}

/** Attempts to read {@param length} bytes from the input stream. If not enough bytes are available it throws [IllegalStateException]. */
...
internal val String.utf8Length: Int
  get() = toByteArray(StandardCharsets.UTF_8).size
