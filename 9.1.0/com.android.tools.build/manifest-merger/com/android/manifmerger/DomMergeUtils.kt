...
object DomMergeUtils {
  ...
  fun cloneAndTransform(document: Document, transform: Predicate<Node?>, shouldRemove: Predicate<Node?>): Pair<Document, Boolean> {
    return try {
  ...
  /**
   * Function goes through dom tree in recursive fashion, skipping nodes per predicate `shouldRemove`, and apply transformation to all
   * others.
   *
   ...
   * @return Pair of cloned node and a flag. Flag shows whether tree was changed. Node can be null when shouldRemove returns true flagging
   *   not to clone this node.
   */
