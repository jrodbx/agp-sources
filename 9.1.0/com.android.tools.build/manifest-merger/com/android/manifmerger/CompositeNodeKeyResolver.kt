...
/**
 * Specific [NodeKeyResolver] for intent-filter and intent elements. These two elements do not have a proper key, therefore their identity
 * is really carried by the presence of their sub-elements. We concatenate the sub-elements' attributes' names and values (after sorting
 * them to work around declaration order) and use that for the intent-filter unique key. An example key might look like
...
internal open class CompositeNodeKeyResolver(val nodeType: String) : NodeKeyResolver {
  ...
  private val dataAttributeNames =
    ImmutableList.of(
  ...
  override fun getKey(element: Element): String? {
    ...
    for (i in 0 until childNodes.length) {
      ...
      if (subElement.type == ManifestModel.NodeTypes.ACTION || subElement.type == ManifestModel.NodeTypes.CATEGORY) {
