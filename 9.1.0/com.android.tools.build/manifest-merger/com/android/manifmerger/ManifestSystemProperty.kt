...
interface ManifestSystemProperty : AutoAddingProperty {
  ...
  companion object {
    ...
    @JvmStatic
    val values: List<ManifestSystemProperty> =
      listOf(Application.values(), Document.values(), Instrumentation.values(), Manifest.values(), Profileable.values(), UsesSdk.values())
        .flatMap { it.asList() }
  ...
  enum class Application(private val override: Boolean) : ManifestSystemProperty {
    ...
    override fun addTo(actionRecorder: ActionRecorder, document: XmlDocument, value: String) {
      val xmlElement = createOrGetElementInManifest(actionRecorder, document, NodeTypes.APPLICATION, "application injection requested")
  ...
  enum class Instrumentation : ManifestSystemProperty {
    ...
    override fun addTo(actionRecorder: ActionRecorder, document: XmlDocument, value: String) {
      addToElementInAndroidNS(
        ...
        actionRecorder,
        value,
        createOrGetElementInManifest(actionRecorder, document, NodeTypes.INSTRUMENTATION, "instrumentation injection requested"),
      )
    }
  }
    ...
    override fun addTo(actionRecorder: ActionRecorder, document: XmlDocument, value: String) {
      ...
      maybeApplicationElement.ifPresent { applicationElement ->
        addToElementInAndroidNS(
          ...
          actionRecorder,
          value,
          createOrGetElement(actionRecorder, document, applicationElement, NodeTypes.PROFILEABLE, "profileable injection requested"),
        )
      }
    }
  }
  ...
  enum class UsesSdk : ManifestSystemProperty {
    ...
    override fun addTo(actionRecorder: ActionRecorder, document: XmlDocument, value: String) {
      addToElementInAndroidNS(
        ...
        actionRecorder,
        value,
        createOrGetElementInManifest(actionRecorder, document, NodeTypes.USES_SDK, "use-sdk injection requested"),
      )
    }
  }
}
...
private fun addToElement(elementAttribute: ManifestSystemProperty, actionRecorder: ActionRecorder, value: String, to: XmlElement) {
  to.setAttribute(elementAttribute.toCamelCase(), value)
  val xmlAttribute =
    to.getAttributeNode(elementAttribute.toCamelCase())?.let { XmlAttribute(to, it, null) }
...
/**
 * utility method to add an attribute in android namespace which local name is derived from the enum name().
 *
...
private fun addToElementInAndroidNS(
  ...
  val toolsPrefix = to.lookupNamespacePrefix(SdkConstants.ANDROID_URI, SdkConstants.ANDROID_NS_NAME, true)
  if (override) {
    to.setAttributeNS(SdkConstants.ANDROID_URI, toolsPrefix + XmlUtils.NS_SEPARATOR + elementAttribute.toCamelCase(), value)
  } else {
    val isModified = ManifestMerger2.setAndroidAttributeIfMissing(to, elementAttribute.toCamelCase(), value)
    if (!isModified) {
      // no reason to record the action if not modified.
      return
    }
  }
  val attr = to.getAttributeNodeNS(SdkConstants.ANDROID_URI, elementAttribute.toCamelCase())
  val xmlAttribute = XmlAttribute(to, attr!!, null)
  recordElementInjectionAction(actionRecorder, to, xmlAttribute)
}

private fun recordElementInjectionAction(actionRecorder: ActionRecorder, to: XmlElement, xmlAttribute: XmlAttribute) {
...
private fun createOrGetElement(
  ...
  return parentElement.createOrGetElementOfType(document, nodeType) { xmlElement ->
    val nodeRecord =
      NodeRecord(
        Actions.ActionType.INJECTED,
        SourceFilePosition(xmlElement.sourceFile, SourcePosition.UNKNOWN),
        xmlElement.id,
        message,
