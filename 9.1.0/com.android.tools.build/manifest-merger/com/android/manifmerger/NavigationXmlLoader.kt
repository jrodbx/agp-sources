...
object NavigationXmlLoader {
  ...
  /**
   * Loads a navigation xml file without doing xml validation and returns a [NavigationXmlDocument].
   *
  ...
  fun load(displayName: String, navigationXmlFile: File, inputStream: InputStream): NavigationXmlDocument {
    val domDocument = PositionXmlParser.parse(inputStream)
    return NavigationXmlDocument(SourceFile(navigationXmlFile, displayName), domDocument.documentElement)
  }

  /**
   * Loads a navigation xml document from its [String] representation without doing xml validation and returns a [NavigationXmlDocument]
   *
