  ...
  fun writeTo(config: ResTableConfig) {
    ...
    if (script.isNotEmpty()) {
      if (script.length > config.localeScript.size) {
        throw IllegalStateException("Locale script '$script' exceeds ${config.localeScript.size} characters.")
      }
    ...
    if (variant.isNotEmpty()) {
      if (variant.length > config.localeVariant.size) {
        throw IllegalStateException("Locale variant '$variant' exceeds ${config.localeVariant.size} characters.")
      }
