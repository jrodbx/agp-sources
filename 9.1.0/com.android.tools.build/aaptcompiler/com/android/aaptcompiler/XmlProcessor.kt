 ...
 * 1. Collect created ID resources. Gather all ids specified in xml attributes with the create marker. i.e.
 *
 *    <TextView android:id="@+id/player_name" android:text="@+string/player_name"/> In this XML, the resource id/player_name would be
 *    exported by this xml file, since it has the "+" resource creation marker. Even though string/player_name is marked with the creation
 *    marker, it is not an id resource and is ignored.
 * 2. Outline inlined XML resources. In android XML files (drawables, layouts, etc.) it is possible to write an XML resource inside of
 *    another XML resource.
 *
 * Consider drawable/player_background: <shape xmlns:android="http://schemas.android.com/apk/res/android" android:shape="rectangle">
 * <gradient android:startColor="#FF000000" android:endColor="#FFAA0000" android:angle="35" /> <corners android:radius="10dp" /> </shape>
 * And consider: <TextView android:id="@+id/player_name" android:text="@string/player_name"
 * android:background=@"drawable/player_background"/>
 *
 * If this drawable is only used here we can simplify this to one file: <TextView android:id="@+id/player_name"
 * android:text="@string/player_name"> <aapt:attr name="android:background"> <shape
 * xmlns:android="http://schemas.android.com/apk/res/android" android:shape="rectangle"> <gradient android:startColor="#FF000000"
 * android:endColor="#FFAA0000" android:angle="35" /> <corners android:radius="10dp" /> </shape> </aapt:attr> </TextView>
 *
 * This is possible by outlining the aapt_attr element into a separate XML file. Effectively undoing the inlining by the developer. So this
 * class outlines the inline XML and sets the parent elements attribute to reference the outlned attr.
 * 3. Flatten XMLs to proto. Flatten all XML files (including outlined XMLs in (2.)) to proto to be written as output.
 *
 * The XMLProcessor handles all of these processes simultaneously to require only a single pass over the XML file.
 *
...
class XmlProcessor(val source: Source, val logger: BlameLogger?) {
   ...
   *
   * Outlines all inline XML aapt:attr resources, collects all created id resources, and flattens the resulting XMLs to proto.
   */
   ...
   *
   * This method flattens the XML tree whose root is [startElement], aggregates all created ids, and outlines any given aapt:attr xml
   * subtrees that may exist in the given XML.
   *
   * @param startElement the start element that represents the root of the XML tree to be flattened. The start element should not be
   *   pointing to a <aapt:attr> element, as those are processed in a different method.
   * @param eventReader the stream from which the XML file is read. The event reader should be in a state where [startElement] was the last
   *   event read. When this method finishes, [eventReader] will be positioned immediately after the [EndElement] that aligns with the
   *   supplied [StartElement].
   * @param collectedIds The map of all created ID resources in the XML document. Any created ids found in the XML tree represented by
   *   [startElement] will be added to this map.
   * @param resourceBuilders The map of all XML resource builders created for this XML document. Any new builders created because of
   *   <aapt:attr> subelements of [startElement] will be added to this map.
   * @param currentBuilder The current xml resource builder that this [startElement] is to be flattened to. When this function returns,
   *   current builder will have the same active element as it did when the method started (or will be finished if this element was the top
   *   element of the builder). The subtree represented by [startElement] will be added as a child element to the current element.
   * @param inheritedNamespaceContext When creating the top element of an extracted <aapt:attr> element, this variable will contain the
   *   context for all the xml namespaces that are active in [StartElement] that will need to be flattened to proto.
   */
  private fun processElement(
    ...
    inheritedNamespaceContext?.namespacePrefixes()?.forEach { prefix ->
      if (namespaces.firstOrNull { ns -> ns.prefix == prefix } == null) {
        currentBuilder.addNamespaceDeclaration(inheritedNamespaceContext.uriForPrefix(prefix)!!, prefix)
      }
    }
    ...
    while (eventReader.hasNext()) {
      ...
      if (nextEvent.isStartElement) {
        ...
        if (isAaptAttribute(elName)) {
          outlineAttribute(currentBuilder, nextEvent.asStartElement(), eventReader, collectedIds, resourceBuilders)
        } else {
          // We're going down a level, so process that element.
          processElement(nextEvent.asStartElement(), eventReader, collectedIds, resourceBuilders, currentBuilder)
        }
        continue
      }
  ...
  /**
   * Outlines the aapt:attr and assigns the correct attibute in the parent XML to the outlined value.
   *
   * @param parentBuilder the xml proto builder that is currently set to the parent element of the aapt:attr. This is so we can set the
   *   attribute on the parent element to a reference of the outlined xml resource.
   * @param attrElement the startElement corresponding to the aapt:attr to be outlined.
   * @param eventReader the stream from which the XML file is read. The event reader should be in a state where [attrElement] was the last
   *   event read. When this method finishes, [eventReader] will be positioned immediately after the [EndElement] that aligns with the
   *   supplied [attrElement].
   * @param collectedIds The map of all created ID resources in the XML document. Any created ids found in the XML tree represented by
   *   [attrElement] will be added to this map.
   * @param resourceBuilders The map of all XML resource builders created for this XML document. At least one new builder will be added from
   *   the outlined aapt:attr (if successful), or more if there are nested aapt:attr definitions.
  ...
  private fun outlineAttribute(
    ...
    if (nameAttribute == null) {
      val exception =
        RuntimeException("${blameSource(source, attrElement.location)}:" + "<${attrElement.name}> tag requires the 'name' attribute.")
    ...
    val resource = processAaptAttr(attrElement, eventReader, collectedIds, resourceBuilders, parentBuilder.namespaceContext)
  ...
  private fun isAaptAttribute(elName: QName): Boolean = elName.namespaceURI == AAPT_ATTR_URI && elName.localPart == AAPT_ATTR_TAG

  /**
   * Processes the element that is the immediate child of an aapt:attr element. This is the root element of the outlined XML resource.
   *
   ...
   * @param resourceBuilders The map of all XML resource builders created for this XML document. Any new builders created because of
   *   <aapt:attr> subelements of [startElement] will be added to this map.
   * @param namespaceContext All inherited xmlns definitions so far in the XML document before the root of this XML subtree.
   * @return the ResourceFile corresponding to the outlined element. This value will be null if the outlining is unsuccessful.
   */
  private fun processAaptAttr(
    ...
    while (eventReader.hasNext()) {
      ...
      if (event.isCharacters) {
        ...
        if (!event.asCharacters().isWhiteSpace) {
          errors += "${blameSource(source, event.location)}: " + "Can't extract text into its own resource."
        }
        continue
      }
      ...
      if (event.isStartElement) {
        ...
        if (foundChild) {
          errors += "${blameSource(source, event.location)}: " + "Inline XML resources must have a single root."
          walkToEndOfElement(event.asStartElement(), eventReader)
          continue
        }
        foundChild = true
        processElement(event.asStartElement(), eventReader, collectedIds, resourceBuilders, outputBuilder, namespaceContext)
      }
    ...
    if (!foundChild) {
      error("${blameSource(source, startElement.location)}:" + " No resource to outline. <${startElement.name}> block is empty.")
    }
  ...
  private fun processText(characters: Characters, builder: XmlResourceBuilder) = builder.addText(characters.data)

  private fun processComment(builder: XmlResourceBuilder) = builder.addComment()

  private fun gatherIds(startElement: StartElement, collectedIds: MutableMap<ResourceName, SourcedResourceName>) {
    ...
    while (iterator.hasNext()) {
      ...
      if (parsedRef.createNew && resourceName.type == AaptResourceType.ID) {
        if (!isValidResourceEntryName(resourceName.entry!!)) {
          error("${blameSource(source, startElement.location)} " + "Id '$resourceName' has an invalid entry name '${resourceName.entry}'.")
        } else {
          collectedIds.putIfAbsent(resourceName, SourcedResourceName(resourceName, startElement.location.lineNumber))
        }
      }
    }
  }
